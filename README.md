...
- **CDT node/slot split for capability moves:** CDT derivation edges are now stored over stable node identities, with CSpace slots mapped to nodes via backpointers; `cspaceMove` updates slot-to-node pointers (`moveSlotNode`) instead of rewriting all global edges.
