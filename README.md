...
| **Production Lean LoC** | 128,431 across 186 files |
| **Test Lean LoC** | 25,163 across 37 test suites |
| **Proved declarations** | 3,712 theorem/lemma declarations (zero sorry/axiom) |
| **Rust crates** | 4 (`sele4n-types`, `sele4n-abi`, `sele4n-sys`, `sele4n-hal`) across 48 source files |
| **Target hardware** | Raspberry Pi 5 (BCM2712 / ARM Cortex-A76 / ARMv8-A) — SMP-by-default at v0.31.7 (Rust HAL boot Phase 5 parses the DTB `/chosen/bootargs` cmdline; default `smp_enabled=true` brings up all 4 Cortex-A76 cores via PSCI CPU_ON loop; cross-core IS-variant TLBI broadcast + GICD_SGIR-based SGI primitives + per-core kprintln macro all wired). SM1 closed at v0.31.8 with WS-SM SM1.I miscellaneous HAL improvements. **WS-SM Phase SM2 CLOSED** post-v0.31.9 — all five sub-phases landed: **SM2.A** (abstract memory model — `MemoryOrder` / `MemoryEvent` / `MemoryTrace` / `synchronizesWith` / `happensBefore` with the four-theorem partial-order witness chain); **SM2.B** (TicketLock FIFO spinlock — 8-conjunct `wf` invariant with the count-parity strengthening INV-T8; 6 substantive theorems including `mutex`, `FIFO`, `bounded_wait`, `release_acquire_pairing` bridging to SM2.A, `reachability`; Rust impl at `rust/sele4n-hal/src/ticket_lock.rs`); **SM2.C** (Verified RwLock — 5-conjunct `wf` invariant with INV-R5 FIFO admission discipline; 10 substantive theorems; bit-packed AtomicU64 Rust impl at `rust/sele4n-hal/src/rw_lock.rs` with documented FIFO divergence in F-02; SM2.C-defer adds D-1..D-6 post-v1.0.0 closure work including a queued variant in `queued_rw_lock.rs` for paths needing strict FIFO); **SM2.D** (FFI bridge + integration — `LockBridge.lean` typed wrappers + RAII combinators; `LockPrimitives.lean` 22-theorem aggregator with `lockPrimitives.length = 22` size witness; cross-language symmetry script `scripts/check_lock_ffi_symmetry.sh`); **SM2.E** (documentation — new SELE4N_SPEC.md §10 normative reference + GitBook chapter 16 + unified refinement-proof methodology hub at `Locks/Refinement.lean` + ARM ARM citation map + decision-rationale and hardware-discipline-limits docstrings). 22 substantive theorems total (4 memory-model + 6 TicketLock + 10 RwLock + 2 refinement) across SM2; zero Lean axioms, zero sorries. SMP completion in flight in WS-SM (foundations + SM1 + SM2 all landed; SM3..SM9 to v1.0.0) |
