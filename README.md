...
| **Target hardware** | Raspberry Pi 5 (BCM2712 / ARM Cortex-A76 / ARMv8-A) — SMP-by-default at v0.31.7 (Rust HAL boot Phase 5 parses the DTB `/chosen/bootargs` cmdline; default `smp_enabled=true` brings up all 4 Cortex-A76 cores via PSCI CPU_ON loop; cross-core IS-variant TLBI broadcast + GICD_SGIR-based SGI primitives + per-core kprintln macro all wired). SM1 closed at v0.31.8 with WS-SM SM1.I miscellaneous HAL improvements. WS-SM SM2.A landed at v0.31.9 (abstract memory model — `MemoryOrder` / `MemoryEvent` / `MemoryTrace` / `synchronizesWith` / `happensBefore` with the four-theorem partial-order witness chain). **WS-SM SM2.B landed** post-v0.31.9 (TicketLock FIFO spinlock — abstract operational spec with 8-conjunct `wf` invariant including the count-parity strengthening INV-T8 that closes a gap in the plan's 7-invariant form; substantive theorems `mutex`, wf-preservation under all kernel transitions (`tryAcquire` / `releaseAndPromote` / `observeServing`), `FIFO`, `bounded_wait` (`nextTicket ≤ serving + numCores`), `release_acquire_pairing` bridging to SM2.A, `reachability` via `KernelStep` inductive; Rust impl at `rust/sele4n-hal/src/ticket_lock.rs` cache-line aligned with RAII `TicketLockGuard`; 21 cargo tests covering panic-safety + cross-thread mutex/FIFO stress; 35 Lean runtime assertions in `lake exe ticket_lock_suite`). SMP completion in flight in WS-SM (foundations + SM1 + SM2.A + SM2.B all landed; SM2.C/D/E + SM3..SM9 to v1.0.0) |
