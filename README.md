...
| **Production Lean LoC** | 115,963 across 178 files |
| **Test Lean LoC** | 21,850 across 30 test suites |
...
| **Target hardware** | Raspberry Pi 5 (BCM2712 / ARM Cortex-A76 / ARMv8-A) — SMP-by-default at v0.31.7 (Rust HAL boot Phase 5 parses the DTB `/chosen/bootargs` cmdline; default `smp_enabled=true` brings up all 4 Cortex-A76 cores via PSCI CPU_ON loop; cross-core IS-variant TLBI broadcast + GICD_SGIR-based SGI primitives + per-core kprintln macro all wired). SM1 closed at v0.31.8 with WS-SM SM1.I miscellaneous HAL improvements. WS-SM SM2.A landed at v0.31.9 (abstract memory model — `MemoryOrder` / `MemoryEvent` / `MemoryTrace` / `synchronizesWith` / `happensBefore` with the four-theorem partial-order witness chain). WS-SM SM2.B landed post-v0.31.9 (TicketLock FIFO spinlock — abstract operational spec with 8-conjunct `wf` invariant including the count-parity strengthening INV-T8; substantive theorems `mutex`, wf-preservation under all kernel transitions, `FIFO`, `bounded_wait`, `release_acquire_pairing` bridging to SM2.A, `reachability`; Rust impl at `rust/sele4n-hal/src/ticket_lock.rs` cache-line aligned with RAII `TicketLockGuard`; 21 cargo tests; 35 Lean runtime assertions). **WS-SM SM2.C landed** post-v0.31.9 (Verified RwLock — abstract operational spec with 5-conjunct `wf` invariant including the INV-R5 FIFO admission discipline strengthening that closes the analogous reachability gap; `AccessMode` + `RwLockState` + `RwLockOp` inductives; `applyOp` / `promoteWaitersOnWriterRelease` / `promoteWaitersIfReadersEmpty` operational transitions with `coreInvolved` no-op gate (fixes a plan-pseudocode INV-R4 violation); 10 substantive theorems (`writer_readers_exclusion`, `reader_multiplicity`, `fifo_admission` as a structural drop-prefix claim with helper corollaries, `bounded_wait_read/write`, `release_acquire_pairing_read/write`, `wf_invariant` aggregate, `reader_batching` with strengthened `admits_at_least_one` / `exact_count` bounds, `writer_safety_under_reader_acquire`); determinism; 4 closure-form aliases; bit-packed encoding with 5 round-trip lemmas; SM2.C.20 refinement bridge `RwLockRefinement.lean` documenting the Rust impl's FIFO divergence; Rust impl at `rust/sele4n-hal/src/rw_lock.rs` cache-line aligned with bit-packed AtomicU64 state, audit-pass-fixed `release_read` using `fetch_sub` (defends release-build state corruption) and `release_write` using `fetch_and(READER_MASK)` (preserves reader bits on misuse); 28 cargo tests including 4 cross-thread stress tests (4-reader stress, mixed reader/writer with counter, deterministic reader-multiplicity via two-phase Barrier, deterministic writer-readers exclusion verifying `rwLock_writer_readers_exclusion` at runtime); 38 Lean runtime assertions in `lake exe rw_lock_suite`). SMP completion in flight in WS-SM (foundations + SM1 + SM2.A + SM2.B + SM2.C all landed; SM2.D/E + SM3..SM9 to v1.0.0) |
