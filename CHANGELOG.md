...
### Audit-pass-5 refinements (third external audit pass)

A third external audit pass found one MEDIUM documentation
overclaim, two LOW scanner-robustness weaknesses, one LOW
architectural inconsistency, and one LOW pre-existing plan
count mismatch.  All addressed:
...
- **MEDIUM (boot.rs docstring overclaim)**: the audit-pass-4
  ...
  install → end of boot).  Added a "Pre-MMU memory safety"
  paragraph documenting why writing the static
...
- **LOW (scanner block-comment bypass)**: build-script scanners
  stripped only `//` line comments.  A pathological refactor
  wrapping the contract calls in `/* ... */` would have
  compiled (block comment elides the body) AND defeated the
  scanner.  Added `strip_rust_comments` helper that handles
  both comment styles; the SM1.I.1 scanner uses it via
  ...
  calls in `/* */` — scanner correctly catches each violation
  with an actionable diagnostic.

- **LOW (200-byte `#[no_mangle]` window fragile)**: the
  SM1.I.1 scanner's preamble window was 200 bytes — currently
  has ~88 bytes of margin, fragile against future docstring
  expansion.  Widened to 4096 bytes (one page) for resilience.
...
- **LOW (scanner-bait architectural inconsistency)**: the
  ...
  SM1.C.5 audit-pass-2 which explicitly removed similar bait.
  Audit-pass-5 makes the binding non-vestigial by consuming
  it in a `debug_assert!` that pins the SM5 contract — "TPIDR_EL1
  yields a valid PER_CPU_DATA index".  Zero release-build
  cost (debug_assert is elided in release), provides actual
  defense-in-depth in test/dev builds.

- **LOW (pre-existing plan count mismatch)**: plan §8
  acceptance gate said "All 8 PSCI primitives wrapped" but
  listed 7 names.  Pre-existing from SM1.A landing; corrected
  to "All 7 PSCI primitives wrapped" in two places.

Stress-test post-audit-pass-5: 20/20 runs of the full Rust
suite (`cargo test --all --features std`) pass.  All Tier
0+1+2+3 still green.  Build-script scanner functionally
verified to catch both `//` AND `/* */` contract-bypass
attempts at elaboration time.
