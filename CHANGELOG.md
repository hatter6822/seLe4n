...
v0.30.6 comprehensive audit ([`docs/audits/AUDIT_v0.30.6_COMPREHENSIVE.md`](docs/audits/AUDIT_v0.30.6_COMPREHENSIVE.md),
196 findings: 2 actionable CRITICAL + 23 actionable HIGH + 71 MEDIUM +
59 LOW + 40 INFO, after C-02 already-resolved and H-22 downgraded).
11 phases (AN0..AN10), 79 sub-tasks, ~144 sub-sub-task commits per
[`docs/audits/AUDIT_v0.30.6_WORKSTREAM_PLAN.md`](docs/audits/AUDIT_v0.30.6_WORKSTREAM_PLAN.md).
No production code changes in AN0; version stays at 0.30.6.

### AN0-A — Baseline snapshot

New file [`docs/audits/AUDIT_v0.30.6_WS_AN_BASELINE.txt`](docs/audits/AUDIT_v0.30.6_WS_AN_BASELINE.txt)
captures the WS-AN start state so AN10 can diff the gate numbers:
`lake build` 260 jobs / 0 warnings; `cargo test --workspace` 414
passing / 0 failed / 0 ignored; `cargo clippy --workspace -- -D
warnings` 0 warnings; Theme 4.7 monolithic file LOCs
(`SeLe4n/Kernel/IPC/Invariant/Structural.lean` 7626,
`SeLe4n/Kernel/InformationFlow/Invariant/Operations.lean` 3768,
`SeLe4n/Kernel/Scheduler/Operations/Preservation.lean` 3633,
`SeLe4n/Kernel/Capability/Invariant/Preservation.lean` 2461,
`SeLe4n/Kernel/Lifecycle/Operations.lean` 1473); fixture byte-identical
to `tests/fixtures/main_trace_smoke.expected` (227 lines); zero
`sorry`/`axiom`/`native_decide` in production proof surface; 24 Lean
test suites / ~1614 assertions; `check_version_sync.sh` PASS at 0.30.6;
`check_website_links.sh` PASS; `test_smoke.sh` / `test_full.sh` /
`test_tier0_hygiene.sh` all PASS. The baseline also records the prior
retirement of `scripts/ak7_cascade_baseline.sh` +
`scripts/ak7_cascade_check_monotonic.sh` + `docs/audits/AL0_baseline.txt`
in commit `8d9e61f` (Apr 19, 2026) — the three primary attack surfaces
the old numeric monotonicity guard tracked (AK7-I null-cap, AK7-F
kind-swap, AK7-E sentinel dispatch) are now closed STRUCTURALLY at the
type system (`NonNullCap` subtype, `storeObjectKindChecked` wrapper,
`Valid{Thread,Obj,SchedContext,CPtr}Id` subtypes), which is strictly
stronger than the retired numeric monotonicity guard. The plan's
AN0-A reference to "17 metrics from `ak7_cascade_baseline.sh` rerun"
is therefore reinterpreted as the structural replacement rather than
the retired script numbers.

### AN0-B — Sub-task inventory (plan)

The WS-AN plan itself was landed earlier on
`claude/audit-workstream-planning-AUBX4`: b2ee03c (plan draft, §§0–21),
7ac49e7 (expansion into 158 sub-sub-tasks, §22), cad00cb (Tier 2
markdown-link CI fix), merged via PR #736. The plan is at
[`docs/audits/AUDIT_v0.30.6_WORKSTREAM_PLAN.md`](docs/audits/AUDIT_v0.30.6_WORKSTREAM_PLAN.md).
The plan supersedes nothing; it is the single source of truth for
WS-AN sub-task IDs, cascade estimates, and closure criteria.

### AN0-C — Branch policy

AN0 is captured on `claude/phase-an0-preflight-9VECN` (this commit).
Subsequent phases AN1..AN10 follow the task-description branch naming
convention and land per `docs/audits/AUDIT_v0.30.6_WORKSTREAM_PLAN.md`
§3. No production branches force-pushed during AN0.

### Documentation synchronization (AN0 scope only)

- `docs/WORKSTREAM_HISTORY.md`: new WS-AN entry (plan, baseline, scope,
  11-phase breakdown, gate-at-AN0-tip) inserted BEFORE the WS-AK
  entry. "What's next" section updated to record WS-AN as active.
- `CLAUDE.md` "Active workstream context": new WS-AN AN0 entry
  prepended before the AK10 closure paragraph.
- `docs/gitbook/navigation_manifest.json`: "Active workstream" bullet
  and `current_workstream_doc` updated to point at the WS-AN plan;
  `docs/gitbook/README.md` regenerated via
  `scripts/generate_doc_navigation.py`.
- `docs/gitbook/05-specification-and-roadmap.md`: "Active workstream"
  table row refreshed.
- `docs/gitbook/07-testing-and-ci.md`: opening paragraph updated to
  reference WS-AN AN0 and the baseline.
- `CHANGELOG.md`: this entry (AN0) prepended before the AK10 entry.

No code paths changed; no `sorry`/`axiom`/`native_decide` regression;
all tier scripts remain PASS at 0.30.6.
