...
the WS-SM SMP multi-core completion workstream.  Twelve sub-tasks
landed in one cut, providing the operational ARMv8.1-A LSE memory
...
New inductive `SeLe4n.Kernel.Concurrency.MemoryOrder` with 5
constructors mapping to the ARMv8.1-A / C++20 memory-order tags:
`.relaxed`, `.acquire`, `.release`, `.acqRel`, `.seqCst`.  Two
Bool selectors (`isAcquire`, `isRelease`) gate the
`synchronizesWith` discipline.  Five lookup witnesses
(`acquire_isAcquire`, `release_isRelease`, `acqRel_both`,
`seqCst_both`, `relaxed_neither`) pin the per-constructor
behaviour at `rfl`.

`DecidableEq`, `Repr`, `Inhabited` derived.

### SM2.A.2 — `AtomicLocation` struct

New structure `SeLe4n.Kernel.Concurrency.AtomicLocation` (single
`id : Nat` field, derived `DecidableEq`).  Three concrete
encoding helpers reserve a small fixed set of slots per lock
instance: `nextTicketOf base = ⟨base⟩`,
`servingOf base = ⟨base + 1⟩` (two slots per TicketLock),
`rwLockStateOf base = ⟨base⟩` (one slot per RwLock).  The
`ticketLock_fields_distinct` witness proves the encoding is
non-aliasing.

### SM2.A.3 — `MemoryEvent` structure

New structure `SeLe4n.Kernel.Concurrency.MemoryEvent` with 6
fields: `core : CoreId`, `loc : AtomicLocation`, `isWrite : Bool`,
`order : MemoryOrder`, `value : Nat`, `seqNum : Nat`.  All fields
have `DecidableEq`, so the structure derives `DecidableEq`
automatically (which in turn gives `LawfulBEq` via the standard
Lean 4 instance).  RMW atomics are modelled as two events with
the same `seqNum`.

### SM2.A.4 — `MemoryTrace` + `empty` + `append`

New structure `SeLe4n.Kernel.Concurrency.MemoryTrace` with a
single field `events : List MemoryEvent`.  Two constructors:
`.empty` (seed) and `.append e` (extension).  Three structural
witnesses: `empty_events` (empty is `[]`), `append_events`
(append extends at tail), `append_length` (append increments
length by 1).

### SM2.A.5 — `wellFormed` + `eventPos`

`MemoryTrace.wellFormed` is the conjunction of two clauses:
...
not in the trace).  Four bridging properties:

- `eventPos_lt_length`: in-trace events have a valid index.
- `eventPos_eq_length_of_not_mem`: out-of-trace events return
  the sentinel.
- `eventPos_get_eq`: `events.get` at the canonical index returns
  the original event (uses `LawfulBEq.eq_of_beq` to lift from
  BEq to structural equality).
- `eventPos_inj`: positions uniquely identify in-trace events.

### SM2.A.6 — `synchronizesWith`

`synchronizesWith t e_R e_A` is the 9-conjunct relation per ARM
ARM B2.3.7 — both endpoints in the trace, `e_R` is a release-
store (`isWrite=true ∧ order.isRelease=true`), `e_A` is an
acquire-load (`isWrite=false ∧ order.isAcquire=true`), same
location, observed value equals released value, and `e_R`
precedes `e_A` in trace order (via `eventPos`).

Two rejection witnesses (`_relaxed_load_rejected`,
`_relaxed_store_rejected`) prove that relaxed-tagged events
cannot participate in a sync edge.

### SM2.A.7 — `sequencedBefore` + `happensBefore`

`sequencedBefore t e₁ e₂` is the 4-conjunct relation — both
endpoints in the trace, same core, `e₁.seqNum < e₂.seqNum`.

`happensBefore t` is an inductive type with three constructors:
`.seq` lifts sequenced-before to hb, `.sync` lifts
synchronizes-with to hb, `.trans` composes two hb edges.

Two foundational lemmas:

- `happensBefore_in_trace`: every hb edge has both endpoints in
  the trace.
- `happensBefore_strict_positional`: every hb edge strictly
  increases trace position under wellFormed.  This is the
  inductive invariant that drives both irreflexivity and
  antisymmetry.

### SM2.A.8 — `happensBefore_irreflexive`

Theorem 3.1.8.1: for any wellFormed trace and any event,
`¬ happensBefore t e e`.  Proved via the strict-positional
invariant and `Nat.lt_irrefl`.

### SM2.A.9 — `happensBefore_transitive`

Theorem 3.1.8.2: hb is closed under composition.  Immediate from
the `.trans` constructor of `happensBefore`.

### SM2.A.10 — `happensBefore_antisymmetric`

Theorem 3.1.8.3: distinct events cannot be mutually hb-related.
Proved via the strict-positional invariant and `Nat.lt_asymm`.

### SM2.A.11 — `happens_before_partial_order` (aggregate)

Theorem 3.1.8: hb is a partial order.  Combines irreflexivity,
transitivity, antisymmetry into a single statement.  The
canonical surface anchor for SM2.B / SM2.C release-acquire
pairing.  Plus `happens_before_strict_partial_order`
(kernel-convenient form) and `happensBefore_no_cycle`
(smoke-test form).
...
- The proofs use a single foundational helper
  (`pairwise_get_lt`, private) that lifts `List.Pairwise R l` to
  per-position ordering `R l[i] l[j]` for `i < j`.  Proved by
...
WS-SM SM1.I (miscellaneous HAL improvements) landed in one cut on
branch `claude/review-hal-improvements-jwTzB`, completing the SM1
Rust HAL surface with the SM5 landing seams + cross-core test
infrastructure required for the SM5+ per-core scheduler integration.

**WS-SM SM1 CLOSED at v0.31.8** — all nine sub-phases LANDED
(SM1.A through SM1.I; 61 total sub-tasks across SM1.A=8, SM1.B=7,
SM1.C=12, SM1.D=6, SM1.E=5, SM1.F=8, SM1.G=4, SM1.H=5, SM1.I=6).

### Six SM1.I sub-tasks

#### SM1.I.1 — Per-core IRQ handler entry

New function `trap.rs::handle_irq_per_core` is the SM5 landing
seam.  Reads the calling core's id via TPIDR_EL1
(`per_cpu::current_core_id_from_tpidr`), records per-core IRQ
dispatch / timer-tick / SGI statistics via
`per_cpu_stats::record_*`, then dispatches via `gic::dispatch_irq`
with per-core attribution in the unhandled-INTID log line.

At SM1.I the assembly entry still calls `handle_irq` (single-core
legacy entry); both functions have identical
`extern "C" fn(&mut TrapFrame)` signatures so SM5's swap of the
assembly entry is a single function-pointer change.  SGI dispatch
increments the per-core counter but does NOT route through the
SGI handler table — `gic::dispatch_irq` discards the source-CPU
bits of GICC_IAR; SM5 will refactor `dispatch_irq` to preserve
the full IAR and route SGIs through `gic::dispatch_sgi`.

New build-script regression scanner
...
`current_core_id_from_tpidr` read at elaboration time so the SM5
swap cannot regress silently.

#### SM1.I.2 — Per-core IRQ priority masking

GICC_PMR is banked per-core at hardware level (GIC-400 TRM
§4.4.2 / §3.1.4) — each core's banked view is reached via the
same MMIO base address.  No software lock or per-core MMIO
address required.  Documentation in `gic.rs` module header
covers: PMR per-core banking, distributor banking for INTIDs
0..31 (GICD_IGROUPR0, GICD_IPRIORITYR0..7, GICD_ICPENDR0,
GICD_ISENABLER0 — explains why `init_cpu_interface_secondary`
must run on every secondary), and DAIF.I per-PE scoping (ARM ARM
C5.2.5).  Cross-referenced in `interrupts.rs` covering DAIF +
PMR composition (two layers of per-core IRQ masking, both
inherently per-PE).

#### SM1.I.3 — Per-core IDLE thread Rust stub

Per-core idle-wait primitives at three layers:
- Rust: `cpu::idle_wait()` (unbounded WFE) and
  `cpu::idle_wait_bounded(max_ticks)` (bounded variant wrapping
  `cpu::wfe_bounded`).
- FFI: `ffi_idle_wait()` and
  `ffi_idle_wait_bounded(max_ticks: u64) -> u64`.
- Lean: `Concurrency.Runtime.idleWait : BaseIO Unit` and
  `Concurrency.Runtime.idleWaitBounded : UInt64 → BaseIO UInt64`.

At SM1.I.3 the Lean kernel has no per-core idle TCB to call
these from; SM5 will wire the idle-thread body to invoke them.
Tier-3 surface scanner pins both FFI exports and Lean typed
wrappers so the SM5 wiring cannot regress.

#### SM1.I.4 — Per-core exception statistics

New module `rust/sele4n-hal/src/per_cpu_stats.rs` (~550 LoC)
introducing `PerCpuStats` struct (`#[repr(C, align(64))]` — one
cache line per slot, eliminates false sharing) with six AtomicU64
counters (`irq_count`, `timer_tick_count`, `sgi_count`,
`syscall_count`, `vmfault_count`, `user_exception_count`) +
2-slot reserved tail for SM5+ growth.  Compile-time `const _: ()`
assertions pin `size_of::<PerCpuStats>() == 64` and
`align_of::<PerCpuStats>() == 64`.  `PER_CPU_STATS` global array
with one slot per core.

Write path: `record_*` functions (Relaxed atomics, wait-free)
called from `handle_irq_per_core` (SM1.I.1) and from
`handle_synchronous_exception` per EC branch.  Read path:
`*_count_for(core_id)` Relaxed-snapshot accessors with defensive
out-of-range return-0 contract.  Inner forms (`record_*_in_slice`)
for unit-testing cross-core scenarios without racing on the
global static.  Aggregate `total_irq_count` /
`total_syscall_count` helpers.

FFI exports: `ffi_per_core_irq_count(core_id)` + 3 siblings.
Lean typed wrappers: `Concurrency.perCoreIrqCount(core : CoreId)`
+ 3 siblings.  Marker theorem
`perCoreIrqCount_returns_baseio_uint64_marker` as Tier-3 surface
anchor.

The synchronous exception handler is also wired to advance the
per-core counter per EC branch (SVC → syscall_count; DABT/IABT
→ vmfault_count; alignment / unknown EC → user_exception_count).

#### SM1.I.5 — SEV / WFE coordination documentation

Comprehensive module-header section in `cpu.rs` (~140 lines)
covering ARMv8-A local event register semantics (ARM ARM B2.10),
`wfe` (C6.2.353) / `sev` (C6.2.243) / `sevl` (C6.2.244)
instruction semantics, inner-shareable broadcast scope (BCM2712
= single-cluster = one IS domain), kernel policy for emitting
SEV (`bring_up_secondaries`, SM2+ lock release, cross-core SGI
fallback), when NOT to use SEV (memory ordering, interrupt
delivery, synchronisation primitive), bounded WFE interaction
with the local event register.

New `cpu::sev()` and `cpu::sevl()` wrappers around the underlying
instructions for testability (the production SEV emission still
lives inline in `smp.rs::bring_up_secondaries_inner` to keep it
co-located with the AN9-J.4.d broadcast point).
...
- `sm1i6_validate_context_id_per_core_dispatch_no_aliasing` —
  PSCI validator bijective dispatch.
- `sm1i6_cross_core_init_helper_idempotent` — per-core init
  helper composition is idempotent.
- `sm1i6_core_ready_flag_monotonic_across_repeated_bringup` —
  CORE_READY flags never false-revert.
- `sm1i6_per_core_stats_total_equals_sum` — aggregate sum
  matches per-slot deltas.
- `sm1i6_per_core_sgi_dispatch_kernel_reserved_intids` — every
  SM0.H reserved INTID dispatched per-core.
- `sm1i6_cross_core_bring_up_with_limit_boundary_progression` —
  limit boundary honoured at every value.
- `sm1i6_full_cross_core_composition` — end-to-end bring-up +
  validator + stats accumulation composes without
  contamination.
...
- 8 in `cpu::tests` for SM1.I.3 + SM1.I.5 wrappers.
- 17 in `ffi::tests` for the new FFI exports.
- 8 in `smp::tests::sm1i6_*` cross-core scenarios.
- 1 deferred (sm1g3 cross-core stress) in `uart::tests`.

Lean-side: 12 new surface anchors + runtime decidable examples
in `tests/SmpFoundationsSuite.lean` covering SM1.I.3 / SM1.I.4
FFI binding well-formedness and Lean typed wrapper structural
witnesses.
...
Two pre-existing parallel-test races surfaced under the SM1.I
expansion (more tests touching shared globals + more parallel
cargo workers).  Both fixed with private `std::sync::Mutex`
serialization within the test profile.  Stress-testing
...
- **UART_LOCK observation tests** (`sm1g4_kprintln_core_*_acquires_lock_*`)
  ...
  this would deadlock-spin and cargo's test timeout surfaces
  the regression.  Plus `SM1G4_OBSERVATION_MUTEX` serializes
  the observation tests against each other.
- **TIMER_INTERVAL / TICK_COUNT tests** wrapped in a private
...
info-flow → release closure) follow per the master overview
...
SMP-M4 (TPIDR_EL1 per-CPU base register) at the Lean ↔ Rust seam.
Pre-SM1.B the `PerCpuData` struct existed as an empty SM0.N stub
(`_reserved: [u64; 8]`) in `smp.rs`, accessible only through the
`per_cpu_slot_addr(context_id)` helper; post-SM1.B the struct
carries a populated `core_id: u64` field, is reachable from
hardware paths via `current_per_cpu()` / `current_core_id_from_tpidr()`
...
### Added — Per-CPU data block module (SM1.B.1, SM1.B.2)

`rust/sele4n-hal/src/per_cpu.rs` (NEW FILE, 650+ LoC):

- **SM1.B.1**: `PerCpuData` struct (`#[repr(C, align(64))]`, 64 bytes
  total).  First field `core_id: u64` (populated, replacing the
  SM0.N `_reserved: [u64; 8]` placeholder); reserved tail
  `_reserved: [u64; 7]` reserved for SM5+ scheduler state.  Two
  const constructors: `new(core_id)` (production initialiser) and
  `zero()` (back-compat alias for `new(0)`).
- **SM1.B.2**: `PER_CPU_DATA: [PerCpuData; MAX_SECONDARY_CORES + 1]`
  static array with each slot pre-populated:
  `PerCpuData::new(0..3)`.  Asm-visible
  `PER_CPU_DATA_SLOT_SIZE` (= 64) and
  `PER_CPU_DATA_SLOT_SIZE_SYM` (`#[no_mangle] static u64`) preserved
  for `boot.S::secondary_entry`'s `madd` stride.  Three
  compile-time `const _: ()` assertions pin the layout:
  `size_of::<PerCpuData>() == PER_CPU_DATA_SLOT_SIZE`,
  `align_of::<PerCpuData>() == 64`, and
  `PER_CPU_DATA.len() == MAX_SECONDARY_CORES + 1`.
- `crate::smp` re-exports `PerCpuData`, `PER_CPU_DATA`,
  `PER_CPU_DATA_SLOT_SIZE*`, `per_cpu_slot_addr`,
  `current_per_cpu`, `current_core_id_from_tpidr`, and
  `check_per_cpu_invariants` so existing callers via the legacy
  module path keep compiling.

### Added — TPIDR_EL1 accessors (SM1.B.3, SM1.B.4)

`rust/sele4n-hal/src/per_cpu.rs`:

- **SM1.B.3**: `current_per_cpu() -> &'static PerCpuData` — reads
  `TPIDR_EL1` on aarch64 (`mrs xN, tpidr_el1` with
  `nomem, nostack, preserves_flags`) and returns a static-lifetime
  reference into `PER_CPU_DATA`.  Host stub returns
  `&PER_CPU_DATA[0]`.  Safety invariants documented inline (EL1
  reachability, TPIDR_EL1 pre-set by boot code, pointer validity).
- **SM1.B.4**: `current_core_id_from_tpidr() -> u64` — fast
  core-id lookup via `current_per_cpu().core_id`.  Preferred over
  the MPIDR + mask path on hot kernel paths (saves the affinity
  mask post-MRS).  Host stub returns 0.

### Added — Lean FFI bridge (SM1.B.5)

- `rust/sele4n-hal/src/ffi.rs`: new
  `#[no_mangle] pub extern "C" fn ffi_current_core_id() -> u64`
  routing through `per_cpu::current_core_id_from_tpidr`.
- `SeLe4n/Platform/FFI.lean`: new
  `@[extern "ffi_current_core_id"] opaque ffiCurrentCoreId : BaseIO UInt64`.
- `SeLe4n/Kernel/Concurrency/Runtime.lean` (NEW FILE, 100+ LoC):
  typed Lean-side wrapper `Concurrency.currentCoreId : BaseIO CoreId`
  with the standard
  `if h : raw.toNat < numCores then ⟨raw.toNat, h⟩ else panic! "..."`
  discipline.  Out-of-range falls back to `panic!`; unreachable on
  hardware under the post-boot invariants enforced by
  `check_per_cpu_invariants`.  Surface-anchor theorem
  `currentCoreId_in_range_marker : ∀ (c : CoreId), c.val < numCores`.
- `SeLe4n/Kernel/Concurrency/Types.lean`: new
  `instance : Inhabited CoreId := ⟨bootCoreId⟩` instance so the
  `panic!` in `currentCoreId` discharges the typeclass obligation.

### Added — Boot-time invariant gate (SM1.B.6)

`rust/sele4n-hal/src/per_cpu.rs`:

- **SM1.B.6**: `check_per_cpu_invariants()` runtime gate — iterates
  `PER_CPU_DATA` and panics if any slot's `core_id` field disagrees
  with its array index.  Catches a regressed const-init table at
  boot rather than at first SMP wakeup.
- `rust/sele4n-hal/src/boot.rs::rust_boot_main`: invokes
  `check_per_cpu_invariants()` in Phase 4 (immediately before the
  `TPIDR_EL1` write) so the invariant is verified before any
  consumer reads it.  Also `kprintln`s the live
  `current_core_id_from_tpidr()` post-TPIDR-set for boot-log
  diagnostics on hardware.

### Added — Test coverage (SM1.B.7)

- `rust/sele4n-hal/src/per_cpu.rs::tests` (NEW, 30 unit tests; 10
  migrated from the SM0.N `smp::tests::sm0n_*` block under
  `sm1b_*` names with expanded coverage, 15 newly authored at
  SM1.B landing for SM1.B-specific functionality, 5 added at
  audit-pass-2 for the `check_per_cpu_invariants_in` inner form
  and its panic path): struct alignment + size,
  ...
  first-slot-wrong, zero-default-regression), pairwise-distinct +
  canonical-range cross-checks on `core_id`, accessor agreement
  with `per_cpu_slot_addr`.
...
- New `#check`s: `Platform.FFI.ffiCurrentCoreId`,
  `Concurrency.currentCoreId`,
  `Concurrency.currentCoreId_in_range_marker`,
  `Concurrency.instInhabitedCoreId`.
- New `example` discharge:
  `(default : CoreId).val = 0` (Inhabited default = bootCoreId),
  `(default : CoreId) = bootCoreId`,
  range marker for `bootCoreId`,
  range marker on every `CoreId` via `c.isLt`.
- New runtime section §2.16 `runCurrentCoreIdChecks`:
  asserts `Inhabited CoreId default = bootCoreId`,
  `Inhabited CoreId default.val = 0`, range marker discharges on
  `bootCoreId`, range marker discharges on every element of
  `allCores`.
...
`Concurrency.Runtime` is in the production import closure via
`SeLe4n/Platform/Staged.lean` (added to
`scripts/staged_module_allowlist.txt` per the WS-RC R12.B
partition gate).  SM5 will move it from staged →
production-reached when per-core scheduler state lands.

### FFI-link discipline note

Per the project's fail-closed FFI convention
(`Platform/FFI.lean` header docstring), the Lean test executables
do NOT link against `libsele4n_hal.a`.  Any path that invokes an
`@[extern] opaque` symbol from host code surfaces as a link error
rather than a silent stub call.  The SmpFoundationsSuite test
therefore exercises only the structural properties of
`currentCoreId` (typed signature, marker theorem, Inhabited
default).  The runtime behaviour of the host stub is covered
exhaustively by the Rust `per_cpu::tests` and `ffi::tests`
