...
- **WS-SM SMP multi-core completion workstream IN FLIGHT (v0.31.2 → v0.31.3 → v0.32.x → v1.0.0,
  ...
  **WS-SM SM1.B LANDED on branch
  ...
  - **SM1.B.7**: 25 unit tests in `per_cpu::tests` (10 migrated
    ...
    with expanded coverage, 15 newly authored for SM1.B-specific
    functionality): struct alignment + size, const-constructor
    `new` and `zero` semantics, byte-level zero discharge for the
    reserved tail, array layout/stride/distinct-addresses,
    asm-stride observability via `PER_CPU_DATA_SLOT_SIZE_SYM`,
    out-of-range panic, `current_per_cpu` returns boot slot on
    host and points inside `PER_CPU_DATA` at a cache-line
    boundary, `current_core_id_from_tpidr` returns 0 on host
    and is in-range, `check_per_cpu_invariants` passes on the
    production initialiser, pairwise-distinct + canonical-range
    cross-checks on `core_id`, accessor agreement with
    `per_cpu_slot_addr`.  3 new tests in `ffi::tests`
    (`ffi_current_core_id` host return 0, range invariant,
    agreement with the per-CPU accessor).  4 back-compat tests
    in `smp::tests` (replacing the 11 sm0n_* tests that
