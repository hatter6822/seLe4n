...
- **WS-AK Phase AK1 COMPLETE** (v0.29.1): Pre-1.0 Release Hardening (v0.29.0 audit) — Phase AK1: IPC Fail-Closed & Rendezvous State. 10 sub-tasks (AK1-A through AK1-J) addressing 2 HIGH, 7 MEDIUM, 6 LOW findings. AK1-A (I-H01/HIGH): `cleanupPreReceiveDonationChecked` error-propagating variant added in `IPC/Operations/Endpoint.lean`; `endpointReceiveDual` no-sender path uses checked variant; bridge lemma `cleanupPreReceiveDonationChecked_ok_eq_cleanup` + 13 preservation theorems updated via outer-case-split pattern. AK1-B (I-H02/HIGH): `endpointReply`/`endpointReplyRecv` fail closed with `.replyCapInvalid` on `replyTarget = none`; soundness bridge `blockedOnReplyHasTarget_implies_some_replyTarget` added. AK1-C (I-M01/MEDIUM): caller-side pendingMessage atomically cleared via `storeTcbIpcStateAndMessage caller (.blockedOnReply _ _) none` on rendezvous (cascaded through 5 preservation theorems in CallReplyRecv.lean). AK1-D (I-M02/MEDIUM): receiver-side atomic `(.ready, senderMsg)` update via `storeTcbIpcStateAndMessage receiver .ready senderMsg` in `endpointReceiveDual` rendezvous — preserves `waitingThreadsPendingMessageNone` structurally; cascaded through 12 preservation theorems (4 in EndpointPreservation, 7 in Structural, 1 NI); new helpers `storeTcbIpcStateAndMessage_tcb_forward` + `storeTcbIpcStateAndMessage_ready_preserves_ipcSchedulerContractPredicates`; `hReceiverNotBlocked` hypothesis dropped. AK1-E (I-M03/MEDIUM): `ensureRunnable` uses `ipcEffectiveRunQueuePriority` — fixes PIP-boosted server placement on wake paths; new correctness lemmas `ensureRunnable_inserts_at_effective_priority`, `ensureRunnable_honors_pipBoost`, `notificationSignal_respects_pipBoost`. AK1-F (I-M04/MEDIUM): `timeoutThread` PIP-revert call-state gap formalised via `blockingServer_isSome_iff_blockedOnReply_some` + `blockingServer_some_implies_blockedOnReply` theorems in `BlockingGraph.lean`. AK1-G (I-M05/MEDIUM): `ipcUnwrapCapsLoop` annotated as internal. AK1-H (I-M06/MEDIUM): composition lemma `endpointQueueRemove_succeeds_under_forwardBackward` composed at call site via `timeoutThread_succeeds_under_preconditions`. AK1-I (I-M07/MEDIUM, NI L-1): send-path missing CSpace root now fails closed with `.invalidCapability`; NI regression test in `tests/InformationFlowSuite.lean`. AK1-J (I-L1..I-L6, IPC INFO): LOW-tier batch documentation at top of `IPC/Operations/Endpoint.lean`. Test: CIC-010 scenario updated to explicit `replyTarget := some replierTid`. Gate: `lake build` (256 jobs) + `test_smoke.sh` + `test_full.sh` + zero sorry/axiom. See `docs/audits/AUDIT_v0.29.0_WORKSTREAM_PLAN.md`
