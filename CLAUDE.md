...
Lean 4.28.0 toolchain, Lake build system, version 0.29.10.
...
- **AK6-F substantive closure — incremental (v0.29.10)**: Opens the per-op discharge of `dispatchCapabilityOnly_preserves_projection`. Three new building blocks: (1) `objects_insert_preserves_projection_high` (Operations.lean) — the universal direct-insert frame lemma, analog of `storeObject_preserves_projection` for ops that mutate `st.objects` without going through the full `storeObject` pipeline (e.g., `updatePrioritySource`, `schedContextBind`); enables discharge of every arm whose underlying op uses `{ st with objects := st.objects.insert … }`. (2) `setIPCBufferOp_preserves_projection` — the first fully-proven per-op preservation theorem for a previously-undischarged cap-only arm (`.tcbSetIPCBuffer`); uses `storeObject_preserves_projection` after validation. (3) Two new frame helpers in `Projection.lean` — `projectState_replenishQueue_eq` (mutating only `scheduler.replenishQueue` preserves projection) and `projectState_scheduler_current_cleared_when_high` (clearing `scheduler.current` when the previous current was non-observable preserves projection). Remaining arms (`.cspaceDelete`, `.lifecycleRetype`, `.vspaceMap`, `.vspaceUnmap`, `.serviceRevoke`, `.serviceQuery`, `.schedContextConfigure/Bind/Unbind`, `.tcbSuspend/Resume`, `.tcbSetPriority/MCPriority`) are tracked as continuation work in `docs/audits/AUDIT_v0.29.0_WORKSTREAM_PLAN.md` §9 AK6-F.2/F.3. `dispatchCapabilityOnly_preserves_projection` retains the structural bridge form with `hArmProj` hypothesis; docstring updated with per-arm discharge table pointing callers at the new building blocks. Gate: `lake build` (260 jobs) + `test_smoke.sh` + `check_version_sync.sh` + zero sorry/axiom.
