...
Lean 4.28.0 toolchain, Lake build system, version 0.23.19.
...
SeLe4n/Kernel/SchedContext/*      Scheduling context types, CBS budget engine, replenishment queue (Z1–Z3)
...
- **WS-U Phase U8 COMPLETE**: Documentation & Closure — 8 sub-tasks (U8-A through U8-H), all complete (v0.21.7). **WS-U PORTFOLIO COMPLETE.** See `docs/dev_history/audits/AUDIT_v0.20.7_WORKSTREAM_PLAN.md`
- **WS-U Phase U7 COMPLETE**: Dead Code & Proof Hygiene — 12 sub-tasks (U7-A through U7-L), all complete (v0.21.6). See `docs/dev_history/audits/AUDIT_v0.20.7_WORKSTREAM_PLAN.md`
- **WS-U Phase U6 COMPLETE**: Architecture & Platform Fidelity — 12 sub-tasks (U6-A through U6-L), all complete (v0.21.5). See `docs/dev_history/audits/AUDIT_v0.20.7_WORKSTREAM_PLAN.md`
- **WS-U Phase U5 COMPLETE**: API & Dispatch Integrity — 14 sub-tasks (U5-A through U5-N), all complete (v0.21.4). See `docs/dev_history/audits/AUDIT_v0.20.7_WORKSTREAM_PLAN.md`
- **WS-U Phase U4 COMPLETE**: Proof Chain & Invariant Composition — 3 sub-task groups (U4-A/B/C/D, U4-K, U4-N), all complete (v0.21.3). See `docs/dev_history/audits/AUDIT_v0.20.7_WORKSTREAM_PLAN.md`
- **WS-U Phase U3 COMPLETE**: Rust ABI Hardening — 10 sub-tasks (U3-A through U3-J), all complete (v0.21.2). See `docs/dev_history/audits/AUDIT_v0.20.7_WORKSTREAM_PLAN.md`
...
- **WS-W PORTFOLIO COMPLETE**: Pre-Release Audit Remediation — 6 phases (W1–W6, 52 sub-tasks), all complete (v0.22.11–v0.22.17). See `docs/dev_history/AUDIT_v0.22.10_WORKSTREAM_PLAN.md`
- **WS-W Phase W6 COMPLETE**: Code Quality & Documentation — 12 sub-tasks (W6-A through W6-L), all complete (v0.22.17). W6-A: TCB existence invariant documented. W6-B: Factored redundant lifecycle preservation proofs (bundled theorems). W6-C: Removed unused `crossSubsystemPredicates`. W6-D: Two-tier API dispatch documented. W6-E: `default_objects_none`/`default_objects_absurd` helpers extracted and applied across 24+ default-state proofs. W6-F: `RHSet.invExtK` public API wrappers. W6-G: Compile-time Lean-Rust constant sync assertions. W6-H: `set_mr`/`get_mr` asymmetry documented. W6-I: CDT edge theorem suite documented. W6-J: 14 RHTable spec surface theorems documented. W6-K: Lifecycle enforcement chain documented (`cleanupTcbReferences`/`cleanupEndpointServiceRegistrations`/`detachCNodeSlots`). W6-L: Documentation sync. Zero sorry/axiom. See `docs/dev_history/AUDIT_v0.22.10_WORKSTREAM_PLAN.md`
- **WS-W Phase W5 COMPLETE**: Test Infrastructure & Coverage — 8 sub-tasks (W5-A through W5-H), all complete (v0.22.16). W5-A: Consolidated 3 duplicate `expectError` implementations into shared `Helpers.lean` with `msgPrefix` parameter. W5-B: Documented 11-suite OID range table. W5-C: chain33 service lifecycle tests (25 assertions). W5-D: `buildChecked` error reporting with check numbers. W5-E: Mutation testing assertions for chain1/chain5. W5-F: Removed unused `_hCap` parameter. W5-G: `resolveExtraCaps` silent-drop documentation. Zero sorry/axiom. See `docs/dev_history/AUDIT_v0.22.10_WORKSTREAM_PLAN.md`
- **WS-W Phase W4 COMPLETE**: Platform & Architecture Hardening — 7 sub-tasks (W4-A through W4-G), all complete (v0.22.15). W4-A: BCM2712 datasheet validation checklist completed (14 constants validated). W4-B: FDT parsing hardened (bounds checks in `readBE32`, `readCString`, `readCells`). W4-C: 3 `native_decide` → `decide` in Board.lean (TCB reduction). W4-D: stale `@[implemented_by]` comment fixed. W4-E: `bootFromPlatformUnchecked` deprecated via docstring. W4-F: MMIO formalization gap documented. W4-G: `encodeMsgRegs` already removed in W3-H. Zero sorry/axiom. See `docs/dev_history/AUDIT_v0.22.10_WORKSTREAM_PLAN.md`
- **WS-W Phase W3 COMPLETE**: Dead Code Elimination — 8 sub-tasks (W3-A through W3-H), all complete (v0.22.14). Verification-first methodology with Category A/B/C classification. Removed ~40 dead definitions across foundation, kernel, architecture, info flow, data structures, and testing layers. Entire fast-projection cluster (~200 lines), 7 monad helpers, 8 alignment predicates, 9 pow2 witnesses, 4 CDT helpers, `resolveCapAddressK`, `severDerivationEdge`, `maxServiceFuel`, `deleteObject`, `encodeMsgRegs`. FrozenOps retained with architectural status documentation. Zero sorry/axiom. See `docs/dev_history/AUDIT_v0.22.10_WORKSTREAM_PLAN.md`
- **WS-W Phase W2 COMPLETE**: Proof Formalism & Architecture — 8 sub-tasks (W2-A through W2-H), all complete (v0.22.12). W2-A: field-disjointness formalism closure (3 frame lemmas + modifiedFields + documentation theorem). W2-B: composition gap documentation. W2-C: `dispatchWithCap_wildcard_unreachable`. W2-D: collectQueueMembers fuel sufficiency. W2-E: serviceHasPathTo fuel exhaustion. W2-F: serviceCountBounded preservation chain. W2-G: enforcementBoundaryExtended unified as abbrev. W2-H: maxHeartbeats documentation. Zero sorry/axiom. See `docs/dev_history/AUDIT_v0.22.10_WORKSTREAM_PLAN.md`
- **WS-W Phase W1 COMPLETE**: Critical Rust ABI Fixes — 9 sub-tasks (W1-A through W1-I), all complete (v0.22.11). See `docs/dev_history/AUDIT_v0.22.10_WORKSTREAM_PLAN.md`
...
- **WS-V Phase V7 COMPLETE**: Performance & Data Structure Optimization — 19 sub-tasks (V7-A through V7-J), all complete (v0.22.8). LawfulBEq explicit API-level requirement for RHTable (18-file ripple), filter_fold_present heartbeat 3.2M→400K, insertLoop preservation heartbeats 800K→420K via noDupKeys_after_set/distCorrect_after_set helpers, CNodeRadix.toList O(n²)→O(n), Boot noDup O(n²)→O(n) via HashSet, native_decide→decide in RegisterFile, general filter_preserves_key theorem. Zero sorry/axiom. See `docs/dev_history/AUDIT_v0.21.7_WORKSTREAM_PLAN.md`
- **WS-V Phase V6 COMPLETE**: Information Flow & Cross-Subsystem — 20 sub-tasks (V6-A through V6-L), all complete (v0.22.7). Cross-subsystem field-disjointness formalization (StateField enum, 10 pairwise disjointness/overlap witnesses, 2 frame lemmas), serviceCountBounded preservation (serviceCountBounded_of_eq, serviceCountBounded_monotone, serviceGraphInvariant_monotone), bibaIntegrityFlowsTo with corrected docstrings + integrityFlowsTo_is_not_biba documentation, LabelingContextValid predicate, ObservableState.serviceRegistry extension with full ripple fixes, enforcementBoundaryComplete_counts + enforcementBoundary_names_nonempty, endpointPolicyRestricted well-formedness, DeclassificationEvent audit trail with authorizationBasis field, kernelOperationNiConstructor 32-variant mapping + niStepCoverage_operational/injective/count, acceptedCovertChannel_scheduling, defaultLabelingContext_insecure warning, enforcementBoundaryExtended 22 entries. 26 V6 test cases. Zero sorry/axiom. See `docs/dev_history/AUDIT_v0.21.7_WORKSTREAM_PLAN.md`
- **WS-V Phase V5 COMPLETE**: Defensive Coding & Robustness — 16 sub-tasks (V5-A through V5-P), all complete (v0.22.5). Defense-in-depth hardening: safe DTB parsing, handleYield guard, configurable defaultTimeSlice, redundant CDT detach removal, frozen ops guards, checked context save/restore, domainTimeRemainingPositive invariant (8th conjunct of schedulerInvariantBundleFull with full preservation proofs), toObjIdVerified. Zero sorry/axiom. See `docs/dev_history/AUDIT_v0.21.7_WORKSTREAM_PLAN.md`
- **WS-V Phase V4 COMPLETE**: Platform & Hardware Fidelity — 26 sub-tasks (V4-A1 through V4-N), all complete (v0.22.3). Complete boot-to-runtime invariant bridge for general configs (`bootFromPlatform_proofLayerInvariantBundle_general`). Zero sorry/axiom. See `docs/dev_history/AUDIT_v0.21.7_WORKSTREAM_PLAN.md`
- **WS-V Phase V3 COMPLETE**: Proof Chain Hardening — 26 sub-tasks (V3-A through V3-M), all complete (v0.22.2). All 8 `True := trivial` documentation theorems replaced with real machine-checked proofs. Zero sorry/axiom/trivial. See `docs/dev_history/AUDIT_v0.21.7_WORKSTREAM_PLAN.md`
...
- **WS-Z Phase Z3 COMPLETE**: Replenishment Queue — 12 sub-tasks (Z3-A through Z3-L), all complete. ReplenishQueue structure (sorted list of SchedContextId × eligibleAt pairs with cached size), operations: empty, insert (sorted O(n)), popDue (prefix split O(k)), remove (filter O(n)), peek (O(1)), hasDue (O(1)). Invariants: pairwiseSortedBy (recursive adjacency predicate), replenishQueueSorted, replenishQueueSizeConsistent, replenishQueueConsistent (parameterized by object store lookup). Preservation theorems: insert_preserves_sorted, popDue_preserves_sorted, popDue_sizeConsistent, remove_preserves_sorted, filter_preserves_pairwiseSortedBy. Length/membership theorems: splitDue_length_additive, insertSorted_length, insert_sizeConsistent, remove_sizeConsistent, mem_insertSorted, subset_insertSorted, empty_sorted/sizeConsistent/consistent. Public helpers: pairwiseSortedBy_cons, pairwiseSortedBy_head_le_second, pairwiseSortedBy_tail, pairwiseSortedBy_nil/singleton. Private helpers: pairwiseSortedBy_head_le_all, insertSorted_head_ge, insertSorted_head_time_ge, filter_head_time_ge. Tier 3 anchors: 13 #check entries. New file: ReplenishQueue.lean. Zero sorry/axiom. See `docs/planning/WS_Z_COMPOSABLE_PERFORMANCE_OBJECTS.md`
...
- **WS-Z Phase Z8 COMPLETE**: API Surface & Syscall Wiring — 17 sub-tasks (Z8-A through Z8-N2), all complete (v0.23.17, audit v0.23.18). Complete public API surface for SchedContext operations. 3 error-exclusivity theorems (`decodeSchedContextConfigureArgs_error_iff`, `decodeSchedContextBindArgs_error_iff`, `decodeSchedContextUnbindArgs_error_iff`). 4 frozen operations (`frozenSchedContextConfigure`, `frozenSchedContextBind`, `frozenSchedContextUnbind`, `frozenTimerTickBudget`). `enforcementBoundary` expanded 22→25 entries (3 new `.capabilityOnly` SchedContext entries). `frozenOpCoverage_count` increased 12→15. 6 budget lifecycle trace scenarios (Z8J-001 through Z8J-006). 8 negative-state tests (Z8-L-01 through Z8-L-08). Audit fix (v0.23.18): AUD-1 `frozenSchedContextUnbind` silent fallback → explicit error. Zero sorry/axiom. See `docs/planning/WS_Z_COMPOSABLE_PERFORMANCE_OBJECTS.md`
- **WS-Z Phase Z9 COMPLETE**: Invariant Composition & Cross-Subsystem — 20 sub-tasks (Z9-A through Z9-P2), all complete (v0.23.19). 3 new cross-subsystem predicates (`schedContextStoreConsistent`, `schedContextNotDualBound`, `schedContextRunQueueConsistent`). `crossSubsystemInvariant` extended from 5 to 8 predicates. Field-disjointness analysis: 14 pairwise witnesses for 8 predicates. 3 frame lemmas. `proofLayerInvariantBundle` extended from 9 to 10 conjuncts (added `schedulerInvariantBundleExtended`). `bootSafeObject` extended with SchedContext wellFormedness constraint (6th conjunct). Boot-time proofs for all 10 bundle components. Operation preservation theorems for timerTick, schedule, donation, cleanup/destroy. Test fixture updated (`crossSubsystemFieldSets` 5→8). Zero sorry/axiom. See `docs/planning/WS_Z_COMPOSABLE_PERFORMANCE_OBJECTS.md`
- **WS-Z PORTFOLIO COMPLETE**: All 9 phases (Z1–Z9, 186+ sub-tasks) delivered. Composable Performance Objects — SchedContext types, CBS budget engine, replenishment queue, scheduler integration, capability-controlled binding, timeout endpoints, SchedContext donation, API surface, and invariant composition. See `docs/planning/WS_Z_COMPOSABLE_PERFORMANCE_OBJECTS.md`
- **Next milestone**: post-WS-Z planning / next audit cycle
