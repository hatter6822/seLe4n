...
Lean 4.28.0 toolchain, Lake build system, version 0.32.73.
...
- `SeLe4n/Model/State.lean` (~3902 lines)
...
- `docs/gitbook/12-proof-and-invariant-map.md` (~3112 lines)
...
- `SeLe4n/Platform/FFI.lean` (~1377 lines)
...
- `SeLe4n/Kernel/Architecture/TlbShootdown.lean` (~1171 lines)
...
- **WS-SM SMP multi-core completion workstream IN FLIGHT (v0.31.2 → v1.0.0)**:
  ...
  **Phase status** (current version: v0.32.73):
  ...
  **Rust HAL at v0.32.73**: 750 tests, zero clippy warnings,
  zero `#[ignore]`'d.
  ...
  **Staged modules**: 57 staged-only (via `Platform/Staged.lean` +
  ...
  extracted from the still-staged `TlbiForSharing`.)
