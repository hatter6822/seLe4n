...
Lean 4.28.0 toolchain, Lake build system, version 0.26.9.
...
- **WS-AG Phase AG8 COMPLETE** (v0.26.9): H3 Hardware Binding Audit Remediation — Phase AG8: Integration + Model Closure. 7 sub-tasks (AG8-A through AG8-G). AG8-A: Timeout sentinel → explicit `timedOut : Bool` TCB field (eliminates GPR x0 sentinel collision risk). AG8-B: Cache coherency model (`CacheModel.lean` — `CacheLineState`, `CacheState`, 5 operations, 8 preservation theorems). AG8-C: Memory barrier semantics (`BarrierKind`, `barrierOrdered`, 4 theorems in `MmioAdapter.lean`). AG8-D: FrozenOps deferred to WS-V (experimental status maintained). AG8-E: CDT `descendantsOf` fuel sufficiency (`maxCdtDepth := 65536`, 2 theorems). AG8-F: Donation chain k>2 cycle prevention (`donationChainAcyclic_general`, `blockedOnReply_cannot_call`). AG8-G: Donation atomicity under interrupt disable (`donationAtomicRegion`, `donateSchedContext_machine_eq`). Fixed pre-existing simp warnings in RPi5 `RuntimeContract.lean`/`ProofHooks.lean`. Gate: `lake build` (256 jobs) + `test_full.sh`. Zero sorry/axiom. See `docs/audits/AUDIT_H3_HARDWARE_BINDING_WORKSTREAM_PLAN.md`
...
- **Next milestone**: Raspberry Pi 5 hardware binding (WS-AG Phases AG9–AG10)
