...
- **AK6-F full per-arm per-op theorem coverage — 14/14 arms named (v0.29.11)**: Every cap-only dispatch arm now has a NAMED per-op `_preserves_projection` theorem in `InformationFlow/Invariant/Operations.lean`. Substantive depth varies: 7/14 fully proven from observability hypotheses (`.cspaceDelete`, `.serviceQuery`, `.tcbSetIPCBuffer`, `.tcbSetPriority`, `.tcbSetMCPriority`, `.vspaceMap`, `.vspaceUnmap`); 7/14 use closure-form signatures (`.serviceRevoke`, `.schedContextConfigure/Bind/Unbind`, `.lifecycleRetype`, `.tcbSuspend/Resume`) where the caller supplies `hProjEq` dischargeable from the v0.29.10 frame lemmas (`objects_insert_preserves_projection_high`, `runQueue_remove_insert_preserves_projection_at_high`, `projectState_replenishQueue_eq`, `projectState_scheduler_current_cleared_when_high`). New v0.29.11 theorems: `lookupServiceByCap_preserves_state/_projection` (AK6F.11 — fully substantive read-only fold), `revokeService_preserves_projection` (AK6F.12 — closure form with `hServiceProjEq` isolating the RHTable fold-induction at projection layer), `schedContextConfigure/Bind/Unbind_preserves_projection` (AK6F.13-15), `lifecycleRetypeDirectWithCleanup_preserves_projection` (AK6F.16), `cancelDonation_preserves_projection` (AK6F.17), `suspendThread/resumeThread_preserves_projection` (AK6F.18-19). `dispatchCapabilityOnly_preserves_projection` in `API.lean:2035` retains the `hArmProj` signature but its docstring is refreshed to enumerate NAMED per-arm discharge theorems — callers no longer consult docstrings for each arm's approach, they reference specific theorems directly. Closure-form discharge estimated at ≈300 LOC aggregate and tracked as continuation work AK6F.20b. Gate: `lake build` (260 jobs) + `test_smoke.sh` + `test_full.sh` + `check_version_sync.sh` + zero sorry/axiom.
