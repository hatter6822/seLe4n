...
Lean 4.28.0 toolchain, Lake build system, version 0.33.1.
...
- `docs/spec/SELE4N_SPEC.md` (~3920 lines)
- `SeLe4n/Kernel/Scheduler/Operations/Preservation.lean` (~3911 lines)
- `docs/gitbook/12-proof-and-invariant-map.md` (~3519 lines)
...
- `SeLe4n/Kernel/InformationFlow/ObservableStatePerCore.lean` (~1059 lines)
...
- `tests/SmpInformationFlowSuite.lean` (~813 lines)
...
- **WS-SM SMP multi-core completion workstream IN FLIGHT (v0.31.2 → v1.0.0)**:
  ...
  **Phase status** (current version: v0.33.1):
  ...
  | SM8.A | LANDED | v0.33.1 | Per-core observable state — the SMP information-flow **observer**.  New staged module `InformationFlow/ObservableStatePerCore.lean` (staged 54 → 55) layered on the SM4.D per-core projections; SM8.B's `crossCoreNonInterference` is the first consumer.  **A.1**: `PerCoreObserver` makes the plan's `(c, L)` pair (Def 3.1.1) a value rather than a two-argument convention, and `ObservableState.onCore` (Def 3.2.1) is *defined as* `projectStateOnCore ctx ⟨L⟩ s c` — one projection function, so the SM8 observer and the SM4.D layer cannot drift; `onCore_bootCore` is `rfl`, so every SM8 theorem at `bootCoreId` is a statement about the live single-core `projectState`, and `lowEquivalent_smp_iff_forall_observer` makes the SM6 cross-core NI theorems (already in `lowEquivalent_smp` form) statements about all per-core observers.  **A.2**: the thirteen `ObservableState` components partition into seven shared + six per-core, and the partition is **total by construction** — `ObservableState.ext_fragments` rebuilds an observable state from its two fragments, so a fourteenth field registered in neither is a compile error (the plan §7 risk "per-core projection missing a field", structurally closed).  Headline `onCore_isProjection_of_globalProjection` (the per-core observer learns exactly the global projection plus core `c`'s six slots) + `onCore_sharedFragment_core_independent` (the core dimension and the label dimension are orthogonal) + thirteen `@[simp]` component accessors.  **A.3**: observable-state equality is **not** decidable (five components are functions over unbounded domains; `machineRegs` carries a `RegisterFile` whose structural `BEq` is provably not lawful), so `onCore_decidable` decides a deliberately distinct *slice* relation, shipping both halves of its own limitation as theorems — `lowEquivalentSliceOnCore_of_lowEquivalentOnCore` (a decided mismatch is a genuine observable difference) and `perCoreSlice_erases_{register,shared}_content` (the converse fails on both halves of the partition).  **A.4**: `onCore_perCore_independence` characterises the read set — six shared components plus core `c`'s five scheduler slots and its register bank, nothing else.  It does **not** follow from the SM4.D `projectStateOnCore_congr`, whose hypothesis is equality of the whole *global* projection and therefore reads the **boot** core's slots; a cross-core transition on core `c'` generally breaks it when `c' = bootCoreId`, which is exactly SM8.B's case.  Twelve corollaries: the six per-core scheduler setters + `setRegsOnCore` at `c ≠ c'`, plus the components outside the read set entirely (replenishment queue, timeout log, `scThreadIndex`, the machine timer, `perCoreTlb`) — invisible on *every* core including the one written.  **A.5**: `onCore_label_monotone` over the new `ObservableState.visibilityLe` preorder, gate by gate from `securityFlowsTo_trans`; a *visibility* order rather than component equality, because a wider clearance may legitimately reveal more of an object it can already see (`projectCNode_lookup_monotone`), bounded by `projectKernelObject_observer_independent_off_cnode` (the CNode arm is the only one reading the observer).  `onCore_schedulingTransparency` restates accepted covert channel CC-1 per core — under SMP the channel exists once per core.  Substrate: the RobinHood filter-lookup characterisation was half-stated, so a monotone predicate change could not be transported through a filter; `RHTable.filter_getElem?_of_pred` + `_iff` complete it.  **A.6**: `tests/SmpInformationFlowSuite.lean` (83 anchors, 15 elaboration examples, **68 runtime assertions / 8 groups**) on a four-thread / four-core fixture with a non-trivial labeling (core 0 low, core 1 high), Tier-2 + Tier-3 wired, fixture OID band 1000–1013 registered.  Every group carries a load-bearing negative: the same write on the observer's *own* core does change its view (the `c ≠ c'` hypothesis is necessary), the high observer strictly outsees the low one on six components, two cores report different active domains, and a purely high remote reshuffle is invisible to the low observer on every core while the high observer's own view moves.  Zero sorry/axiom; trace byte-identical. |
  | SM8.B–SM8.E | PENDING | — | Per-core NI proofs (`crossCoreNonInterference`, the 32 per-constructor variants, CC-5 lock-contention), per-core declassification audit, information flow under fine locks, closure |
 ...
 **v0.32.99 PR #845 review round 3**: five findings, all consequences of this PR's own changes — (1) the page-alignment guard reached only two of the four checked map wrappers; both remaining ones guarded and `checkedMapWrappers_reject_unaligned` now pins all four to the same rejection (the duplication across wrappers is what let two drift).  (2) `checkedDispatch_capabilityOnly_eq_unchecked` was advertised as complete but enumerated 14 of 16 arms, omitting `.vspaceUnifyInstruction` and `.mintReplyCap`; both added plus their per-arm theorems.  (3) `.vspaceUnifyInstruction` was outside `lockSetTransitions_within_bound`, which supplies the size premise for bounded-wait/WCRT reasoning; `lockSet_vspaceUnifyInstruction_size_le` added.  (4) **the clean-to-PoU obligation was vacuous** — `kernelCodeWriteOwesPoUClean` ignored its site argument and asserted only barrier-list membership, mentioning neither `DC CVAU` nor any instruction invalidate, so it stayed provable with every clean absent; it is now a `PoUCleanObligation` naming the concrete per-site operations (`DCacheMaintenance.isClean` rejects invalidate-only, which would discard the bytes just written), with `kernelCodeWriteSites_emission_pending` recording as a decidable fact that the obligation is declared rather than discharged.  (5) the `sele4n-sys` wrapper documented `AsidNotBound` for an unbound ASID, but the capability binding returns `IllegalAuthority` first (deliberately — no ASID-existence oracle); contract corrected.
  ...
  **Staged modules**: 55 staged-only (via `Platform/Staged.lean` +
