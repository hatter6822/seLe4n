...
- **WS-AH Phase AH1 COMPLETE** (v0.27.2): Pre-Release Comprehensive Audit Remediation — Phase AH1: Critical IPC Dispatch Correctness. 5 sub-tasks (AH1-A through AH1-E). AH1-A (H-01/HIGH): `endpointSendDualChecked` delegates to `endpointSendDualWithCaps` (was `endpointSendDual`) — checked `.send` performs IPC capability transfer, 3 new params, return type `Kernel CapTransferSummary`. AH1-B: Updated `dispatchWithCapChecked` `.send` arm. AH1-C: 7 NI theorems updated (Wrappers, Soundness, Operations). AH1-D (M-01/MEDIUM): `validateVSpaceMapPermsForMemoryKind` wired into `.vspaceMap` dispatch — device memory execute permission blocked. AH1-E: 10 test call sites updated. Gate: `lake build` (256 jobs) + `test_smoke.sh` + `test_full.sh`. Zero sorry/axiom. See `docs/audits/AUDIT_v0.27.1_WORKSTREAM_PLAN.md`
