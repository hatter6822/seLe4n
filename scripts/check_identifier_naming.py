...
NO_CONTENT_SCAN = {
    ...
    ".hex": ("a device-tree blob rendered as hex by scripts/generate_dtb_corpus.py; "
             "its code is hex digits and its comments are the generator's "
             "annotations, so the names in it are the generator's, scanned there"),
}
...
EXTENSIONLESS_STRIPPERS = {
    ...
    # `tests/fixtures/dtb/MANIFEST`: `#` comments; the first column is the
    # corpus's fixture names, which are maintained names.
    "MANIFEST": strip_config,
}
