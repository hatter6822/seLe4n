...
run_check "DOC" rg -n '(^- \*\*Current completed slice:\*\* post-M7 comprehensive-audit execution portfolio \(WS-B1 through WS-B11 complete\)\.$)|(^- \*\*Current active slice:\*\* comprehensive-audit v0\.9\.32 WS-C execution kickoff \(Phase P0/P1 transition; execution now beginning on WS-C workstreams\)\.$)|(^- \*\*Current active slice:\*\* comprehensive-audit v0\.9\.32 WS-C execution \(WS-C1 \+ WS-C2 \+ WS-C3 completed; WS-C4\+ follow-on execution in progress\)\.$)|(^- \*\*Current active slice:\*\* comprehensive-audit v0\.9\.32 WS-C execution \(WS-C1 \+ WS-C2 \+ WS-C3 \+ WS-C4 completed; WS-C5\+ follow-on execution in progress\)\.$)|(^- \*\*Current active slice:\*\* comprehensive-audit v0\.9\.32 WS-C execution \(WS-C1 \+ WS-C2 \+ WS-C3 \+ WS-C4 \+ WS-C5 completed; WS-C6\+ follow-on execution in progress\)\.$)' docs/SEL4_SPEC.md
...
run_check "DOC" rg -n '(^- \*\*Phase P2:\*\* WS-B5, WS-B6, WS-B2 \(WS-B1/WS-B2/WS-B5/WS-B6 complete; WS-B7 completed\)$)|(^1\. Pick one coherent WS-B target\.$)|(^1\. Pick one coherent WS-C target \(prioritize Phase P1 blockers first\)\.$)|(^- \*\*Phase P2:\*\* WS-C5 \+ remaining WS-C4 assurance expansion$)|(^- \*\*Phase P2:\*\* WS-C5 assurance expansion$)|(^- \*\*Phase P2:\*\* WS-C5 assurance expansion \(completed\)$)' docs/gitbook/06-development-workflow.md
run_check "DOC" rg -n '(^- \*\*Phase P2:\*\* WS-B5 \+ WS-B6 \+ WS-B2 \(WS-B1/WS-B2/WS-B5/WS-B6 complete; WS-B7 completed\)$)|(^- \*\*WS-C1:\*\* IPC handshake correctness \(critical\)$)|(^- \*\*WS-C1:\*\* IPC handshake correctness \(critical; execution beginning\)$)|(^- \*\*WS-C1:\*\* IPC handshake correctness \(critical; completed — notification badge OR accumulation \+ waiter ipcState transitions validated\)$)|(^- \*\*WS-C3:\*\* Proof-surface de-tautologization \(critical; completed\)$)|(^- \*\*WS-C4:\*\* Test validity hardening \(high; completed\)$)' docs/gitbook/24-comprehensive-audit-2026-workstream-planning.md
