...
- **active workstream:** **WS-AE Phase AE5 COMPLETE** (v0.25.19): Platform, Service & Cross-Subsystem — 7 sub-tasks (AE5-A through AE5-G). `collectQueueMembers` fuel-safe `Option` return (U-22), `registryEndpointUnique` invariant (U-20), `registryInterfaceValid` in 9-predicate `crossSubsystemInvariant` (SVC-04), boot bridge limitation documentation (U-21), NI boundary documentation (U-10), `LabelingContextValid` deployment documentation (IF-14). Phases AE1–AE4 also complete. See `docs/audits/AUDIT_v0.25.14_WORKSTREAM_PLAN.md`. **WS-AD PORTFOLIO COMPLETE** (v0.25.11–v0.25.14). **WS-AC PORTFOLIO COMPLETE** (v0.25.3–v0.25.10). 6 phases (AC1–AC6), 42 sub-tasks. Phase AC6 (v0.25.10): Documentation, Testing & Closure — T-03 `DecodingSuite.lean` (40 tests across Layer 1 RegisterDecode + Layer 2 SyscallArgDecode), coding conventions consolidated, audit severity table errata appended, workstream history updated. Phase AC5 (v0.25.9): Cross-cutting & infrastructure. Phase AC4 (v0.25.8): Architecture & platform tightening. Phase AC3 (v0.25.7): IPC atomicity & invariant strengthening. Phase AC2 (v0.25.6): Scheduler & SchedContext hardening. Phase AC1 (v0.25.5): High-severity fixes. **WS-AB Phases D1–D6 COMPLETE** (v0.24.0–v0.25.5). D1: Thread Suspension & Resumption. D2: Priority Management. D3: IPC Buffer Configuration. D4: Priority Inheritance Protocol. D5: Bounded Latency Theorem — proof-only phase, `bounded_scheduling_latency` WCRT = D*L_max + N*(B+P), PIP enhancement, 58 surface anchor tests. Zero sorry/axiom across all phases. **WS-AA Phase AA2 COMPLETE** (v0.23.23). CI & Infrastructure Hardening — 6 sub-tasks. **WS-AA Phase AA1 COMPLETE** (v0.23.22). **WS-Z PORTFOLIO COMPLETE** (v0.23.0–v0.23.21). **WS-Y PORTFOLIO COMPLETE** (v0.22.23–v0.22.26). **WS-X PORTFOLIO COMPLETE** (v0.22.18–v0.22.22). All prior portfolios complete: WS-W (v0.22.11–v0.22.17), WS-V (v0.22.0–v0.22.9), WS-U (v0.21.0–v0.21.7), WS-T (v0.20.0–v0.20.7), WS-S–WS-B (see `docs/WORKSTREAM_HISTORY.md`),
