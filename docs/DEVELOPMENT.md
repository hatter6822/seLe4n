...
- Trace and probe harnesses now exercise policy-checked wrappers (`endpointSendDualChecked`, `cspaceMintChecked`) by default; unchecked operations remain available for research experiments. (WS-Q1: `serviceRestartChecked` removed — service lifecycle simplified to registry-only model.)
