...
- **active workstream:** **WS-AG Phase AG2 Audit COMPLETE** (v0.26.2): Post-implementation audit — fixed critical `sched_context_configure` IPC buffer overflow bug (domain parameter not written to overflow slot), clarified `DomainId::MAX_VALID` type-level bound, corrected CHANGELOG version text. **Phase AG2 COMPLETE** (v0.26.1): Pre-Hardware Rust ABI Fixes — 3 sub-tasks (AG2-A through AG2-C). AG2-A (R-05): `MAX_DOMAIN` constant fix (255→15). AG2-B (R-01): SchedContext typed wrappers (all 25 syscalls). AG2-C (RUST-04): Rust workspace version sync (0.25.6→0.26.1). **Phase AG1 COMPLETE** (v0.26.0): Pre-Hardware Lean Code Fixes (6 sub-tasks). Plan: [`docs/audits/AUDIT_H3_HARDWARE_BINDING_WORKSTREAM_PLAN.md`](audits/AUDIT_H3_HARDWARE_BINDING_WORKSTREAM_PLAN.md). **WS-AF PORTFOLIO COMPLETE** (v0.25.22–v0.25.27). **WS-AE PORTFOLIO COMPLETE** (v0.25.15–v0.25.21). **WS-AD PORTFOLIO COMPLETE** (v0.25.11–v0.25.14). **WS-AC PORTFOLIO COMPLETE** (v0.25.3–v0.25.10). **WS-AB PORTFOLIO COMPLETE** (v0.24.0–v0.25.5). **WS-AA COMPLETE** (v0.23.22–v0.23.23). **WS-Z PORTFOLIO COMPLETE** (v0.23.0–v0.23.21). All prior portfolios complete: WS-Y, WS-X, WS-W, WS-V, WS-U, WS-T, WS-S–WS-B (see `docs/WORKSTREAM_HISTORY.md`),
