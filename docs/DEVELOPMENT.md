...
- **WS-H6 status:** in progress (v0.12.20-dev) — scheduler proof-surface strengthening slice landed (`flat_wf_rev`, `membership_implies_flat`, `mem_toList_iff_mem`, `isBetterCandidate_transitive`, `bucketFirst_fullScan_equivalence`),
...
- **WS-H6:** in progress (v0.12.20-dev). Scheduler proof completion slice landed: reverse RunQueue consistency bridge (`flat_wf_rev`, `membership_implies_flat`, `mem_toList_iff_mem`) and candidate-selection lemmas (`isBetterCandidate_transitive`, `bucketFirst_fullScan_equivalence`) to close M-04/M-05/M-06 foundation gaps.
