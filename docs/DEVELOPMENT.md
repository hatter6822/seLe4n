...
insert/erase/resize, `distCorrect` preservation, noDupKeys preservation,
`probeChainDominant` preservation, bundle preservation theorems, lookup
correctness (`get_after_insert_eq`, `get_after_insert_ne`, `get_after_erase_eq`).
All TPI-D items (D1–D6) completed. `RHTable.invariant` unified from former
`invExt` to use `probeChainDominant` (moved to Defs.lean). Robin Hood test suite
added. Zero sorry/axiom in production proofs. Remaining: N3 (kernel API bridge),
