...
- **completed workstream:** WS-K (full syscall dispatch completion) — **PORTFOLIO COMPLETE** (v0.16.0–v0.16.8). All 8 phases completed: **K-A** (v0.16.0): extended `SyscallDecodeResult` with `msgRegs` field, proved length invariant and round-trip lemmas. **K-B** (v0.16.1): per-syscall typed argument decode layer (`SyscallArgDecode.lean`) with 7 structures, 7 decode functions, 14 theorems. **K-C** (v0.16.2): wired all 4 CSpace syscalls through `dispatchWithCap`, 4 delegation theorems. **K-D** (v0.16.3): wired all 3 remaining stubs — zero `.illegalState` stubs, 13/13 dispatch; `objectOfTypeTag`, `lifecycleRetypeDirect`, `PagePermissions.ofNat`/`toNat`. **K-E** (v0.16.4): `ServiceConfig`-sourced policy, IPC message body population, 5 delegation theorems. **K-F** (v0.16.5–v0.16.6): layer-2 round-trip proofs, extraction round-trip, preservation composition, `retypeFromUntyped` NI proof. **K-G** (v0.16.7): `lifecycleRevokeDeleteRetype` NI proofs, 34 `NonInterferenceStep` constructors. **K-H** (v0.16.8): documentation sync, metrics regeneration, version bump, workstream closeout. See [`AUDIT_v0.15.10_SYSCALL_COMPLETION_WORKSTREAM_PLAN.md`](./dev_history/audits/AUDIT_v0.15.10_SYSCALL_COMPLETION_WORKSTREAM_PLAN.md). WS-J1 portfolio fully completed (J1-A through J1-F, v0.15.4–v0.15.10). WS-F portfolio fully completed (F1..F8),
