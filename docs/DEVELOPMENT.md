...
- **active workstream:** WS-T (Deep-Dive Audit Remediation) — Phases T1–T6 **COMPLETE** (v0.20.0–v0.20.5), T7–T8 in progress. 8 phases, 94 sub-tasks, v0.20.0–v0.20.7. Plan: [`AUDIT_v0.19.6_WORKSTREAM_PLAN.md`](audits/AUDIT_v0.19.6_WORKSTREAM_PLAN.md). T5 hardens lifecycle retype safety, fixes service graph cleanup, and extends cross-subsystem invariants: `KernelObject.wellFormed` predicate with retype validation, mid-queue intrusive link splice for `removeFromAllEndpointQueues`, `noStaleNotificationWaitReferences` predicate in `crossSubsystemInvariant`, `cleanupEndpointServiceRegistrations` dependency edge cleanup, `threadPriority_membership_consistent` scheduler proof. All prior portfolios complete: WS-S (v0.19.0–v0.19.6), WS-R (v0.18.0–v0.18.7), WS-Q (v0.17.7–v0.17.14), WS-N (v0.17.0–v0.17.5), WS-M (v0.16.14–v0.17.0), WS-L (v0.16.9–v0.16.13), WS-K (v0.16.0–v0.16.8), WS-J1 (v0.15.4–v0.15.10), WS-F (F1..F8),
