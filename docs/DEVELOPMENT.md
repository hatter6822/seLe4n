...
- WS-E4 dual-queue endpoint operations (`endpointSendDual`/`endpointReceiveDual`) use intrusive-list queue boundaries (`sendQ`/`receiveQ`) with per-thread links stored in `TCB.queuePrev`/`TCB.queuePPrev`/`TCB.queueNext`; `endpointQueueRemove` now provides O(1) arbitrary queue-member removal using `queuePPrev` + `queueNext` rewiring. Invariant checks include `intrusiveQueueWellFormed` validation for both endpoint queues (head/tail shape, cycle-free traversal, and per-node `queuePrev`/`queuePPrev`/`queueNext` linkage), and `negative_state_suite` adds runtime queue-link assertions for send/receive FIFO-dequeue paths, queue drain, middle-node removal rewiring, and dual-queue double-wait rejection (`alreadyWaiting`).
