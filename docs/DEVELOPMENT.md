...
All WS-H workstreams and WS-F1..F6 are completed. The remaining low-priority
WS-F7..F8 workstreams close testing and cleanup gaps from the v0.12.2 audit.
...
| ID | Focus | Priority | Status |
|----|-------|----------|--------|
| **WS-F5** | Model fidelity (word-bounded badge, order-independent rights, deferred ops) | Medium | **Completed** |
| **WS-F6** | Invariant quality (tautology reclassification, notification blocking, VSpace isolation) | Medium | **Completed** |
...
- **WS-F5:** completed (v0.14.9). Model fidelity — word-bounded `Badge` with `machineWordBits`(64)/`ofNatMasked`/`bor` and validity preservation theorems (`badge_bor_valid`, `badge_bor_comm`, `badge_bor_idempotent`); `notificationSignal` migrated to `Badge.bor`; `badgeWellFormed` invariant with `notificationBadgesWellFormed`/`capabilityBadgesWellFormed` and preservation proofs for signal/wait/mint/mutate; order-independent `AccessRightSet` bitmask replacing list-based rights with O(1) `mem`/`subset`/`ofList_comm`; deferred operations documented in API.lean, SELE4N_SPEC.md, and CLAIM_EVIDENCE_INDEX.md. Closes CRIT-06, HIGH-04, MED-03.
