...
- **WS-H6:** in progress. Scheduler proof-surface extension started with RunQueue reverse structural invariant `flat_wf_rev` and theorem `membership_implies_flat` (`membership → flat`) to complete the bidirectional bridge needed for O(1) membership reasoning in scheduler proofs.
