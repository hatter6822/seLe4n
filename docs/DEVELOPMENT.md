...
- **active workstream:** **WS-AC Phase AC5 COMPLETE** (v0.25.9). Cross-cutting & infrastructure: X-05 field-disjointness complete pairwise coverage (28/28 pairs, 12 disjoint + 16 shared, compile-time `native_decide` summary), X-08 GitBook content-hash drift detection in `test_docs_sync.sh`, S-05 `saveOutgoingContext_scheduler` theorem verified and cross-referenced, F-04 codebase-wide catch-all audit (zero risky patterns), F-02 `AccessRightSet` operational safety theorems (`inter_valid_left`, `subset_sound`, `mem_bit_bounded`), F-03/IF-01 coding conventions verified. **WS-AC Phase AC4 COMPLETE** (v0.25.8). Architecture & platform tightening. **WS-AC Phase AC3 COMPLETE** (v0.25.7). IPC atomicity & invariant strengthening. **WS-AC Phase AC2 COMPLETE** (v0.25.6). Scheduler & SchedContext hardening. **WS-AC Phase AC1 COMPLETE** (v0.25.5). **WS-AB Phases D1–D6 COMPLETE** (v0.24.0–v0.25.5). D1: Thread Suspension & Resumption. D2: Priority Management. D3: IPC Buffer Configuration. D4: Priority Inheritance Protocol. D5: Bounded Latency Theorem — proof-only phase, `bounded_scheduling_latency` WCRT = D*L_max + N*(B+P), PIP enhancement, 58 surface anchor tests. Zero sorry/axiom across all phases. **WS-AA Phase AA2 COMPLETE** (v0.23.23). CI & Infrastructure Hardening — 6 sub-tasks. **WS-AA Phase AA1 COMPLETE** (v0.23.22). **WS-Z PORTFOLIO COMPLETE** (v0.23.0–v0.23.21). **WS-Y PORTFOLIO COMPLETE** (v0.22.23–v0.22.26). **WS-X PORTFOLIO COMPLETE** (v0.22.18–v0.22.22). All prior portfolios complete: WS-W (v0.22.11–v0.22.17), WS-V (v0.22.0–v0.22.9), WS-U (v0.21.0–v0.21.7), WS-T (v0.20.0–v0.20.7), WS-S–WS-B (see `docs/WORKSTREAM_HISTORY.md`),
...
- **latest audit:** [`AUDIT_v0.25.3_COMPREHENSIVE`](audits/AUDIT_v0.25.3_COMPREHENSIVE.md) — full-kernel Lean + Rust audit (0 CRIT, 3 HIGH, 9 MED, 14 LOW); remediation plan in [`AUDIT_v0.25.3_WORKSTREAM_PLAN`](audits/AUDIT_v0.25.3_WORKSTREAM_PLAN.md),
...
9. **`AccessRightSet` operational safety (F-02/AC5-E)**: `subset` is sound
   ...
   the left operand is valid (`inter_valid_left`); membership checks are
