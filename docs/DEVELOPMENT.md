...
- **active workstream:** WS-K (full syscall dispatch completion, v0.16.4–v0.16.8). **K-A completed** (v0.16.0): extended `SyscallDecodeResult` with `msgRegs` field, updated `decodeSyscallArgs` to extract message register values, proved length invariant and round-trip lemmas. **K-B completed** (v0.16.1): per-syscall typed argument decode layer (`SyscallArgDecode.lean`) with 7 structures, 7 decode functions, 7 determinism theorems, 7 error-exclusivity theorems. **K-C completed** (v0.16.2): wired all 4 CSpace syscalls (`cspaceMint`, `cspaceCopy`, `cspaceMove`, `cspaceDelete`) through `dispatchWithCap` using decoded message register arguments, changed `dispatchWithCap` signature to accept `SyscallDecodeResult`, 4 delegation theorems proved. **K-D completed** (v0.16.3): wired all 3 remaining syscall stubs (`lifecycleRetype`, `vspaceMap`, `vspaceUnmap`) through `dispatchWithCap` — zero `.illegalState` stubs remain, all 13 syscalls fully dispatch; added `objectOfTypeTag` (with dedicated `invalidTypeTag` error variant), `lifecycleRetypeDirect`, `PagePermissions.ofNat`/`toNat` with round-trip and delegation theorems. **K-E completed** (v0.16.4): replaced `(fun _ => true)` service policy stubs with `ServiceConfig`-sourced predicates; populated IPC message bodies from decoded registers via `extractMessageRegisters` (`Array RegValue` → `Array Nat`); 5 delegation theorems; zero policy stubs, zero empty IPC payloads remain. Extends WS-J1's decode layer with per-syscall argument decode and full dispatch for all 13 syscalls. See [`AUDIT_v0.15.10_SYSCALL_COMPLETION_WORKSTREAM_PLAN.md`](./audits/AUDIT_v0.15.10_SYSCALL_COMPLETION_WORKSTREAM_PLAN.md). WS-J1 portfolio fully completed (J1-A through J1-F, v0.15.4–v0.15.10). WS-F portfolio fully completed (F1..F8),
...
The active near-term portfolio is **WS-K** (v0.16.4): full syscall dispatch
...
**K-D completed** (v0.16.3), **K-E completed** (v0.16.4). See
