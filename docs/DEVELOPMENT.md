...
- CDT modeling now uses stable node identities for derivation edges plus explicit slot↔node projection bindings. This makes `cspaceMove` a slot-pointer rebind (`rebindSlotNode`) with node backpointer fixup instead of an O(|edges|) edge rewrite, while preserving the CSpace-observed CDT via `projectedEdges`.
