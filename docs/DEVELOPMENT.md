...
- **active workstream:** **WS-V Phase V2 COMPLETE** (API Surface Completion) — 9 sub-tasks (V2-A through V2-I): added `notificationSignal`/`notificationWait`/`replyRecv` syscalls (`SyscallId` count 14→17), wired both unchecked and checked dispatch paths with enforcement wrappers and delegation theorems, updated Rust variants 14–16, bounded `MessageInfo` label to 20 bits (seL4 convention) in Lean and Rust, made cap-recv slot configurable via `capRecvSlot`, added `SyscallArgDecode` structs and round-trip theorems (v0.22.1). **WS-V Phase V1 COMPLETE** (Rust ABI Hardening, v0.22.0). All prior portfolios complete: WS-U (all 8 phases, v0.21.0–v0.21.7), WS-T (v0.20.0–v0.20.7), WS-S (v0.19.0–v0.19.6), WS-R (v0.18.0–v0.18.7), WS-Q (v0.17.7–v0.17.14), WS-N (v0.17.0–v0.17.5), WS-M (v0.16.14–v0.17.0), WS-L (v0.16.9–v0.16.13), WS-K (v0.16.0–v0.16.8), WS-J1 (v0.15.4–v0.15.10), WS-F (F1..F8),
