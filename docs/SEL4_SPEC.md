...
All policy decisions -- including memory allocation policy, driver isolation, and
filesystem semantics -- are delegated to user-level components. The kernel provides
mechanisms; user space defines policy.

The kernel is extremely small: approximately 8,700--10,000 lines of C (depending on
architecture) plus ~600 lines of assembly. Binary images range from approximately 66 KiB
(RISC-V 64-bit) to 162 KiB (AArch64 with hypervisor support).

Key design principles:

- **No dynamic memory allocation in the kernel.** The kernel has no heap. All kernel
  objects are created explicitly from user-controlled memory.
- **All authority is capability-mediated.** No operation can be performed without holding
  a capability with sufficient rights.
- **Minimal trusted computing base.** Fewer lines in privileged code reduce the attack
  and bug surface.
- **Formal verification as a first-class design constraint.** The kernel was designed
  from the start to be verifiable.
...
| Object | Size | Purpose |
|---|---|---|
| **Untyped Memory** | Variable | Raw physical memory; the foundation of all memory allocation. New kernel objects are created by retyping untyped memory. |
| **CNode** | Variable (2^k slots) | Capability Node; a table of capability slots forming a thread's CSpace. |
| **TCB** | Fixed | Thread Control Block; represents a thread of execution. Holds references to CSpace root, VSpace root, IPC buffer, fault endpoint, priority, and register state. |
| **Endpoint** | Fixed | Synchronous IPC channel. Threads block on endpoints to send or receive messages. |
| **Notification** | Fixed | Asynchronous signalling mechanism (logically an array of binary semaphores). Supports Signal and Wait operations. |
| **Reply** (MCS only) | Fixed | Container for the one-off reply capability generated by `seL4_Call`. Tracks scheduling context donation. |
| **SchedContext** (MCS only) | Variable | Represents CPU time as a budget/period pair. Enables temporal isolation via sporadic servers. |

### 2.2 Architecture-Specific Objects (Virtual Memory)

| Object | Purpose |
|---|---|
| **VSpace** | Root of a virtual address space. Concrete type is architecture-dependent (e.g., PML4 on x86_64, top-level PageTable on RISC-V, PageDirectory on ARM32). |
| **PageTable / PageDirectory / PDPT / PML4** | Intermediate paging structures. Architecture-dependent hierarchy. |
| **Frame (Page)** | Physical memory frame. Available in architecture-dependent sizes (e.g., 4 KiB, 2 MiB, 1 GiB on x86_64; 4 KiB and 2 MiB on ARM). |
| **ASID Control** | Singleton; used to create ASID Pools. Controls the limited supply of address-space identifiers. |
| **ASID Pool** | Created from a 4 KiB untyped; holds up to 1,024 ASID slots. A VSpace must be assigned an ASID before it can be used. |

### 2.3 Interrupt Objects

| Object | Purpose |
|---|---|
| **IRQControl** | Singleton capability; used to create IRQHandler capabilities for specific interrupt sources. |
| **IRQHandler** | Manages a single hardware interrupt. Bound to a Notification for interrupt delivery. Must be acknowledged after each interrupt. |

### 2.4 Platform-Specific Extension Objects

| Object | Architecture | Purpose |
|---|---|---|
| **IOPort** | x86 | Access to hardware I/O port ranges. |
| **IOPageTable** | x86 (IOMMU) | IOMMU page tables for DMA isolation. |
| **VCPU** | ARM, x86 | Virtual CPU for hypervisor/virtualization support. |
...
### 4.2 MCS-Specific System Calls

The MCS configuration modifies and adds system calls:
...
### 4.3 Object Invocations

Beyond the core system calls, kernel services are accessed via **object invocations** --
method calls on kernel objects performed through capabilities. These are encoded as
`seL4_Send` or `seL4_Call` operations on the target object's capability, with the method
and parameters encoded in the IPC message. Examples: `seL4_Untyped_Retype`,
`seL4_TCB_Configure`, `seL4_CNode_Mint`.
