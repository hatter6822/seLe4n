...
> **Status**: **IN FLIGHT — BP0, BP1, BP2, BP3, BP4, BP5 and BP6 LANDED at `v0.36.2`; BP7.10 LANDED at `v0.36.3`; BP7.1 slices 1–3 of 4 LANDED at `v0.36.4`–`v0.36.6`**; the rest of BP7, and BP8, not started.
...
| Sub | Description | Files | Est |
|-----|-------------|-------|-----|
| BP7.1 | **IN PROGRESS — slices 1, 2 and 3 of 4 LANDED (`v0.36.4`, `v0.36.5`, `v0.36.6`).**  **Slice 3 (`v0.36.6`): memory returns to its untyped.**  `SyscallId.untypedReset` (discriminant 37) is seL4's `resetUntypedCap`, invoked on an untyped capability carrying `.retype`: it refuses unless every carved child is a frame and no capability anywhere names one — decided over the whole object store (`untypedChildrenUnreferenced`), since the watermark is the untyped object's and a derivation-free sibling copy of its capability would pass a per-slot test — then removes every mapping of a page meeting the region through the `.vspaceUnmap` arm's own verified transition and checks that none survives (`untypedRegionUnmapped`), erases the carved frames (`retireFrame`, the one primitive that erases, frames only, so ids and store capacity return) and resets the watermark.  `untypedReset_ok_unmapped`, `untypedReset_ok_unreferenced`, `untypedReset_ok_children_absent`, `untypedReset_ok_untyped` and `untypedReset_preserves_ipcInvariantFull` are its theorems; `tests/VSpaceCapabilityBindingSuite.lean` §5e carves, maps, revokes, resets and re-carves the same page and id.  The in-place retype of a frame stays refused — memory returns through its untyped, as in seL4 — and revocation does not yet unmap (registered, owner slice 4).  **Slice 2 (`v0.36.5`): memory reaches a thread only by a carve.**  `SyscallId.untypedRetype` (discriminant 36) is seL4's `seL4_Untyped_Retype` at the frame type — `untypedRetypeFrame` carves the page at the untyped's watermark through `retypeFromUntyped` (whose authority, capacity, fresh-id, alignment and device guards it reuses; the device rule became `!memoryBacked`, so a device untyped yields a device frame), zeroes a RAM page, installs a read/write/grant frame capability at an empty slot of a CNode the caller holds `.write` to, and records it as a CDT child of the untyped capability (`DerivationOp.retype`).  `untypedRetypeFrame_ok_frame` and `untypedRetypeFrame_preserves_ipcInvariantFull` are its theorems; `tests/VSpaceCapabilityBindingSuite.lean` §5d carves and maps the carved page.  **Slice 1 (`v0.36.4`): memory is authority.**  A translation-table base a PE can be told to use needs a page of physical memory the kernel owns, and a page the kernel can hand to a table is a page the model must be able to *name* — so the row opens with the seL4 memory-as-authority model rather than with the base field.  `FrameObject` (`base`, `isDevice`, its lock) is the ninth kernel-object kind (`KernelObjectType.frame`, retype tag `8`), locked at `LockKind.page`, which becomes a modelled kind like `Reply` did at SM6.D; `KernelObjectType.memoryBacked` names the two kinds that *are* physical memory (untyped, frame).  **The first slice closes a vulnerability**, found while scoping this row and reported before the fix: `.vspaceMap` read MR2 as a raw physical address, so one VSpace-root capability was authority over every page of physical memory, the kernel image included.  MR2 is now the address of a **frame capability** in the caller's CSpace (`VSpaceMapArgs.frame`), resolved with `.read` (`resolveVSpaceMapFrame`); the page installed is that frame's own `base`; a writable mapping needs `.write` on the frame capability and a device frame is mapped neither executable nor cacheable (`frameMappingAdmissible`), and `vspaceMapFromFrameCap` is the one definition the arm and its theorems read.  Three things close the routes around it: an in-place retype refuses a memory-backed replacement (`retypeReplacementAdmissible`'s third conjunct, `.illegalState`), the pre-retype cleanup refuses to destroy a frame in place (`.revocationRequired` — permanently since slice 3, which returns a frame's memory through its untyped), and the boot refuses a configured frame (`bootSafeObjectCheck`, `bootSafeObject`'s last conjunct), since no boot check places one.  `decodeVSpaceMapArgsChecked` is retired with the operand it bounded.  **Remaining slices, in order:** ~~(2) a live untyped carve that mints frames out of an untyped the caller holds, so a frame capability can be obtained at all~~ (landed `v0.36.5`, above); ~~(3) frame destroy with unmap~~ (landed `v0.36.6` as the untyped reset, above); (4) page-table objects carved the same way, whose physical base is this row's original subject and what the next row installs — with the reset extended to retire them and child untypeds, and a frame capability recording the mapping made through it so that its deletion unmaps.  *The row as registered:* Extend the model's `VSpaceRoot` with a translation-table **physical base**.  The model carries an ASID and an abstract `VAddr → PAddr` table, which is not something a PE can be told to use | `SeLe4n/Kernel/Architecture/VSpace.lean`, `SeLe4n/Model/` | L |
