...
> **Status**: **IN FLIGHT — BP0, BP1 and BP2.1 LANDED at `v0.36.2`**; BP2.2..BP2.6 and BP3..BP8 not started.
...
| BP1.3 | **LANDED `v0.36.2`** (`scripts/test_lean_aarch64_archive.sh`, CI job `Lean aarch64 Archive`).  Archive the objects as `libsele4n.a` (deterministic, members sorted), the name `rust/sele4n-hal/src/boot.rs` already asserts is linked, and add the CI lane that builds it.  The archive is checked, not trusted: one module initializer per object and per closure module, every initializer referenced defined, no symbol defined twice, and **every regenerated stdlib module defines exactly the global symbols the toolchain's own object for it defines** (609 of 609).  Its unresolved set is written **attributed by provider**, each class derived from the provider's own object code or declarations — 1 allocator (`lean_alloc_small`, which the mimalloc host runtime lacks), 221 runtime (`libleanrt.a`), 80 compiler builtins (Rust's `compiler_builtins` for the target), 73 HAL (the kernel's `@[extern]`s), 6 stdlib externs no provider supplies (`acosh`/`asinh`/`atanh` and their `f` forms) — and an unattributed symbol stops the build.  That report is what the runtime phase's libc-surface row consumes | `scripts/`, `.github/workflows/` | M |
...
| Sub | Description | Files | Est |
|-----|-------------|-------|-----|
| BP2.1 | **LANDED `v0.36.2`** (`rust/sele4n-hal/src/lean_heap.rs`, `link.ld`).  A kernel heap arena in the image's own memory, and the allocator behind `lean_alloc_small` / `lean_alloc_object` / the free paths.  Its extent is a boot-time constant the linker script places, not a runtime negotiation: `link.ld`'s `NOLOAD` `.lean_heap` section of `LEAN_HEAP_SIZE` (64 MiB), bounded by `__lean_heap_start` / `__lean_heap_end`, above the image and both stack regions, with three `ASSERT`s — whole pages, page-aligned, and ending inside the smallest board's `[0, 1 GiB)` — each proved live by `scripts/check_link_script.py` (cross lane step 6), which links a probe under the script and mutates it until every assertion fires.  The HAL exports `lean.h`'s three small-allocator functions under `hw_target` (512 classes, indexed as `lean_get_slot_idx` indexes them) and a general `malloc`-shaped interface over the same arena, so `lean_alloc_object`'s big path is served by the one heap the next row's libc surface calls.  **All allocator state is out of band** — a page map, a free-page bitmap and per-page occupancy bitmaps in the arena's leading pages — so the allocator never touches the memory it serves, every free is validated (a double free is refused, not absorbed), each operation is bounded (eight words per small allocation; a bitmap scan from the first free word per page run), an emptied small page returns to the pool, and the C entry points halt on a refusal and on exhaustion.  The boot map's critical ranges include the arena (`mmu::image_ranges`), and the Lean lane now verifies that the HAL's object code defines every symbol the archive attributes to it (the allocator and the 73 production `@[extern]`s).  **What the runtime build inherits**: the runtime's `alloc.cpp` defines the same three functions under `LEAN_SMALL_ALLOCATOR`, over per-thread heaps built on `new` and `std::vector`; the runtime the next row builds replaces that block with forwards to this one (`lean::alloc` / `lean::dealloc` over `Heap::alloc` / `Heap::free`) rather than linking two allocators | `rust/sele4n-hal/src/`, `rust/sele4n-hal/link.ld` | L |
| BP2.2 | The libc surface the runtime calls — `memcpy`, `memset`, `memcmp`, `strlen` and whatever the measured link demands — as `no_std` definitions, derived from the *unresolved symbols of the BP1.3 archive* rather than from a guessed list.  **Measured at `v0.36.2`**: the kernel's Lean objects call **no** libc symbol at all — the archive's report attributes every unresolved symbol, and the only class no provider supplies is six libm functions (`acosh`, `asinh`, `atanh` and their `f` forms) that Rust's `compiler_builtins` omits.  The libc surface is therefore the **runtime's**, and it is measured when BP2 builds that runtime — whose C++ sources the Lean toolchain does not ship, so BP2 compiles them from the pinned `lean4` release at the toolchain's own commit, against the kernel's `lean/config.h` (the allocator is a contract between every object's inline paths and the runtime that serves them) | `rust/sele4n-hal/src/` | L |
...
- [x] `check_kernel_entry_exports.py` reads the cross archive (BP1.4).
  Executed at `v0.36.2`: all 8 HAL kernel-entry declarations defined in both
  archives, `lean_kernel_main` reconciled as expected unresolved.
