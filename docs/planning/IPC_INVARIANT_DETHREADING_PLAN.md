...
| D2′ lifecycle/retype third clause | ⏳ 2 `lifecycleRetypeObject` bundles (`EndpointReplyAndLifecycle.lean`): retype creates fresh objects (never `.blockedOnReply`), so the clause is framed — needs a retype objects frame (Capability subsystem). | — |
