...
| D2 replyCallerLinkage (third clause) + consumer | ✅ **LANDED (all 3 establish cases)**: consumer (v0.31.157), full frame family incl. `endpointQueuePopHead`/`endpointQueueEnqueue` frames + `linkCallerReply`/`linkServerStashedReply`/`cleanupPreReceiveDonation` establishes + `wakeThread_…_of_ready` (per-core); single-core `endpointCall`/`endpointReceiveDual_establishes_…` (v0.31.159) **and** per-core `endpointCallOnCore_establishes_…` (v0.31.160); **all 3 bundle theorems that can introduce `.blockedOnReply` de-threaded** (thread `replyCallerLinkageReciprocal st'`, establish the third clause). Preserve-only bundles de-threaded: `endpointSendDual` (v0.31.161, pure frame); `notificationSignal`/`notificationWait` (v0.31.162, via generic `storeObject_nonTcb_…` + `storeTcbIpcState_fromTcb_nonBlocked_…` frames). **6 of 11 bundles de-threaded.** Remaining (each needs new frame machinery in its subsystem): `endpointReply`/`endpointReplyRecv`/`consumeCallerReply` (reply-consumption frame: unblock + `consumeReply`), the 3 WithCaps (`ipcUnwrapCaps` cap-transfer frame), 2 lifecycle/retype bundles (Capability subsystem). Reciprocal-half de-thread (optional). | v0.31.162 |
