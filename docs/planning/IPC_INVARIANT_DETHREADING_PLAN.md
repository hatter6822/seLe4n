...
| D6 donationOwnerValid + donationBudgetTransfer + passiveServerIdle | 🔄 **IN PROGRESS (UNBLOCKED by Finding F-3)** — **`donationBudgetTransfer` PARTIAL (3/13 bundles, v0.31.178)**: reusable `sameSchedContextBindings` frame (`donationBudgetTransfer_of_sameSchedContextBindings` + the store-op family `storeObject_{modifiedTcb,nonTcb}` / `storeTcbIpcState{,_fromTcb,AndMessage}` / `storeTcbReceiveComplete` `_sameSchedContextBindings` + `of_objects_eq`) de-threads `hDBT'` from `notificationWait` / `notificationSignal` / `endpointReply` (the binding-free transitions establish it by the frame; core IPC transitions never write a `schedContextBinding`). **Remaining**: the 10 binding-free/binding-touching bundles — `endpointSendDual` / `endpointCall` / `endpointReceiveDual` / `endpointReplyRecv` (queue-op `sameSchedContextBindings` for `endpointQueue{PopHead,Enqueue}`), the 3 `WithCaps` (`ipcUnwrapCaps`), `endpointCallOnCore`, the 2 `lifecycleRetypeObject` (`newObj` side-condition); plus `donationOwnerValid` + `passiveServerIdle` (per-transition establish/preserve: the donation-return composition — `donationOwnerValid` needs bare `endpointReply` not to wake a donation owner; `passiveServerIdle` per-transition block treatment). | v0.31.178 |
