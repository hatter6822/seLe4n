...
| D3 blockedOnReplyHasTarget + pendingReceiveReplyWellFormed | 🔄 **`blockedOnReplyHasTarget` frame family + ALL establishes/preserves built** (every transition; the `.blockedOnReply` stores use `(some receiver)` so the store establishes the clause directly). `endpointCall`/`endpointReceiveDual` bundles **wired** (`hBRT'` de-threaded); remaining ~11 bundles' wiring mechanical. `pendingReceiveReplyWellFormed` ⏳ | v0.31.166 |
