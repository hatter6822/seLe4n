...
| D-5 | `QueuedRwLock` impl (MCS-style per-core slots, NOT lock-free linked-list); refinement bridge updated; **≥10 cross-thread tests** covering FIFO + mutex + mixed-stress + panic-safety + slot-ownership; **`cargo +nightly miri test -p sele4n-hal --lib queued_rw_lock` passes with `-Zmiri-strict-provenance`**; **`cfg(loom)` exhaustive-interleaving runs pass on every two-thread program of one contract-respecting operation unit per thread — at most five lock operations, enumerated from the lock's own unit list (`every_pair_of_units_is_safe`) — plus the targeted scenario models, with no preemption bound** (restated at v0.34.55, PR #890 review round 2: "op-sequences of length ≤ 4" named a bound the handwritten scenarios did not enumerate); FIFO test iteration count ≥ 10⁴ per run |
...
`docs/REGISTERED_DEBT.md` is updated to mark SM2.C-defer as CLOSED.
...
| D-5 release-path successor-signaling race | MED | HIGH | The remaining principal risk; mitigated by `loom` exhaustive-interleaving tests over every pair of contract-respecting operation units on two threads, unbounded (audit M-10 acceptance gate, restated at PR #890 review round 2) and `cargo +nightly miri test` runs |
...
- [ ] **Documentation synchronized** — the full canonical set (matches
  ...
  - [ ] `docs/REGISTERED_DEBT.md` if workstream status changes
  - [ ] `CHANGELOG.md` entry
