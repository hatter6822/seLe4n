...
**50 sub-tasks across 9 phases `BP0..BP8`**, in execution order, with
...
[`SMP_BOOT_PATH_PLAN.md`](SMP_BOOT_PATH_PLAN.md)'s 50 sub-tasks, and `SM10.1.1`
...
[`SMP_BOOT_PATH_PLAN.md`](SMP_BOOT_PATH_PLAN.md)** (WS-BP, 50 sub-tasks
...
|-----|-------------|-------|-----|
| SM10.1.1 | **Produced at `v0.36.2`** by WS-BP BP5.1–BP5.4 (`scripts/build_rpi5_image.sh`, CI artifact `rpi5-kernel-image`); what the release cut still owes is consuming it.  Bootable image target — `kernel8.img` + `config.txt` packaging, and the `sele4n-kernel` bootable binary it packages | `scripts/build_rpi5_image.sh` | XL |
