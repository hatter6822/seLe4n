...
| RR7 | Medium-severity sweep, plus the §7 rows RR0.11 routes here.  **LANDED v0.34.47 → v0.34.92** (RR7.1–RR7.41, every row marked) | 41 | M |
| RR8 | Phase closure and hand-off to SM10.  **LANDED v0.35.55 → v0.35.203** (RR8.1–RR8.16) | 16 | L |
...
`bootAndInitialiseRPi5OrHalt` applied to a configuration, decided by a
reduction directed at the approved call and one reducible `isDefEq` on the
result (PR #892 review round 2 replaced round 21's single unbounded `isDefEq`,
which opened the approved call itself).  Stronger than the walk (which admitted any extra action that did
...
### RR7 — Medium-severity sweep — **LANDED v0.34.47 → v0.34.92**
...
|-----|-------------|----------|-----|
| RR7.1 | Boot MMU corrections: 960 MiB of RAM mapped as Device, and nothing mapped above 4 GiB (§4).  **LANDED v0.34.57**: the tables are derived from `mmu::boot_mapping_for` (mirroring `rpi5MemoryMapForConfig`) and sized to the board's own `/memory` node — since PR #892 review round 3 (v0.34.111), to the **contiguous** run it reports from address 0 (`cmdline::contiguous_ram_top`, retired with the rest of the device-tree-sized boot map at WS-BP BP2.6; the peripheral window is the one gap the walk may cross), where the maximum fold of the first cut mapped every hole between two claims Normal-cacheable; and since round 4 (v0.34.112) a bank whose `status` is not `okay`/`ok` — DRAM the firmware has withheld — contributes nothing, and `init_mmu` refuses, before translation is enabled, a clamped top that does not cover the image, its stacks and the blob's own extent (`boot_critical_ranges_mapped`, retired at WS-BP BP2.6 for a map built from the image and board constants; the PE parks in `cpu::fatal_halt`, since Phase 2 runs on the boot core before the GIC exists).  The same cut fixed a defect the finding did not name — the table `TTBR0_EL1` pointed at held *level-1* block descriptors while `T0SZ = 16` makes level 0 the initial lookup level, so the boot MMU enable would have translation-faulted on the first fetch | 1 | L |
...
| RR7.19 | Fine-lock migration mediums.  **LANDED v0.34.70**: three register rows.  **Finding 7** ("Tracks B, C and D are entirely unstarted") is closed by the rows above and *recorded*: the plan's status header said "2 of 12 PRs landed" for ten cuts after Track B started landing, and now says **9 of 12** with the per-track landing versions and the honest scope of the v1.0.0 claim — the syscall seam brackets, the three per-core scheduler entries do not, and `ExportCommitDisciplineCensus` measures which is which rather than the sentence asserting it.  **Finding 8** is closed both halves: Tracks B and C are named in `SMP_RELEASE_CLOSURE_PLAN.md` §2 (which named the fine-lock plan nowhere), and the SMP-C3 closure line now says what is dischargeable — the syscall path — and what is not until Track D lands.  The WS-RA obligation (2) in the same section is marked closed by RR7.14.  **Finding 9** is the code half: `capabilityOp_modifiedFields` was corrected at RR7.9, but the finding's *second* clause — "and has no consumer" — was still open, and it applied to **all six** `*_modifiedFields` lists.  `SystemState.fieldEq` (total over `StateField`, so a new field is a missing case), `preservesFieldsOutside` and `predicateFramedByDisjointWrites` make them load-bearing: a read-set disjoint from a write-set now *entails* preservation, so both families are premises and an under-declared write-set licenses a conclusion the operation does not earn.  Two real consumers land with it (`storeObject_preserves_registryDependencyConsistent`, `revokeService_preserves_noStaleEndpointQueueReferences`) plus the load-bearing negative that `storeObject` is **not** framed away from the queue-reference invariant.  **A second under-declaration surfaced immediately**: `storeObject_modifiedFields` omitted `.asidTable`, which `storeObject`'s own record update writes when the stored or displaced object is a `.vspaceRoot`.  Inert before this row (no consumers), a soundness hazard the instant they exist — `asidTable` is the ASID → VSpaceRoot map.  Corrected, with `storeObject_preservesFieldsOutside` false at the old list so the correction cannot be reverted; the retype and IPC lists are **defined as** `storeObject`'s rather than copied, so a future correction cannot reach one and miss the other, and the IPC list is widened rather than resting on two unproved conditional facts.  Eight runtime checks.  **RR7 audit round (v0.34.109)**: the row's own claim — "each operation carries a `_preservesFieldsOutside` theorem at its own list" — was true of two lists of six.  Writing the other four found the IPC list short by `.scheduler` and `.scThreadIndex` (both dual-queue operations wake or deschedule; the receive path's donation return rewrites the SchedContext → threads index), the capability list short by `storeObject`'s three index/ASID fields (the very class this row found in `storeObject`, under the sentence it had retracted for the IPC list), and `StateField` naming sixteen of `SystemState`'s twenty-seven fields — so the `scThreadIndex` write could not have been declared at all.  `SystemState.eq_of_fieldEq_all` now holds the enumeration to the structure (agreement on every constructor is state equality, through `SystemState.mk.injEq`), both lists are widened and stated as relations over `storeObject`'s, and all six theorems exist, the composites proved from one lemma per primitive | 3 | M |
| RR7.20 | TLB shootdown mediums.  **LANDED v0.34.71**: three register rows, all of which are deferrals whose owner had evaporated.  **Finding 29** (SM7.D's SM10.1-owned cache items appear in no SM10 sub-task): two of the four had closed already (item 1 at `v0.34.57`, item 3 at `v0.33.37`); the remaining three each get a named owner instead of a phase that no longer exists, since SM10.1's content became WS-BP.  Item 4 — the `.bootImageLoad` clean-to-PoU, the *only* unemitted kernel-code-write site — is scheduled as **`BP4.5`** with the acceptance criterion SM7 already stated, and it must run in the boot seam before any user code can be fetched.  Item 2 — the post-state is published before the maintenance is emitted — is **not** boot-path work: its design belongs to the fine-lock plan's **Track D**, the row that changes the commit model, and its validation to **BP8**; it has its own debt row rather than a phase name.  Item 5 — `scrubExtent` is the model's abstract convention, not the allocator's — is AN4-G.3/LIF-M03, a **data-disclosure** gap once bootable, needs kernel-model work no boot row covers, and is registered post-v1.0.0 against the untyped/retype surface.  **Finding 30** (two SM7 debts named SM8, which closed without absorbing either) closes by recognising they are **one root**: `asidAllocateWithShootdown` is complete, proven and unreachable because the ASIDControl/ASIDPool family does not exist, so building its initiator-atomic seam would violate the wire-it-into-the-consumer rule — the consumer is what is missing.  Both re-target to **WS-AP**, a new post-v1.0.0 workstream registered with three ordered items and the honest consequence: v1.0.0 ships no userspace-reachable ASID surface and the release note must not imply one.  **Finding 31** needed no plan change and gets none: the §8 box stays **unchecked**, with `BP8.4` named as the run that decides it and the instruction to check it in the same cut.  A SKIP is not a pass, and restating the box as "the script exists" would trade a behaviour criterion for an artefact-existence one — exactly what RR7.16 refused for the two SM1.H boxes | 3 | M |
| RR7.21 | Debt-register mediums.  **LANDED v0.34.72**: three rows, two of which closed in earlier cuts and are recorded here rather than re-done.  **Finding 36** (SM7's ASID gap named SM8, which closed without it) closed at `v0.34.71` as **WS-AP**, post-v1.0.0, in the implement-the-improvement direction the row asks for — build the object family, not a caveat.  **Finding 37** (three `@[export]` runtime-seam modules staged-only, outside the production closure) closed at `v0.34.48` (RR5.15): all three are in `SeLe4n.lean`'s closure and out of the allowlist, and `check_kernel_entry_exports.py` verifies each against the **built static archive** rather than a text anchor.  **Finding 38** is the substantive one and closes here for the register half: all three of SM6's tracked-debt items stated a closure target as a *theorem name* and none stated an owner or a version — a theorem name is a destination, not a schedule, and that is precisely how a recorded debt stops being tracked.  Item 1 (bound-notification delivery) is owned by the cross-core IPC row below, which carries the same gap from the audit side; item 2 (`withLockSet` bundle preservation) by the fine-lock Track D, post-v1.0.0, because the lock-write congruences are worth building only once the bracket is what the live kernel runs everywhere — seven seams commit and two bracket at `v0.34.66`; item 3 is non-load-bearing by its own description and is **deliberately unowned**, which is now stated rather than left ambiguous.  Nothing was re-scoped and no claim was weakened | 3 | M |
| RR7.22 | Cross-core IPC mediums.  **Finding 3 LANDED v0.34.72**: the bound-notification delivery path's non-interference, built on the endpoint-splice projection engine that did not exist.  **Finding 4's engine LANDED v0.34.73**: `QueueSplicePreservation.lean` decomposes `endpointQueueRemoveDual` once as `SpliceShape` and carries **all twenty** `ipcInvariantFull` conjuncts across it — nineteen unconditional, the membership conjunct relaxed at the removed thread because the splice deliberately leaves that thread's `ipcState` alone — with the one fact the bundle does not entail stated as `splicePredecessorBlocked` rather than assumed.  **The first of Finding 4's two consumers LANDED v0.34.92**: the pair closes — `storeTcbReceiveComplete_closes_exceptMembership` turns the relaxed bundle back into the full one, and the three detachment facts it asks for are the ones the splice establishes (`endpointQueueRemoveDual_removed_detached`, from the cleared back-link, the four-shape boundary analysis, and the head/tail-blocked conjuncts the splice already carries).  So `notificationSignalBoundOnCore` carries `ipcInvariantFull`, every core's `ipcInvariantFull_perCore`, and the bundle at both dispatch forms including the flow-checked one the live SM9 arm runs.  **`cancelIpcBlockingOnCore`'s whole-bundle theorem remains**, and its scope is now stated rather than implied: its blocked-on-endpoint arm does not use the splice at all — it runs `removeFromAllEndpointQueues`, a `RobinHood.fold` over the whole object store preceded by `spliceOutMidQueueNode`, for which the tree has frames and `ipcInvariant` but no queue-shape conjunct.  Carrying twenty conjuncts across that fold is phase-sized, not residual-sized; re-registered in `docs/REGISTERED_DEBT.md` with a closure target of *before RR8 closes* | 2 | S |
| RR7.23 | Declassification mediums.  **LANDED v0.34.74**, both in the implement-the-improvement direction.  **Finding 5** proposed registering the service-registry serialization gap as an `UncoveredLockDomain`; the gap was closable instead, so it is closed: all four registry writers declare `stateLevelLock` by name — the trio in write/write/**read** and, the one the prose kept naming and no footprint declared, `lockSet_lifecycleRetype`, whose `cleanupEndpointServiceRegistrations` sweep writes the same map.  `serviceRegistry_footprints_share_serialization` pins it and a new writer must extend it.  **Finding 6** (the wait-before-signal declassified badge, undeliverable until the context restore is live) is kept as boot-path work and made an *executed* acceptance criterion: `BP7.7`, with its own box in the phase acceptance line and in the WS-BP gate.  **Follow-up at v0.34.76**: the widened retype footprint falsified a Tier-2 pin in `SmpInformationFlowSuite` that had listed `lockSet_lifecycleRetype` among the footprints free of the state-level write; corrected to the positive form by the assertion block's own criterion, since a retype is not a hot IPC path and does write global structure | 2 | S |
| RR7.24 | Panic-hang remediation mediums.  **LANDED v0.34.75**, both by implementing the mechanism.  **Finding 12**: three of the six definition-of-done gates were unrunnable *as written* — a gate nobody can invoke reports nothing, which is worse than one that fails.  All three shared one defect, a proxy standing in for the fact: `cargo test --release` could not compile because `ffi.rs`'s `panic = "abort"` guard read `not(debug_assertions)` as a stand-in for "the image" and a release *test* build satisfies it (its condition is now `target_os = "none"`); `ITER_OVERRIDE` was read by nothing, so the plan's 1000-iteration invocation ran the compiled-in count and reported success for a run it had not performed; and the "13 tests" figure had drifted to 14, so the gate is restated over the module rather than over a number.  **Finding 13**: the `Locks/Refinement.lean` methodology hub is implemented rather than struck — and as the *shared engine*, not a fourth statement of the method: the block fold all three bridges are built on is defined once there and each bridge's fold is pinned to it by `rfl` | 2 | S |
| RR7.25 | RwLock-deferred mediums — the verification row §2.3 describes, and it verifies by *running* rather than by observing that a script exists.  **LANDED v0.34.76**: **finding 27** (D-5's miri and loom gates unmet) closed at `v0.34.50` in RR6 and is confirmed by execution — miri passes 71 tests under `-Zmiri-strict-provenance`, loom's 19 exhaustive-interleaving models all pass on the per-push lane, `loom` is `cfg(loom)`-conditional so the cross build resolves none of it, and the iteration counts meet the ≥ 10⁴ figure by default rather than by an override path.  **Finding 28** (the pool instantiating the non-FIFO lock) closed at RR6.10 and is confirmed: the pool is `[QueuedRwLock; _]`, every `ffi_rw_lock_*` entry resolves through it, the kernel instantiates the CAS-retry lock nowhere, and `build.rs` pins the element type so a revert fails the build.  The sweep RR6 did not owe: `cross_thread_reader_stress`'s docstring still described a 100-iteration smoke test and deferred the gate to an env-override path that did not exist until RR7.24 | 2 | S |
...
| RR7.27 | Implement-the-improvement sweep: the DeviceTree-to-`PlatformConfig` boot bridge — a platform/boot surface unrelated to the row above, so its own task.  **LANDED v0.34.58**: `PlatformConfig.fromDeviceTree` plus the board-versus-binding check (RAM against `rpi5MachineConfig`, MMIO against `RPi5.mmioRegions`) and the production consumer `bootAndInitialiseRPi5FromDtbOrHalt`, which boots through the checked entry or parks the PE.  `DeviceTree.fromDtbFull` has a caller.  The pointer→`ByteArray` read stays SM10.1's, as the finding assigns it.  **PR #892 review round 2** (v0.34.110): the check compared every board against the fixed 4 GiB `rpi5MachineConfig`, so the 1 and 2 GiB boards `BCM2712Config` has declared since V4-D were refused, and a single-entry reading of "covered" refused an aperture the firmware reports in two `reg` pairs.  The binding now installs the largest of `rpi5Variants` the board's account covers (`PlatformBinding.bindMachineConfig`, `rpi5BoundMachineConfig`; the smallest when none is covered), the bridge validates the board against that same function (`rpi5PlatformConfigFromDtb_ok_binds_detected_variant`), and "covered" is the union of same-kind entries (`memoryRegionCoveredByUnion`, sound by `memoryRegionCovered_sound`) in `Platform/Boot/MemoryCoverage.lean`.  PR #892 review round 5 (v0.34.113) hardened the parser the bridge reads: `parseFdtNodes` must reach a top-level `FDT_END` and every partial exit is `.malformedBlob` (the Rust walk's round-1 fix, swept to its Lean sibling); the memory selector is `memoryNodeReg?` over that same tree — one walk, one answer (`findMemoryRegPropertyChecked_eq_memoryNodeReg?`) — with the `status`, `device_type` and top-level filters the Rust selector applies; and a peripheral's `reg` is translated through its ancestor buses' `ranges` (`FdtAddressContext`), so a child-relative address is never reported as a physical one and a bus with no `ranges` hides its children, per Devicetree Specification v0.4 §2.3.8 | 1 | S |
| RR7.28 | IPC de-threading medium — the verification row §2.3 describes for RR3, and like RR7.25 it verifies by *measuring* rather than by reading.  **LANDED v0.34.77**: the remedy finding 1 asks for is built (`*_preserves_replyCallerLinkageReciprocal` for every transition in the family, consumed at each bundle's construction site), and `--report` measures **0 threaded statements out of 158**, 0 post-state bindings, all four payoff theorems present.  The sweep RR3 did not owe, and the reason a verification row earns its place: that census figure was quoted as **146** in `CLAUDE.md`, `AGENTS.md` and `docs/spec/SELE4N_SPEC.md` — the count before RR7.22 added twelve statements two cuts earlier.  A hand-maintained number beside a derivation had gone stale inside the documented evidence for the de-threading claim itself.  Corrected, and made self-checking: the gate holds every tracked Markdown file outside `CHANGELOG.md` and `docs/dev_history/` to its own `len(bundles)` and refuses an unreadable spelling rather than skipping it, with four token-preserving witnesses | 1 | S |
| RR7.29 | Reply objects medium — the #2.d "end-to-end" acceptance chain never crosses the syscall gate.  **LANDED v0.34.78** by implementing the missing coverage rather than re-scoping the claim: `sd058_mintReplyCapThroughTheSyscallGate` drives `.mintReplyCap` through `dispatchSyscall` in nine checks — grant required and sufficient, the same call without it `.illegalAuthority`, a wrong-kind primary capability and a non-Reply source both refused with nothing written, the minted handle validated as a **resolution** at `syscallRequiredRight .reply`'s own right rather than as a field read, and the CDT edge recorded and revoked.  Pinned by a token-preserving mutation: weakening the arm's required right from `.grant` to `.write` fails the suite and left the #2.d chain green.  The chain keeps its place and names its companion | 1 | S |
| RR7.30 | SMP foundations medium — `numCores` is a literal, so "the `coreCount := 1` Sim binding can never shape kernel state".  **LANDED v0.34.79**: the premise is false at HEAD (PR #889 rounds 3, 5 and 20 made `coreCountLe` a class obligation, `declaredCores` bound the boot's idle install, and `MachineConfig.declaredCoreCount` carry the number into the machine the live affinity transition reads), and the root cause of the misreading was the constant's own docstring, which still described the pre-`coreCountLe` design.  Rewritten to state the actual relation and its three enforcement points; the one genuinely unstated link is now `determineTargetCore_lt_declaredCoreCount`, closing the **unpinned** half so no thread of any kind is enqueued on a PE the machine does not have, with four runtime checks on a one-PE and a four-PE machine.  The proposed remedy (make `numCores` be `PlatformBinding.coreCount`) is declined **with its argument**: `CoreId = Fin numCores` fixes every per-core `Vector`'s width, so a typeclass-derived bound would make every kernel theorem relative to a binding — seL4 takes the same build-time constant, and `coreCountLe` already fails a too-wide binding's elaboration rather than shipping a wrong image | 1 | S |
| RR7.31 | Master plan medium — §2.1 and §7.2 assert live fine-lock concurrency and a 720 µs WCRT the shipping kernel does not have.  **LANDED v0.34.80**, and the figure was arithmetically wrong as well as unshipped.  §2.1 now opens with what ships (the global entry lock, RR7.12's eight bracketed arms, the unbracketed scheduler entries) and names the three fine-lock Track C closure rows below, plus Track D, as what removes it — by ID there rather than here, since a row body naming a higher-numbered one reads as a dependency to the plan gate and this is a forward *pointer*, not a consumption.  §7.2's `4 × 3 × ~60 µs ≈ 720 µs` took a **typical** footprint size for the ceiling: at `maxLockSetSize` — 9 since RR7.11, 8 before — the product is 1620 µs and 1440 µs, both outside the 1 ms tick, and at 60 µs the boundary is five locks.  Its third factor is measured nowhere in the tree.  Replaced by the budget condition solved for the measurable factor — `admissibleCriticalSection`, **37 µs** for the 1 ms tick at this cut's ceiling of nine (the WS-OD donation-chain work raised the ceiling to eleven at `v0.34.128`, moving the figure to **30 µs**; `docs/REGISTERED_DEBT.md` carries the live value) — with two payoff theorems, three `decide`-checked negatives and six runtime checks, so the plan now claims no numeric syscall WCRT and a future move of `maxLockSetSize` confronts a theorem | 1 | S |
| RR7.32 | Doc-sync medium — an open IPC verification workstream registered in no canonical source, with a payoff theorem that did not exist.  **LANDED v0.34.81**: both halves closed and verified by running rather than reading (WS-DT is a section-A row *and* a workstream-registry entry in `REGISTERED_DEBT.md`, closed at `v0.34.43`; `--report` lists all four payoff theorems as present).  The sweep is the mechanism the class lacked: `check_workstream_plan.py` now holds every plan under `docs/planning/` to being named in a canonical index, derived on both sides, and found one live orphan on its first run — `SMP_FOUNDATIONS_PLAN.md`, reachable only through a sibling plan's prerequisites — now linked from the status index.  Five witnesses, including the orphan's own shape and the fail-closed direction | 1 | S |
| RR7.33 | Unwired proven structures (§7).  **Finding 15 LANDED v0.34.82**: `ipcUnwrapCaps`'s dead `senderCspaceRoot` is gone, and with it the decision §9.2 had been waiting for — the fail-closed branch is **not** wanted on its own terms, because the transfer reads `TransferCap.srcNode` (minted at resolution against that very root), because the receive arm's lookup made the *receiver's* syscall fail on a fact about the *sender's* TCB, and because a destroyed source is already declined per capability by `sourceRevoked`.  The deadness propagates, so the parameter goes from the send/call `WithCaps` wrappers, their `OnCore` and cross-core-dispatch forms, the flow-checked wrappers and the dispatch arms too — ~330 sites, every proof repaired.  **Finding 98 LANDED v0.34.83**: the four accessors have a reader (`perCoreStats`, pinned against dropping one of the four) and the reader has a meaning — `perCoreStatsPlausible` is the disjoint-subset containment `per_cpu_stats.rs` states in prose and nothing in Lean stated, with the module's own "a core that ran for ≥ 1 tick saw ≥ 1 IRQ" as a theorem and seven runtime checks, three of them negatives in the shape a mis-wired accessor makes.  Its *invocation* needs a booted machine and is **BP8.5** with its own acceptance box | 2 | M |
| RR7.34 | Plan-named artefacts that do not exist (§7).  **LANDED v0.34.84**, each authored or corrected, none struck.  **Two authored**: `donation_perCore_consistent` is §10's catalogue entry over all three live SchedContext hand-offs (`PerCoreDonationStep` is the relation; the proof is the three per-path migration theorems and nothing else), and `declassifyStoreOnCore_state_log_independent` says the audit trail does not steer the downgrade it records — over two runs differing in exactly the trail, both required to succeed, since the trail genuinely decides *whether* the step is admitted.  **One gate built**: SM0 §6.3 claimed Tier 0 forbids `native_decide` and nothing in the tree checked it; it is now in the forbidden-marker alternation, pinned by a planted-token mutation, and inert today because the scan reads the comment-free code view.  **Citations corrected where the artefact exists under an accurate name**: the plan's `notification_waiters_nodup` was one namespace short (verified by running all five of that plan's reachability checks), five more dangling names were found in **live SM8 docstrings**, and four of `CLAIM_EVIDENCE_INDEX.md`'s 117 citations resolved to nothing.  **Made self-checking**: `scripts/check_claim_evidence_citations.py` (Tier 0, nine witnesses) derives both the citation set and the declaration set across five languages, so the class cannot recur in the one document whose job is to name evidence | 5 | L |
| RR7.35 | Gate coverage the claims assume (§7).  **LANDED v0.34.85**, four claims each given the mechanism it had been asserting.  **Finding 78**: `lakefile.toml`'s three interpreter-run suites were declared and compiled by nothing — including `negative_state_suite`, the one CLAUDE.md names as build-fragile, so the exact hazard the interpreter runs route around was the one no gate checked.  Tier 2 now *derives* the executables from its own `lake env lean --run` lines through `lakefile.toml`'s `root =` field, so an interpreted suite added later is compiled without a second list and a module with no `lean_exe` is reported rather than skipped.  **Finding 22**: nine `docs/dev_history` cross-references survived a claim that they were gone; all redirected to the live canonical source, and Tier 0 holds it — as `run_prose_negative_check`, since every reference was a comment and a code-view scan would have found none of them and reported absence.  **Finding 28** is the same claim's other half and shows why it drifted: "closed in SM0" was made on **three different surfaces** (this plan's §11 command, the foundations plan's "production sources", the register's), each true of its own, none owning the difference — the gate scans the union of all three plus `tests` and `rust`, where two more references were hiding, so every spelling is true at once.  **Finding 87**: eleven i18n READMEs published a `v0.33.101` snapshot and four GitBook surfaces two *further* stale generations, because `sync_readme_from_codebase_map.sh` drives two files while the sync matrix claims the translations mirror.  `scripts/sync_translated_metrics.py` closes it — and does so *correctly for the languages that inflect*: Russian, Ukrainian and Arabic decline the counted noun, so a digit-only rewrite would have published `11 287 деклараци**я**` and `11,287 **إعلان**`.  Each such noun is a declared slot selected by CLDR plural category; a category with no declared form **fails the run** by name rather than guessing, and the alternation is exactly the declared forms, so the table cannot silently drift from the translation it models.  Twenty-four plural cases and eleven token-preserving witnesses, wired into `test_docs_sync.sh` beside its own self-test | 4 | M |
| RR7.36 | Boot-core-pinned thread-state classification (§7).  **LANDED v0.34.86**, and the verification found the sweep was not empty.  The lift itself closed at RR5.10 and is confirmed: `inferThreadState` asks every core, and the boot-state theorems pin it.  **Three consumers RR5.10 did not touch.**  (1) *A second answer to one question*: RR5.10 wrote `threadRunningOnSomeCore` / `threadQueuedOnSomeCore` as fresh folds over `allCores` in a module that does not import the one where SM5.C.1's and SM5.D.4's `runnableOnSomeCore` / `runningOnSomeCore` already asked it — and that pair diverging would let the cross-core wake's single-placement guard and the classification disagree, i.e. a thread enqueued twice while still classifying as running.  They are now *defined as* those predicates, so the divergence is impossible rather than merely unchecked.  (2) *The registered debt itself*: `threadInactiveFlagConsistent` — the relation `tcbSuspend` / `tcbResume` / the cancellation and fault suspends actually read — is now **preserved by the per-core context switch** and by the preempt it composes, under two side conditions that are the two ways it breaks (a displaced thread stranded off every queue; a dispatch of a thread classified `.Inactive`).  The reusable machinery is `threadPlacedOnSomeCore`, `inferThreadState_eq_inactive_iff` and two frame lemmas, so the wake, lifecycle and IPC writers that remain are per-transition applications rather than fresh arguments, and the register row says so.  (3) *The detector nobody called*: `assertStateInvariantsWithoutSync` had zero callers — built to catch operational drift and never invoked — and the full classification it runs is false after any dispatch, so a live check could not exist.  `threadInactiveFlagConsistentChecks` / `assertLiveThreadStateInvariants` are that check, and five runtime witnesses run it either side of a real `switchToThreadOnCore`, including the negative that the *full* classification does not survive (the reason the narrow relation had to be stated) and a queued thread flagged `.Inactive` (the premise, exhibited).  The first run of it failed on the suite's own fixture, whose user TCB took the field's `.Inactive` default while being enqueued | 1 | S |
| RR7.37 | Test-surface corrections (§7).  **LANDED v0.34.87.**  **Finding 59**: §8's D-1 gate asks for "≥3 `decide`-checked fixtures (success, reader-batching tie, writer-after-readers)" and every `decide` in the suite was over `applyOp`, `writerWaitDepth` or the concrete event model — the *temporal* claim had no executable witness.  All three now exist over `enqueueStep`/`admissionStep`, plus the shape that shows why D-1.9's enqueue premise is needed (a lone writer is admitted at step 1 and `enqueueStep` answers `none`, because it never was a waiter), plus `rwLock_fifo_admission_temporal` **instantiated** on a two-writer trace with every premise — the LC1 no-withdrawal window included — discharged by `decide` rather than assumed.  Mutation-tested by moving the tie.  **Finding 74**: the fifteen `r4a_`/`r4c_` identifiers are renamed to what they test.  The naming gate cannot catch them and correctly does not try: `r` is a *declined* single-letter family with its measurement recorded (105 collisions — ARM registers `r0`/`r1`, `hR0` hypotheses), so this is the rule holding where no scanner can.  **Finding 84**: `svc_stub_returns_not_implemented` asserted `NOT_IMPLEMENTED != 0` — a fact about a constant, true before the handler existed — under prose retired twice (AN9-F wired the dispatch; RR5 put the readiness gate ahead of it).  Replaced by the property it meant, driven through the seam.  **The sweep it opened is the larger half**: that test aborted the *whole* library test binary, because a `fatal_halt` inside an `extern "C"` handler hits Rust's abort-on-unwind guard, and the library binary's readiness bit is *owned* by one timer test that asserts it unset when it starts.  Two more library tests were in the same race and passed only on a favourable schedule; all three now live in `readiness_gate_after_mark.rs`, which marks the core itself.  `build.rs` keeps the class closed with a **derived** gate — the halting seam set is a fixpoint over the crate's own call graph seeded from `halt_*_before_lean_ready`, read on the *host* build's view so a `hw_target`-only halt is not one — refusing any library test that selects the halting arm.  Five witnesses, and one of them caught a defect in the scanner itself: it searched the strings-blanked view for a marker whose predicate *is* a string literal, so it had been blanking no block at all | 3 | M |
| RR7.38 | Splice-neighbour queue ownership (`UncoveredLockDomain.queueOwnershipProtocol`) — the third of the three SM3.B-owned domains the register's §6 finding 16 names, the other two having closed at RR7.8 and RR7.9.  **LANDED v0.34.88**, by the cheaper of the two options the domain's own entry costed.  The gap was that a suspend's neighbour link writes are *authorized* by the queue owner's write lock but not *excluded* against other writers of the same TCB: `lockSet_tcbSetPriority` wrote a queued neighbour holding neither the endpoint lock nor a lock the suspend takes.  **Option A taken**: the eleven footprints that can write a queued TCB — `tcbSetPriority`, `tcbSetMCPriority`, `tcbSetIPCBuffer`, `tcbSetAffinity`, `tcbSetFaultHandler`, `tcbResume`, `tcbBindNotification`, `tcbUnbindNotification`, `schedContextConfigure`, `schedContextBind`, `schedContextUnbind` — each declare the queue owner's write lock, resolved from the pre-state by `queueOwnerAt`.  Option B (let the suspend name both neighbours' `tcbLock`s) was declined *with its cost*: it raises `maxLockSetSize` and so moves the WCRT ceiling `admissibleCriticalSection` RR7.31 had just made a theorem, for every syscall.  The parameter is a `QueueOwner`, not an `Option LockId`, and that is the load-bearing choice: a `LockId` carries an arbitrary kind, so a footprint taking one could only be admitted by a `permittedKinds` arm listing *every* kind — the `.declassify` shape, honest there and dishonest here, since a wait queue is owned by an endpoint or a notification and nothing else.  `QueueOwner.lock_kind` fixes it to two, so the eleven arms admit exactly `.endpoint` and `.notification` and the fixed part of each footprint stays pinned.  No parameter takes a default, so every bound theorem is restated at the new arity and the RR7.18 census checks it.  **Deletion last** (RR7.8's order): the `¬` that stated the gap and its `_omits_endpointLock` witness are deleted, the constructor and its registry entry are gone (the inventory falls to four), and three Tier-3 `run_negative_check`s pin that none of them may come back | 1 | M |
| RR7.39 | Fine locks, Track C closure — the scheduler domain (`UncoveredLockDomain.schedulerDomain`; the fine-lock plan's named follow-on SM3.C.9.b). The per-core scheduler entries the RR7.12 bracket does not cover — the timer tick, the `.reschedule` SGI receiver and the secondary bring-up entry — commit run-queue and replenish-queue state under the global entry lock only. Declare their `SchedLockId` footprints from the model footprints that already exist (`timerTickOnCoreLockSet`, `timerTickOnCoreCompleteLockSet`, `chooseThreadOnCoreLockSet`, `switchToThreadOnCoreLockSet`, `wakeThreadLockSet`, `enqueueIdleThreadOnCoreLockSet`, `advanceDomainOnCoreLockSet`), bracket the export bodies through `withLockSet` with RR7.12's revalidate-and-refuse discipline, prove each transition's write set within its footprint, and only then delete the constructor, its violation theorem and the inventory arithmetic, replacing the Tier-3 `run_check`s with `run_negative_check` pins (RR7.8's deletion-last order). Consumes RR7.12.  **LANDED v0.34.89**, with two departures from the row as written, both stated rather than absorbed.  (1) **The domain had no runtime at all.**  `SchedLockId.runQueue` / `.replenishQueue` named locks `SystemState` had no word for, so a bracket over a scheduler footprint could sort the list, walk it and change nothing — the row's "bracket the export bodies through `withLockSet`" was not implementable as written.  The cut therefore builds the domain: `SystemState.schedulerLocks` (the per-core words, beside `objStoreLock` and *not* inside `SchedulerState`, so a lock write frames the data it guards), the `SchedLockId` acquire/release/withdraw primitives whose `.object` arm calls SM3.C's own, the `SchedLockSet` footprint type with a fail-closed `ofList?`, and `schedulerLockBracketDomain`.  The bracket itself is **shared**: `Concurrency.runBracketed` in the new `Locks/LockBracket.lean`, of which RR7.12's `runUnderDeclaredLockSet` is now definitionally the object-domain instance — one answer to "what does a revalidating 2PL bracket do", per the one-question-two-answers rule.  (2) **A finding blocked the row and is fixed in it**: `timerTickOnCoreCompleteLockSet` named at most two run-queue locks (`bootCoreId` and `c`) while the tick's replenish-drain and timeout wakes both place via `determineTargetCore` — an arbitrary core — since SM5.F and PR #880 rounds 7–8 made them target-aware.  A declared set that does not cover a write is a false footprint, so the segment is now **every** core's run-queue write lock; `timerTickOnCoreCompleteLockSet_serialises_pairwise` makes the zero-concurrency-cost argument checkable (every tick footprint already holds the object-store *table* lock).  Containment is proved for both steps: `perCoreRescheduleStep_coversWrites` (production) and `perCoreTimerTickStep_coversWrites` (staged, its frame chain running through the staged CBS modules).  The constructor is **narrowed, not deleted**: this row closes the *entries*' half, and `syscallSeamSchedulerDomain` records the half that remains — moving the RR7.12 syscall seam onto this domain, which needs per-arm resolved wake targets.  (This row's stated reason for that — "because those footprints hold `stateLevelLock` rather than the table lock" — is **false**, and was corrected when the syscall half landed at `v0.35.181`: `stateLevelLock` IS the table lock, `acquireLockOnObject`'s `.objStore` arm reading nothing of the `LockId` but its kind.  The remedy it licenses is therefore a *unified* footprint rather than a second bracket, and the constructor is deleted with it.)  Owner **RR8** | — | L |
| RR7.40 | Fine locks, Track C closure — the dynamic PIP chain (`dynamicPipChain`). The boost/revert walk's per-member TCB and home-core run-queue write locks are discovered as the walk proceeds, so no pre-state footprint can name them. Route the walk through SM3.C.11's `DynamicChainExtension` inside the RR7.12 bracket — extend the held set hand-over-hand as each member is discovered, in `LockKind` order so the deadlock-freedom argument survives — prove the walk's writes stay within the extended set and that the extension composes with the bracket's revalidation, and only then delete the constructor under the same deletion-last discipline. Consumes RR7.12  **LANDED v0.34.90**, with the acquisition shape stated rather than assumed.  The locks are nameable because the preceding row gave the scheduler domain a runtime: `PriorityInheritance.pipChainSchedFootprint` declares every visited thread's TCB write lock **and** its home core's run-queue write lock — the member `updatePipBoostOnCore`'s bucket migration writes and `LockSet` had no constructor for.  **Not hand-over-hand, and the row's wording is the departure**: the `SchedLockId` order is `object < runQueue < replenishQueue`, so per-member coupling walks the ladder backwards at the second member; the footprint is two segments (all TCB locks in path order, then all home-core run-queue locks in `CoreId`-ascending order), which is the only shape the ladder admits and is strictly stronger — the whole chain is held before any bucket moves.  The run-queue segment is `allCores.filter`, so it is ascending, duplicate-free and `numCores`-bounded by construction rather than by a `dedup` needing its own ordering proof.  The extension is `Concurrency.runChainExtension`, shared with `withDynamicChainExtension`, which was repointed onto it in the same cut so "acquire a discovered chain, act, unwind" has one answer.  `propagatePipChainCrossCore_coversWrites` proves the walk writes nothing outside the footprint, over a `pipChainVisited` derived from the transition's **own** `blockingServer` recursion; `runBracketed_chainExtension_composes` states the composition with the declared bracket, and says why the nesting order is fixed.  The footprint is bounded by the chain length and `numCores`, **not** by `maxLockSetSize`, and says so — a reader who assumed otherwise would read the WCRT figures as covering the PIP walk.  PR #892 review round 5 (v0.34.113) closed the ordering: the chain is resolved from `blockingServer`, which is not ordered by object id, so a footprint could be declared *descending* and `schedulerLockBracketDomain` acquired the declared list verbatim — a lock-order inversion against the SM0.I ladder.  The domain now sorts (`SchedLockSet.lockAcquireSequence`), as `objectLockBracketDomain` has since SM3.B, so ladder order is the domain's rather than a convention each resolver has to keep; `pipChainSchedExtension_acquires_in_ladder_order` takes no hypothesis about the walk.  Constructor deleted | — | L |
| RR7.41 | Fine locks, Track C closure — the interior CNodes of a multi-level CSpace walk (`cspaceWalkInteriorCnodes`). `resolveCapAddress` descends through child CNodes the arguments cannot name, so every CPtr-resolving footprint holds only the root. Make the walk lock-coupling — hand-over-hand read locks down the CSpace, each child acquired before its parent is released, the dynamic-acquisition shape RR7.40 establishes — prove that a concurrent `cspaceDelete` of an interior slot conflicts with a resolution passing through it, and only then delete the constructor under the same deletion-last discipline. Consumes RR7.12 and RR7.40  **LANDED v0.34.90**, with a stated departure from the row's mechanism.  `Capability.cspaceWalkPath` derives the CNodes a resolution passes through from `resolveCapAddress`'s **own** recursion — same guard check, same radix split, same child selection, same bit descent — and `cspaceWalkLockSet` declares a **read** lock on each; `cspaceWalk_conflicts_with_delete` is the payoff the root-only footprint could not state: a `cspaceDelete` whose target lies on the path shares a conflicting lock with the resolution, so SM3.E's conflict order separates them (the delete's half, `lockSet_cspaceDelete_target_write_mem`, is stated beside the footprint it is about).  **The walk is not made hand-over-hand**, and the reason is the ladder: the CSpace's edges do not follow `ObjId.val`, so a coupling walk acquires out of SM0.I order and would need its own deadlock-freedom argument over the capability graph — an argument every future capability operation would then have to maintain.  Instead the discovered set is **sorted** into ladder order and acquired through RR7.12's revalidating bracket unchanged (`resolveCapAddressUnderWalkLocks`), which buys the same exclusion while keeping the one total order the whole tree rests on; the revalidation has real content here, unlike at the scheduler entries, because the interior is discovered by reads its own locks do not yet protect.  Read locks, so two resolutions through one CNode still do not exclude each other.  The root-only footprints the live seam declares are **complete**, not approximate, and v0.34.91 proves it: `abiEntryGate` admits a resolution only when the root consumes every address bit, and `cspaceWalkPath_single_level` says such a walk reads exactly its root, so `cspaceWalkLockSet_single_level` makes the declared root lock the whole CNode footprint.  A multi-level walk — the one case a root-only footprint would be false for — is refused at that gate, so no footprint is declared for one; the mechanism above is what a future consumer admitting one would take.  PR #892 review round 4 (v0.34.112) closed two relations in that mechanism: the declaration is **refused** above `maxLockSetSize` (`declaredLockSetForCSpaceWalk` answers `none` for a walk past the ceiling — one key per level, so a CSpace deeper than nine levels — and the bracket falls back, per RR7.12's rule that a footprint the bound is false for is never claimed; the RR7.18 census cannot see a state-derived footprint, and could not state a bound there is none of), and a **failed** lookup is a read the footprint names: the `none` arm records the key and `cspaceWalkKeyLock` declares `stateLevelLock` in read mode for a key holding no CNode, which conflicts with the write every structural writer declares (`mem_cspaceWalkLockSet_missing`; `cspaceWalkPath_dropLast_cnode` — every key before the path's last holds a CNode — bounds it to one such member).  Nothing is owed per arm.  Constructor deleted | — | L |
...
### RR8 — Phase closure and hand-off to SM10 — **LANDED v0.35.55 → v0.35.203**
...
| RR8.3 | State `queuePPrev` consistency as an invariant — the prerequisite the next row's collapse is blocked on, the dual removal's precondition needing to be discharged rather than assumed.  **LANDED v0.35.57**: `queuePPrevAgreesWithPrev` is `dualQueueSystemInvariant`'s fourth conjunct (so `ipcInvariantFull`'s twenty conjuncts and the 178-bundle family are unmoved), the removal's guard is the named `dualQueueRemovalGuard` the transition itself reads, `dualQueueRemovalGuard_eq_position_and_pair` factors it into a position question and the pairing, and `dualQueueRemovalGuardHolds` / `…_of_dualQueueSystemInvariant` discharge the pairing half from the invariant.  Membership stays a hypothesis and the witness measures why: a detached thread carrying `(none, .endpointHead, none)` *satisfies* the pairing and still fails the guard | `SeLe4n/Kernel/IPC/Invariant/`, `SeLe4n/Kernel/IPC/DualQueue/Core.lean` | L |
| RR8.4 | Collapse the two endpoint-queue removals onto one definition, and restate the endpoint arm's bundle over it.  They write the same fields to the same values today, which is exactly why they can drift; consumes RR8.3.  **LANDED v0.35.58**, and the row's own premise was **false**: the two agreed on the head and parted on the tail, because `endpointQueueRemove` asked `q.tail = some tid` (the fact) while `endpointQueueRemoveDual` asked `removed.queueNext = none` (a **proxy** for it) and derived the tail from `queuePPrev`.  Under a connected queue the two coincide; `ipcInvariantFull` joins a queue's boundaries nowhere, so on a state it admits — a queued thread with no successor that is not the tail — the inferring form cleared the tail and **stranded the queue's real tail**, a thread that could then never be dequeued, which is WS-OD OD3.9's defect class.  `queueRemoveBoundary` (`Model/Object/Types.lean`, beside `queueUnlinkPredecessor` / `queueUnlinkSuccessor` — the third and last piece of *what does unlinking write*) is the shared definition and asks the fact, with the four shapes a guarded removal writes named once (`queueRemoveBoundary_{headLast,headMore,midLast,midMore}`).  Asking the fact leaves the proxy's direction to be established, so the dual now **checks** the two answers agree — `queueTailPairAgrees`, the first factor of `dualQueueRemovalGuard`, with `dualQueueRemovalGuard_eq_position_and_pair` restated over three factors — and refuses that state instead of mishandling it: fail-closed where the inference was fail-open.  The payoff is that WS-OD OD1.3's `spliceRemovedIsTailWhenLast` is **deleted**, not merely unused: every consumer was conditioned on the dual succeeding (`dualRemovalEnabled`), so the guard discharges what the hypothesis supplied, `SpliceShape`'s two no-successor branches carry `hTail` outright, and `endpointQueueRemove_agrees_with_dual`, `endpointQueueRemove_establishes_ipcInvariantFullExceptMembership` and `abortPendingIpcOnEndpoint_preserves_ipcInvariantFull` each shed a hypothesis.  The fact did not vanish and the deletion is not its closure: queue connectivity is still a missing invariant, now `dualQueueRemovalGuardHolds`'s `hTailLast`, an obligation on whoever *calls* a removal rather than one carried by every theorem about one.  The dual's boundary write is pinned by `endpointQueueRemoveDual_writes_queueRemoveBoundary` (hypothesis-free, replacing two consumer-less WS-L3 tail theorems); the single's by the four existing `endpointQueueRemove_ok_*` shape theorems, measured rather than assumed — a no-op mutation of its Step 3 fails four of them — so a fifth restatement was **not** added.  **And the boundary question had four askers, not two**: this row's own enumeration was wrong twice over — OD3.9 had already established there are *three* removals, and the third splits its work, so `spliceOutMidQueueNode` writes only the neighbour TCBs while its boundary write is `removeThreadFromQueue`, a fourth spelling of the same expression.  Found by sweeping for who computes a queue boundary rather than by trusting the row; it already asked the fact, so unifying it is a de-duplication (`removeThreadFromQueue_eq_queueRemoveBoundary`) and not a behaviour change, and its `lookupTcb`-absent arm stays its own because that arm is the *absence* of a removal.  **Running the sweep again found a fifth**: `frozenQueueRemove`, the frozen mirror, which a kernel-tree sweep misses structurally — that surface is reached by neither library root — and which is why `queueRemoveBoundary` lives in the *model* rather than the IPC layer.  It already asked the fact, so it was right where the live dual was wrong and nothing compared them there; its *guard* carries none of the live removal's three refusals, so it succeeds where the kernel refuses, which is registered debt (§C) rather than absorbed, the guard family being in the layer `FrozenOps` does not import | `SeLe4n/Model/Object/Types.lean`, `SeLe4n/Kernel/IPC/DualQueue/{Core,Transport}.lean`, `SeLe4n/Kernel/IPC/Invariant/{Defs,QueueSplicePreservation,TimeoutAbortPreservation,Structural/PerOperation}.lean` | M |
| RR8.5 | Collapse the two spellings of "tear down the caller↔Reply link" — `SystemState.consumeCallerReply` against `Lifecycle.Suspend.consumeReplyLink`.  It precedes the reply-arm bundle below, which is then stated over the surviving definition rather than restated after it.  **LANDED v0.35.63**: the reply path's monadic step is the survivor — it is the one the chain results are stated over, and it writes through `storeObject`'s bookkeeping where the cancellation twin wrote raw — and the cancellation path reads it through a *pure projection*, `SystemState.consumeCallerReplyLink`, the one state the infallible step leaves (error arm eliminated by `consumeCallerReply_isOk`, not defaulted), with `consumeCallerReply_eq_link` the bridge every cancellation-side fact is derived through.  `consumeReplyLink` is that projection under the victim's own `replyObject`; the raw-insert twins `clearTcbReplyObject` / `clearReplyObjectCaller` and the twenty-eight theorems stated over them are deleted (thirty declarations, twelve added), every surviving `consumeReplyLink_*` theorem keeps its statement — the projection lemma gains the index-completeness hypothesis the reply path's own projection theorem reads — and is a corollary of its `consumeCallerReply_*` twin, and `cancelIpcBlocking_lifecycle_eq` is deleted rather than given a third hypothesis (with the teardown through `storeObject`, its definitional lifecycle frame holds on no reachable `.blockedOnReply` state, and nothing consumed it).  The reply arm's projection composite now carries the index completeness the reply path's projection theorem reads, through two new restore lemmas.  The census registers the projection as a primitive and the teardown as a site; Tier 3 refuses both retired names tree-wide and pins the teardown's body to the projection.  The collapse *measured* the drift it is one instance of — sixty executable definitions still write the object table raw beside `storeObject` — and registered it (`REGISTERED_DEBT.md` table C) rather than absorbing it, since that migration is sixty proof repairs and not an M | `SeLe4n/Kernel/Lifecycle/Suspend.lean`, `SeLe4n/Model/State.lean` (the row named `Model/Object/Reply.lean`, which the cut did not need: the survivor is a `SystemState` step, so the projection belongs beside it) | M |
| RR8.6 | The deschedule sweep: `descheduleThread` and `cancelIpcBlockingOnCore` still resolve the victim's core at `determineTargetCore`, which PR #895 review round 10 records as equally a proxy for placement.  Repoint both onto `placedCoreOf?`, as that review's two closed sites already are.  **LANDED v0.35.79**: both repointed, and the sweep found a third — `suspendThreadOnCore`'s G4/G4b removal pair (home, then running core), which misses a victim *queued* off its home, so a `.tcbSuspend` on an unpinned thread preempted on a secondary core left it dispatchable while `.Inactive`.  All three read the placement through one primitive (`descheduleAt` / `descheduleAtPlacement`), the poke is resolved on the same placement (`descheduleSgi?`, the wake's ghost-guard deliberately not mirrored), the composite is `descheduleThread` on the post-wake state by `rfl`, the three scheduler-domain footprints are keyed on the placement with `cancelIpcBlockingOnCoreSchedLockSet_covers_deschedule` relating the pre-state footprint to the post-wake removal, and §3.24 of the cancellation suite computes the retired reading beside the live one on the queued-off-home shape | `SeLe4n/Kernel/IPC/CrossCore/Cancellation.lean`, `SeLe4n/Kernel/IPC/CrossCore/EndpointCall.lean` | L |
| RR8.7 | `cancelIpcBlocking`'s **reply** arm carries `ipcInvariantFull` — the first half of the RR7.22 residual, the last of the five arms without a bundle.  Consumes RR8.5.  **FIRST CUT LANDED v0.35.80** — and what it found first is that the arm's *teardown* had two bundle theorems that asserted **nothing**: both asked for `ipcInvariantFull` of the state they run on **together with** that state's answered caller not being `.blockedOnReply`, which `replyCallerLinkage`'s second direction refutes, so their premises held on no state while their names read as coverage.  Reported as a finding, proved in Lean (`replyCallerLinkage_refutes_woken_linked_caller`, kept as a permanent pin), and closed by the *narrowest* relaxation that admits the state — `replyCallerLinkageExcept` keeps the reciprocal pair required at the woken thread and drops the blocking clause as a **disjunct** — with `ipcInvariantFullExceptReplyLinkage` registered as a de-threading pre-state predicate.  The **unit** is the restore-and-teardown pair, each half being the other's repair, so `restoredAndConsumed_preserves_ipcInvariantFull` is the cut's keystone; one theorem was generalised, one restated over the relaxed pre-state, and one deleted behind a tombstone with its residue registered (table C).  `notQueueBlocked_bounds_no_endpoint_queue` gives the “does this victim bound an endpoint queue?” question one owner, shared with the notification arm.  **SECOND CUT LANDED v0.35.82** — the arm keystone `cancelIpcBlocking_replyArm_preserves_ipcInvariantFull`, over `returnDonationToCancelledCaller_preserves_ipcInvariantFull` (itself over `abortHolderPendingIpc_preserves_ipcInvariantFull` and `spliceThreadReplyFrameOut_preserves_ipcInvariantFull`), with `cancelIpcBlocking_reply_arm_eq` pinning the arm to its four steps by `rfl`.  Two things the cut measured rather than planned.  The **timeout-budget discipline had to cross the reclaim** and nothing carried it, so the cut added that chain — and naming `endpointQueueRemove_timeoutBudgetFrame` retired an *inline second derivation* of the same fact in `TimeoutAbortPreservation.lean`, which had reached it through the dual removal at the cost of `ipcInvariantFull` and `dualRemovalEnabled`; the `upToField` route needs neither.  And the arm passes the **pre-cancellation** TCB to a teardown running on a state that holds that TCB with its binding rewritten, so `restoredAndConsumed_congr` states what the teardown actually reads (the reply object, and nothing else of the record).  One anchor was written and **deleted before it shipped**: a negative on the ungated coherence hypothesis fires on a clean tree, because that spelling is legitimately present on the abort's own carriage — the M0 baseline of the block's mutation harness caught it, and the positive on the gated form is the relation check instead | `SeLe4n/Kernel/Lifecycle/Invariant/CancellationReplyShape.lean`, `SeLe4n/Kernel/IPC/Invariant/Defs.lean`, `SeLe4n/Kernel/IPC/Invariant/Structural/DualQueueMembership.lean`, `SeLe4n/Kernel/Lifecycle/Invariant/CancellationQueueShape.lean` | L |
| RR8.8 | **CLOSED v0.35.193.**  Discharge `abortHolderProjectionStable` as far as the labelling can carry it, and **absorb the prerequisite** the register scheduled against no row — the fact that a donated context's holder is at least as high as its donor.  Consumes RR8.5 and RR8.6.  **FIRST CUT LANDED v0.35.83** (plan and prose repair, ahead of any proof, because the scheduled remedy was refuted): the register closed all three of these obligations against *an endpoint/notification queue label-uniformity invariant, established on every enqueue path*, and that invariant is **unestablishable**.  The live send / call gate is `endpointFlowGate ctx ep (threadLabelOf sender) (endpointLabelOf ep)` — an **order**, not an equality — so a lower-labelled and a higher-labelled sender are both admitted onto one higher-labelled endpoint by design, `publicLabel → kernelTrusted` being the flow `securityFlowsTo_prevents_label_escalation` documents as intended.  `endpointAdmissionAdmitsMixedObservability` (`SeLe4n/Kernel/InformationFlow/Projection.lean`) is that admission as a theorem, `decide`-checked, with an observer that sees exactly one of the two; establishing uniformity would mean narrowing the gate to equality and refusing a lower-labelled client's send to a higher-labelled server.  What the gate **does** give is the other direction, and it settles the scope: every waiter's label flows to its endpoint's, so of the abort's three write classes the *endpoint object* and the *holder's own TCB* both close from the labelling (the second through the donating `Call`'s gate composed with the server's receive gate), and the *queue neighbours* close from nothing — their labels are constrained only against the endpoint's.  So the residue is **representational, not a missing invariant**: a queue's content must live in an object whose label dominates every member's, which the endpoint is and a member's own TCB is not.  Taking the links out of the TCB is the remedy, measured at **2272 whole-word occurrences of `queueNext` / `queuePrev` / `queuePPrev` across 67 tracked `.lean` files** — far past this phase, so it is registered with its own closure target rather than absorbed here, and this row closes having reduced the obligation to that one class and named it.  A finding rode along and is registered with the same measurement: the links **survive `projectKernelObject`**, so an observable thread's projection already names a non-observable one's identity whenever both are admitted onto one endpoint — standing, not created by the cancellation, and the same class as the `replyObject` erasure SM6.D landed.  **SECOND CUT LANDED v0.35.84** — the labelling layer, and it found the `v0.35.83` prose asserting an inference the tree could not make: `endpointFlowGate` compares `endpointLabelOf` while `objectObservable` decides visibility from `objectLabelOf`, and `LabelingContext` carries those as **independent** fields with nothing relating them, so *one question had two answers* and "the endpoint object is non-observable whenever a waiter is" was not derivable.  `LabelingContextValid.endpointObjectCoherence` is the missing conjunct, discharged structurally for every constructed context from the new `DeploymentLabeling.hEndpointObjectCoherence`, which the one base constructor meets by reflexivity — so it costs the production family nothing and no existing theorem had made the conflation.  On it: `donationOwnerFlowsToHolder` (the absorbed prerequisite, stated over `replyDonationReturn?`, the tree's single reader of that question) and `blockedSenderFlowsToEndpoint`, with `donationHolderHigh_of_donorHigh`, `endpointObjectHigh_of_admittedThreadHigh` and `blockedSenderEndpointObjectHigh` as their consequences.  **Two of the three write classes are discharged** and the third isolated: `abortHolderSpliceHigh_of_victimHigh` derives the endpoint object's and the holder's own non-observability and takes the neighbour clause as its one parameter, stated over `endpointSpliceHigh` — **relocated** from `IPC/CrossCore/NotificationSignalNI.lean` to `InformationFlow/Invariant/Operations.lean`, because the predicate had one owner that one of its two askers could not reach, and the relocation repaired no proof, which is the evidence the layer was wrong.  **What the second cut did NOT do**, stated rather than implied at the time: the reduction stopped at `endpointSpliceHigh`.  `endpointQueueRemove` — the *single* removal the abort runs — had no projection lemma where the dual does, so `abortHolderProjectionStable` still carried its whole obligation; and writing that lemma had to bridge a mismatch the reduction found, `endpointSpliceHigh` naming the predecessor through `queuePPrev` while the single removal reads `queuePrev`.  **THIRD CUT LANDED v0.35.193 — the row CLOSES.**  The mismatch was a mismatch of *spelling*: `queuePPrev` carries exactly one bit beyond `queuePrev`, so RR8.3's `TCB.queuePPrevAgreesWithPrev` makes the two predecessors one thread (`endpointSpliceHigh_queuePrev_high`), read at a thread through the new `queuePPrevAgreesWithPrev_lookupTcb` rather than by re-opening the `getTcb?` / `objects[…]?` bridge at each consumer.  On it: `queueNeighbourPatch_preserves_projection_high`, `endpointQueueRemove_preserves_projection{,_and_invExt}`, `abortPendingIpcOnEndpoint_preserves_projection`, `abortHolderPendingIpc_preserves_projection` and the discharges `abortHolderProjectionStable_of_{spliceHigh,neighbourHigh}` — the last taking the neighbour clause and nothing else.  Three things the cut records.  **The chain is over TABLES, not states**: the single removal's four writes are raw `RHTable.insert`s inside one record update, so it composes `objects_insert_preserves_projection_high` at `{ st with objects := · }` rather than the two store primitives' frames the dual's lemma uses — the same property, a different proof, because the two removals are written differently.  **Two pins moved down a layer**: `endpointQueueRemove_ok_getEndpoint?` and `endpointQueueRemove_eq_patches` sat in `Lifecycle/Invariant/CancellationReplyShape.lean`, which is *incomparable* with the module owning `endpointSpliceHigh`, so the projection lemma could not reach the equation that decomposes the operation it is about; they are beside `queueNeighbourPatch` and `spliceOutMidQueueNode_eq_patches` now — the sibling answer to the same question — and `_eq_patches` spells its boundary and link clear through `queueRemoveBoundary` / `tcbWithQueueLinks` rather than restating both inline, retiring a live instance of the duplication RR8.4 removed (still `rfl`).  And **the residue is exhibited**: `tests/SmpInformationFlowSuite.lean` §14 runs the live abort on one state under two labellings differing in exactly one object's label — with the neighbour high the low view is unchanged at every written key, with the neighbour low it moves at that key and no other.  Six prose passages were corrected a second time, in the same six places, because a correction is a claim and gets the same scrutiny as a claim.  Five passages across three docstrings in `CancellationNI.lean` — `abortHolderProjectionStable`, `abortHolderWakeHigh`, and `cancelIpcBlockingOnCore_reply_cancellation_NI`, whose docstring carries three of them — and the G3 note in `InformationFlow/Invariant/Operations.lean` named the refuted invariant, one of them naming the **impossible** direction (*a low endpoint holding a high waiter*, which the gate forbids); all six are corrected | `SeLe4n/Kernel/InformationFlow/Projection.lean`, `SeLe4n/Kernel/IPC/CrossCore/CancellationNI.lean`, `SeLe4n/Kernel/InformationFlow/Invariant/Operations.lean` | L |
| RR8.9 | Discharge `abortHolderWakeHigh`, the scheduler twin of RR8.8 — a run-queue insert is filtered by the inserted thread's own observability, and the holder's label is not determined by the victim's.  Consumes **RR8.8** (for the donor-dominance invariant), not RR8.6 alone.  **LANDED v0.35.84 — it closes OUTRIGHT, in the same cut as the prerequisite it consumes.**  Landing it beside RR8.8's invariant layer rather than one cut later is this plan's own rule applied: splitting them would have left `donationHolderHigh_of_donorHigh` with no consumer, and an unconsumed lemma is what the project forbids.  `abortHolderWakeHigh_of_donationOwnerFlowsToHolder` is the discharge, with `cancelAbortedHolderWake?_donation` relating the wake's resolved holder to the donation resolver's.  Re-scoped at v0.35.83:  Its own docstring said it waited on the endpoint-queue label-uniformity invariant `abortHolderProjectionStable` waited on; it never needed it.  The obligation is a single `threadObservable ctx observer holder = false`, so the donor-dominance invariant RR8.8 absorbs is the whole of it — nothing here asks about any queue's representation, which is why this row does **not** inherit RR8.8's registered residue and is re-rated S.  **Since `v0.35.158`** the reclaim deschedules the holder it unbinds instead of waking it; the obligation is `descheduledHolderHigh`, discharged by `descheduledHolderHigh_of_donationOwnerFlowsToHolder` from the same invariant — a removal is filtered by the removed thread's own observability exactly as the insert was, so the row's closure is unchanged in content | `SeLe4n/Kernel/IPC/CrossCore/CancellationNI.lean`, `SeLe4n/Kernel/InformationFlow/NonInterferenceCrossCore.lean` | S |
| RR8.10 | The composite over all the teardown's arms lifted to `cancelIpcBlockingOnCore` — the second half of the RR7.22 residual, and what retires box 4's confinement note.  Consumes RR8.4, RR8.6 and RR8.7.  **LANDED v0.35.85.**  `cancelIpcBlocking_preserves_ipcInvariantFull` over the four bodies the six `ipcState` constructors are serviced by (this row's own heading said *five arms*; the count is four bodies over six constructors, the three endpoint states sharing one), and `cancelIpcBlockingOnCore_preserves_ipcInvariantFull` over the transition the live `.tcbSuspend` dispatch runs, both in the new production `SeLe4n/Kernel/IPC/Invariant/CancellationBundle.lean` — a module of its own because the three arm theorems live in three, so there is no natural home among them, and imported from `SeLe4n.lean` and from the Tier 1 de-threading census so it is inside every derived domain rather than exempt by omission (the `v0.35.60` lesson).  **The premises are per-arm because the facts are**: `ipcInvariantFull` entails none of the queue-coherence facts the three bodies need, so `cancelIpcBlockingArmPremises` gates each group on that arm's own `ipcState` equation — the equation each arm theorem already takes — and the `.ready` arm, which commits no write, owes nothing.  **The cross-core lift adds no premise at all**, which is the cut's substantive claim: only `passiveServerIdle` of the twenty conjuncts reads the scheduler, so `ipcInvariantFull_of_descheduleFrame` (RR2.5, factored for exactly this) transports nineteen; the migration frames because it writes neither a run queue nor a current slot; the holder wake frames *unconditionally* because an **insert** cannot break a conjunct whose antecedent is "not queued"; and the victim's placement removal is discharged from `cancelIpcBlocking_victim_ready` — a cancelled victim ends `.ready` on every arm, established per arm rather than assumed.  Two things the cut found rather than planned.  The de-threading census counts the two new statements, so the family figure moved **184 → 186** across five prose sites, and the Tier 1 elaborator census had to import the module — its **own** closure is what the semantic layer sees, and Tier 0's text scan counting a statement the elaborator cannot reach is precisely the drift that check exists to catch.  And `restoreToReadyStaging_preserves_objects_invExt` in the **staged** `CancellationNI.lean` was a byte-for-byte duplicate of the production `restoreToReadyStaging_invExt`: one question, two answers, with the production one unreachable from the staged copy's readers and the staged one unreachable from production.  It is deleted and its two consumers repointed | `SeLe4n/Kernel/IPC/Invariant/CancellationBundle.lean`, `SeLe4n.lean`, `SeLe4n/Testing/IpcDethreadingEnvironmentCensus.lean`, `SeLe4n/Kernel/IPC/CrossCore/CancellationNI.lean` | L |
| RR8.11 | The replenish-migration congruence: that `cancelIpcBlockingMigrated` *establishes* `replenishQueueAffinityConsistent_smp` on the composite.  Consumes RR8.6 and RR8.10.  **LANDED v0.35.86.**  **The congruence could not be stated of the code as written**, which is this row's finding: the migration aimed at `determineTargetCore st victim` — the home of the thread the reclaim is *about to* bind the context to — and the reclaim's guards are fail-closed, so on a refusal it moved `scId`'s replenishments to a core no thread bound to `scId` is homed on.  The reply path never had it, its migration sitting in the `.ok` continuation of its return: one question, two spellings, the pure one wrong.  **The destination is the fact now** — `replenishHomeOfSchedContext` of the post-teardown state (`SeLe4n/Kernel/SchedContext/ReplenishAffinity.lean`), whose `_spec` *is* `migrateSchedContextReplenishment_preserves_affinityConsistent_smp`'s destination obligation, so a refused reclaim degenerates to a self-migration and `cancelIpcBlockingMigrated_establishes_replenishQueueAffinityConsistent_smp` plus `cancelIpcBlockingOnCore_establishes_replenishQueueAffinityConsistent_smp` take **no** hypothesis beyond `objects.invExt` and the pre-state invariant.  The general form `migrateSchedContextReplenishment_to_home_preserves_affinityConsistent_smp` states what any rebind-then-migrate transition owes and requires the *source* to be where the invariant currently puts the context, since a context bound to no thread locates no entries.  Three teardown frames were built for it and are the cut's bulk: `cancelIpcBlocking_getSchedContext?_eq_of_reclaim_frame` (one six-arm analysis, two corollaries) and `cancelIpcBlocking_affinity_frame`, the `Option.map` form whose `none` case is *the teardown materialises no thread* — the direction the tree lacked at the abort and the consume, so `consumeCallerReply_getTcb?_backward` is new.  `cancelIpcBlocking_determineTargetCore_eq` moved here and was generalised from the victim to any thread (its old proof could only be stated at the victim).  `consumeCallerReply_getSchedContext?_eq` and `spliceReplyFrameOutOrSelf_getSchedContext?_eq` were *extracted* from `removeCallerReplyFrame_getSchedContext?_eq`, which had them inlined.  Behaviour on every reachable state is unchanged (`cancelIpcBlockingMigrated_eq_victim_home_of_committed`, golden trace byte-identical), and §3.25 of the cancellation suite computes the retired destination beside the live one on a refused reclaim across two cores | `SeLe4n/Kernel/SchedContext/ReplenishAffinity.lean`, `SeLe4n/Kernel/IPC/CrossCore/Cancellation.lean`, `SeLe4n/Kernel/IPC/Invariant/CancellationBundle.lean`, `SeLe4n/Kernel/IPC/Invariant/Structural/DualQueueMembership.lean`, `SeLe4n/Kernel/IPC/CrossCore/EndpointCall.lean`, `tests/SmpCancellationSuite.lean` | M |
| RR8.12 | `UncoveredLockDomain.syscallSeamSchedulerDomain`: per-arm resolved wake targets, so the syscall seam's scheduler writes are covered by a footprint rather than by the free over-approximation.  RR7.39's residual, and independent of RR8.3–RR8.11.  **Landing across three cuts, in the order the numbering rule's semantic half requires** — the segment the per-arm footprints are built from, then the footprints and their coverage, then the bracket and the deletion (RR7.8's deletion-last order).  **Cut 1 LANDED v0.35.87**: the per-arm footprints need a same-kind segment over a core set of *unbounded* arity (a `.replyRecv` names four run queues and up to six replenish queues), and the tree had `sortedSchedCorePair` and `sortedSchedCoreTriple` — one question at two arities, each with its own `_map_fst_mem` and `_pairwise_le`.  A third arity would have been the enumeration-standing-in-for-a-derivation shape, so both are **deleted** and `schedCoreSegment (f : CoreId → SchedLockId) (cs : List CoreId)` takes the set, with seven lemmas replacing four and `runQueueLock_injective` / `replenishQueueLock_injective` as the injectivity witnesses.  That the two were instances was **measured** exhaustively over the concrete core enumeration at both lock constructors before they were deleted, so no footprint's value moved; 61 use sites across five files were repointed.  It was a **fifth** spelling, not a third: `cancelDonatedDonationOnCoreSchedLockSet` spelled the sorted pair out again as an `if`-chain forty lines below a comment saying the pair had been relocated so "every use below is unchanged", and its `_keys_nodup` / `_pairwise_le` fell from 30 and 35 lines to five and six.  The canonicalisation is shared too — `Concurrency.canonicalCores` is RR7.40's own `allCores.filter` derivation stated once, read by `pipChainHomeCores` and by every new segment — and `allCores_pairwise_le` (over `pairwise_finRange_le`, by induction rather than by `decide` at the literal) retired an `unfold Concurrency.allCores; decide` that `allCores_nodup`'s own docstring says not to write.  Three Tier 3 positives are repointed, the OD5.3 suspend anchor pins the three-core *list* as a relation, and each retired name is a tree-wide negative, mutation-tested by reintroducing it as **code** (both survive in prose, which the code view strips).  Golden trace byte-identical; `maxLockSetSize` unmoved.  **Cut 2 LANDED v0.35.90**, and it is a **security fix rather than the declaration work the row planned**: reading the `.tcbSuspend` arm for its resolved wake target found that the arm performs none — WS-OD OD1.7's holder wake and WS-RR RR7.22/RR8.11's replenishment migration both sit in `cancelIpcBlockingOnCore`, a composite no production path calls, while the live arm and the `suspend_thread_cross_core` seam run `suspendThreadOnCore`, whose G2 took the *bare* teardown.  Measured on the live transition: an aborted donation holder ends `.ready`, `.unbound` and on no run queue on any core — a permanent denial of service against a passive server, reachable with a TCB write capability over a thread in one's own call chain — and the reclaimed reservation's replenishment is stranded on the holder's home core beside a SchedContext the same step deactivates.  `cancelIpcBlockingReclaimed` names the composite's prefix so the shared step is reachable from both consumers, G2 reads it, and the two declarations that were silent about the wake grew by its core (`suspendThreadOnCoreSchedLockSet`'s `wakeCore`, `suspendThreadOnCoreWriteSet`'s first entry — the holder's *placed* core, `holderPlaced` / `cancelUnboundHolderCore?`, since `v0.35.158`).  `tests/SmpCancellationSuite.lean` §3.26 computes the retired G2 beside the live one; two Tier 3 anchors pin G2's identity, mutation-verified both ways.  It lands **before** the declaration work because a resolved wake target the live transition does not write would be a footprint wider than its operation.  Two obligations registered in `docs/REGISTERED_DEBT.md` §A against **Cut 3**: OD1.7's payoff lifted through the six stages after G2, and the single-core reference path, which cannot see the shared step across the import boundary.  **Cut 3 LANDED v0.35.91**, and it is the *class* behind Cut 2 rather than the declaration work: `SeLe4n/Testing/KernelTransitionReachabilityCensus.lean` (Tier 1) derives every definition whose result type transforms `SystemState` — **528**, of which **288** are reachable from one of the 7 committing `@[export]`s and **240** are not — and reconciles the unreachable half against a pin in both directions.  It closes the register row Cut 2 opened, **and correcting that row was part of landing it**: the row said a reachability census "would have failed on the day" OD1.7 added its step, which is false — `cancelIpcBlockingOnCore` already existed and was already unreachable, so a partition cannot see a step added *inside* a registered entry.  The `standsBesideLive` rows are what can: each names the surface, the live definition that re-composes it, and a pin theorem mentioning both, verified by inserting a step into the composite and watching `cancelIpcBlockingOnCore_eq_reclaimed_deschedule` stop elaborating.  Its first run produced three findings, each registered: a docstring family calling an unreachable definition "the live `.lifecycleRetype` seam" (corrected at all three sites, the same reading that let Cut 2's defect survive), the revocation family having **no syscall arm at all**, and four transformers consumed by nothing.  Seven mutations, each keeping every token and breaking only the relation, plus a clean-tree control.  **Cut 4 LANDED `v0.35.92`**, closing the first of the two obligations Cut 2 registered: `suspendThreadOnCore_holder_still_placed` lifts WS-OD OD1.7's payoff through the six stages after G2, so *a suspend does not strand the server its victim called* is a theorem about the transition the live `.tcbSuspend` runs rather than something `tests/SmpCancellationSuite.lean` §3.26 measures.  It is the composition of frames the register row predicted, and landing it established two things the row did not.  The **holder-is-not-the-victim** fact — what licenses G4's removal to leave the holder alone — was a sentence in `suspendThreadOnCore`'s own G4 comment and is now `cancelAbortedHolderWake?_ne_victim`, derived from the wake's two guards asking the same TCB's `ipcState` for incompatible constructors.  And the holder's **resolvability travels with its placement**: a scheduling point strands a thread whose TCB does not resolve, so every stage carries `(getTcb? holder).isSome` forward and `switchToThreadOnCore_preserves_threadPlacedOnSomeCore` states that side condition instead of hiding it.  (**Superseded at `v0.35.158`**: the reclaim deschedules the holder it unbinds — PR #897's review having measured that the woken holder ran at its legacy band on no reservation — so the lift is `suspendThreadOnCore_holder_unplaced`, the not-the-victim fact is the structural `cancelUnboundHolder?_ne_victim`, and the resolvability scaffolding is retired with the wake; the relocations below stand.)  The chain walk's run-queue and `current` frames moved from `IPC/Invariant/FaultProgress.lean` — which imports `IPC.CrossCore.Fault`, which imports `Cancellation`, so the payoff could not reach them — to `Scheduler/PriorityInheritance/Propagate.lean`, beside the walk, and their three `_not_mem_of_not_mem` forms were **retired**: the biconditional is the answer and `updatePipBoostOnCore_mem_runQueueOnCore` had been the step-level one since WS-RR RR2.6.  `threadPlacedOnSomeCore_congr_at` (agreement **at** the thread, which the equality form cannot give for a step that clears another thread's `current` slot) and `threadPlacedOnSomeCore_dispatch_ne` / `_dispatch_self` are the shared answers, the second pair read by `threadInactiveFlagConsistent_dispatch`, which had the argument inline.  Three anchor mutations decide the new checks -- a retired name reintroduced as **code**, the derivation replaced by a hypothesis with the name still present, and the conclusion moved to the state G2 leaves -- each with a clean-tree control.  **Cut 5 LANDED `v0.35.93`**, closing the second obligation: the boot-pinned single-core `Lifecycle.Suspend.suspendThread` reached for the *bare* teardown, so WS-OD OD1.7's strand was reachable on the tree's single-core reference — and it could not have reached the shared step, because `cancelIpcBlockingReclaimed` was declared in `IPC/CrossCore/Cancellation.lean`, a module that **imports** `Lifecycle/Suspend.lean`.  *When a question has one owner and an asker that cannot see it, the owner is in the wrong layer*, and it was: all 31 relocated declarations read a `TCB`, a run queue or a replenish queue and none reads anything cross-core.  They keep the `SeLe4n.Kernel` namespace they were declared in, so the move **renames nothing** and every one of the 168 references in the tree is untouched; the single new import (`SchedContext.ReplenishAffinity`, whose own closure is 63 modules and excludes `Suspend`) was measured cycle-free before the move.  §3.26 (vi) drives the reference path on the stranding state and computes the retired bare G2 beside it; both new negatives are mutation-verified with clean-tree controls.  The structural divergence between the reference and the live path (`revertPriorityInheritance` vs `propagatePipChainCrossCore`) is unchanged and was never what the row was about.  **Cut 6 LANDED `v0.35.94`**, and it is the shared answer the per-arm footprints are built from rather than the footprints themselves: the three-domain ladder `(object, .write) :: runSegment ++ replenishSegment` was spelled at **seven** definitions, **four** of them carrying a byte-identical twenty-five-line `_pairwise_le` — one question with four answers, about to become nine as the five undeclared arms are added, which is the shape Cut 1 closed one level down and this cut closes one level up.  `schedFootprintOfCores (runCores replenishCores : List CoreId)` is that answer, with `_pairwise_le`, `_write_only`, `_keys_nodup`, `_length_le`, `_subset` and one characterisation (`mem_schedFootprintOfCores_iff`) proved once; the three footprint modules shed **263 lines against 120 added** while the shared constructor cost 250 once, and the five new arms will cost none.  It closes Cut 1's **unswept sibling**: `cancelIpcBlockingOnCoreSchedLockSet` answered a question about a *set* with an `if placed = some c then … else … ++ [(runQueue ⟨c⟩, .write)]` deduplication branch, which is precisely the defect Cut 1 recorded against `cancelDonatedDonationOnCoreSchedLockSet` in the same file and did not sweep onto its neighbour; with the branch gone `…_contains_wake_runQueue_write` is **unconditional**, where it needed `placed ≠ some c`.  *That the argument is a set is now a theorem* — `Concurrency.canonicalCores_congr` / `schedFootprintOfCores_congr`, with `canonicalCores_singleton` the `Option CoreId` arm — rather than a docstring claim.  The criterion for which footprints are this constructor is stated as the **shape of the argument** (a core *set*) rather than as a list of names, so the three fixed-single-core literals (`wakeThreadLockSet`, `descheduleThreadLockSet`, `cancelBoundDonationOnCoreSchedLockSet`) are excluded by a rule a future footprint is measured against; `ChainFootprint.lean`'s ladder is deliberately its own, its object segment being a per-thread TCB lock rather than the table lock.  Seven Tier 3 relation anchors (each footprint's own **body** names the constructor, declaration-bounded) and three negatives (the deduplication branch, the two-core replenish list, a re-inlined `runQueue_lt_replenishQueue` under the IPC and Lifecycle surfaces), all six mutation-verified in both directions with a clean-tree control.  No footprint's *set* of members moved, so the golden trace is byte-identical and `maxLockSetSize` is unmoved.  **Cut 7 LANDED `v0.35.95`**: the first **three** arms declare a scheduler-domain footprint — `.notificationSignal` (through the *bound* arm the live dispatch routes to), `.notificationWait` and `.send` — each `schedFootprintOfCores` of the arm's own SM8.B **write set** rather than of a second resolution of the same cores, so a footprint and a confinement claim naming different cores is unstateable rather than merely refuted.  That required moving three write sets and one wake frame into **production**: they were declared in the staged `InformationFlow/NonInterferenceCrossCore.lean` (which imports `Kernel.API`) and in the staged `PerCoreCbs.lean`, where the `_local` suffix on `wakeThread_replenishQueueOnCore` was the signal — Cut 5's rule, *when a question has one owner and an asker that cannot see it, the owner is in the wrong layer*, for the fourth time in this row.  An **empty replenish segment is a theorem**, not a reading of the body: `observableSlotsConfinedToCores` covers six per-core slots and the replenish queue is not one of them, so the four `…_replenishQueueOnCore` frames are what make the declaration honest.  Twelve Tier 3 anchors, three of them **relation** positives over each footprint's body, and every positive pins the name *followed by its parameter list* — a mutation found that `^theorem X` matches a suffix-renamed `X_local`, so the old form was a presence check one character down.  Two stale positives repointed onto the relocated definitions.  Six mutations, all decisive, with a clean-tree control.  **`.receive` and `.replyRecv` are deliberately left undeclared** at Cut 7: both donate, so their replenish segments are non-empty and their cores come from the migration rather than from a confinement write set, and `.receive`'s chain leg is not pre-state computable at all.  **That reason was measured and is only half the obstacle** (`v0.35.145`, Cut 8b-i): `replyRecvBodyWriteSet` *is* a pre-state-computable confinement write set, so what actually blocked `.replyRecv` was **layering** — it and four siblings (`replyRecvPostReceiveDonationWriteSet`, `replyRecvServerDescheduleWriteSet` — `replyRecvHolderDescheduleWriteSet` since `v0.35.149` — `replyRecvDescheduleAndWalkWriteSet`, `receiveLegPipHandoffWriteSet`), plus the `pipChainWriteSet` they all compose, were declared in the **staged** `InformationFlow/NonInterferenceCrossCore.lean`, which a production footprint cannot read, while the arm's spine (`replyRecvBody`, `replyRecvPopDonation`, `replyRecvPostReceiveDonation`, `replyRecvServerDeschedule`, renamed `replyRecvHolderDeschedule` at `v0.35.149`) sits in `Kernel/API.lean`.  Cut 8b-i relocates all six beside the transitions they describe — the chain write set to `Scheduler/PriorityInheritance/Propagate.lean`, the hand-off's to `IPC/Operations/Donation.lean`, the four `.replyRecv` ones to `API.lean` — keeping the `SeLe4n.Kernel` namespace so nothing is renamed, with the confinement theorems left staged because `observableSlotsConfinedToCores` is that module's predicate.  *A premise that survives the cut it motivated will be cited by the next one*, which is why the row is corrected rather than quietly satisfied.  **Cut 8a-ii LANDED `v0.35.107`** (`.receive`, narrowed at `v0.35.112` and Cut C1 `v0.35.160`; the block-path return migrated at `v0.35.161`).  **Cut 8b-ii LANDED `v0.35.162`**: `schedLockSet_endpointReplyRecvOnCore`, `schedFootprintOfCores` of `replyRecvBodyWriteSet` and of `replyRecvHandoffReplenishCores` — the arm's **three** hand-offs (the pop, the block-path return, the re-donation) each read at the state it runs on through its own arm selector, since the pop rewrites the receiver's binding between the legs and a pre-state reading would be a proxy; the block-path pair became one definition for both receiving arms (`receivePreReturnReplenishCores`); three coverage theorems at the migrations' own cores, the empty segment exact in both directions (`replyRecvBody_replenishQueueOnCore_of_no_donation`, over a new reply-leg frame), and `tests/SmpIpcSuite.lean` §3.29 driving three live shapes.  Declaring it found a third instance of the WS-CB passive/legacy split — the never-donated arm does not donate a dequeued `Call`'s context where `.receive` would — registered on that row rather than fixed, since a donation added to a live transition is a bundle and payoff obligation, not a footprint edit.  **Cut 8b-iii LANDED `v0.35.163`** (Cut C3a): `schedLockSet_endpointCallOnCore`, `schedLockSet_endpointReplyOnCore` and the arm's `schedLockSet_replyTransferOnCore`, each `schedFootprintOfCores` of the arm's SM8.B write set (five write sets and one frame relocated to production from the staged non-interference module, with tombstones) and of a replenish segment that mirrors the dispatch's own donation guard at the state the dispatch asks it; the pop's pair given one owner (`replyDonationReturnReplenishCores`) for both reply-shaped arms; coverage at the resolved cores, with the RR2.4 shape covered member for member and the RR2.10 shape's executing-core member recorded as an over-declaration on a false justification; the empty segments exact in both directions over four new frames; witnessed in `tests/SmpIpcSuite.lean` §3.30 and `tests/FaultHandlingSuite.lean` §7c.  **Fix cut LANDED `v0.35.164`** (between C3a and C3b, found while surveying C3b's `.lifecycleRetype` write set; register row 62): the retype's TCB cleanup ran the bare donated return and an index-only removal for a `.bound` thread, so a successful retype falsified `schedContextBindingConsistent` and `replenishQueueAffinityConsistent_smp` — the arm is `cancelDonationArmOnCore` now, the suspend's G3 match named beside the cleanup with the two per-core arms relocated to reach it, `cancelDonationOnCore` defined through it, `retypeTargetDetached.tcbNotBound` added, and the arm carrying the affinity theorem neither caller had; witnessed in `tests/SmpIpcSuite.lean` §3.31.  What it left is register row 63 — the SchedContext-target arm and the composite's own theorem — owned by the fix cut before C3b.  **Second fix cut LANDED `v0.35.165`** (register row 63's arm half): the same cleanup's `.schedContext` arm refused only a stack head, so a retype of a context **bound** to a thread left that thread naming a destroyed object with its index entry and replenish entries in place — `releaseSchedContextBinding` is seL4's `schedContext_unbindAllTCBs` per core, with its own unconditional affinity theorem and the witness §3.32; what the row still carries is the composite's own theorem, blocked by two sweeps' frames being private in a module downstream of both programs.  **Third fix cut LANDED `v0.35.166`** (register row 63's *layering* half): each of those frames moved to the module holding the fact its proof rests on — the two generic accessor bridges to `Model/State.lean`, the splice's to `CleanupPreservation.lean`, each sweep's to its own `Cancellation*Shape.lean` — and `Lifecycle/Invariant/RetypeReservation.lean` (library root, importing `CancellationNotificationShape`, the only layer that sees every frame it composes) proves `cleanupTcbReferences_preserves_replenishQueueAffinityConsistent_smp` and `lifecyclePreRetypeCleanup_preserves_replenishQueueAffinityConsistent_smp` over all six object kinds, with **no** detachment pack, so it covers exactly the states on which the runtime arms do the work.  `replenishQueueAffinityConsistent_smp_frame` joins `_smp_congr` for the three arms that write no object at all.  **Fourth fix cut LANDED `v0.35.183`** (register row 63's remaining half): the effort fact the third cut left — no `preserves_schedContextBindingConsistent` theorem existed anywhere in the tree — is paid.  `schedContextBindingConsistent` reads exactly two projections, so `schedContextBindingConsistent_transfer` beside the predicate carries it whole given both frames (framing the binding alone leaves the backward clause unsupported); `spliceOutMidQueueNode`'s field frame gets one owner over an arbitrary projection, with the affinity reading from `v0.35.166` and the new binding reading as instances and `tcbQueueLinkRewrite` as the relation that makes one owner possible; and eight preservation theorems run from `returnDonatedSchedContext` — the pop, which moves one whole reciprocal pair and takes its uniqueness obligations from Z4-O itself — up to `lifecyclePreRetypeCleanup_preserves_schedContextBindingConsistent`.  The `.schedContext` arm is excluded by `hNotSc` and **refuted rather than proved** (`releaseSchedContextBinding_refutes_schedContextBindingConsistent`, the RR8.7 pattern): the arm deliberately leaves the destroyed context's `boundThread` naming the thread it has just unbound, for the retype's own `storeObject` at that key to replace, so writing `boundThread := none` there would add a store to an object the very next step replaces; `retypeTargetDetached`'s `notSc` makes the hypothesis free at the live call site.  What row 63 still carries is the retype *composite*'s two theorems — its `storeObject` at `target` preserves the invariant exactly when no surviving context is bound to the destroyed thread — and the model-level hole measuring their hypotheses found: neither `KernelObject.wellFormed` nor `retypeReplacementFresh` constrains a replacement SchedContext's `boundThread`, so the admissible replacement space holds a context claiming a thread that does not name it back, which is the class SM6.D closed for `Reply` one field over and which the live path avoids only because `objectOfKernelType` installs `SchedContext.empty`.  **Cut C3b-i LANDED `v0.35.167`**: the three TCB-control arms — `.tcbResume`, the `.tcbSetPriority` / `.tcbSetMCPriority` pair (one footprint, SM8.B giving them one write set) and `.tcbSetAffinity` — declare theirs in a module of their own, `SeLe4n/Kernel/SyscallSchedFootprint.lean`, because `SchedLockId` is declared in `Scheduler/Operations/PerCoreChooseThread.lean`, which imports `Lifecycle/Suspend.lean` and `IPC/Operations/Endpoint.lean`, so every module holding a lifecycle, priority, affinity, SchedContext or retype transition is outside its reverse closure and cannot name a `SchedLockId` at all; moving the identifier down was rejected (it is declared with `RunQueueLockId`, `ReplenishQueueLockId` and the cross-domain order over them, which is what `schedFootprintOfCores` is about), so the header states the rule instead — *a resolved scheduler footprint lives beside its transition where that module can name a `SchedLockId`, and here where it cannot*, the shape the object domain reached at `LockSetTransitions.lean`.  Each is `schedFootprintOfCores` of the arm's own write set, which moved the three SM8.B sets out of the staged non-interference module; the affinity arm's replenish segment follows the thread's **binding** and every empty segment is a theorem against the declaration's own half, so each narrowing is exact in both directions; coverage against the *transition* is its own statement (`schedLockSet_setThreadCpuAffinityOnCore_covers_migration`), the transition resolving its destination at the post-affinity-write state where the footprint resolves it from the argument.  Asking for `schedLockSet_setThreadCpuAffinityOnCore_covers_parametric` is what found the SM5.H.4 parametric family (`setThreadCpuAffinityWithMigrationLockSet`, `migrateRunQueueOnAffinityChangeLockSet`) still in the staged `PerCoreCbs.lean` twenty lines below the tombstone RR2.4 left when it relocated `migrateSchedContextReplenishmentLockSet` out of that same file for that same reason — *a fix applied at one site and not at its sibling*, invisible until a production footprint had to state the relation; both are production now, with four more frames beside the definitions they frame.  A write set's content is deliberately **not** anchored: each footprint's `_contains_*_runQueue_write` theorem is `simp [<the write set>]`, so a mutation dropping a core fails to elaborate — measured rather than asserted.  Witnessed by `tests/SmpCbsSuite.lean` §4.5 (the decisive one: one state, a thread on a reservation and a thread on none, the same migration, opposite segments, with the parametric footprint computed beside the resolved one), `tests/SuspendResumeSuite.lean` SR-035 and `tests/PriorityManagementSuite.lean` PM-FP-01.  `maxLockSetSize` is unmoved.    **Cut C3b-ii LANDED `v0.35.168`**: `.schedContextConfigure`, `.schedContextBind` and `.schedContextUnbind` declare theirs in the same module, each `schedFootprintOfCores` of the arm's own SM8.B write set (four more relocated out of the staged non-interference module) and of a replenish segment read from the arm's own purge.  The configure's keys on the **SchedContext resolving** rather than on its being bound -- an unbound SC has no home, the purge falls back to the boot core, and a stale entry from an earlier binding is what it drops -- and when the SC is bound the purge and the re-bucket land on one core.  The unbind's is the first segment in this family that is **every** core: its sweep arm runs `purgeReplenishmentFromAllCores` because with the bound TCB gone there is no `cpuAffinity` left to read, both arms are decided on the pre-state, and a footprint naming only the home core would be *false* there.  The bind declares none, with the absence stated.  Every narrowing has its theorem in both directions except the sweep arm, which needs none.  Declaring them found a **duplicate resolver**: `SchedContextOps.schedContextBoundThread?`'s docstring has claimed since SM8.B that it is single-sourced *because a second copy would drift*, while the staged module carried `schedContextSubject?`, clause for clause the same function, which the very write set that docstring names read instead; the copy is deleted and refused tree-wide -- *a docstring naming a drift hazard is not a check that the hazard is closed*.  A Tier 3 anchor pinning a relocated write set at its old home is repointed rather than deleted.  Witnessed by `tests/SmpCbsSuite.lean` §4.6, which computes the retired home-only reading beside the live segment on the sweep fixture.  `maxLockSetSize` is unmoved.    **Cut C3b-iii LANDED `v0.35.169`**: the live `.lifecycleRetype` arm declares `schedLockSet_lifecycleRetypeOnCore`, and declaring it made visible that SM8.B's `lifecycleRetypeWriteSet` is a **run-queue** write set -- `observableSlotsConfinedToCores` covers six per-core slots and the replenish queue is not one of them -- so it says nothing about the two reservation steps `v0.35.164` and `v0.35.165` put on the destroy path and a footprint built from it alone would be *false* of the operation.  The replenish segment is keyed on the object kind, with exactly two kinds naming a core: a `.tcb` target's is the donation arm's (nothing for `.unbound`, the thread's home for `.bound`, the return's two migration endpoints for `.donated`, the destination read at the post-return state), a `.schedContext` target's is the release's (the bound thread's home, or **every** core where that thread is already gone -- the second `allCores` segment in the module), and every other kind's is empty with the declaration's own half stated.  Both resolvers read the **pre-state**, which is a fact rather than a convenience: for a SchedContext target every earlier step of the cleanup is the identity, and for a TCB target the donation arm *is* the first step.  Exactness is composed over all six kinds; `threadOccupiedCores` and the two retype write sets moved to production with tombstones and two stale anchors repointed; `tests/SmpIpcSuite.lean` §3.33 computes the run-only reading beside the live footprint on both target shapes.  `maxLockSetSize` is unmoved.    **Cut C3b-iv LANDED `v0.35.170`**, the sixteenth and last of the arms Cut 7's sequence enumerated: `schedLockSet_suspendThreadOnCore` is the live `.tcbSuspend` arm's footprint (which of the remaining nineteen arms write a scheduler slot at all is `declaredSchedFootprintSyscall`'s question, and Cut 8's), and declaring it found that **neither** parametric footprint over the suspend's G2 named the replenish queues that step writes.  Since RR8.11 (`v0.35.86`) the teardown is `cancelIpcBlockingMigrated` and since this row's second cut (`v0.35.90`) the live pipeline runs it, moving the reclaimed reservation from the **holder's** home core to the home the context is bound to at the torn state — and `cancelIpcBlockingOnCoreSchedLockSet`'s replenish segment was `[]` while `suspendThreadOnCoreSchedLockSet`'s was G3's migration read off the *victim's* binding, a different thread and a different state.  That second cut widened the *run* segment by the holder's placed core and did not ask the same question of the replenish segment: *a fix applied at one site and not at its sibling*.  Latent rather than live (the seam does not yet bracket the scheduler domain), so everything stated over those footprints was **silent** about the two queues rather than conservative — RR8.11's and OD3.9's own posture.  Both are fixed here, each taking a `reclaimReplenish : List CoreId` whose resolver `cancelIpcBlockingReplenishCores` lives beside `cancelIpcBlockingMigrated`, reading the same `let`s, because both footprints must name it and neither can see the resolved-footprint module.  Neither half of the arm's own replenish segment is pre-state computable — G3's resolver is read at the **post-revert** state, the reclaim having rebound the victim (WS-OD OD5.3's second pop migrates to the *outer caller's* home) — so the segment re-runs the spine as `replyRecvBodyWriteSet` does.  Exactness is over the whole arm (`suspendThreadOnCore_replenishQueueOnCore_ne`, seven stages, two of which move a reservation and five of which frame every replenish queue); the donation-arm frame gained **one owner at an explicit purge core** (`donationArmAt_replenishQueueOnCore_ne`, the destroy path's now its instance), since the two askers hand it different cores; coverage against the parametric form is stated over the **run-queue half alone**, the honest scope for a form whose replenish segment is four free parameters.  `tests/SmpCancellationSuite.lean` §3.27 computes both retired readings beside the live ones with core 3 as the control; nine anchor mutations, all decisive, one of which found a positive matching a suffix-renamed definition (Cut 7's own finding, one character down).  `maxLockSetSize` unmoved; golden trace byte-identical.  **Cut C4 LANDED `v0.35.171`**: `schedLockSetForSyscall`, the scheduler domain's `lockSetForSyscall` — a `match` over `SyscallId` dispatching to each arm's own resolved footprint, `declaredSchedFootprintSyscall` as the inventory, and the undeclared-none negative over it, which is the load-bearing direction for the object domain's own reason (a caller reading `some S` treats `S` as the complete set of *cores* the transition writes).  The other drift direction is closed by the per-arm `_isSome_iff` family.  `SyscallLockOperands` gains the scheduler domain's five fields rather than a record of its own — the two domains ask different questions of one syscall's operands, and the affinity operand is doubly optional because its inner `Option` is the unpin request.  `.notificationSignal` routes to the bound arm's footprint and `.reply` to the arm's rather than the dispatch's, both pinned as relations with the wrong resolver refused.  `tests/SmpCbsSuite.lean` §4.7 is the witness; seven anchor mutations, all decisive.  **Cut C4b LANDED `v0.35.172`**: `declaredSchedLockSetForAbiEntry`, the seam-level twin of `declaredLockSetForAbiEntry` — `abiEntryPlan`, then `abiEntryLockOperands` on that plan's answer, then the domain's own resolver.  Cut C4 could not have it because that builder supplied none of the five scheduler operands, so `.call`, `.reply`, `.replyRecv` and `.tcbSetAffinity` would each have answered `none` — sound, an undeclared arm establishing no exclusion, and four arms silently outside the coverage this workstream is building.  C4b extends **that** builder rather than adding a second, because a second is what lets one domain's footprint be acquired around the other domain's transition: `declaredSchedLockSetForAbiEntry_shares_decode` states that both footprints are functions of one `(tid, decoded, stFilled)` and one operand record, and a negative refuses the resolver re-deriving the gate or the capability lookup.  What one record costs is a congruence the object domain must satisfy — `lockSetForSyscall_ignores_sched_operands`, over all five fields at once, so a sixth added without extending it is a field nothing has checked — and it is measured at the seam's own operands in the witness, which is what makes “the object domain is byte-identical to Cut C4's” a theorem rather than a reading of two definitions.  Four arms grew the operands their scheduler footprint refuses without, each naming what its own live dispatch arm names (the `.replyRecv` payload is MR0-stripped and badged with the **reply** capability's badge, SM6.D's own distinction, refused in the wrong spelling by a negative); eight more are there because the scheduler domain declares for them, with `.schedContextBind` naming the decoded `threadId` argument rather than the capability's object.  `abiEntryGate_cspaceRoot` and `abiEntrySchedReceiverCspaceRoot` make the `.replyRecv` footprint's CSpace root and the gate's one lookup.  `tests/SmpCrossCoreCallSuite.lean` is the witness, state-dependent by construction — an `.Inactive` victim declares the object-store lock alone and an active one, one field apart, additionally declares the executing core's run queue; sixteen anchor mutations, all decisive.  **Cut C5 LANDED `v0.35.173`**: `SeLe4n/Testing/SchedFootprintCensus.lean` (Tier 1), the scheduler domain's counterpart of `LockFootprintBoundCensus`, which Cut 8a-ii's own measurement asked for — thirty-three of this family's forty-seven theorems had neither a consumer nor an anchor, every one silently deletable, and eight hand anchors were the stopgap.  Two questions.  **Every footprint is `schedFootprintOfCores` applied to two core lists at its full arity**, which is not a style rule but the premise every generic lemma is consumed under: the domain restates none of `_write_only` / `_pairwise_le` / `_keys_nodup` / `_subset` / `mem_…_iff` per footprint because all five are stated once of that function and inherited by shape, so a footprint written any other way loses all five SILENTLY — and `_keys_nodup` is `SchedLockSet.ofList?`'s own obligation, so the constructor then refuses and the arm answers `none`, an undeclared arm rather than a failure.  The question goes to the elaborator by reducing TOWARDS the constant (`Meta.whnfUntil`), since `whnf` would run past it into the `List.cons` the body builds.  **And every footprint is NAMED by `schedLockSetForSyscall` or registered with a reason** — named rather than reached, because a transitive closure would count a footprint as consumed for being mentioned by a reachable helper, silencing the census exactly where it fires; the register holds two supersessions (the bare notification signal, the dispatch-level reply footprint) reconciled in both directions.  Reported: **17 footprints, all canonical, 15 consumed, 2 registered**.  Neither failing branch can fire on the live tree, so the module carries plants on the far side of each decision — a canonical footprint and a hand-written ladder carrying a member the canonical form also carries, a constant with the family's NAME and not its TYPE, and a namer pair separating *named* from *reached* — plus a WIRING case the plants cannot supply, drawn from the live tree: a write-set helper named by a footprint and by no arm.  Seven census-branch mutations and ten anchor mutations, all decisive; five of the anchors were rewritten first, a Lean `Name` literal's double backtick inside a double-quoted `bash -lc` argument being an empty command substitution that deletes itself from the pattern.  **Cut C6a LANDED `v0.35.174`**: the coverage, which the numbering rule's semantic half puts BEFORE the bracket — a bracket acquiring a footprint nobody proved covers the writes hands out exclusion the runtime never established.  `SeLe4n/Kernel/SyscallSchedContainment.lean` (staged, as `SchedLockTimerContainment` is, every proof consuming an SM8.B confinement theorem) holds one bridge and one application per arm: `schedFootprintCoversWrites_of_cores` (production, beside the obligation) makes the OBJECT clause structural — a canonical footprint always names the object-store table write lock — and `schedFootprintCoversWrites_of_confined` supplies the RUN-QUEUE clause from `observableSlotsConfinedToCores`; the REPLENISH clause has no bridge and cannot, confinement covering six per-core slots of which the replenish queue is not one, which is why every donating arm carries a frame of its own.  The split from C6b is semantic: an empty replenish segment makes the clause a whole-state frame and a core-naming one makes it an exactness claim, two different propositions with two different frames — so the seven empty-segment arms (`.notificationWait`, `.notificationSignal`, `.send`, `.tcbResume`, `.tcbSetPriority`, `.tcbSetMCPriority`, `.schedContextBind`) are here plus `.tcbSuspend`, whose `_ne` frame the fourth cut already built.  Eight proved theorems cannot be wrong and could be VACUOUS, so the module carries a refutation per clause and a Tier 3 negative refuses `schedFootprintCoversWrites_refl` inside it — discharging an arm with the no-op lemma is the token-preserving weakening this family admits.  Eight anchor mutations, all decisive.  **Cut C6b LANDED `v0.35.175`**: the first three core-naming segments — `.schedContextConfigure`, `.schedContextUnbind`, `.tcbSetAffinity` — each covered by one application of the bridge, because each arm's `_ne` frame is now keyed on the FOOTPRINT's own replenish segment rather than on a resolution: a resolution-keyed frame answers a different question every consumer must case-split to reach, so the footprint-keyed form takes the plain `_ne` name, the resolution-keyed one is `_ne_of_sc` / `_ne_of_tcb`, and a Tier 3 negative refuses the plain name re-acquiring the narrower hypothesis.  Two readings the proofs make explicit: an unresolved segment is a REFUSAL rather than a gap (both SchedContext arms' transitions fail there, and the proof derives the contradiction rather than assuming resolution), and `allCores` is a legitimate segment on which the clause is vacuous because there is no core outside it.  Seven anchor mutations, all decisive.  **Cut C6c LANDED `v0.35.176`**: the two IPC spines' exactness frames and the `.call` arm's coverage.  The IPC arms' replenish segments are COMPUTED by running the transition, so these frames are the one place a footprint and its operation could describe different migrations — and each is one case split, because Cut C3a made the segment's branch structure and the transition's the same structure.  Six new frames: an `_ne` beside each donation step's `_of_no_donation` (the existing ones say the hand-off moves NOTHING when the resolver declines, the new ones say WHERE it moves when it answers), the two dispatches' frames over their own segments, and the `.reply` arm's, stated PER BRANCH rather than through the hypothesis-parameterised `…_of_dispatch` — that form asks for the dispatch's frame at EVERY message and the segment is message-dependent, the fault branch composing the dispatch at `IpcMessage.empty` and the ordinary branch at `msg`.  `.call`'s coverage is stated of the UNCHECKED dispatch, which is what its write set and confinement result are stated at and what the checked arm equals wherever its flow gate admits.  Seven anchor mutations, all decisive — one of which first read MISSED because the harness's row filter matched a DIFFERENT anchor, which is *run the mutations before believing the cases* applied to the harness rather than to the fixtures.  **Cut C6d LANDED `v0.35.177`**: the live `.reply` ARM's coverage — the ARM, not the dispatch beneath it.  `schedLockSet_replyTransferOnCore` had nothing behind it because the confinement surface stopped at `endpointReplyCrossCoreDispatch`, whose post-state is not the arm's: the arm adds the delivered-message staging on an unfaulted caller and the decoded outcome — a restart frame, or an abandon that DESCHEDULES — on a faulted one.  Five confinement theorems (§5c-bis), each at the write set its own definition derives, plus the two missing machine frames (`applyFaultRestart_machine_eq`, `faultAbandonOnCore_machine_eq`), make the coverage proof one application of `schedFootprintCoversWrites_of_confined`.  **The cut's own first draft overstated its finding and the tree's measurement caught it**: the abandon does NOT name a core the dispatch never names — every successful dispatch opens its write set with the answered thread's home and no step of it writes a `cpuAffinity`, so the append is a DUPLICATE, which `FaultHandlingSuite` §7c had been measuring since Cut C3a.  The draft was written from the shape of the definition rather than from the measurement beside it; the claim is corrected at both sites and the measurement is now asserted with the restart's empty append as its control.  Eleven anchors, nine mutations all decisive, 254 of 254 sweep rows with 0 deferred.  **Cut C6e LANDED `v0.35.178`**: the live `.replyRecv` ARM covered — the one declared arm whose footprint covers its WHOLE body, both chain walks being static members of its run segment (Cut C2), so this is a complete coverage claim rather than one about a prefix.  Five replenish exactness frames, one per stage of the four-stage composition, each keyed on the SUB-SEGMENT the definition appends at that stage, composed through `replyRecvHandoffReplenishCores_eq_of_legs`.  Two things the composite needed and the tree lacked: the general `replyRecvPostReceiveDonation_preserves_objects_invExt` (only the `returned? = none` form existed, so nothing could carry integrity past a donating arm) and `descheduleAtPlacement_preserves_objects_invExt`, spelled inline at three sites.  **A negative anchor that could never fire**: the one refusing the return of a retired hand-kept count read PASS on the mutation that restored it, because the sentence lives in a `--` comment the code view blanks — *gates read code, prose reads prose* meeting the case where the subject IS the text, found by the mutation run and not by reading.  Thirteen anchors, twelve mutations all decisive, 576 of 576 sweep rows with 0 deferred; §3.29 gains the direction its segment assertions cannot see, with the EMPTY-segment shape as the decisive one.  **Cut C6f LANDED `v0.35.179`**: the live `.receive` ARM covered — the arm whose coverage claim is about a SUB-COMPOSITION, deliberately: its run segment is the LEG's, because `.receive` is the one declared arm whose chain walk sits outside it (declared dynamically through `pipChainSchedFootprint`), so the claim is stated at the leg composed with WS-OD OD3.6's donation and a Tier 3 negative refuses the whole-hand-off spelling.  Two lemmas made it statable at all — the thread the leg reports IS the pre-state send queue's head, or the receiver where it dequeued nobody — which is what lets a footprint resolved before the transition and a donation resolved after it name the same two threads; every other rendezvous frame existed because until a coverage proof nothing had to relate the leg's OUTPUT to the resolver.  The block path needs no post-state guard at all (`callDonationSchedContext?_self`: a thread donates nothing to itself), and `queueHeadBlockedConsistent` is taken for exactly one corner, a rendezvous whose sender is not a `Call` yet whose donation resolver answers `some`.  Two confinement theorems were written and REJECTED AS ALREADY DECLARED — *before writing a helper, find the one this tree already has*, and the build is the fastest way to find out.  Eleven anchors, nine mutations all decisive, 547 of 547 sweep rows with 0 deferred; §3.26 gains the direction its segment assertions cannot see, with a strict-subset assertion so it is not vacuous.  **Cut C6g LANDED `v0.35.180`**: the live `.lifecycleRetype` ARM covered, and with it the SIXTEENTH and last declared arm, so every arm `schedLockSetForSyscall` answers a footprint for now carries one.  The unit is the ICACHE-LAYERED arm the syscall dispatches, not the retype core: the `.aside1` shootdown round with the initiator's own TLB drain and the domain-wide `IC IALLUIS` each write kernel state, so a claim at `lifecycleRetypeDirectWithCleanup` would be about a program the arm does not run (a Tier 3 negative refuses that spelling), and both layers being scheduler-silent by theorem is what lets the exactness frame descend by citation rather than by a second case analysis.  Both segments are read at the PRE-state and have to be — the object a retype destroys is gone from the post-state, so a post-state reading would name the empty set on exactly the transition the segments exist for — so this arm has no instance of WS-HP HP10.8's resolution asymmetry.  THREE RELOCATIONS out of the staged non-interference module (`retypeInitiatorDrain_scheduler`, `_machine`, and a private `lifecycleRetypeDirect_framed` deleted for a public sibling), the fourth cut in this sequence to pay `v0.35.59`'s layering rule, which is the signal that the class is a convention and not four accidents.  Every mutation of the footprint fails to ELABORATE rather than failing the suite (§3.23's situation), so §3.33 (d) is a differential inside the suite instead: the same measurement over `runOnlyRetypeFootprint` must FAIL.  Eleven anchors, three mutations all decisive, 456 of 456 sweep rows with 0 deferred.  **Cut B2 LANDED `v0.35.182`**: the register's own WS-CB row named this row's declaration of `.schedContextBind` as the blocker for making a bind PLACE a parked runnable thread (seL4-MCS's `SCHED_ENQUEUE` tail), and with the arm declared and bracketed it landed here.  The declared footprint did **not** move: its run segment was already the bound thread's home core, which is the core the placement inserts on — a declaration written for the operation rather than for the branch it takes is what makes a behavioural widening free, and the two docstrings that predicted a widening are corrected rather than left standing.  Fourteen anchors; every mutation of the production side fails to ELABORATE, so both witnesses are differentials against the retired queued-only reading, each with an already-queued control.  **Cut C6h LANDED `v0.35.181`**: the syscall seam BRACKETS on the scheduler domain and the constructor is DELETED, so the row is closed and the register's uncovered-domain inventory falls from two to one.  The design was decided by a measurement that contradicted this row's own premise: the object-domain footprints' `stateLevelLock` and the scheduler footprints' `schedObjStoreLockId` are the SAME WORD (`acquireLockOnObject`'s `.objStore` arm reads nothing of the `LockId` but its kind), so nesting two brackets would take it twice and walk the SM0.I ladder backwards — the seam acquires ONE unified `SchedLockSet` instead, which is what `SchedLockId` was introduced for at SM5.A.2, with `canonicalSchedLockOfObject` collapsing the two spellings onto one key and the aliasing pinned at ALL FOUR primitives because a renaming is sound only if every operation on the two keys agrees.  The deletion rests on ONE generic bridge rather than sixteen instances: `unifiedSchedLockSetForSyscall_coversWrites` over `schedFootprintCoversWrites_mono`, coverage travelling upward because every clause of the predicate names a lock the footprint does NOT have.  The fallback is unchanged — an arm neither domain declares is the bare step, bit-identical — which is what makes landing it ahead of the remaining object-domain declarations safe.  Fifteen anchors; every mutation of the production side fails to ELABORATE rather than failing the suite (§3.23's situation), so the witness group is a differential against the object domain's own footprint | `SeLe4n/Kernel/Concurrency/Types.lean`, `SeLe4n/Kernel/Scheduler/Operations/PerCoreChooseThread.lean`, `SeLe4n/Kernel/Scheduler/PriorityInheritance/ChainFootprint.lean`, `SeLe4n/Kernel/IPC/CrossCore/{EndpointCall,EndpointReply,Cancellation}.lean`, `SeLe4n/Kernel/Concurrency/Locks/LockSetForSyscall.lean`, `SeLe4n/Kernel/{SyscallSchedFootprint,SyscallLockBracket}.lean`, `SeLe4n/Testing/SchedFootprintCensus.lean`, `SeLe4n/Kernel/{SchedLockBracket,SyscallSchedContainment}.lean`, `SeLe4n/Kernel/IPC/Operations/Donation.lean`, `SeLe4n/Kernel/IPC/CrossCore/{EndpointCallDispatch,EndpointReplyDispatch,Fault}.lean`, `SeLe4n/Kernel/InformationFlow/FineLockFlow.lean` | L |
| RR8.13 | Update `UNFINISHED_SMP_WORK.md`: mark each closed finding with its version, leaving open items visible  (**Formerly RR8.2** — renumbered at `v0.35.56` when RR8 grew the twelve rows above; a citation naming RR8.2 means *this* row.)  **LANDED v0.35.89.**  All **26** findings carry a status marker — 23 CLOSED with the version, 2 PARTIALLY CLOSED with the residual named, 1 REGISTRATION CLOSED with the residual owned by WS-SL — and each was checked against its own artefact in the tree rather than against a CHANGELOG heading.  **Doing it that way found three things no row had.**  §4 finding 11 was **still live**: `trap.rs`'s SVC comment named the deleted `syscall_dispatch_inner` export *and* described the ABI v1 status convention (a raw discriminant in `x0`, wrapped as `DispatchError::Kernel`), so a reader following it would have looked for a symbol that does not exist and then decoded the wrong register — fixed at the seam, naming `lean_syscall_dispatch_cross_core` and the v3 label encoding.  §4 finding 2's stated closure target — endpoint/notification queue label uniformity — was **retracted** at `v0.35.83` (RR8.8) as *unestablishable*, the admission gate being an order rather than an equality, so its closer would have gone after a premise the gate refutes; the note now records the retraction, what `v0.35.84`'s `endpointObjectCoherence` conjunct does cover, and that the residue is representational with non-intrusive endpoint queues as the closure.  And §4 findings 3 and 4 *read* as unregistered debt because they appear in no debt row, when in fact they closed at `v0.34.57` (RR7.1/RR7.2) — absence from the register meant **done**, not forgotten, which is worth knowing before reading another row's silence as a gap.  The findings are left as the audit wrote them; only markers are appended, since this register records what was found and `docs/REGISTERED_DEBT.md` is the status board | `docs/planning/UNFINISHED_SMP_WORK.md`, `rust/sele4n-hal/src/trap.rs` | M |
| RR8.14 | Retire the RR0.3 standing constraint from `CLAUDE.md` and `AGENTS.md` once RR3 has closed — it says two conjuncts remain threaded and `ipcInvariantFull` is not end-to-end checked, which becomes false at RR3.25 and would otherwise misdirect every later contributor  (**Formerly RR8.3** — renumbered at `v0.35.56` when RR8 grew the twelve rows above; a citation naming RR8.3 means *this* row.)  **LANDED v0.35.88.**  **The misdirection this row was written against was already gone** — RR3.1's gate and the cuts after it corrected the bullet in place, so by the time the row came due it said the opposite of what the row quotes.  What was left was the *shape* the row's own last line named: a 4,577-character standing constraint that was four fifths narrative — how the family-size figure drifted, which inhabitation witnesses fire which premises, that WS-DT is closed and its plan retired — beside about eight hundred characters a contributor must actually respect.  It is compressed to those, as four numbered obligations: cite the production payoff and never the two staged ones from production, supply the pack rather than citing a payoff bare, the packs are inhabited per arm with the two out-of-reach interiors registered, and the three confinements are stated.  The narrative is where this file says narrative belongs (`CHANGELOG.md`, `docs/REGISTERED_DEBT.md`), and the **enforced** family-size figure moves out of guidance into `docs/spec/SELE4N_SPEC.md`, which still carries it along with three other sites — so the de-threading gate holds a prose claim exactly as before, one document further from the reader it was misdirecting | `CLAUDE.md`, `AGENTS.md` | S |
| RR8.15 | Hand-off check **before** the closure entry: confirm SM10's §2 dependencies are genuinely met and its §1 scope statement matches the tree. Ordered first deliberately — each row may land as its own PR, so recording closure first would advertise the workstream complete for an intervening release, and an unmet dependency found afterwards would have to be retracted rather than simply fixed  (**Formerly RR8.4** — renumbered at `v0.35.56` when RR8 grew the twelve rows above; a citation naming RR8.4 means *this* row.)  **LANDED v0.35.186.**  Recorded as [`SMP_RELEASE_CLOSURE_PLAN.md`](SMP_RELEASE_CLOSURE_PLAN.md) §2.1: every §2 dependency re-measured against the artefact that decides it, not inherited from the phase that claimed it, and §1's scope confirmed (its *State at the audited cut* table is labelled a measurement of `v0.34.3` and each moved row carries its closing version).  **Running the dependencies rather than reading them is what earned the row**, and it found three things a read would not have.  `test_tier5_cross_language.sh`'s **central comparison did not decide**: it read its mismatch count as `grep -c … || echo 0`, and on a clean run `grep -c` prints `0` *and exits 1*, so the `||` fired, the variable held two lines, `[ -gt ]` died with `integer expression expected`, and the gate took the else arm and printed PASS — agreeing with the truth by accident of which arm `[`'s failure takes, while an **unreadable** log answered exactly as a clean one.  Fixed with `grep`'s status captured, `>1` a named gate failure, and the exclusion count refusing an unreadable log; five mutation cases decide it, one restoring the retired reading and asserting the shell error **with exit 0**.  The Track B/C bullet's closing qualifier — *must not read it as covering the scheduler path* — was **stale** five versions after RR8.12 Cut C6h made the seam cover exactly that.  And §8's acceptance gate said *README + 10 i18n* where the tree carries **11** — the identical figure §4 recorded as drifted and corrected at `v0.34.29`, unswept onto its sibling eleven lines away.  Tier 0–3, Rust, the aarch64 cross build and Tier 5 all exit 0; Tier 4 with `NIGHTLY_ENABLE_EXPERIMENTAL=1` exits 77, every runnable check passing and the QEMU boot-check gate NOT RUN for want of `qemu-system-aarch64` | `docs/planning/SMP_RELEASE_CLOSURE_PLAN.md`, `scripts/test_tier5_cross_language.sh` | S |
| RR8.16 | WS-RR closure entry in `docs/REGISTERED_DEBT.md`; update the CLAUDE.md phase table — last, on evidence RR8.15 established.  **It also closes the residue the hand-off check surfaces**, which is not scope creep but the shape of a closure phase: the register is read against the tree, and a row this workstream owns and can close is closed here rather than re-homed.  (The row's own text said *RR8.15* reads the register; RR8.15 as written and as landed measured SM10's §2 dependencies and §1 scope, so the register reading is this row's, and the closure cut corrected the sentence rather than inheriting it.)  **LANDED v0.35.203.**  Seventeen closure cuts, which is the length of the enumeration below and not a figure kept beside it — `v0.35.200` wrote *nineteen* where thirteen had been the count at `v0.35.199` and one cut had landed since, which is this project's own hand-kept-figure hazard arriving in a plan row — `v0.35.126` (the two information-flow gate facts: transport, derivation, establishment at the one production write, and inhabitation by the boot; the per-transition lift to the two checked dispatches stayed registered), `v0.35.188` (the removal's bundle statement at the honest pre-state, which needed the relaxed linkage *split* so a `replyLinkageFrame` has a unit to transport), `v0.35.189` (the object-domain donation members narrowed onto the donation's own guard, register row 56 — the row predicted it would ride Cut C4 and Cut C4 did not touch it, which is recorded rather than quietly satisfied) and `v0.35.190` (**`seL4_CNode_Revoke` gets an arm** — the row's own remedy, since this project's implement-the-improvement rule says wire a missing syscall rather than document its absence.  `SyscallId.cspaceRevoke` dispatches the CDT-traversing `cspaceRevokeCdt`, and the cut had to build the family's *IPC*-bundle chain from nothing: `donationReadAgreement` was generalised from `pendingMessage` **equality** to `pendingMessageReadAgrees`, since the in-flight sweep is the one transition in the tree that rewrites that field while the thread stays blocked and equality was strictly more than the bundle reads; the scaffold's case analysis and the traversal's induction became predicate-free and moved beside their definitions, so the capability bundle's argument and the IPC bundle's are one; and the content-flow gate's `CONTENT_WRITE_EXEMPT` is the honest form of "the write is real and moves no tracked payload", reconciled both ways.  What the row was closed *down to* is the three reporting variants, which have no in-kernel caller.) and `v0.35.191` (**the `v0.35.126` lift itself** — `endpointSendCrossCoreDispatchChecked` and `endpointCallCrossCoreDispatchChecked` carry `blockedSenderFlowsToEndpoint`, each under its own `endpointFlowGate`, in the production `IPC/Invariant/BlockedSenderPreservation.lean`.  The per-TCB dichotomy the register row estimated was **not needed** and the correction is the cut's finding: every step of both composites either preserves every `ipcState` (getting `ipcStateFrame`, the relation the tree already owned), writes `.ready` (so it *shrinks* the blocked-sender set), or **is** the blocking store the establishment already covered.  The sibling fact is lifted at the send over the send's own `sameSchedContextBindings` frame, which the `.call` chain has had since RR2 and the send did not; the `.call` arm had no counterpart from that direction, its dispatch minting a donation and the receiving gate being recorded in no state — that residue and the `ctx`-parametrised reachable pack opened as their own register row rather than being approximated, and `v0.35.196` below closed it.)  `v0.35.192` (**the four unconsumed state transformers judged both ways**, register row 59 — `KernelTransitionReachabilityCensus` found them and registered a wire-or-retire judgement it had never had; two were **deleted** with tombstones and their pin entries, two were kept with the reason, and the witness computes the retired reading beside the live one so the judgement is measured rather than asserted).  `v0.35.193` (**the SINGLE removal's projection lemma**, register row 70 — `v0.35.84` proved the labelling layer under `abortHolderProjectionStable` and reduced it to the holder's queue neighbours, and for nine cuts nothing related `endpointSpliceHigh` to `abortHolderPendingIpc`: the prefix runs the single `endpointQueueRemove` and only the *dual* removal had such a lemma, so a reduction and a closure read alike in a bundle search.  The one mismatch the row named turned out to be a mismatch of *spelling*, closed by RR8.3's pairing; two `_eq_patches` pins moved to a layer both askers reach, shedding a live instance of the duplication RR8.4 removed; and §14 of the information-flow suite exhibits the residue rather than describing it).  `v0.35.194` (**the citation gate's two domains derived**, register row 141 — a pattern requiring an underscore put every lowerCamelCase Lean `def` outside the gate, and a one-path file list put every other document outside it; the column split makes the widening affordable, the exemption table falls 18 → 4 and is reconciled both ways, and the same cut swept register row 71, whose open closure target had outlived its own recorded closure by a hundred and nine versions — which is what a closure phase reading the register against the tree is *for*).  `v0.35.195` (**the payoff's `.reply` arm covers a faulted caller**, register row 81 — the row named the missing lemma exactly and the closure found two things it did not predict.  The transition-level bundle the row called *proven* was **vacuous on the path it is named for**, composing the non-donating reply form whose `hNoDonationOwnedBy` a faulted thread holding a reservation refutes; and the `ipcState` frame the dispatch-level statement composes had to move beside the transition it frames, `v0.35.191`'s home for it being outside the closure of every asker on the reply path.  The pack's confinement field is retired for `replyFaultStage` — the same five pre-state conditions the ordinary branch already carries, at `IpcMessage.empty` — and §7f of the fault suite exhibits the donating state by ordinary operations, with the control that an unbound faulter's delivery donates nothing).  `v0.35.196` (**the RECEIVING side of the endpoint gate**, register row 183 — the successor row `v0.35.191` opened, closed in the shape the row itself prescribed.  `blockedReceiverFlowsFromEndpoint` records what the receiving gate checked, established at the **same** production write from the receive arm's own gate, transported by a `blockedReceiverShrinks` twin, and read **off the state** by `donationFlowToBlockedReceiver` where its sibling takes it as an argument — which is why no dispatch could discharge that one.  So `endpointCallCrossCoreDispatch{,Checked}_preserves_donationOwnerFlowsToHolder` exist and the arm that *mints* a donation carries the flow fact.  **The judgement the row asked for** is that the extra `ipcInvariantFull` conjunct is not an asymmetry between two halves of one question: the sending gate is evaluated on the invoking thread, whose identity the transition holds, while the receiving gate is evaluated on a thread the rendezvous *finds* on a queue, so `queueHeadBlockedConsistent` is the **resolution** of the receiver rather than an extra assumption.  `ipcReachableUnder ctx` is opt-in and exactly as strong as the `ipcReachable` it extends — neither is claimed preserved along a trace, which is that pack's own shape.  One de-duplication rode along: the chain walk's frame is stated at the **record**, so the `ipcState` and `schedContextBinding` readings are two instances of one proof.  The retiring artefact is a *negative* rather than a definition — `v0.35.191`'s anchor forbade asserting the `.call` counterpart without the predicate row 183 named, so supplying the predicate is what retires it, and a negative kept past its own remedy would refuse the fix it was written to demand).  `v0.35.197` (**the two cross-subsystem bundles get their FRAMES**, register row 85's first half — the row asks for `schedulerInvariantBundle` and `capabilityInvariantBundle` across the cross-core Call and reply chains, and the measurement was that neither had a frame at all: twelve per-conjunct lemmas across the *scheduler* transitions and none for a step that writes nothing the invariant reads, so each IPC step's lift would have been a fresh case analysis.  What landed is the read set stated once — the scheduler frame takes TCB **survival** rather than store equality, with a narrower one at the two `SchedulerState` fields of nine the invariant reads; the capability frame states the **direction** each conjunct transports in, three backward over CNodes and two forward over presence and Replies, with a kind-preserving pointwise instance and the `storeObject`-level instance every chain is built from — plus the three steps both chains share: the priority-inheritance walk (both bundles, and *not* a frame application, since it re-buckets), the placement removal and the replenishment migration.  One retirement, found because the asker could not see the owner: `migrateSchedContextReplenishment_currentOnCore` was a second answer to a question the migration's own module already answers.  Mutation-testing its anchors found a class of its own — a bare `^theorem foo` anchor is satisfied by a suffix rename, 2404 of 7061 — registered with its measurement and its two-part remedy).  `v0.35.198` (**a bare-name anchor pins nothing**, the class `v0.35.197`'s own mutation run found and registered — `rg '^theorem foo'` matches `theorem fooX`, so an anchor over a declaration with no other consumer goes on reporting PASS once the name it pins is gone.  2543 positive anchors bounded; negatives deliberately left, since bounding one can stop it firing on a name it was catching; and `unbounded_declaration_anchors` refuses the next bare positive, driven by the gate's **own** anchor parser rather than by a second regex — which is what found the last 41 the hand-rolled sweep missed.  The sweep's measurement is the argument for it: **eight anchors pinned nothing they name**, six naming the prefix `_preserves_ipcInvariant` where the declaration is `_preserves_ipcInvariantFull`, one naming a file whose bare match was a different declaration, and one correctly bare as a family count).  `v0.35.199` (**the reply chain carries both cross-subsystem bundles**, register row 85's reply half — the relation `v0.35.197`'s two frames read acquired a *name*, which is what turns a chain into citations: `kindPreservingWrite` is what both `_of_kindPreserving` instances take, so a widening reaches the scheduler bundle and the capability bundle by construction, where an inline pointwise condition would have reached whichever consumer a review named.  Each store primitive then answers it beside itself, and the **in-place** one needs no side condition at all — `rewriteAdmissible` already carries the kind equality *and* `rewriteNeutral`, which is `false` at `.cnode`, so both of the store lemma's hypotheses are inside the rewrite's proof argument, and a negative refuses one coming back.  On that the `.reply` chain is four citations, and the **asymmetry between a transition's two lifts is the claim**: the capability bundle reads the store and the two CDT tables, so it is unconditional, while the scheduler bundle reads `currentOnCore` and inherits the wake's `hNotCur`, stated on the pre-state where a caller can discharge it.  One anchor was **repointed** rather than deleted, the exclusion it pinned having moved from the consumer into the relation.  What remained was the call chain and the fault composition, measured rather than estimated, and the measurement held).  `v0.35.200` (**the call chain and the fault composition CLOSE register row 85** — every call-side store primitive got the `kindPreservingWrite` / CDT pair beside itself, with `SystemState.updateTcb_kindPreservingWrite` the one owner for *a typed TCB rewrite keeps every key's kind*, and `faultDeliverOnCore` / `faultReplyOnCore` — the transitions the row is named for — carry both bundles.  **The capability half REDUCES rather than discharges, and asking why found a live Medium defect.**  The `.call` chain's capability lift takes `ipcUnwrapCapsPreservesCapabilityBundle` from `endpointCallWithCapsOnCore` up; that step's *own* externalised premises are **refuted** (`hSlotCap` against a CNode at capacity), so importing them would have been a vacuity, and the reduction is exhibited by `…_of_noGrant` instead.  Neither fault transition owes anything — `faultMessage` carries no capabilities and the leg short-circuits before resolving the receiver's root.  Pulling on *what discharges the hypothesis* found a **High**-severity defect: no CSpace destination slot is validated against the target CNode's radix width at any of the four capability-insert paths, so one `seL4_CNode_Copy` with a raw out-of-range `dstSlot` grows a fixed-size kernel object without bound and falsifies `cspaceSlotCountBounded`; registered with its end-to-end `#eval` measurement, the central remedy (the check belongs in `cspaceInsertSlot`) and its own closure target.  And the donation's lifts sit with the chain rather than with the bundles, because `applyCallDonationOnCore` is declared *above* the scheduler-invariant layer while `returnDonatedSchedContext` is below it — the import graph's asymmetry, stated at the declaration.  **Eight frames moved to production**: the four fault-path and two chain-level `_preserves_objects_invExt` lemmas sat in the staged `FaultPreservation.lean`, out of reach of the production consumers that needed them, which is this row's own complaint one level up; `ipcUnwrapCaps_getTcb?_eq` was `private` in a cross-core *reply* module while framing a model-layer primitive the `.call` leg asks the same question of; and a `private` duplicate of `storeTcbIpcStateAndMessage`'s CDT frame was deleted rather than kept.  The one gap left behind is the one the reply half already named: `hNotCur` is stated rather than derived, and a per-core `currentThreadIpcReady` retires it — along with `endpointReceiveHeadsNotCurrent`, the state-level form the fault delivery takes because it resolves its own handler endpoint).  `v0.35.201` (**a capability is installed only at a slot the target CNode can address**, register row 111 — the High-severity defect `v0.35.200` found and registered, closed in the shape that row prescribed: `CNode.slotAddressable` at `cspaceInsertSlot`, the one primitive all four installs pass through, and `findFirstEmptySlotChecked` on the transfer path so a full receiver answers `.noSlot`.  **What landing it found is the sharper half**, and it was found by landing the guard rather than by auditing fixtures: six fixture CNodes were built on the defect, the trace harness's own **bootstrap root CSpace** among them — `radixWidth := 0`, *one* slot, holding capabilities at 0, 5 and 6, so `cspaceSlotCountBounded` was false of the state every trace scenario starts from.  Two of the six carried a comment naming the radix the code did not have.  With the six repaired the golden trace is byte-identical but for the check count the cut's new runtime check moves, which is the measurement that the guard refuses only what was already unreachable — and that check exists because **nothing asserted the conjunct**: `slotCountBounded` appeared nowhere under `SeLe4n/Testing/`, which is RR8.3's *a conjunct is checked at runtime, not only proved* rule meeting one that predates this repository's visible history and was checked never.  The guard also forced a sweep of **eight** inline re-derivations of what a successful insert consists of, all of which now read one owner, with three frames moved beside the primitive.  Two residues are registered rather than absorbed: the refusal's error code, and the boot check bounding the slot **count** rather than the slot **indices**.)  `v0.35.202` (**a tactic that unfolds an accessor is not adoption of it** — the metric defect the cut above registered, found because the metric *scored an improvement as a regression*: collapsing eight inline re-derivations lowered `GETCNODE_ADOPTION` 147 → 129 and failed a should-**grow** floor, and 45 of its 172 lines were tactic references to the accessor's definition.  One place asks the question for all eight `*_ADOPTION` metrics, the scope is stated as a floor over *recognised* uses because the exact question is about elaboration, and the four self-test cases include an **unreadable input** — without which the I/O-status check is a condition no input reaches.  All eight floors re-anchor onto the corrected scale, which is what fixing a metric's subject costs.  The two scripts are **renamed** in the same cut — `store_reader_hygiene_baseline.sh` and `check_store_reader_hygiene_monotonic.sh` — because a Tier 3 anchor must name the file it pins, so leaving the phase code would have grandfathered nine fresh occurrences of it into live code.)  `v0.35.203` (**the closure entry itself, and the register read against the tree**).  The reading touched **fifteen** rows of table A, fourteen of which named a **finished** phase in their closure cell — RR2.18, RR7, RR7.20, RR7.22, RR7.30, RR7.36, the RR7.7–RR7.41 fine-lock span, SM10.1 and this closure row itself — which is the shape a workstream leaves behind when it closes: a cell naming a phase that cannot act reads as owned and is not.  **Two of the fifteen were already closed and nobody had said so**: SM6's tracked-debt row, discharged by RR7.21 at `v0.34.72` and unrecorded for **276** versions, and the `numCores` row, whose head RR7.30 made **false** at `v0.34.79` — *a narrower binding does shape kernel state*, which the constant's own docstring has said for 269 versions while the register said the opposite.  A refuted row reads exactly like an open defect, which is why a closure phase reads the register against the *tree* and not against its own CHANGELOG — and the sharpest evidence is that the tree had **already said so in the other artefact**: RR8.13 (`v0.35.89`) marked the same `numCores` finding CLOSED in `UNFINISHED_SMP_WORK.md`, with the argument and the declined remedy, and swept no debt row, so for 113 versions one register said closed and the other said open.  A third had been closed in prose and named `RR7` in the cell it is scanned by.  The remaining **twelve** are re-homed **in place** — to WS-BP BP7.6, to WS-BP BP7.8 (added to that plan in this cut rather than left as a wish), to WS-CB, to fine-lock Track D, to the untyped/retype surface, and to *no owner before v1.0.0* with what each constrains v1.0.0 from claiming — and **six** §C rows whose targets named RR6–RR8, RR5.14 or WS-HP were re-pointed the same way.  **Two findings of its own.**  A `row N` citation of table A, B or C is its **line number**, so every row added above it renumbers the citation — measured: `v0.35.196` cites *row 183*, `v0.35.201` added four rows above it, and *row 183* is now a different row — registered post-v1.0.0 with the namespace collision its fix must resolve, and mitigated now by a convention (cite the number *and* the opening phrase).  And the gate half of that collision was **live**: `check_deferral_registration.py` ran its row pattern over the whole register, and the WS-IN remainder table's first column is an occurrence count, so six counts (63, 78, 88, 94, 104, 292) sat in the index as debt rows and a comment citing *row 292* in `scripts/test_tier3_invariant_surface.sh` would have satisfied both halves of the gate — existence and file-agreement — while naming no row at all; none is written today, which is what makes this a hole rather than a finding.  `enumerated_table` scopes the parse to §C.1 and **refuses** a register whose heading it cannot find, with three mutations each caught.  (**Formerly RR8.5** — renumbered at `v0.35.56` when RR8 grew the twelve rows above; a citation naming RR8.5 means *this* row.)  | (3 files) | S |
...
- [x] `SMP_RELEASE_CLOSURE_PLAN.md` §1 scope and estimate match the tree.
      — established **v0.34.26** (RR0.4, RR1.11); **re-verified at `v0.35.186`**
      (RR8.15), which is why RR8.1 left it unticked: a hundred and sixty patch
      versions had landed since, so a tick taken from RR0's cut would have
      certified a measurement nobody repeated.  The evidence is
      [`SMP_RELEASE_CLOSURE_PLAN.md`](SMP_RELEASE_CLOSURE_PLAN.md) §2.1, a row
      per dependency naming the artefact that decides it — and the walk found
      three drifts, one of them a Tier 5 gate whose central comparison errored
      out and printed PASS.
- [x] The SM10 theorem tally includes SM9 and is generated, not hand-summed.
      ...
      16 inventories, 1140 entries of which 922 are theorems, 16 more in the
      assumption ledgers.
...
- [x] `UNFINISHED_SMP_WORK.md` updated with closing versions.
      — **RR8.13's output** at `v0.35.89`, which RR8.1 could not tick because
      the row had not run.  Re-measured at `v0.35.186`: all **26** findings
      carry a status marker (23 CLOSED with the version, 2 PARTIALLY CLOSED
      with the residual named, 1 REGISTRATION CLOSED with its residual owned by
      WS-SL).  The box stayed unticked for ninety-seven patch versions after its
      own row landed, which is *an acceptance box is a present-tense claim* —
      the rule RR8.1's own preamble states — failing at the next opportunity.
...
|---------------|-------|------|
| A delegated `.reply` does not deschedule a *preempted* recorded server | RR8 | **Code sweep — closed at RR8.6 (`v0.35.79`).** The defect is fixed at the two sites PR #895 rounds 10–11 named, at the two this row named (`descheduleThread`, `cancelIpcBlockingOnCore`), and at the one the sweep found (`suspendThreadOnCore`'s G4/G4b pair); every state-resolved deschedule reads `placedCoreOf?`. |
...
| `UncoveredLockDomain.syscallSeamSchedulerDomain` (inside the fine-lock row) | RR8 | **CLOSED at RR8.12 Cut C6h (`v0.35.181`).**  Per-arm resolved wake targets for all sixteen declared arms, each with a coverage proof, and the seam bracketed on `schedulerLockBracketDomain` over one unified footprint spanning both domains — the constructor is deleted and the inventory falls from two to one.  The row's own premise about *why* this needed the free over-approximation to be unavailable (that the object footprints hold `stateLevelLock` "rather than the table lock") was **false**: the two are one word, which is why the cut unifies the footprints rather than nesting two brackets.  RR7.39's residual. |
...
| ...and so is the holder *wake* | **WS-OD** | A second one, of `abortHolderWakeHigh` (`descheduledHolderHigh` since `v0.35.158`, when the wake became a deschedule; same discharge). |
