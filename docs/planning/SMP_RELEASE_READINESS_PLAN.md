...
|-----|-------------|----------|-----|
| RR7.1 | Boot MMU corrections: 960 MiB of RAM mapped as Device, and nothing mapped above 4 GiB (§4).  **LANDED v0.34.57**: the tables are derived from `mmu::boot_mapping_for` (mirroring `rpi5MemoryMapForConfig`) and sized to the board's own `/memory` node — since PR #892 review round 3 (v0.34.111), to the **contiguous** run it reports from address 0 (`cmdline::contiguous_ram_top`; the peripheral window is the one gap the walk may cross), where the maximum fold of the first cut mapped every hole between two claims Normal-cacheable; and since round 4 (v0.34.112) a bank whose `status` is not `okay`/`ok` — DRAM the firmware has withheld — contributes nothing, and `init_mmu` refuses, before translation is enabled, a clamped top that does not cover the image, its stacks and the blob's own extent (`boot_critical_ranges_mapped`; the PE parks in `cpu::fatal_halt`, since Phase 2 runs on the boot core before the GIC exists).  The same cut fixed a defect the finding did not name — the table `TTBR0_EL1` pointed at held *level-1* block descriptors while `T0SZ = 16` makes level 0 the initial lookup level, so the boot MMU enable would have translation-faulted on the first fetch | 1 | L |
