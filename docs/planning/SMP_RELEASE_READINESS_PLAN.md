...
| RR7.5 | SM10 plan-schedule corrections: re-sequence SM10's sub-phases so the image build precedes its consumers — SM10.B.7 and SM10.B.10 consume SM10.E.D1's image and SM10.C cuts the tag before SM10.E runs, a backward dependency the plan-numbering rule forbids and a sequencing note does not repair (532 `SM10.<letter>` citations across 60 files, so a phase of its own); and add the three `contextRestoreSeamLive` prerequisites absent from SM10's dependencies, sub-tasks and acceptance gate. §1's false "all substantive SMP work is complete" phase goal was the third item here and is **closed** — RR0.4 rewrote it at v0.34.26 | 2 | L |
