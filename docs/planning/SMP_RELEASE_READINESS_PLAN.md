...
| RR8.16 | WS-RR closure entry in `docs/REGISTERED_DEBT.md`; update the CLAUDE.md phase table — last, on evidence RR8.15 established.  **It also closes the residue the hand-off check surfaces**, which is not scope creep but the shape of a closure phase: RR8.15 reads the register against the tree, and a row this workstream owns and can close is closed here rather than re-homed.  Two so far — `v0.35.126` (the two information-flow gate facts: transport, derivation, establishment at the one production write, and inhabitation by the boot; the per-transition lift to the two checked dispatches stays registered) and `v0.35.188` (the removal's bundle statement at the honest pre-state, which needed the relaxed linkage *split* so a `replyLinkageFrame` has a unit to transport).  (**Formerly RR8.5** — renumbered at `v0.35.56` when RR8 grew the twelve rows above; a citation naming RR8.5 means *this* row.)  | (3 files) | S |
