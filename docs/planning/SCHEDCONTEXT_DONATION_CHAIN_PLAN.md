...
| OD1.3 | Twenty-conjunct carriage for OD1.2.  **The engine does not cover it today**: `QueueSplicePreservation` is stated entirely over `endpointQueueRemoveDual`, which is a separate implementation rather than a wrapper over the `endpointQueueRemove` the timeout uses.  Three ways out, and the row picks one on evidence rather than by default — extend the engine to the single-queue removal; move `timeoutThread` onto the dual-queue removal, which is the consolidation but changes its error surface; or prove the two agree under the invariants and transport.  Whichever is chosen, the abort and the timeout must end up removing with the **same** function, because one question with two answers is how these two drift apart | `SeLe4n/Kernel/IPC/Invariant/QueueSplicePreservation.lean`, `SeLe4n/Kernel/IPC/DualQueue/Core.lean`, `SeLe4n/Kernel/IPC/Operations/Timeout.lean` | L |
...
| OD3.2 | Re-base the remaining thirty-five attached theorems.  Two change *statement*, not only proof: the one asserting the return touches no Reply becomes a Reply **frame**, and the binding trichotomy widens at the target.  Consumes OD3.1 | `SeLe4n/Kernel/IPC/Operations/Endpoint.lean`, `SeLe4n/Kernel/IPC/Invariant/DonationPreservation.lean` | XL |
