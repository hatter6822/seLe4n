...
> **Sub-task count**: 44 across 6 phases (OD1..OD6), each phase numbered in the
...
| OD3 | The pop, generalised and behaviourally inert — signature, head validation, pre-state resolver, and the footprint split its growth needs | 10 | XL |
...
**25 µs**, and the uniform envelope is 2340 µs.  Every later row measures against
...
> **Superseded by OD3.5 and OD3.7.**  This section records the budget as it stood
> at `v0.34.105`.  OD3.5 made the victim's splice neighbours arm-selected, taking
> the reply arm to eight; OD3.7 added the two below-head reads, taking it to ten;
> and `maxLockSetSize` is now thirteen.  The live figures are
> `lockSet_cancelIpcBlockingOnCore_size_le_ten` and `maxLockSetSize`, never the
> numbers below.
