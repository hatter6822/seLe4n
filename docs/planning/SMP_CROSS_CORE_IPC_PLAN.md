...
| SM6.F.5 | `scripts/test_qemu_smp_ipc.sh` | the Tier-4 QEMU `-smp 4` cross-core IPC handshake exerciser, registered in `test_tier4_smp_bootcheck.sh`; SKIPs (with the formal-coverage banner) until the SM10.E bootable kernel-image `[[bin]]` target exists — the sibling discipline of the SM5 QEMU scripts; pass gate: `[smp-test] cross-core-ipc: reply delivered across cores` | ✓ |
...
- [x] Tier 0..4 green (SM6.F, v0.32.67 — Tiers 0–3 substantive; the Tier-4 QEMU sub-tests SKIP by design until the SM10.E bootable kernel-image target exists, exactly as the SM1–SM5 QEMU siblings).
