...
| RA.E.6 | Documentation sync + closure record: spec, GitBook, `CLAIM_EVIDENCE_INDEX`, `REGISTERED_DEBT`, `CLAUDE.md`/`AGENTS.md`; **correct `readReturnValue`'s docstring** in `Platform/FFI.lean`, whose "documented current behaviour" paragraph is the artefact this workstream removes | spec + docs | M |
