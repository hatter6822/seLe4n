...
| HP6.2 | **The two reply footprints repointed onto the head-driven trigger** — moved ahead of the splice, see the preamble's finding 4.  `lockSet_endpointReplyOnCore` / `lockSet_endpointReplyRecvOnCore` resolve their donation members through `answeredReplyObject?` composed with `replyFrameHeadHolder?`, the same expression the pop reads, so the footprint and the transition cannot disagree about which objects are written.  `lockSet_endpointReplyOnCore_covers_headDrivenPop` — HP4.4's stand-in — is **deleted** by this row, since the members then come from the trigger itself; it therefore consumes HP4.4.  Sound today (under `severAtCut` the two resolvers agree), so this row is behaviour-preserving and verifiable on its own.  **The row's own claim about the sharp bounds was wrong and is corrected here** (`v0.35.43`), measured against the composite's three write sites rather than reasoned from the resolver: `endpointReplyOnCore` writes the answered caller, `propagatePipChainCrossCore` writes the **recorded server** (`expected`), and `applyReplyDonationOnCore` writes the SC, the **holder** (`sc.boundThread`, `.unbound`) and the answered caller (the rebind).  So the repoint *reuses* the `donatedOriginalOwnerTid` slot for the holder — the recipient needs no slot at all, being `replyTargetTid` by construction — and the declared arity and count do **not** move.  What changes is which fact licenses the 18 -> 17 merge: it was `replyDonationOwnerIsAnsweredCaller` (owner = answered caller) and becomes *holder = recorded server*, which the splice can falsify at an orphan head and which is `answeredHeadContextIsServerDonation`'s content — so the sharp bounds stay **conditional** here and a later row discharges them.  One coherence fact does become free at this row: `replyStackHeadIsAnsweredReply`'s consumer `replyStackHead?_none_of_answeredFrameAbove` is subsumed by HP2.4's `answeredFrameHeadContext?_head_is_answered_reply` and `answeredReplyFrameAbove?_none_of_headContext`, which carry no hypothesis.  Consumes HP6.1, HP4.4 | `SeLe4n/Kernel/IPC/CrossCore/EndpointReply.lean`, `SeLe4n/Kernel/Concurrency/Locks/LockSetTransitions.lean`, `SeLe4n/Kernel/Concurrency/Locks/ResolvedFootprintBounds.lean`, `SeLe4n/Kernel/IPC/CrossCore/EndpointReplyDispatchInvariant.lean` | L |
...
| HP7.2 | `replyDonationOwnerIsAnsweredCaller` has **no consumer left** once the footprints are repointed, and the two sharp bounds become **unconditional** here rather than there — their merge is *holder = recorded server*, which this phase is what derives (corrected `v0.35.43`) — verify that, rather than remove it again (the row said "removed … the sharp bound becomes unconditional", which HP6.8 now does; corrected at `v0.35.40`, since the two rows as written each waited on the other). What remains here is the sweep: every other citation of the three facts outside the definitions themselves. Consumes HP7.1, HP6.8 | `SeLe4n/Kernel/Concurrency/Locks/ResolvedFootprintBounds.lean` | M |
