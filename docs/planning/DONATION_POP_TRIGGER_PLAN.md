...
| HP10.7 | **The arm flips**: the pop's bottom-of-stack recipient is the recorded origin when `donationRecipientAcceptable` holds of it, the answered caller otherwise, with `_eq_legacy_of_no_origin` the definitional equality that carries every existing result.  **SCOPE, established while landing HP10.6 and binding on this row: the redirect goes in the three REPLY-path pops** — `applyReplyDonation`, `applyReplyDonationOnCore`, `replyRecvPopDonation` — **and not in the shared `returnDonatedSchedContextResolved`.**  All six pop call sites thread that resolver, so putting it there would redirect the two pre-receive cleanups (`cleanupPreReceiveDonationChecked`, whose recipient is the *binding's* recorded owner), the cancellation reclaim and thread destruction as well — and only the two reply footprints declare an origin member, so `lockSet_endpointReceive`, `lockSet_replyRecv`'s pre-return group, `lockSet_cancelIpcBlocking`, `lockSet_cancelDonation` and `lockSet_tcbSuspendOnCore` would each be **false** of their own transition.  It is also the right scope on the merits: the registered defect is the reply path's, and the reclaim's recipient is resolved off the victim's own frame head (`cancelledCallerDonation?`), which is a different question.  Widening the redirect is therefore a row that declares first.  The no-regression fact this row owes is that the redirect never introduces a refusal — `donationRecipientAcceptable` of the redirected recipient follows from `donationOriginRecipient?_acceptable` — so every reply that succeeded before still succeeds.  Consumes HP10.6 | `SeLe4n/Kernel/IPC/Operations/Endpoint.lean`, `SeLe4n/Kernel/IPC/Invariant/DonationPreservation.lean` | L |
| HP10.8 | **The FROZEN arm flips, in the same cut as HP10.7.**  `frozenEndpointReplyWithDonationReturn` calls `frozenApplyReplyDonation st' holder scId targetId` — the answered caller as the recipient, exactly as the live composite passes `targetVtid` — and `frozenReturnDonatedSchedContextResolved` mirrors the live resolver guard for guard, HP4.6's recipient check included.  So flipping only the live arm leaves two programs that disagree on precisely the states this phase exists for, under `frozenBranchOperationChecked .endpointReplyToBlockedCaller = true`, which is a machine-checked claim that the two are run beside each other.  That is HP4.7's situation verbatim, and it gets HP4.7's answer: the mirror flips in the **same cut**, not a later one — a window with the live arm flipped and the mirror not is a window in which that claim is false.  The row is separate from HP10.7 so the frozen work is visible, not so it can land later.  Needs the frozen origin read (`FrozenKernelObject.schedContext` carries the live record, so there is no field to add) and a `FO-044` whose two halves are an origin that differs from the answered caller and one that does not — because a selector that fires unconditionally passes the first and breaks the second, which is the shape §3.9b was built against.  Consumes HP10.7 | `SeLe4n/Kernel/FrozenOps/Operations.lean`, `SeLe4n/Kernel/FrozenOps/Core.lean`, `tests/FrozenOpsSuite.lean` | L |
| HP10.9 | **The payoff**: `donationAccountingPreserved_atCallDepthTwo`, and §3.20's accounting halves invert from COST to PAYOFF — the same inversion HP6.9 performs at depth three, on the shape HP6 provably could not reach.  The depth-three payoff must pass byte-identically, which is the measurement that this phase is confined to the reachability gap rather than changing the chain.  Consumes HP10.8, HP10.2 | `SeLe4n/Kernel/IPC/Invariant/DonationPreservation.lean`, `tests/SmpIpcSuite.lean` | L |
