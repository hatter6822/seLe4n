...
* `noteDeactivated st tid` — called where a thread becomes inactive: in
  **`removeRunnableOnCore`**, the per-core primitive that the cross-core
  send, reply, signal, fault and cancellation paths call directly, firing iff
  the thread was a member of that core's queue — so the `bootCoreId` wrapper
  `removeRunnable` and the all-core fold `removeRunnableFromAllCores` fire it
  at most once, a thread being queued on at most one core; in
  `suspendThreadOnCore`, the cancellation and fault suspends,
  `cleanupTcbReferences` on retype, and the dispatch paths that clear a
  `current` slot without re-enqueueing.  It sets the leaf's count to `0` and
  propagates the crossing.
