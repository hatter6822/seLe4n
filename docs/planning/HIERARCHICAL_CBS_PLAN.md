...
| D21 | `maxLockSetSize` moves **from its value at the time CB4.4 lands** to that value `+ 2`, when the activation paths gain the ancestors' SchedContext locks, with the constant-dependent WCRT terms re-derived, and is re-verified when the per-core admission slot joins the admitting footprints (§4.12).  **Re-baselined at `v0.34.128`**: the constant was `9` when this decision was written (WS-RR RR7.11 raised it from `8`) and is `11` since WS-OD OD3.5, which declared the second SchedContext hand-off `.replyRecv` performs and the recorded server's TCB; `lockSet_tcbSuspend` moved from eight to nine in the same cut.  So the arithmetic to re-run at CB4.4 is `11 → 13`, not `9 → 10`, and CB4.4 must **measure rather than quote** — every figure in this row is a snapshot of a constant two workstreams write to | Leave the bound and take the ancestor locks outside the set; lower `maxServerDepth` to `2` | `lockSet_replyRecv` is at the ceiling at its widest and a member leaf adds up to two ancestors; a lock taken outside the set is invisible to the deadlock-freedom and serializability theorems, and depth two forbids the root server → server → leaf shape D9 exists for |
...
/-- The thread whose reservation a leaf is, while a donation is in flight:
    `boundThread` is then the **donee**, and the owner is recorded only in the
    donee's `.donated scId owner` binding, reached through the leaf's
    `scThreadIndex` entry.  Every rule that needs the owner reads it here —
    `chargedCoresOf`, the affinity rule, bind's refusal and the departure
    refusals (§4.6, §4.8) — so no second derivation from `boundThread` can
    name the donee twice. -/
def donationOwnerOf? (st) (scId) : Option ThreadId          -- `some owner` iff a donation is in flight
def midDonation (st) (scId) : Bool := (donationOwnerOf? st scId).isSome
/-- The cores a root's utilisation is charged to (§4.6): a root **server**'s
    `serverCore`; a root leaf's bound thread's home core — the donee's while
    donated — **and** the recorded owner's home core while a donation is in
    flight (`donationOwnerOf?`; the eighth cut derived the second core from
    `boundThread` again, which named the donee twice and left the owner's core
    open to admissions the promised return would then overcommit); `[]` for
    members and for a parentless unbound leaf.  A live `residual` (below) is
    charged on its own core besides. -/
def chargedCoresOf (st) (scId) : List CoreId
/-- The core whose scheduler state a key change on this context reaches
    (§4.4, §4.12, §4.13): a leaf's bound (or donee) thread's core; a server's
    `serverCore`, since every member's path — and so every member thread's
    key — moves with the server's window, priority or eligibility; `none` for
    an unbound parentless leaf.  `keyRescheduleFootprint` and
    `schedContextWriteSet` are both read off it, so the lock footprint and the
    information-flow write set of a key-moving transition cannot disagree. -/
def seamCoreOf? (st) (scId) : Option CoreId
```
...
| Rule | When | Effect |
|------|------|--------|
| (a) configure | `schedContextConfigure` on an **unconfigured** context (`budget = 0`, which every retyped context is and which a successful configure never leaves behind, so the test is exact) succeeds at time `t` — the only fresh window a context ever receives; a configured context that has been emptied or unbound is still configured and takes rule (f) | `budgetRemaining := budget`, `periodStart := t`, `deadline := t + period`, `replenishments := []`, purge |
...
| (d) landing | the drain pops `(scId, refillAt)` at `now ≥ refillAt` | if `sc.replenishments` names this `refillAt` **and** `refillAt ≥ periodStart + period`: `budgetRemaining := budget`, `replenishments := []`, and the window is `[refillAt, refillAt + period)` while it is still current (`periodStart := refillAt` if `now < refillAt + period`) and `[now, now + period)` once it has passed (`periodStart := now` otherwise) — a landing never revives an expired window (`cbsLandRefill_deadline_future`); otherwise the queue entry is **stale** and is dropped with no other change.  Under `pendingRefillMirroredOnCore` the stale arm is unreachable (`cbsLandRefill_stale_unreachable_of_mirrored`); it stays total |
...
| (f) reconfigure | `schedContextConfigure` on any **configured** context — bound or not, populated or empty — at time `t`, after re-admission (§4.6) | `budget := Q'`, `period := P'`, `budgetRemaining := min budgetRemaining Q'`, `deadline := periodStart + P'`; a pending refill becomes `{amount := Q', eligibleAt := max (periodStart + P') (t + 1)}` and is re-keyed; then, if the entity is active, rule (e) at `t` — never rule (a) (D17) |
...
**An eligibility gain never yields an expired window**
(`eligibilityGain_window_current`): every rule that can make a node
eligible-active leaves `now < deadline` — (a) and (e1) by construction, (d) by
its window choice, (f) through the rule (e) it ends with on an active entity,
(e2) because it gains nothing and (e3) because `deadline > t` is in its guard —
so T15's contained-window accounting sees every tick of execution inside the
executing node's current window.  The seventh cut of this plan held the
property by an incidental composition — the activity sync re-applied rule (e)
after a landing — and lost it when that sync stopped re-arming; a property two
...
from `t` or from a landing time `≤ now`, and the premise T5's dead-time bound
needs (the eleventh cut stated T5 without it, and a window placed in the
future refutes it).
...
whose flag is set always takes one (`tick_takes_scheduling_point_of_pending`),
so a locally owed reschedule is enacted within a tick — the enqueue-only arm's
contract today, now stated.  The eighth cut stated the flag-to-SGI relation
over every core and so promised an SGI the gated local arm never sends.
...
```
eligibleActive sc := sc.budgetRemaining.val > 0 ∧ sc.activeDescendants > 0   -- the node's own fields, nothing else
```

— and `activeDescendants` counts, at a leaf, its thread (`1` iff the thread is
active — a **maintained** field, written by the transitions that change the
thread's activity and related to `threadActive` only by the invariant), and,
at a server, the **members that are eligible-active**.  The predicate reads
the node's own fields, so every decision it drives is taken under the locks
the transition already holds; the global `threadActive` appears in
`activeDescendantsConsistent` (§4.10) and nowhere at runtime — a proof
obligation, not a read.  A sync that recomputed the leaf's count from
`runnableOnSomeCore ∨ runningOnSomeCore` — the shape of an earlier cut — read
every core's run queue and current slot under none of their locks, so on SMP
the stored count could reflect neither the pre-state nor the post-state.  One
...
linkActivity st child now   := if eligibleActive child then propagateCrossing st child up now
unlinkActivity st child now := if eligibleActive child then propagateCrossing st child down now
...
top of it would move *every* late drain's window rather than only an expired
one.  Linking and unlinking use the
...
One bracket, on derived triggers.  `withEligibilityFlip st scId now f`
evaluates `eligibleActive` on the node before and after the field update `f`
and climbs with `propagateCrossing` on a flip; around an update that does not
flip it is the update (`withEligibilityFlip_eq_of_no_flip`).  Every write to
a node's `budgetRemaining` or `activeDescendants` goes through it — a crossing
cannot be recovered from the post-state, so a bracket is the only shape that
sees both sides.  The leaf's count is written by
`noteLeafActivity st scId active now`, the bracket around
`activeDescendants := if active then 1 else 0`, where `active` is the
post-state activity of the leaf's thread **as the primitive knows it from the
core it holds locked**: `enqueueRunnableOnCore c tid` passes `true`;
`removeRunnableOnCore c tid` passes `false`, since by `queueCurrentConsistent`
and the one-core invariant a thread it removes from `c`'s queue or current slot
is active nowhere else; a preemption that re-queues `c`'s current thread passes
`true` (`1 → 1`, no flip).  No other core's queue or current slot is read.  It
is idempotent — the same value twice changes nothing
(`noteLeafActivity_idempotent`) — and a site that omits it fails
`activeDescendantsConsistent`.  The call sites are **derived** from the three
things eligibility depends on, not enumerated:
...
* every transition that changes `threadActive tid` for a bound `tid`:
  ...
  caller is current, not queued — and is **suppressed inside a donation
  composite** (`removeRunnableOnCoreNoNote`), whose rendezvous removes the
  donor before the rebind: the composite notes once after the rebind, so the
  leaf's count goes `1 → 1` and no ancestor sees the `1 → 0 → 1` that would
  have invoked rule (e) on a reservation whose work merely changed threads
  (`donation_activity_transfer_no_crossing`); `suspendThreadOnCore`, the cancellation and
  fault suspends, `cleanupTcbReferences`, and the dispatch paths that clear a
  `current` slot without re-enqueueing — each noting **after** the leaf's own
...
* every transition that changes **which leaf a thread's activity is
  ...
  receiver, so the leaf is noted **after the rebind** and the donor's
...
* every transition that moves a node's `budgetRemaining` across zero — **at
  ...
  (f) (`0 → 1`) and the surrender (c) each run under `withEligibilityFlip` on the
...
deadline)`, §4.1) — and the set of those transitions is **read off
`chargedCoresOf` and `U`**, not listed: a departure is any step after which a
core that was in the root's `chargedCoresOf` no longer is, or on which its `U`
is lower.  Today that is `schedContextUnbind` of a root leaf (its whole `U`),
an affinity move (on the source core), a donation's return (on the donee's
core, which the return no longer releases), a reconfiguration to a smaller
reservation (the difference), `schedContextBindServer` of a root **leaf or
root server** (its `U`, now counted under the parent as well — a server that
drew on its window and then lost its members is exactly the cell a leaf-only
rule missed), and **destruction**, which removes every core: the destroyed
object cannot carry a residual, so `lifecyclePreRetypeCleanup` refuses a root
context that counts a **positive** share on a core — a bound root leaf, a
configured root server — until its current window ends (`now ≥ deadline`,
`.illegalState`, at most one period), and the object itself is the residual;
an unconfigured context (`U = 0`, a zero-share root server among them) or an
unbound parentless leaf counts nothing and is destroyed at once.  `rootUtilisationOnCore` sums live
...
required, so none is ever dropped (the bounded ledger is Q15's alternative).
The one departure whose success is *promised* — the return of a cross-core
donation — **reserves** the slot: `donationAdmissible?` refuses a cross-core
donation of a leaf carrying a live residual, and while the donation is in
flight the leaf admits no other departure — its owner's or its donee's
affinity move (the owner is treated as bound to the leaf, §4.8), a shrink
(which would record on both charged cores), `schedContextUnbind`, a link under
a server, retype — each `.illegalState` until the return, so the return finds
the slot empty and never fails (`residualReservedByDonation`).  The rule, not
the list: while a promised departure is outstanding every other departure is
refused, and the list is what that says of the transitions that exist.  T15's
contained-window demand then includes the departed windows,
...
blocker is dispatchable on the core — runnable there, in its active domain,
and `pathBudgetEligible`, since inheritance moves a key and not a budget and
the selector skips an exhausted or out-of-domain thread whatever its key —
when the blocker and the waiter have the **same parent** (both roots, or siblings
...
the negative suite, across: for a SchedContext — `{unconfigured, configured}`
(a retyped context is `SchedContext.empty`: budget zero, period `10000`, so
under D22 its derived deadline is `10000`, and binding it before its first
configuration is legal today — the configuration must then be rule (a), not a
clamp to the zero it holds) × `{root, member}` ×
...
**A composite's checks precede its first write, and its hooks fire once.**
The live chains compose transitions in a fixed order — the call rendezvous
before the donation, the donor's removal before the receiver's rebind — so a
refusal placed in a constituent is raised after an earlier constituent has
committed, and a hook placed in a primitive fires once per constituent rather
than once per chain.  Every refusal in the tables below is therefore evaluated
by the chain's **entry point** on the pre-state, before the chain's first
write, and a hook a chain would reach more than once is suppressed in the
constituents and fired once at the chain's end (§4.5's donation transfer).
The ninth cut placed the donation guard inside the donation composite, which
the live dispatch runs *after* the rendezvous, and the eighth cut's activity
note in the removal primitive gave a donation a `1 → 0 → 1` crossing; both are
the same omission, a rule walked against a transition and not against the
chain that runs it.
...
| effect | `serverCore := some core`; the window and budget are **kept** — conversion changes the context's role, never its reservation, so a used leaf cannot be laundered into a fresh budget by becoming a server (D17) — and both refill representations are already empty on a quiescent parentless leaf (rule (g) at its unbind; `unhomedNoPendingRefill`); the first member's activation lands in rule (e) with the window the leaf left, which re-arms only under the density rule |
...
| effect | `child.parentServer := some server`; `server.serverMembers += child`; `linkActivity` on the child (its own count unchanged; rule (e) on every ancestor that crosses `0 → 1`); if the child was a root that counts on a core — a leaf with a bound thread, or a server — its root share on that core becomes a residual until its deadline (D23 — the link is refused while a live residual that would not coalesce exists); reschedule |
...
| `lifecyclePreRetypeCleanup` of a SchedContext | a populated server is refused, and so is a context carrying a live residual or an in-flight donation, and so is a **root context that counts a positive share on a core** — a bound root leaf, a configured root server — until its current window ends (D23, §4.6 — destroying it would drop the share early, and a destroyed object cannot carry a residual, so the object is the residual; every refusal here lasts at most one period), while an unconfigured root server — `U = 0`, the shape `schedContextConfigureServer` on a retyped context produces, carrying the derived deadline `10000` of a default period — reserves nothing and is destroyed at once; **any** member — a leaf or an empty server — is unlinked as `unbindServer` would before destruction, so no `serverMembers` entry outlives its object; rule (g); `noteLeafActivity` before the unlink | `.illegalState` |
...
`replyRecvBody` for `replyRecvReturnDonation`'s forward half — not inside the
...
and changes nothing.  The live call dispatch runs `endpointCallWithCapsOnCore`
— caller blocked, receiver woken, capabilities transferred — and only then
`applyCallDonationOnCore`, returning the rendezvous state with the error when
the donation fails, so a guard placed in the composite (the ninth cut's
placement) could not make a refused Call state-preserving.  The composites
take admissibility as a hypothesis (`hAdmissible`) that the dispatchers
discharge, and `endpointCallCrossCoreDispatch_refused_donation_unchanged`
...
elaborator refuses to compile until the three arms exist.  This table gives the
**final** value each takes once CB6.6 wires the arms; the table after it gives
the **placeholder** each differing row holds between CB6.1 and CB6.6, so that
neither cut's tree is described by the other's values:
...
|-------|------------------------------------------|
| `syscallDelegates` | the wildcard refusal arm — no transition |
| `enforcementBoundary` + `syscallIdToEnforcementName` | the refusal path's class and name |
...
**Every activation caller** writes the leaf and its ancestors (§4.5), and
reads nothing else — the bracket decides on the node's own fields and the
primitive's own core, never on another core's queue or current slot — so
every footprint that reaches `noteLeafActivity` gains the
ancestors' SchedContext locks and no scheduler slot; the locks — `ancestorLockSetOf st tid`, at most
...
| `schedContextBindServer` | caller TCB (read), CNode root (read), server (write), the server's ancestors (write, ≤ 1 at depth 3), child (write), the child's bound TCB (write) when present, the home core's replenish queue (write — rule (e1) on a `0 → 1` crossing), the run queue of the child's `seamCoreOf?` (write — `keyRescheduleFootprint`, the seam the bind ends with) whenever the child has one: a bound leaf's thread core, or a server child's `serverCore` — an empty server child has no bound TCB and still a seam core; the admission slot of the core the child's share moves under (§4.12) |
...
with the seam composes it for `seamCoreOf?` of the context it moves (§4.1):
the bound thread's core for `schedContextBind`, `schedContextUnbind` and
`schedContextConfigure` on a leaf, `serverCore` for `schedContextConfigure` on
a populated server, the receiving thread's core for the donation composites,
the child's seam core for `schedContextBindServer` and
`schedContextUnbindServer`, when it has one.
The eighth cut listed the three hierarchy footprints as object locks and the
replenish-queue slot while their transitions ended with a scheduling point;
the slot is now derived from the seam's writes, so a new caller inherits it.

**Admission is a per-core slot, and the read is part of the footprint.**
`checkRootAdmissionOnCore st c …` reads `rootUtilisationOnCore st c`, a sum
over every root and residual counting on `c`, and the decision it yields is
committed by a write to one context.  Two callers that read the sum under
their own object locks alone — a Call donating onto `c` and a configure on
`c`, or two Calls onto `c` — each see `900 ‰`, each admit `100 ‰`, and the
core ends at `1100 ‰` with `hierarchicalAdmissionHolds` false and T13
refuted, because nothing in either footprint serialises the read with the
other's commit.  So the scheduler domain gains a third slot,
`SchedLockId.admission (c : CoreId)` (CB2.1) — exclusive, ordered after every
object lock and before the run-queue locks, by core among themselves — and
every transition that admits on `c` composes `admissionLockOf c` and holds it
from the read of the sum through the commit of the share or residual:
`schedContextConfigure` and `schedContextConfigureServer` (the candidate's
charged cores), `schedContextBind` of a root leaf, `setThreadCpuAffinityWithMigration`
(the destination), the call dispatch for a cross-core donation (the donee's
core), `schedContextBindServer` and `schedContextUnbindServer` (the core the
linked or detached root counts on).  Member admission needs no new slot:
`memberUtilisation` sums the parent's members, every member's reconfiguration
...
the projection erases **per object, never per field**.  A SchedContext visible
to the observer keeps `parentServer`, `serverMembers`, `serverCore`,
`activeDescendants` and `residual` with its other fields; an invisible one is
erased whole, as today.  Erasing the hierarchy fields of a *visible* object as
scheduling plumbing — the seventh cut's choice — made a state where a visible
leaf hangs under a visible same-label server low-equivalent to one where both
are roots, and the tick charges the server in the first only, so a visible
`budgetRemaining` diverged between low-equivalent states and the
non-interference lift could not hold.  Label uniformity (D8) is what makes
per-object erasure consistent: a visible leaf's ancestors carry its label and
are visible with it, so the projection of a visible tree is the tree.  The
rule the projection is derived from: a field a visible transition reads to
produce a visible effect is visible, pinned by `projection_determines_charge`
(two low-equivalent states charge the same visible path).  `periodStart`
follows whatever class `deadline` is in today; `inheritedDeadline` follows
`pipBoost`.  `schedContextWriteSet` becomes `(seamCoreOf? st scId).toList`
(§4.1): the bound thread's core for a leaf, as today, and `serverCore` for a
server.  The existing derivation from the bound thread returned `[]` for a
server, so the configure confinement theorem would have claimed that
reconfiguring a populated server changed no core-local slot while
`keyRescheduleOnCore serverCore` rewrote that core's flag, run queue and
current slot.  The write set and the seam's lock footprint (§4.12) are one
derivation, so they cannot disagree; CB5.6 re-proves the configure confinement
and non-interference surface over the new set.
...
| `SeLe4n/Kernel/Scheduler/Liveness/EdfGuarantee.lean` (new) | T14–T16 | staged like `PerCoreWcrt.lean`, which imports it: `Platform/Staged.lean` import if reachability does not already provide one, and allowlist line `SeLe4n.Kernel.Scheduler.Liveness.EdfGuarantee  # marker: CB7.2 guarantee until the composition closes`, both added in CB7.2 |
| `SeLe4n/Kernel/SchedContext/HierarchyInventory.lean` (new) | theorem inventory | staged, reached through the staged phase manifest: `Platform/Staged.lean` import if reachability does not already provide one, and allowlist line `SeLe4n.Kernel.SchedContext.HierarchyInventory  # marker: inventory claimed by the phase manifest`, both added in CB8.2 |
...
Every new module whose import closure is staged takes its allowlist line in the
row that creates it: the partition gate computes the staged-only set from the
tree and holds it equal to the allowlist in both directions, so a module this
table names as staged without a line fails Tier 0 the day it lands — which the
twelfth cut's two rows would have done.
...
the sweep the compiler forces — and a row names only symbols a lower-numbered
row or the row itself defines, so a pure definition lands with the data it
reads (a constant or record predicate with the record, a state query with the
queries, a bundle after every conjunct it names), never in the cut where it
first matters, the ledger below being the check; (2) every live definition the row changes has
every theorem that unfolds it re-proved in the row — the frozen twin of an
operation `frozenOpCoverage` covers, and its agreement theorem, among them,
since a twin without the change is the old behaviour under another name; (3) every behaviour the
row makes reachable is described by the specification and pinned by a test in
the row — a rule that can leave the current thread ineligible with the
scheduling point that enforces it among them, since a CBS rule without its
preemption is a budget the thread overruns until the next tick; (4) every table over a total type states what is **true after the
...
| `pathBudgetEligible` | CB3.1 |
| `resolveEffectiveSchedPath` | CB3.2 |
| `isBetterPath` | CB3.3 |
| `chooseBestRunnableHierarchical` (path form) | CB3.4 |
| `candidateOutranksCurrentOnCore`, `keyRescheduleOnCore`, `edfCurrentEarliestOnCore`, `currentPathEligibleOnCore` in path form | CB3.5 |
| `chargeSchedPath`; `withEligibilityFlip`, `noteLeafActivity`, `removeRunnableOnCoreNoNote`, `propagateCrossing`, `linkActivity`, `unlinkActivity` | CB4.1 |
| the three hierarchy transitions and their footprints | CB5.1, CB5.3, CB5.4, CB5.14 |
| the three `SyscallId`s and their encoders | CB6.1, CB6.2 |
...
| Sub | Description | Files | Est |
|-----|-------------|-------|-----|
| CB2.1 | Add `parentServer`, `serverMembers`, `serverCore`, `activeDescendants` and `residual` to `SchedContext` with defaults (§4.1); the constants `maxServerDepth := 3` and `maxServerMembers := 16` (D9), since `MemberList`'s bound names the latter; `MemberList`; `isServer`, `isLeaf`; the record predicate `eligibleActive` (§4.5 — budget and count, the node's own fields, so it is defined with them); extend the manual `BEq` instance; **in the same row as** the sweep of every constructor-arity destructuring the build now rejects — `schedContextReferencesReservedIdleSlot`, `bootSafeSchedContextCheck`, the freeze mirror and siblings — classifying the new fields (a member or parent id naming a reserved idle object is refused; a boot SchedContext is a parentless leaf with no active descendants), since a structure extension and the sweep the compiler demands cannot build apart; the scheduler-domain lock id `SchedLockId.admission (c : CoreId)` with its place in the order — after every object lock, before the run-queue locks, by core among themselves — and `admissionLockOf` (§4.12) | `SeLe4n/Kernel/SchedContext/Types.lean`, `SeLe4n/Kernel/Scheduler/Operations/PerCoreChooseThread.lean`, `SeLe4n/Kernel/SchedContext/Hierarchy.lean` (new), `SeLe4n/Platform/Boot.lean`, `SeLe4n/Model/FrozenState.lean`, whatever else the build names | M |
...
| The activation footprints (CB4.4) push the widest lock set past `maxLockSetSize` | CERTAIN | MED | Measured at planning time and **re-measured at `v0.34.128`**: `lockSet_tcbSuspend` is nine at its widest and the ceiling is 11, so the ancestors' two take it to the cap; the constant moves to thirteen (D21) with the WCRT terms re-derived, rather than a lock taken outside the set.  CB4.4 re-measures again — this row's numbers are a snapshot |
...
| Q12 | `maxLockSetSize` up by two (D21) — or `maxServerDepth` to `2`? | up by two | Depth two forbids the root server → server → leaf shape; the constant stays, CB4.4 shrinks to the footprint additions.  The *value* was `10` when this was answered and is `13` after WS-OD OD3.5; the answer is the direction, not the number |
...
| `timerTickOnCore_preserves_perCoreCbsInvariant` (re-proved), `cbsReplenish_server_reschedules_local`, `propagateCrossing_stops_at_first_non_flip`, `noteLeafActivity_idempotent` | CB4 | the CBS bundle survives path charging; a server refill triggers the executing core's reschedule decision; the counter walk is the crossing walk |
...
9. **Thirteen review rounds, ninety-two findings, one plan.**  The automated
   ...
   | The deactivation hook keyed on queue membership, so the executing caller `removeRunnableOnCore` clears from `currentOnCore` was never deactivated | one idempotent `noteLeafActivity`, keyed on the before/after `threadActive` predicate (§4.5) |
   ...
   | The activity sync recomputed `threadActive` over every core's queue and current slot under none of their locks | eligibility reads the node's own maintained fields; the primitive notes the activity it knows from the core it holds; the global predicate lives in the invariant only (§4.5, §4.12) |
   | A refill landing after its whole window revived the expired window with a full budget, outranking current work | rule (d) lands into `[now, now + P)` once `[refillAt, refillAt + P)` has passed; `eligibilityGain_window_current` is held by each rule, no longer by the sync the seventh round stopped re-arming (§4.2) |
   | A context bound before its first configuration was "live", so its configuration clamped to zero and its first budget waited for the derived deadline `10000` | `{unconfigured, configured}` joins the state dimensions; rule (a) applies to an unconfigured context whether bound or not (§4.2, §4.8, CB1.6) |
   | A donated root leaf's owner could migrate, so the return — promised never to fail — needed a second residual the one slot cannot hold | a promised departure reserves the slot: a leaf mid-donation admits no departure but the return (§4.6, §4.8, §4.10, D23) |
   | Bind of a running thread left ineligible by rule (e2) had its scheduling point in CB1.7, one cut after the rule | the eligibility scheduling point lands with the rule in CB1.6; §7's third question names the pairing (§4.2, §7) |
   | The EDF response bound and restated liveness hypotheses landed two rows after the order switch | the liveness restatement joins the order switch, CB1.7, and its own row is retired (§7) |
   | Erasing the hierarchy fields of visible objects made a linked and an unlinked tree low-equivalent while the tick charged only one | the projection erases per object, never per field; `projection_determines_charge` pins it (§4.13, CB2.8) |
   | `chargedCoresOf` derived both charged cores from `boundThread`, which is the donee twice during a donation, so the owner's core took new admissions the promised return then overcommitted | the owner is read by one accessor, `donationOwnerOf?`, wherever a rule needs it (§4.1, §4.8, CB2.3) |
   | `MemberList`'s bound named `maxServerMembers` one row before it was defined; the bundle filtered by `eligibleActive` two phases before it was defined | the constants and the record predicate land with the record (CB2.1), `threadActive` with the queries (CB2.3); §7's first question now requires it and a ledger of every §4 symbol's defining row is the check (§7) |
   | The three hierarchy footprints listed object locks and the replenish-queue slot while their transitions ended with the reschedule seam, which writes the core's scheduler state | the seam has a footprint, `keyRescheduleFootprint`, and every caller composes it (§4.12, CB1.3, CB5.14) |
   | The flag-to-SGI theorem was stated over every core, and the gated local arm sets the executing core's flag with no SGI to send | the theorem is stated over remote cores; the executing core's flag is consumed by its next scheduling point, which the tick guarantees within one tick (§4.4, CB1.3, CB1.7) |
   | An empty root server that had drawn on its window could be destroyed at once, dropping its share before the deadline | the departures are read off `chargedCoresOf` and `U`, and destruction removes every core: a root context that counts on a core is destroyed only after its window ends, the object being its own residual (§4.6, §4.8, D23, CB5.11) |
   | Linking an empty root server recorded no residual, since the rule named a root leaf | the same derivation: a root leaf or root server that counts on a core leaves a residual when linked (§4.6, §4.8) |
   | T18 concluded a dispatch for a blocker the selector would skip — exhausted, or out of the active domain | T18 requires the blocker dispatchable on the core: runnable, in the active domain, `pathBudgetEligible` (§4.7) |
   | T14's single-domain hypothesis let the guaranteed root's threads sit outside the core's active domain, where the selector never picks them | H4 requires `e`'s domain to be the core's active domain over the window (§4.11) |
   | `schedContextWriteSet` derived a server's write set from its bound thread and returned `[]`, while reconfiguring a populated server rewrites `serverCore`'s scheduler state through the seam | the write set and the seam's lock footprint are one derivation from `seamCoreOf?` (§4.1, §4.12, §4.13, CB5.6) |
   | A configured server emptied by unbinding and detaching its leaves was "not live", so reconfiguring it minted a fresh budget inside a window it had drawn on; conversion of a used leaf did the same | rule (a) is for an unconfigured context alone; every configured context, bound or not, takes rule (f), and conversion keeps the window (§4.2, §4.8, D17, CB5.1) |
   | The removal primitive's activity note ran before the donation's rebind, so a Call gave the leaf a `1 → 0 → 1` crossing and rule (e) re-armed ancestors for work that only changed threads | the note is suppressed inside a donation composite and fired once after the rebind, a `1 → 1` transfer (§4.5, §4.8, CB4.4, CB5.10) |
   | The donation guard sat inside a composite the live dispatch runs after the rendezvous, so a refused Call had already blocked the caller and woken the receiver | the guard is the dispatcher's, on the pre-state before the first write; the composites take admissibility as a hypothesis (§4.8, §4.12, CB5.10) |
   | The frozen twin of `schedContextConfigure` kept the ungated write of priority and domain while `frozenOpCoverage` claimed a counterpart | the twin takes the caller and the two refusals in CB0.3; §7's second question names frozen twins (§7, CB0.3) |
   | `currentBudgetPositiveOnCore` kept reading the leaf's budget after the selector switched to the path, so the bundle certified a current under an exhausted ancestor | every per-core conjunct that reads a budget switches to the path form at CB3.6, `currentPathEligibleOnCore` among them, the set swept rather than named (§4.10, CB3.5, CB3.6) |
   | T17 bounded consumption over an interval by one `U` and one budget, which a reconfiguration inside the interval changes | T17 is per window — consumption inside a window is at most its opening budget — and T15 sums opening budgets, each admitted under the utilisation in force at release (§4.11) |
   | T5's dead-time bound was false for a window placed in the future | `windowStartNotInFuture` (`periodStart ≤ now`) joins `schedContextStoreConsistent`, preserved by every rule, and is T5's premise (§4.2, CB1.2, CB1.6) |
   | §4.8 moved the donation guard to the dispatchers; CB5.2 still assigned it to the composites and named no dispatch file | CB5.2 wires the guard into the three dispatchers, the composites take the hypothesis; the dispatch files are in the row (CB5.2) |
   | CB5.2 changed live transitions and left their preservation to CB5.12, ten rows later | the preservation of every live transition CB5.2 changes lands in CB5.2; CB5.12 keeps the hierarchy bodies (CB5.2, CB5.12) |
   | S10 expected `900 ‰` right after the reply, the pre-D23 release the residual exists to prevent | S10 reads `1000 ‰` after the return and `900 ‰` after the residual's deadline (§4.15) |
   | The total table was labelled the CB6.1 sweep while holding CB6.6's values, the paragraph beside it demanding placeholders | two tables: final values, and the placeholders each differing row holds between the cuts (§4.9) |
   | A zero-share root server — `configureServer` on a retyped context — was unretypable for a default period | the destruction delay is for a root that counts a positive share; `U = 0` reserves nothing and is destroyed at once (§4.6, §4.8, CB5.11) |
   | Two new staged modules had no allowlist line and no row adding one | `EdfGuarantee` and `HierarchyInventory` take their lines and `Staged.lean` imports in the rows that create them, and §4.14 states the rule (§4.14, CB7.2, CB8.2) |
   | The hierarchy footprint rows composed the seam slot "when bound", while an empty server child has no bound TCB and a seam core all the same | the rows follow `seamCoreOf?`, as the paragraph beside them already did (§4.12, CB5.14) |
   | Admission read a per-core sum under object locks alone, so two concurrent admissions could each see `900 ‰` and commit `1100 ‰` | `SchedLockId.admission c`, held from the read to the commit by every admitting transition; §7's fifth question now covers the reads a decision depends on (§4.6, §4.12, §7, CB2.1, CB5.2) |
   | A donation could hand a context to a receiver in another domain, which the selector skips while H5 saw a runnable descendant | the donee's domain must equal the context's, as bind requires; `bindingDomainConsistent` covers `.donated`; H5 names the active domain (§4.8, §4.10, §4.11, CB5.2) |
   | The Rust configure wrapper kept forwarding an arbitrary deadline for five cuts after the kernel began refusing it | the Rust side of the contract — wrapper, struct docs, tests — lands in CB1.6, which joins the cross-build list (§8, CB1.6, CB6.2) |

   The sub-task count moved from 93 to 72: CB1, CB3, CB4 and CB6 each took
   ...
   cuts that make their subject reachable, the compiler sweeps and the server
   refills joined the cuts that cannot build or hold without them, and the
   liveness restatement joined the order switch it describes.
...
10. **What the forty-five findings had in common, and what changed so the
    ...
    **derivations themselves** into §4.4 and §4.5 — the key's inputs and the
    ...
    The eighth round's seven findings fall into the same five classes — two
    sequencing splits (the bind scheduling point, the liveness restatement), a
    state dimension the list omitted (unconfigured), a projection that erased
    per field, an enumeration (D23's refusals) whose rule is *a promised
    departure reserves the slot* — and two of them were **consequences of the
    seventh round's fixes**: the one-slot residual rule refused what the return
    donation promises, and the walk that stopped re-arming the landing node
    stopped repairing the expired window a late landing revives.  That is the
    sixth thing to check, and it is the first five applied to the fix itself:
    **a fix is a transition — walk it against the properties the mechanism it
    replaces was holding**, incidentally or not, and re-home each one in the
    rule responsible.  The proxy read — `threadActive` recomputed over every
    core's slots by a sync whose footprint held the ancestors' locks and none
    of theirs — is the standing constraints' *a proxy is not the fact*, and its
    exit is the same: the transition already holds the fact, locally and
    locked; read that, and leave the global predicate to the invariant.

    The ninth round is the definitions-before-consumers shape for the third
    time — the admission arithmetic in the seventh, `maxServerMembers` and
    `eligibleActive` now — and each time the instance was moved and the class
    stayed open, because the plan gate resolves row *citations* and cannot see
    a *symbol*.  A table a reader can check is the nearest thing to a gate a
    plan has, so §7 now carries the ledger of every §4 symbol's defining row
    and the first per-row question requires it.  The other three are old
    classes at new sites: the donation owner derived twice from `boundThread`
    (one fact in two places, closed by one accessor); the seam's footprint
    listed rather than derived from what the seam writes (enumeration where a
    derivation was needed, closed by composing `keyRescheduleFootprint`); and a
    theorem stated over every core when its hypothesis holds only for remote
    ones (the textbook-theorem class — walk the statement against every arm,
    the gated local one included).

    The tenth round is the derivation rule at the one place §4.6 had still
    enumerated: the departures were a list — unbind, move, return, shrink, the
    link of a leaf — and the list lacked destruction and the root-server kind,
    exactly the two cells a derivation from `chargedCoresOf` and `U` produces
    on its own.  Two theorems had hypotheses that had not been walked against
    two dimensions the §4.8 preamble lists for every *rule* — eligibility and
    the active domain — which is the textbook-theorem class again, and its
    rule gains a clause: **walk a theorem's hypotheses against the same state
    dimensions the rules are walked against**.  And the information-flow write
    set was a second derivation of a fact the lock footprint had just been
    derived from — the core a key change reaches — closed by deriving both from
    `seamCoreOf?`.

    The eleventh round is the fifth class — a rule walked against a
    transition and not against the **chain** that runs it — and it is now
    written down as such (§4.8): the live dispatch runs the rendezvous before
    the donation, so a guard inside the donation composite refuses after the
    caller is blocked, and the removal primitive runs before the rebind, so a
    hook inside it fires twice across one transfer.  Checks belong to the
    chain's entry point, on the pre-state; hooks fire once, at the chain's
    end.  The fresh-window finding is the nominal-state class at rule (a):
    "not live" was a state the caller can *manufacture* inside a window by
    unbinding and detaching, so the only state that earns a fresh window is
    the one no caller can return to — unconfigured.  The frozen twin is the
    ledger rule's cousin: an operation and its frozen twin are one definition
    with two spellings, and §7's second question now says so.  And the
    per-core budget conjunct is the enumeration rule at the selection switch:
    "the conjunct" was one when the sweep says two.

    The twelfth round is the sweep rule failing at this plan's own
    restatements, four times: §4.8 moved the donation guard to the dispatchers
    and CB5.2 kept it in the composites; D23 landed in the sixth cut and S10
    still expected the release it forbids; the unconfigured dimension was added
    in the eighth cut and the destruction rule written in the tenth was not
    walked against it; the staging table named two modules staged and gave
    neither the line the partition gate holds the tree to.  A rule changed at
    its statement and not at its restatements is the *one fact in two places*
    class with the plan itself as the second place, and the remedy is the
    sweep: every citation of the decision, every row, scenario and table that
    restates it, walked when the decision moves.  This cut ran that sweep for
    D23, D17 and the unconfigured dimension and records that the drifts it
    found were the ones reported.  Two theorems were walked against the
    reconfigure and time dimensions — T17 is per window, T5 takes the
    window-start invariant — and CB5.2 carried live transitions with their
    proofs ten rows behind: the sequencing class at a row the inert-then-switch
    shape had not reached because nothing called it a switch.

    The thirteenth round adds one rule and repeats three.  The rule: **a
    decision's read is a footprint** — §7's fifth question covered every write
    and not the per-core sum an admission is decided on, so two admissions
    could interleave between read and commit; the remedy is a slot for the
    resource the decision is about, `SchedLockId.admission c`, held through
    the commit, and the question now names reads.  The repeats: the footprint
    table said "when bound" one round after the paragraph beside it was
    rewritten to follow `seamCoreOf?` (a fix is a transition — its own
    restatements included); the domain dimension was walked through T14 in the
    tenth round and not through donation (when a dimension is found missing at
