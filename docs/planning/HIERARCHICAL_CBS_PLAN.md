...
structure SchedContext where
  ...
  /-- A root share that left a core — by unbind, a move, a donation's return,
      ...
      deadline)` while live, cleared once `deadline ≤ now`. -/
...
One helper, on derived triggers.  `syncLeafActivity st scId now` sets the
leaf's count to `1` if its bound thread is `threadActive` and to `0`
otherwise; if the count crossed, it applies `cbsActivate` to the leaf on
`0 → 1` and propagates the crossing.  It is idempotent — a site that calls it
twice changes nothing, a site that omits it fails `activeDescendantsConsistent`
— and its call sites are **derived** from the two things the count depends
on, not enumerated:
...
* every transition that changes `threadActive tid` for a bound `tid`:
  ...
  `current` slot without re-enqueueing;
...
untouched; bindServer/unbindServer of a member call `propagateActivation` on
the (un)linked child with the sign of the child's count, so a whole subtree's
activity enters or leaves the ancestors' counts through the same crossing walk
(§4.8).  The invariant
...
optimistic) and may not be retyped; `.illegalState` otherwise, for at most
one period.  T15's contained-window demand then includes the departed windows,
...
| effect | `child.parentServer := some server`; `server.serverMembers += child`; `propagateActivation` on the child with `+1` if its count is positive (rule (e) on every ancestor that crosses `0 → 1`); if the child was a root leaf with a bound thread its root share on that core becomes a residual until its deadline (D23); reschedule |
...
| CB5.7 | `schedContextUnbind` on a member leaf: today's effect plus `syncLeafActivity` (the ancestors' counts through the crossing walk if the thread was active); a member leaf keeps its refill (it is still homed) and records no residual (it never counted as a root); `schedContextUnbindOnCore` follows; the existing unbind surface re-elaborated in the row | `SeLe4n/Kernel/SchedContext/Operations.lean`, `SeLe4n/Kernel/SchedContext/OperationsPerCore.lean` | S |
...
9. **Six review rounds, fifty-one findings, one plan.**  The automated
   review of the second through seventh cuts found defects in the *design*, not
