...
with the seam composes it for the core it names: `schedContextBind`,
`schedContextUnbind` and `schedContextConfigure` (the bound thread's core),
the donation composites (the receiving thread's core), `schedContextBindServer`
and `schedContextUnbindServer` (the child's bound thread's core, when bound).
...
| `parentChain?`, `pathLength?`, `rootOf?`, `isAncestorOf`, `schedPath?`, `donationOwnerOf?`, `midDonation`, `chargedCoresOf`, `threadActive`, hierarchy-aware `replenishHomeOf?`; `rootCountsOnCore`, `rootUtilisationOnCore`, `checkRootAdmissionOnCore`, `memberUtilisation`, `checkMemberAdmission`, `residualLive`, `residualWellFormed` | CB2.3 |
...
9. **Nine review rounds, seventy findings, one plan.**  The automated
   review of the second through tenth cuts found defects in the *design*, not
