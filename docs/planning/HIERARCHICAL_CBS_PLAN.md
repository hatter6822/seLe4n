...
| `schedContextBindServer` | caller TCB (read), CNode root (read), server (write), the server's ancestors (write, ≤ 1 at depth 3), child (write), the child's bound TCB (write) when present, the home core's replenish queue (write — rule (e1) on a `0 → 1` crossing), the child's bound thread's run queue (write — `keyRescheduleFootprint`, the seam the bind ends with) when bound |
| `schedContextUnbindServer` | caller TCB (read), CNode root (read), child (write), parent (write), the parent's ancestors (write, ≤ 1), the child's bound TCB (write) when present, the former home core's replenish queue (write — rule (g)'s purge), the child's bound thread's run queue (write — `keyRescheduleFootprint`) when bound |
...
| `parentServer`, `serverMembers`, `serverCore`, `activeDescendants`, `residual`; `maxServerDepth`, `maxServerMembers`; `MemberList`, `isServer`, `isLeaf`, `eligibleActive` | CB2.1 |
| `parentChain?`, `pathLength?`, `rootOf?`, `isAncestorOf`, `schedPath?`, `donationOwnerOf?`, `midDonation`, `chargedCoresOf`, `seamCoreOf?`, `threadActive`, hierarchy-aware `replenishHomeOf?`; `rootCountsOnCore`, `rootUtilisationOnCore`, `checkRootAdmissionOnCore`, `memberUtilisation`, `checkMemberAdmission`, `residualLive`, `residualWellFormed` | CB2.3 |
...
9. **Twelve review rounds, eighty-eight findings, one plan.**  The automated
   review of the second through thirteenth cuts found defects in the *design*, not
