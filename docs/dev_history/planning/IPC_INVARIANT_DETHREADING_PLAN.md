...
> **Status**: **IN FLIGHT — the de-threading itself is DONE; the payoff is
...
> `dispatchSyscall_preserves_ipcInvariantFull` exists, and they are not a
...
| `blockedThreadsPendingMessageConsistent` | **wiring** | all 7 base transitions |
...
| D8 close-out + payoff theorem | 🔄 **DISPATCH INTEGRATION IN PROGRESS (Slice 1 LANDED v0.32.46).** **Slice 1 (v0.32.46)**: the three single-core `*WithCaps` `ipcInvariantFull` bundles now *establish* `allPendingMessagesBounded` / `endpointQueueNoDup` / `ipcStateQueueMembershipConsistent` from the pre-state (9 threading sites removed). New `ipcUnwrapCaps_preserves_{allPendingMessagesBounded,endpointQueueNoDup,ipcStateQueueMembershipConsistent}` cap-transfer frames (objects-backward — the transfer writes only a CNode at `receiverRoot`) + the 9 `*WithCaps` establishers composing base-establish ∘ cap-transfer-frame. **Slice 2 (v0.32.47)**: the cap-transfer `badgeWellFormed` frame is built — `ipcUnwrapCaps_preserves_badgeWellFormed` (+ the `cspaceInsertSlot`→`ipcTransferSingleCap`→`ipcUnwrapCapsLoop` chain in the Capability invariant layer), gated on a "message caps badge-valid" precondition (dischargeable from the sender's pre-state `capabilityBadgesWellFormed`). Because the frame lives in the Capability invariant layer (which does not import the IPC structural layer), the `*WithCaps` badge establishers + the `hBadge'` discharge are **inherently dispatch-payoff-layer** work (the API layer is the first to import both layers). **Slices 3–4 (v0.32.48–49)**: the staged cross-core `endpointCallOnCore_preserves_ipcInvariantFull` bundle now *establishes* **both** queue conjuncts (`endpointQueueNoDup` v0.32.48 + `ipcStateQueueMembershipConsistent` v0.32.49) from the pre-state — new cross-core frames `{removeRunnableOnCore,wakeThread}_preserves_{endpointQueueNoDup,ipcStateQueueMembershipConsistent}{,_of_ready}` + the two `endpointCallOnCore_preserves_{endpointQueueNoDup,ipcStateQueueMembershipConsistent}` establishers (faithful cross-core mirrors of the single-core proofs: `ensureRunnable`→`wakeThread`, `removeRunnable`→`removeRunnableOnCore`; the `linkServerStashedReply_preserves_{…}` lemmas promoted public). The cross-core call bundle now threads only `hWtpmn'` + `hRCLRecip'`. **Remaining `*WithCaps`-threaded** (next slices): `hDualQueue'` (relocate the `ipcUnwrapCaps_preserves_dualQueueSystemInvariant` frame from `PerOperation` to `DualQueueMembership` so the in-file bundle can call it — IPC-layer-internal, de-threadable in place); `hBadge'` (frame built; establish at the payoff layer); `hWtpmn'` + `hRCLRecip'` (D8-layer / composite). **Cross-core dispatch payoff is blocked on SM6.D** — `endpointReceiveDualOnCore` / `endpointReplyOnCore` / `endpointCallWithCapsOnCore` lack full `ipcInvariantFull` (20-conjunct) preservation bundles (only the 15-conjunct `ipcInvariant` core exists); building those is the SM6.D "per-core IPC invariant bundle", a separate pending phase. After Slice 2c (v0.32.45) every IPC transition establishes all *structurally-establishable* conjuncts. The remaining threaded hypotheses are **not** per-transition de-threads — they are either (i) **dischargeable reachability preconditions** (`hCallerReady`/`hReceiverReady`/`hWaiterReady`/`allTimeoutBudgetsNone`/`*NotReply`/`*NotUnbound`/freshness — properties of the running syscall thread, supplied by a reachability bundle); (ii) **genuine composites** correctly threaded at the bare level and restored at the dispatch layer — `donationOwnerValid` for the 2 reply bundles (bare reply wakes the owner; `applyReplyDonation` returns the donation).  *`replyCallerLinkageReciprocal` left this class at v0.32.57*: the PR #827 review-#3 reply-fold moved the single-use consume **into** `endpointReply`/`endpointReplyRecv`/`endpointReplyOnCore`, so the direct reply establishes reciprocity itself (`endpointReply_preserves_replyCallerLinkageReciprocal`) and `endpointReply_preserves_ipcInvariantFull` no longer threads `hRCLRecip'`; the `endpointReplyRecv` + queue-transition bundles still thread it (their receive/link legs create a *new* caller↔Reply edge — the `endpointReceiveDual` composite reciprocity establish); or (iii) **`blockedThreadsPendingMessageConsistent`** (genuinely per-transition, establishers exist in `PerOperation` downstream — relocatable, but the plan's intended path is D8-layer discharge). **D8 payoff scope**: `dispatchWithCap_preserves_ipcInvariantFull` composes, per IPC arm, the cross-core dispatch wrappers (`endpointSendDualWithCaps`+`clearWokenReceiverStash`, `endpointReceiveDualOnCore`, `endpointCallCrossCoreDispatch`, `endpointReplyCrossCoreDispatch` — the single-use consume is folded into the reply transitions themselves as of v0.32.57, no separate dispatch-layer consume remains) — each needs a `_preserves_ipcInvariantFull` built over the now-de-threaded per-transition bundle — plus object-frame preservation for every capability/cspace arm + `dispatchCapabilityOnly`, all gated by the reachability bundle. Large multi-slice integration; the per-transition establishers it consumes are all in place. | — |

### WS-RR RR3 (v0.34.43) — D1 and D6 closed, D8's de-threading half closed

| Slice | Status | Where |
|---|---|---|
| D1 `blockedThreadsPendingMessageConsistent` | ✅ **FULLY DE-THREADED** (RR3.2–RR3.6).  The module-ordering obstruction was the whole of it: the establishers moved up beside the bundles that call them | `IPC/Invariant/Structural/`, `IPC/Invariant/WaitingThreadHelpers.lean` |
| D2 `replyCallerLinkageReciprocal` (the second half the census named) | ✅ **FULLY DE-THREADED** (RR3.7–RR3.10) via the new `replyLinkageFrame` family and per-transition establishers; the `consumeCallerReply` documented exception closed rather than re-recorded | `IPC/Invariant/Structural/DualQueueMembership.lean` |
| D1′ `dualQueueSystemInvariant` / `badgeWellFormed` (not in the original census) | ✅ **FULLY DE-THREADED** (RR3.11).  Two vacuity defects closed with it: an "every ObjId is a CNode" side condition that made the `*WithCaps` bundles vacuous on their own success path, and `badgeWellFormed` constraining badges *at rest* while the transfer installs an in-flight capability verbatim — closed by the new `pendingMessageCapBadgesWellFormed` | `IPC/Invariant/Structural/`, `IPC/Invariant/Defs.lean` |
| D6 `donationOwnerValid` (not in the original census) | ✅ **FULLY DE-THREADED** (RR3.12).  The conjunct was **false** of a bare reply's post-state, not merely unproven, so nine reply bundles were vacuous rather than conditional; answered with `donationOwnerValidExcept` plus the donation return's upgrade, and the live cross-core `.reply` dispatch now covers the donating path | `IPC/Invariant/Defs.lean`, `IPC/CrossCore/EndpointReplyDispatchInvariant.lean` |
| D8 pre-state discharge | ✅ **LANDED** (RR3.13, RR3.14) — `ipcReachable` collects the state-shaped preconditions, the running-caller and queue-tail ones are *derived* from `ipcInvariantFull` itself, and the boot state is proved to satisfy the bundle so it is inhabited | `IPC/Invariant/Reachability.lean` |
| D8 payoff theorems | 🔄 **OPEN**, registered — see the header | `docs/planning/ipc_dethreading_pending.txt` |
