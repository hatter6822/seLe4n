...
**WS-DT (IPC `ipcInvariantFull` de-threading) — CLOSED at v0.34.43** — in
flight since v0.31.157; registered here by RR0.1 at v0.34.26 after the audit
filed its absence from this file as blocker 1; closed end to end by WS-RR
phase RR3 (RR3.1–RR3.26), dispatch payoff included.  See the **WS-DT**
section below.
...
and the hardware IRQ path is closed.
...
chosen `KernelObject` at a chosen id.  SM9.C is the syscall that moves the data.

**The transition.**  `notificationSignalDeclassifiedOnCore`
(`InformationFlow/DeclassifiedSignal.lean`, **production** — the live arm calls
it, so staging it would break the partition gate) is a real SM6.B bound signal:
the badge lands where the ordinary signal would put it, a waiter or the
notification's bound TCB is woken on its **own** home core, and the committed
post-state is literally SM6.B's with `declassificationAuditLog` replaced
(`notificationSignalDeclassifiedOnCore_frame`).  That frame is why the whole IPC
invariant surface transfers rather than being re-proven, and why
...
`declassifiedSignal_audits_each_hop` records both in order, the first naming the
notification and the second the receiver's own TCB;
`declassifiedSignal_no_invented_edge` is the property a single record could not
have, since collapsing `high → mid` and `mid → low` puts a direct `high → low`
edge in the trail that no run of `declassificationDecision` ever returned `.ok`
for.  `declassifiedSignal_never_unaudited` is the fail-closed half: a delivery
that cannot record **all** of its downgrades performs none of them.
...
**A footprint is not an authorization.**  `declassifiedSignalEffectFootprint` is
defined **once** (notification ⊕ resolved receiver ⊕ SM6.B's own
`notificationSignalBoundWriteSet`) and read by both the confinement proof and
the non-interference theorem, so the two cannot name different cores.  It takes
**no policy** — which is what makes `footprint_does_not_authorize` provable
rather than plausible: a receiver squarely inside the footprint is still refused
when the policy refuses it.  Reading a footprint as a permission would be the
SM6.B badge-leak class one abstraction up.

**Declassification-relative non-interference.**  Because this flow is meant to
be seen, its bound is a write set **plus** a recording obligation rather than an
equality of projections.  `declassificationRelativeNonInterference` has three
conjuncts — confinement on every core outside the footprint, every visible
difference recorded with a domain pair the decision authorized, and the object
effect bounded to the ordinary signal's — and the sharpest of the runtime
negatives is that this transition is **not** plain non-interference at all: the
footprint's core really changes, so a theorem asserting an unchanged view
everywhere would be *false* of it.

**The NI taxonomy that deliberately did not grow.**  The plan's SM9.C.7 asked
for `KernelOperation.all` 35 → 36.  Carrying that out would have been wrong:
every `NonInterferenceStep` constructor concludes the projection is *unchanged*
(`step_preserves_projection`, uniformly on the exhaustive match), so an
operation whose defining property is an authorized *visible* flow cannot
...
reported as coverage of the whole.  SM8.C had set the precedent already
(`declassifyObjectFromCore` is likewise absent).  So the inventory that grew is
`CrossCoreTransition` (28 → **29**, live arms 21 → **22**, remote writers 22 →
**23**, delegation-backed 13 → **14**), and `KernelOperation`'s docstring now
records the exclusion with a Tier-3 negative pinning it, so a later cut that
adds one has to delete the note first.

**Registries, and what three total-match tables forced.**
`SyscallId.declassifySignal = 33` (count 33 → **34**), both Rust mirrors,
conformance, a `sele4n-sys` wrapper.  `refusalSeamClass` → `.records`, so a
refused downgrade through a signal reaches the SM9.B ledger rather than
bypassing it — that seam's own tripwire fired exactly as its docstring predicted
it would, and `refusalSeamClass_records_iff` became a disjunction.
`syscallReturnShape` → `.unit` (the badge goes to the receiver, not back to the
signaller); `syscallChecksTargetFirst` → `false` (the authority is an ordinary
notification capability, so there is no wrong-kind refusal to sequence ahead of
the rights gate).  Enforcement boundary 42 → **43** canonical / 57 → **58**
per-core, **policy-gated** and naming the live arm.  `lockSet_declassifySignal`
is *composed* — `lockSet_notificationSignal` extended with the state-level write
its trail append needs — rather than rewritten, so the notification half cannot
drift (`lockSet_declassifySignal_extends_notificationSignal` is the tie);
inventory 107 → **109**; and `auditState_footprints_share_serialization` grows a
fourth conjunct, because this is the one trail writer whose footprint is
*dominated* by object-level members and would otherwise look like an ordinary
IPC syscall.  The arm is delegation-backed
(`syscallDelegates_declassifySignal`), and the per-core routing gate passes with
**zero** allowlisted exceptions.
...
`endpointQueueRemoveDual_frame` — which the audit-trail frame this phase needed
...
SM9.C.8/.C.9 surfaces; `SmpSurfaceAnchors` §11; four `declassifying signal`
...
The transition consulted the declassification plan before the delivery's typed
lookup, so a writable capability to a non-notification object read policy
state off the error discriminant (`.declassificationDenied` vs
`.invalidCapability`) — an invalid capability as a policy oracle, and an
inconsistency with the sibling `.declassify`, which has always validated its
target before `authorizeDeclassificationOnCore`.  The transition now validates
the operand as a live notification ahead of every policy read, answering the
ordinary signal's own recovery errors (`.invalidCapability` wrong-kind /
...
(bumped to **3** at v0.34.44, WS-RR RR4 audit round: the status label moved to
the top of the label range so a delivered fault tag is never read as an error).
...
(the `.vspaceUnifyInstruction` code-publication syscall).  No residuals
remain.  Prior: SM7.C per-core TLB model LANDED (v0.32.80); operative
...
closes the *cache* half, and the two hierarchies are architecturally
asymmetric in the way that decides the whole design: the **data** caches of
the PEs in a shareability domain are hardware-coherent and `DC` maintenance
by VA to the Point of Coherency is architecturally visible to every agent
(ARM ARM B2.7 / D7.4), while the **instruction** caches are coherent with
nothing — neither the data side nor each other.

**The gap this closed.**  Before SM7.D the kernel performed **no
instruction-cache maintenance on any live path**: `ic_iallu` had an FFI seam
with no caller, and the SMP-correct broadcast variant `ic_ialluis` had no
seam at all.  Instruction caches are physically tagged from software's point
of view (ARM ARM D7.2), so a line cached from a page whose executable mapping
is later torn down stays hittable through *any* later executable mapping of
...
theorems mirroring `TlbInvalidation`; the effect algebra
`applyICacheInvalidation` (removal / selectivity / monotonicity / idempotence
/ `iallu`-empties / survivor lemmas); the mounted per-core state
`SystemState.perCoreICache : Vector ICacheState numCores` under the SM4.B
path-a discipline; and the three model operations — `icFetchOnCore` (the
hardware instruction fetch, an *environment* step), `icInvalidateOnCore`
(`IC IALLU`, whose `…_icacheOnCore_ne` **states the SMP hazard**, with
`icInvalidateOnCore_remote_line_survives` as the non-vacuity witness), and
`icInvalidateBroadcast` (`IC IALLUIS` / `IC IVAU`).  Headline:
`icInvalidateBroadcast_reaches_all_cores`, the instruction-side analogue of
Theorem 3.3.1, plus the platform instantiation over `icBroadcastReach`.
`reach` is a parameter for the SM7 §3.4 reason `targets` is: a multi-cluster
port leaves the Inner Shareable domain and needs an SGI protocol.

**SM7.D.2 — D-cache at the PoC is system-wide, modelled not assumed.**
`DCacheMaintenance` over the AG8-B operations, and `dcMaintenanceAllCores`,
which takes **no target set at all** — the absence of a reach parameter *is*
the formal content of "at PoC, already system-wide".
`dcMaintenanceByVA_reaches_all_cores`, `dcacheCoherentAcrossCores` + cold-boot
witness + preservation, and the asymmetry as a theorem
(`icInvalidateOnCore_vs_dcMaintenance_reach`).

**SM7.D.3 — DMA out of scope, as a tripwire.**  `modeledCoherentAgents` is
exactly the PEs; `dcMaintenance_covers_all_modeled_agents` proves the
maintenance reaches all of them and `modeledCoherentAgents_no_dma_master`
proves the enumeration contains no non-coherent bus master.  Adding a DMA
agent breaks the latter, so the buffer-ownership obligation cannot be
forgotten.  `Architecture/Assumptions.lean`'s AG8-B entry is rewritten from
"sequential model — cache coherency is trivially satisfied under single-core
operation" to the per-structure proved/assumed split.

**SM7.D.4 — the 14th `proofLayerInvariantBundle` conjunct.**
`icacheCoherent_perCore`: on every core, every cached line still has a live
**executable** mapping.  An `ICacheLine` records the executable translation
the fetch resolved through (`ICacheLine.toTranslation`), so the whole
page-table frame algebra proven for `tlbEntryConsistent` carries over
unchanged.  Unlike the 13th conjunct it needs no pending-allowance disjunct:
instruction-cache maintenance is a *synchronous* broadcast instruction, not a
queued request/acknowledge round.  Boot witness, op-level preservation
(fetch with the authorisation contract, local invalidate, broadcast),
decidable checker `icacheCoherentCheck_perCore` (+ `_iff` + `Decidable`), the
capstone `cacheCoherency_cross_subsystem`, and the joint
`icInvalidateBroadcast_preserves_perCore_memory_invariants`.  Carried through
freeze (`FrozenSystemState.perCoreICache`, **required**), congruence
(`OffSchedulerAgrees.perCoreICache`), boot
...
**Live wiring.**  `.vspaceUnmap` →
`vspaceUnmapPageWithShootdownAndIcacheBroadcast` (targeted `IC IVAU` at the
retired page's physical address, when and only when the mapping was
executable — read from the *pre*-state; a data-page unmap is provably inert).
`.lifecycleRetype` → `lifecycleRetype{Direct,}WithCleanupShootdownPerCoreIcache`
(unconditional `IC IALLUIS`: a retype scrubs and re-purposes the target's
backing memory and the abstract state cannot enumerate which physical lines it
covered, so the sound choice is the full invalidate — and the payoff is that
the hardest transition for cache reasoning becomes the easiest to discharge,
its post-state having every cache cold).  Both seams carry machine-checked
`…_preserves_icacheCoherent_perCore` **and**
`…_preserves_tlbInvalidationConsistent_perCore` plus the joint capstones.
Runtime seam: `SyscallDispatchEntry.completeIcacheMaintenance`, keyed on the
same shootdown diff the TLB round uses, ordered after it, inert otherwise.

**Rust HAL** 782 → 789 tests, clippy-clean: `cache::ic_ivau`,
`cache::ic_invalidate_all_inner_shareable`, typed `ICacheInvalidation` +
fail-closed `decode_icache_invalidation`, exports `cache_ic_ialluis` /
`cache_ic_maintenance`; Lean bindings `ffiIcIalluIs` / `ffiIcMaintenance` +
the typed wrapper `icMaintenanceBroadcast`.  `Architecture.CacheModel`
promoted staged → production (staged 56 → 55).  Tests:
`tests/SmpCacheMaintenanceSuite.lean` — 56 runtime assertions across 9 groups
plus 100+ surface anchors, including the non-vacuity witness that a cached
...
**SM7.D residual closure (v0.32.96) — `.vspaceUnifyInstruction`.  SM7.D is
CLOSED.**  The v0.32.95 cut left exactly one residual: maintenance was wired to
the paths that *destroy* an executable mapping, but nothing served the dual — a
subject that **writes** instructions.  On ARMv8-A those stores land in the
D-cache while the instruction fetch reads at the Point of Unification, so
without a kernel operation a JIT, loader, or dynamic linker had **no way** to
make its own writes fetchable.  seL4 exposes exactly this as
`seL4_ARM_Page_Unify_Instruction`; seLe4n now does too.

`Architecture.vspaceUnifyInstructionPage asid vaddr` is a **pure cache**
transition — `_frame` proves the object store, page tables, TLB and shootdown
state are all unchanged — and is fail-closed on both authority legs
(`.asidNotBound` when the ASID is unbound, `.translationFault` when the address
is unmapped in that address space), so a caller can only maintain memory it
already holds a translation for.  It is deliberately **not** gated on the
mapping being executable: the writer holds the *data* mapping, so an execute
gate would make the operation useless for its only purpose.  Authority is the
`.write` right, so a read-only VSpace capability is refused with
`.illegalAuthority`.

The operand is a *third* `ICacheInvalidation` constructor, `.unifyPage`, not a
reuse of `.ivauPage`: the data side must be cleaned to PoU **before** the
instruction side is invalidated, and folding the two would silently drop the
clean.  `join` gives `.unifyPage` dominance over `.ivauPage` on the same page —
upgrading is sound, downgrading is not — so an accumulated round can never
weaken a unify into a bare invalidate.  Theorems:
...
targets `ObjId × objectTypeAllocSize`, which `ScrubAndUntyped.lean` and
`SELE4N_SPEC.md` both identify as an abstract model convention.  The real child
extent is the untyped allocator's `regionBase + offset`, so on hardware the
`DC CVAU` would sweep memory the allocation does not occupy.

**Valid, and both narrower and wider than it reads.**
...
*Wider*: if the scrub does not reach real memory, the re-typed object is handed
...
two computations".  It was not: its right-hand side restated `retypeIcacheOp`'s
own definition and never mentioned `scrubObjectMemory`, so it held for any
extent whatsoever and pinned nothing — had the scrub's range changed, the
theorem would still have gone through while the operand silently diverged.  The
section comment claimed the two "diverge together, because both sides read the
same convention"; in fact they read two *identical copies* of it, which is not
the same property.

**The fix — one extent, two consumers.**  `scrubExtent objectId objType` now
...
construction and the AN9 bridge rewrites that single function.
`retypeIcacheOp_cleans_scrub_extent` is restated against `scrubExtent` — it now
relates two *different* functions and so fails if either moves independently —
and `scrubObjectMemory_cleaned_by_retype` closes the loop from the scrub's side
by naming the pair handed to `zeroMemoryRange`.

The new body is definitionally the old one (the bridge theorem is `rfl`), so
the other 80 `scrubObjectMemory` references are untouched; the single structural
unfold in `InformationFlow/Invariant/Operations.lean` gains `scrubExtent` in its
`simp only` set.

The v0.32.100 notes also implied more hardware fidelity than the change had.
The section comment now says plainly that the extent is the model's convention,
that hardware fidelity rides AN4-G.3, and why the operand follows the scrub
rather than leading it.

Coverage: `SmpCacheMaintenanceSuite` §3.13 gains the correspondence *exercised*
rather than asserted — dirty all of memory, run the real scrub, confirm the
zeroed bytes lie inside the cleaned range, and check the byte just past the
extent is untouched so the bound is exact (114 → 118).  Tier-3 anchors pin
`scrubExtent`, both bridge theorems, and — negatively — that neither
`retypeIcacheOp` nor `scrubObjectMemory` mentions `objectTypeAllocSize` in its
own body, which is what would reintroduce the second copy.  Lean-only; Rust HAL
untouched at 798.  Trace byte-identical; zero sorry/axiom.
...
`join (.unifyPage p) .iallu = .iallu` would have dropped that operand's clean
to the Point of Unification — under-maintenance, the unsafe direction.  Nor
does any single operand cover two distinct `unifyPage`s.  Not reachable at
v0.32.95 (one maintenance-bearing transition per syscall, drained atomically
with the commit, so every record started from an empty ledger and the binary
join arms were dead code) but a latent trap.  The join is replaced by a
**coverage preorder over a list**: `pendingIcacheMaintenance` becomes
`List ICacheInvalidation`, appended in record order and drained wholesale, and
reduces only where one entry provably **covers** another —
`ICacheInvalidation.covers`, grounded in the model's own effect by
`icacheLineMatches_of_covers` and `applyICacheInvalidation_subset_of_covers`,
with `iallu_not_covers_unifyPage` stating the exclusion as a theorem.
`recordIcacheMaintenanceList_covered` / `_mem_of_mem` are the no-loss
properties; `_length_le` bounds each record at one entry, so the live ledger is
a singleton and still needs no capacity invariant and no bundle conjunct.

Live through `API.dispatchWithCap`
(`dispatchWithCap_vspaceUnifyInstruction_delegates`), so the operand rides the
v0.32.95 emission ledger into the syscall-entry drain and out through
`cache_ic_maintenance`; the HAL realises it as
`cache::unify_instruction_page_inner_shareable` — a 64-line `DC CVAU` loop →
`DSB ISH` → a 64-line `IC IVAU` loop → `DSB ISH` → `ISB`, the same canonical
sequence `kernelCodeWriteSites_owe_pou_clean` names for the kernel's own code
writes.  ABI: `SyscallId.vspaceUnifyInstruction = 29`, count 29 → 30, threaded
through the Lean encodings, both Rust mirrors, ABI conformance, the frozen-ops
classifier, the argument decoder, the information-flow enforcement registry
(37 → 38, capability-only 22 → 23), and the lock-set inventory —
`lockSet_vspaceUnifyInstruction` takes the VSpaceRoot in **read** mode, since it
modifies no page table (inventory 99 → 101, categories 29 → 30).  Rust HAL
792 → 795; suite 72 → 93 assertions / 12 groups; the golden trace's
`[XVAL-002]` line moves 29 → 30 variants, everything else byte-identical.
...
mirror: home-core G7-precapture, per-core donation arms, home-core
removal, local `handleRescheduleSgiOnCore` inline / remote `.reschedule`
SGI seam) is live behind `API.dispatchCapabilityOnly`'s `.tcbSuspend` arm
(executing core via `determineExecutingCore`); the SM5.F.4 diff seam
gained the SM6.E **descheduled-current rule**
(`crossCoreSgiBody_remote_deschedule`: a victim actively current on its
remote home core in `pre` and neither current nor queued there in `post`
fires the home-core poke; single-core inertness preserved), and the AN9-D
atomicity bracket flipped its Rust extern to the new commit-then-fire FFI
seam `suspend_thread_cross_core`
(`SyscallDispatchEntry.suspendThreadCrossCoreEntry`) — golden trace
byte-identical, Rust HAL 724 green.  **(2) cancellation
non-interference** landed as the staged
[`SeLe4n/Kernel/IPC/CrossCore/CancellationNI.lean`](../SeLe4n/Kernel/IPC/CrossCore/CancellationNI.lean)
(staged-only 56 → 57): every SM6.E-new state effect discharged
substantively (`descheduleThread_cancellation_NI{,_smp}`, the fully
substantive ready-arm composite
`cancelIpcBlockingOnCore_ready_cancellation_NI{,_smp}`, ∀-core
replenish-queue/migration projection frames, the donated-arm migration
leg); the blocked-arm composites consume exactly the single-core teardown
projection obligation the production closure form (AK6-F.18 G3) has
always documented.  **(3) whole-bundle groundwork**: the `ipcInvariant`
conjunct is CLOSED across the entire cancellation surface — the RHTable
fold keystone `fold_preserves_of_lookup` (the fold step learns the
visited key's stored value), the state-correcting
`notificationQueueWellFormed_filter_correct`, twelve single-core and five
OnCore `_preserves_ipcInvariant` lemmas, and the reusable transports
(`ipcInvariant_of_objects_eq`, `ipcInvariant_insert_*`,
`notification_lookup_of_insert_no_notification`); the remaining
`ipcInvariantFull` conjuncts stay tracked with the per-key engine now in
place.  **(4) the resolution frames** are discharged closed-form: the
per-key TCB frames (`spliceOutMidQueueNode_tcb_lookup`, the sweeps'
`_tcb_lookup`/`_no_tcb` families, the teardown helpers' `_tcb_lookup`)
compose into `cancelIpcBlocking_determineTargetCore_eq` /
`cancelIpcBlocking_getTcb?_isSome_eq` and the invExt-only
`cancelIpcBlockingOnCore_eq_descheduleThread_closed`.  The cut also
landed observational atomicity (the SM3.C.7 **guarded** observer layer
`AcquireInsensitiveOn`/`ReleaseInsensitiveOn`/`lockSet_observer_atomic_on`
in `LockSet2PL` §4c, instantiated at the victim-`ipcState` observer:
`updateObjectLockAt_getTcb?_ipcState` + the capstone
`cancelIpcBlockingOnCore_observer_atomic`), the boot-instance bridges
(`cancelIpcBlockingOnCore_bootHome_state_eq`,
`cancelDonationOnCore_bootHome_{ok,error}`), the SM5.H corollaries
(`descheduleThread_fully_descheduled`,
`cancelBoundDonationOnCore_replenishments_purged`), the donated-arm §2b
**replenishment migration** (`cancelDonatedDonationOnCore` now migrates
the returned SC's replenishments to the original owner's home core via
the production `migrateSchedContextReplenishment`; self-migration — the
whole single-core config — is a definitional no-op,
`cancelDonatedDonationOnCore_eq_of_sharedHome`), the SM6.E
`SchedLockId`-domain footprints (`descheduleThreadLockSet` +
cancellation/suspend companions with the standard lemma family), and
fixes for **two pre-existing single-core defects** the proof work
surfaced: `removeFromAllNotificationWaitLists` left a sole-waiter
cancellation in an `ipcInvariant`-violating `.waiting`-with-`[]` state
(now state-correcting), and `suspendThread`'s G7 reschedule guard read
the current slot after G4 had already cleared it (dead code — the
suspend-the-running-thread reschedule never fired; now a G7-precapture,
mirrored by `suspendThreadOnCore`).
[`tests/SmpCancellationSuite.lean`](../tests/SmpCancellationSuite.lean)
grew to 76 assertions in 13 scenario groups (new: sole-waiter state
correction + two-waiter retention, 3-deep mid-queue splice, mirror SGI,
the live per-core suspend incl. diff-seam recovery and inline successor
dispatch, send-/receive-blocked arms, migration assertions).

**Prior sub-phase record (SM6.E landing, v0.32.60).**
The two cancellation sub-operations of the suspend pipeline are lifted to
SMP under the SM3.B per-object lock-set discipline, in the new
**production** module
[`SeLe4n/Kernel/IPC/CrossCore/Cancellation.lean`](../SeLe4n/Kernel/IPC/CrossCore/Cancellation.lean)
(imported by `SeLe4n.lean`; the staged partition is unchanged at 56).
`descheduleThread` is the SM5.C `wakeThread` **dual**: it removes a thread
from its *home* core's run queue / current slot (`determineTargetCore` →
`removeRunnableOnCore`) and surfaces a `.reschedule` SGI exactly when the
victim was **actively current on a remote home core** — a merely-queued
remote victim needs no poke, a victim current on the executing core is
rescheduled synchronously by the caller, and an unresolvable `tid` pokes
nothing (the wake ghost-guard, mirrored).  `cancelIpcBlockingOnCore` is the
suspend pipeline's G2+G4+G7 slice generalised across cores: the pure
single-core `cancelIpcBlocking` object teardown (endpoint/notification
dequeue, reply-link consume, TCB IPC-field clear), then the home-core
deschedule + remote poke, with the home core pre-resolved from the
pre-state (justified by `cancelIpcBlocking_scheduler_eq`; the coincidence
with post-state resolution is explicit in
`cancelIpcBlockingOnCore_eq_descheduleThread`).  The donation side fixes a
genuine single-core hardcode: `cancelBoundDonationOnCore` purges the
SchedContext's pending replenishments from the **victim's home core's**
replenish queue (SM5.H `replenishQueueAffinityConsistentOnCore` places
them there) instead of `cancelBoundDonation`'s pinned `bootCoreId`, with
the single-core form recovered definitionally
(`cancelBoundDonationOnCore_bootCoreId`); the dispatcher
`cancelDonationOnCore` resolves the purge core via `determineTargetCore`
and returns the pre-state on error (the SM6.A/B/C bracket convention).
**Lock-set migration (SM6.E.1/.3)**: the plan §3.1 cancellation rows land
as `lockSet_cancelIpcBlocking` (victim TCB W; blocked-on
endpoint/notification W, pre-resolved from `ipcState`; **consumed Reply
object W**) and `lockSet_cancelDonation` (victim TCB W; SC W; donated-arm
original-owner TCB W), with state-resolved `…OnCore` forms, consistency
against `permittedKinds .tcbSuspend`, `Nodup` correctness, and
member-by-member coverage by the enclosing suspend footprint
(`lockSet_tcbSuspend_*_write_mem` ×6).  This closed a real **footprint
gap**: the SM6.D reply-object fold made `cancelIpcBlocking` sever
`reply.caller` on the `.blockedOnReply` arm, a write the suspend footprint
never declared — `permittedKinds .tcbSuspend` now includes `.reply` and
`lockSet_tcbSuspend` carries an optional `consumedReplyId` (a 5-extension
footprint, size 8 = `maxLockSetSize` exactly; `size_le_5` +
`lockSet_consistent_base_plus_five_opts` added).  **Atomicity
(SM6.E.2/.4)**: `cancelIpcBlocking_atomic_under_lockSet` and
`cancelDonation_atomic_under_lockSet` (the exact plan-named theorems) plus
the `…OnCore_atomic_under_lockSet` companions, all via
`lockSet_atomic_under_2pl`; the mode-aware membership lemma
`mem_insertOrMerge_write_of_mem_write` (write survives any
`insertOrMerge`, no key-distinctness side-condition) powers the coverage
family.  **Flagship (SM6.E.5)**: `cancellation_cross_core_correct` — for a
victim actively current on a remote home core, the cancellation surfaces
the `.reschedule` SGI to that core ∧ fully deschedules the victim there
(∉ run queue ∧ ≠ current) ∧ leaves every other core's scheduler state
exactly the pre-state's ∧ has object-level effect exactly the single-core
teardown's.  Supporting invariant surface (production):
`cancelIpcBlocking_preserves_objects_invExt` and the donation arms'
`_preserves_objects_invExt` (with per-helper lemmas for
`spliceOutMidQueueNode` / `removeFromAllEndpointQueues` /
`removeFromAllNotificationWaitLists` / `consumeReplyLink` in
`CleanupPreservation.lean` / `SuspendPreservation.lean`;
`returnDonatedSchedContext_preserves_objects_invExt` de-privatised), plus
the per-core donation frames (`…_replenishQueue_purged` /
`_replenishQueue_ne` / `_runQueue_current_eq` and the ∀-core
`cancelDonatedDonation_scheduler_eq`).  All axiom-clean.  Tests:
[`tests/SmpCancellationSuite.lean`](../tests/SmpCancellationSuite.lean)
(54 runtime assertions in 8 scenario groups — endpoint-/notification-/
reply-blocked victims homed on a remote core, the running-remote SGI vs
running-local no-SGI duals, the bound-donation home-core purge, the
donated return-to-owner, dispatcher identity + ghost + `withLockSet`
bracket; Tier-2 `smp_cancellation_suite` + Tier-3 anchors).  The four
tracked-debt items this landing recorded were closed or narrowed by the
v0.32.61 completion cut above.

**Prior sub-phase: SM6.D IPC across-core invariant bundle LANDED
...
production) mirrors the twenty-conjunct `ipcInvariantFull` (the plan's
fifteen R4-era conjuncts plus the five SM6.D reply-object-hardening
conjuncts), one named field per conjunct: thread-subject conjuncts restrict
to the threads homed on core `c` (`threadHomeCore` = `cpuAffinity` defaulting
to `bootCoreId`, provably the operational wake target —
`determineTargetCore_eq_threadHomeCore`); shared-object clauses are carried
whole in every core's view; the scheduler-reading `passiveServerIdle` uses
the SM4.D per-core form.  The decomposition is **exact**
(`ipcInvariantFull_smp_iff_full_and_passive_smp`: the ∀-core aggregate ↔ the
global bundle + the per-core passive slices), with the four plan-named
conjuncts (SM6.D.3–D.6) additionally shipping per-conjunct exactness iffs
(`ipcStateQueueMembershipConsistent/endpointQueueNoDup/
queueNextBlockingConsistent/queueHeadBlockedConsistent_smp_iff`) and a boot
witness (`default_ipcInvariantFull_smp`).  Preservation (SM6.D.2,
[`PerCoreBundlePreservation.lean`](../SeLe4n/Kernel/IPC/Invariant/PerCoreBundlePreservation.lean),
production): all six IPC operations — send, receive, call, reply (+
replyRecv), signal, wait — preserve every core's bundle view
(`…_preserves_ipcInvariantFull_perCore` × 7), plus the cross-core call
flagship `endpointCallOnCore_preserves_ipcInvariantFull_perCore` (staged,
with SM6.A): nineteen conjuncts ride the existing single-core whole-bundle
theorems through the exact-decomposition bridges, and the per-core
`passiveServerIdle` slice rides a new core-parameterised pullback-frame
family (`passiveServerIdleFrameOnCore` + per-store micro-frames + per-op
compositions) built on the SM4.B independence algebra with **no idle-core
...
- **SM1.B.1**: `PerCpuData` struct migrated from `smp.rs` (where
  SM0.N parked the empty seam) to the new dedicated module
  `rust/sele4n-hal/src/per_cpu.rs`.  The SM0.N
  `_reserved: [u64; 8]` placeholder is replaced with a populated
  `core_id: u64` first field plus a `_reserved: [u64; 7]` tail
  (still one cache line wide for SM5+ scheduler-state fields).
  Two const constructors: `new(core_id)` (production initialiser)
  and `zero()` (back-compat alias for `new(0)`).
- **SM1.B.2**: Static array population — `PER_CPU_DATA[i].core_id
  ...
  `const _: ()` assertions pin
  `size_of::<PerCpuData>() == PER_CPU_DATA_SLOT_SIZE`,
  `align_of::<PerCpuData>() == 64`, and
  `PER_CPU_DATA.len() == MAX_SECONDARY_CORES + 1` (= 4 =
  `PlatformBinding.coreCount`).  Asm-visible
...
- **SM1.B.3**: `current_per_cpu() -> &'static PerCpuData` —
  reads `TPIDR_EL1` on aarch64 and returns a static-lifetime
  reference into `PER_CPU_DATA`.  Host stub returns
  `&PER_CPU_DATA[0]`.  Safety invariants documented inline (EL1
  reachability, TPIDR_EL1 pre-set by boot code, pointer
  validity).
- **SM1.B.4**: `current_core_id_from_tpidr() -> u64` — fast
  core-id lookup via `current_per_cpu().core_id`.  Preferred over
  the MPIDR + mask path on hot kernel paths.  Host stub returns 0.
- **SM1.B.5**: Lean FFI — Rust
  `#[no_mangle] pub extern "C" fn ffi_current_core_id` in
  `ffi.rs`; Lean
...
- **SM1.B.6**: `check_per_cpu_invariants()` runtime gate —
  ...
  `boot.rs::rust_boot_main` Phase 4 before the `TPIDR_EL1` write,
  so the invariant is verified before any consumer reads it.
...
`ffi::tests` modules; the hardware behaviour (the actual
`mrs tpidr_el1` read) will be covered by SM1.H's QEMU `-smp 4`
boot-trace test.
...
rewriting the previously-thin `rust_secondary_main` shell into
the full per-core boot sequence (MMU → VBAR → GIC → timer → IRQ
→ Lean kernel) shared symmetrically with the primary boot path:

- **SM1.C.1**: `mmu::init_mmu_secondary(core_id)` + extracted
  `mmu::init_mmu_per_core(core_id)` helper.  The primary's
  `init_mmu()` now routes through `init_mmu_per_core(0)` after
  `build_identity_tables()`; secondaries skip the table-build
  step (boot L1 table is a read-only global) and apply the
  per-core MMU enable sequence with the AK5-C SCTLR_EL1 bitmap.
  Audit follow-up cfg-gated the `pt_pa_raw < 2^44` debug_assert
  to aarch64 (host x86_64 PIE binaries routinely exceed 2^44).
- **SM1.C.2**: `boot::install_exception_vectors()` — VBAR_EL1
  install extracted from the formerly-private `set_vbar` and
  made `pub` so the secondary path reaches it via the same
  module name the primary uses.  Two new build.rs scanners pin
  the primary/secondary call-site symmetry.
- **SM1.C.3**: `gic::init_cpu_interface_secondary(core_id)` —
  wraps the existing `init_cpu_interface(GICC_BASE)` (banked
  per-core) with a per-core diagnostic kprintln.  The global
  distributor is initialised once by the primary's `init_gic()`.
- **SM1.C.4**: `timer::init_timer_secondary(tick_hz) ->
  Result<(), TimerError>` — per-core timer arming.  Deliberately
  does NOT reset the global `TICK_COUNT` (primary-owned monotonic
  counter) or rewrite `TIMER_INTERVAL` (primary already populated
  it; same value on every core).  Failure on a secondary halts
  just that core via WFE loop.
- **SM1.C.5**: `rust_secondary_main` body rewrite — eight-step
  pipeline (CORE_READY wait, MMU, VBAR, GIC, timer, IRQ unmask,
  Lean kernel entry, idle fallback).  A new build.rs scanner
  enumerates the six required call sites by name and fails the
  build if any is silently dropped.
- **SM1.C.6**: Lean `SeLe4n.Kernel.SecondaryEntry.secondaryKernelMain
  : UInt64 → BaseIO Unit` with `@[export lean_secondary_kernel_main]`
  — new module file.  Body is `pure ()` at SM1.C (deliberate
  placeholder; SM5 replaces with per-core scheduler entry).
  Marker theorem `secondaryKernelMain_returns_unit_marker`
  witnesses the placeholder by `rfl`.  Module reached via
  `Platform.Staged`; added to staged-module allowlist.
- **SM1.C.7..C.11**: documentation-only sub-tasks — per-core
  stack reservation in `link.ld`, MMU page-table reuse rationale,
  per-core SCTLR_EL1 + VBAR_EL1 (covered by SM1.C.1/SM1.C.2
  helpers), SError handler masked policy retained.
- **SM1.C.12**: 32 new host tests across `mmu::tests`,
  `boot::tests`, `gic::tests`, `timer::tests`, `smp::tests`
  (sm1c1_*, sm1c2_*, sm1c3_*, sm1c4_*, sm1c5_* prefixes
  respectively) covering callability, signature pinning,
  panic-path coverage, monotonic-counter preservation,
  aggregate idempotence, and `#[no_mangle]` discipline.  Plus
  12 new Lean assertions in `tests/SmpFoundationsSuite.lean`
  (surface anchors + marker-theorem discharges + runtime
  BaseIO invocation + boundary input tolerance).

**Test coverage**: 313 HAL tests (was 281 at SM1.B close), zero
`#[ignore]`'d, zero clippy warnings workspace-wide.  Tier
...
- Build-script regression scanners:
...
- **SM3.E.7/E.8**: `tests/SerializabilitySuite.lean` (27 runtime
...
**SM3.E audit-pass-1** (post-landing self-audit): closed three
honesty/completeness gaps — orientation completeness
(`conflictPrecedes_total_of_distinct_commit` +
`conflictPrecedes_strict_total_of_distinct_commit`), strict-2PL
grounding (`conflictsCommitOrdered` +
`serializability_under_2pl_of_conflicts_ordered`), non-vacuous
Cor 2.1.11 (`withLockSet_preserves_objStoreLock_wf`), and
`conflictOrder` connectivity (`conflictOrder_implies_conflictPrecedes`
+ `commitSorted_respects_conflictOrder`).  Inventory 68 → 78.

**SM3.E audit-pass-3** (deepest deep audit): closed the three
honestly-deferred gaps the initial landing documented but did not
implement (per the `implement-the-improvement` rule).  (1) **Atomicity
bridge** — `withLockSet_observation_eq_action` +
`applySequentialWithLockSet` + `applySequentialWithLockSet_observation`
connect the bare-action `applySequential` model back to SM3.C.7's
`withLockSet` atomicity (`lockSet_observer_atomic`), so the headline
theorem reasons about real 2PL-wrapped transitions rather than an
idealised raw-action fold.  (2) **Observational serializability** —
...
structural headline `serializability_under_2pl` reorders via `Eq`,
which write/write pairs do NOT satisfy since the Robin-Hood slot layout
is insertion-order-dependent), threading `invExt` through the entire
`commitSort` reorder; `objStoreWriteInstance` is the canonical covered
instance.  (3) **Second real Cor 2.1.11** —
...
with `invExt`), demonstrating the lever generalises beyond
`objStoreLock.wf` to the object-store structural invariant class
SM4..SM6 consume.  Inventory 78 → 106 (+5 preservation, +5
...
kernel operations and extending the static lock-set declarations;
per CLAUDE.md's `Implement-the-improvement` rule, no findings
are documented away — each is materialised as a code change on
the lockSet declaration; all closures land in the same v0.31.9
release cut):

* **P1 `lockSet_tcbSetPriority`** + `_tcbSetMCPriority`: gain
  `boundSchedContextId : Option SchedContextId` arg.
  `setPriorityOp` / `setMCPriorityOp` call `updatePrioritySource`
  which writes the bound SchedContext's `priority` field via
  `st.objects.insert scId.toObjId (.schedContext sc')` when the
  target's binding is `.bound`/`.donated`.
* **P1 `lockSet_tcbSetIPCBuffer`**: gains
  `targetVSpaceRootObjId : Option ObjId` arg.
  `setIPCBufferOp` → `validateIpcBufferAddress` reads the
  target's VSpaceRoot and traverses `root.lookup addr`.
* **P2 `lockSet_serviceRegister`**: gains a mandatory
  `endpointObjId : ObjId` arg.  `registerService` reads
  `st.objects[epId]?` to verify the endpoint kind tag.
* **`permittedKinds` extensions**: `.tcbSetPriority` /
  ...
  `.serviceRevoke` / `.serviceQuery` split out unchanged.
* **Consistency-proof updates**: `_serviceRegister` gains a 3rd
  literal-list rcases branch; `_tcbSetPriority` /
...
* **Source-level tracing**: each finding was verified against
  ...
  `lookupServiceByCap` confirmed NOT to read kernel objects.
* **Test-coverage expansion**: 106 → 133 runtime assertions
  (+27 across §4, §7, §11, §17 — new §17
  `runAuditPass6FootprintChecks` validates SC inclusion in
  bound `setPriority`/`setMCPriority`, VSpaceRoot inclusion in
  `setIPCBuffer` and absence in the none case, endpoint
  inclusion in `serviceRegister`, plus canonical-sort
  hierarchy-level cross-checks).
...
- **SM4.A.1 + SM4.A.2**: `SeLe4n.PerCoreVector` bootstrap in
  ...
  SeLe4n.PerCoreVector`): `get_set_eq`, `get_set_ne`, `toList_length`,
...
SM4.D (cross-subsystem migrations) — **LANDED at v0.31.30, see below**;
SM4.E (witness retirement); post-SM4.C: per-extended-conjunct per-op
preservation (4 SC-using × 5 ops = 20 additional theorems).

**WS-SM SM4.D LANDED at v0.31.30** (cross-subsystem per-core invariant
migration; plan §5.4).  Mirrors the SM4.C pattern exactly: every
...
SMP aggregate.  Each per-core form at `bootCoreId` is *equivalent* to the
existing single-core predicate (`SchedulerState.runnable` is the `abbrev`
`(runQueueOnCore bootCoreId).toList`, so the run-queue reads coincide
definitionally; the TCB-lookup reads bridge through `getTcb?_eq_some_iff`
since the per-core bodies use the typed `getTcb?` accessor for the SM4.C
AK7 discipline).  Bridges close by `getTcb?_eq_some_iff` (TCB predicates),
`Iff.rfl` / `rfl` (endpoint/notification, IF projections), or `and_congr`
(aggregates).  The live invariant surface is untouched and stays green.
Five new staged modules:

- **SM4.D.1/.2** `SeLe4n/Kernel/IPC/Invariant/PerCore.lean` (~550 LoC):
  the 12 IPC↔scheduler coherence predicates (`runnableThreadIpcReady`,
  the 5 `blockedOn…NotRunnable`, `currentThreadIpcReady`,
  `currentNotEndpointQueueHead`, `currentNotOnNotificationWaitList`,
  `passiveServerIdle`, + the `ipcSchedulerContractPredicates` /
  `currentThreadDequeueCoherent` aggregates) → `_perCore` forms (TCB
  lookups via the typed `getTcb?` accessor — `RAW_LOOKUP_TID` unchanged,
  `GETTCB_ADOPTION` 186→233) + 12 boot-core bridges + 10 frame lemmas +
  12 defaults + 3 `∀ c` SMP aggregates (`_smp` + `_smp_at` +
  `_smp_to_singleCore` + defaults) + 9 per-conjunct projections.  The
  `∀ c` form is the right SMP semantics (e.g. a send-blocked thread is
  not runnable on *any* core's queue).
- **SM4.D.3/.4** `SeLe4n/Kernel/Capability/Invariant/PerCore.lean`:
  `cleanupNoStaleSchedRef_perCore` + `cleanupHookDischarged_perCore` +
  bridges + frame + default + the SMP form `cleanupNoStaleSchedRef_smp`
  ("a retyped object's TCB must not be runnable on **any** core" — per
  the implement-the-improvement rule, the SMP-correct retype precondition,
  strictly stronger than the boot-core single-core check).  Default-state
  + frame are provided for the scheduler conjunct only (the `ScrubToken`
  conjunct is an opaque cleanup-ran witness, not inhabited on `default`).
- **SM4.D.9** `SeLe4n/Kernel/Architecture/InvariantPerCore.lean`:
  `registerDecodeConsistent_perCore` + bridge + frame + default + `∀ c`
  SMP aggregate (a *sibling* module, not an in-file extension, so it
  stays staged-only).
- **SM4.D.12/.13/.14** `SeLe4n/Kernel/InformationFlow/ProjectionPerCore.lean`:
  the 6 scheduler-reading IF-M1 projections (`projectRunnable`,
  `projectCurrent`, `projectActiveDomain`, `projectDomainTimeRemaining`,
  `projectDomainScheduleIndex`, `projectMachineRegs`) + the aggregate
  `projectStateOnCore` → `…OnCore` forms + 7 boot-core bridges (`rfl`) +
  6 per-core observability frame lemmas + `projectStateOnCore_congr`.
  These are the per-core observability locality the SMP non-interference
  proofs require (a write to core `c'`'s slot leaves core `c`'s
...
- **SM4.D.19** `SeLe4n/Kernel/CrossSubsystemPerCore.lean` (capstone):
  ...
  12-conjunct invariant; only the ninth conjunct
  `schedContextRunQueueConsistent` reads scheduler state, migrated by
  SM4.C) and `crossSubsystemSchedulerContract_perCore` (the bundle of
...
N/A sub-tasks (documented, not weakened per the implement-the-improvement
rule): operation files (IPC/Lifecycle/Service ops, Platform/FFI, API)
read `currentOnCore bootCoreId` as the *running* core — core-
parameterisation is SM5 (per-core scheduler) work, not invariant
migration (SM4.B already routed their reads through the accessor); frozen
state (SM4.D.16) stays scalar per the SM4.B phase-1 decision; Service /
ExceptionModel / InterruptDispatch / VSpace / SyscallEntry / IF-Invariant
files define no scheduler-reading predicate (confirmed by an exhaustive
codebase scan).  Test: `tests/CrossSubsystemPerCoreSuite.lean`
(`lake exe cross_subsystem_per_core_suite`) — 89 Tier-3 surface anchors +
17 elaboration examples + 18 runtime assertions (incl. value-level
decidable IF projection checks on the default state); Tier-2 + Tier-3
wired.  Axiom budget: 0 axioms, 0 sorries (only `propext` / `Quot.sound`
/ `Classical.choice`); +108 theorems; partition gate 40 staged-only
modules (was 35); default build green (320 jobs); trace fixture
...
**WS-SM SM5.A LANDED at v0.31.38** (per-core `chooseThread`; opens SM5, the
per-core scheduler phase).  All eight sub-tasks per
[`docs/planning/SMP_PER_CORE_SCHEDULER_PLAN.md`](planning/SMP_PER_CORE_SCHEDULER_PLAN.md)
§3.1/§5 landed in one cut.  Axiom-clean (`propext` / `Quot.sound`); the legacy
single-core `chooseThread` migrates to delegate to the per-core form with **zero
behavioural change** (default build green, 320 jobs; trace fixture
byte-identical).

- **SM5.A.1 + SM5.A.5** (production, `Scheduler/Operations/Selection.lean`):
  `chooseThreadOnCore (st) (c : CoreId) : Except KernelError (Option ThreadId)`
  — per-core selection reading only core `c`'s run-queue + active-domain slots.
  Plan §3.1's bare `Option ThreadId` is adapted to the `Except`-returning form
  so `chooseBestInBucket`'s `schedulerInvariantViolation` (corrupt run queue) is
  preserved, not collapsed to `none`.  Legacy `chooseThread` now *defined* as
  `chooseThreadOnCore` at `bootCoreId` lifted into the `Kernel` monad
  (`chooseThread_eq_chooseThreadOnCore_bootCore`, `rfl`); 5 production proof
  sites that `unfold chooseThread` updated.
- **SM5.A.2** (staged, `Scheduler/Operations/PerCoreChooseThread.lean`):
  `RunQueueLockId` (`CoreId`-keyed per-core run-queue lock, distinct from the
  SM0.I object-lock hierarchy — run queues are `SchedulerState` fields, not
  kernel objects) + read-only `chooseThreadOnCoreLockSet c = [(⟨c⟩, .read)]`
  reusing SM3.B's `AccessMode`, with 4 structural witnesses.
- **SM5.A.3**: per-core independence (Theorem 3.1.2) — `chooseThreadOnCore_frame`
  + 4 corollaries (sibling-core run-queue / active-domain writes; **any** core's
  `current` write; the lock-set bridge).
- **SM5.A.4**: idle-fallback completeness — 3 `chooseBestRunnableBy` induction
  lemmas lift to `_ok_of_runnableTCBs` (never errors on a valid state),
  `_none_no_eligible` (idle fallback only when no in-domain runnable thread),
  `_some_of_eligible` (an in-domain runnable thread ⇒ `some`; SM5.E discharges
  with the idle thread for `chooseThreadOnCore_always_succeeds`) + the
  `schedulerInvariant_perCore` corollary.
- **SM5.A.6**: selection soundness — `_some_mem_runQueueOnCore` (chosen ∈ run
  queue) + the `Kernel`-monad preservation form + the invariant corollary.
...
**WS-SM SM5.B LANDED at v0.31.39** (per-core `switchToThread` — the per-core
context-switch transition).  All nine sub-tasks per
[`docs/planning/SMP_PER_CORE_SCHEDULER_PLAN.md`](planning/SMP_PER_CORE_SCHEDULER_PLAN.md)
§3.2/§5 landed in one cut.  Every new theorem axiom-clean (`propext` /
`Quot.sound` / `Classical.choice` only); default build green (320 jobs); trace
fixture byte-identical; Tier 0–3 green; full Rust HAL suite green (718 tests),
zero clippy warnings.

- **Foundation (SM5.B.4)**: `TCB.cpuAffinity : Option CoreId := none`
  (`none` = unbound, runs on any core; `some c` = pinned to core `c`) added to
  `Model/Object/Types.lean`, pulled forward from the plan's nominal SM5.C.7
  slot because the reject-remote semantics cannot be a non-vacuous theorem
  without a per-thread core binding (a stubbed reject-remote would be a
  security/correctness shortcut, forbidden by the implement-the-improvement
  rule).  Default `none` keeps the single-core boot trace byte-identical; the
  manual `BEq TCB` grows 23 → 24 conjuncts and `TCB.ext` gains the
  `cpuAffinity` hypothesis; TCBs pass `FrozenState.freezeObject` unchanged (no
  freeze cascade).  `KernelError.threadOnDifferentCore` (discriminant 53) added
  across the Lean enum + `Platform.FFI.KernelError.toUInt32` + the Rust
  `sele4n-types` enum (`from_u32` / `Display`) + the `sele4n-abi` decode layer,
  with every discriminant-pinning test (Lean + Rust `error.rs` / `conformance.rs`
  / `decode.rs`) growing in lock-step (54 variants, kernel range 0..=53,
  unknown-code fallback from 54).
- **SM5.B.1/.3/.4** (production `Scheduler/Operations/Selection.lean`):
  ...
  per-core analogue of seL4's `switchToThread`: preempt core `c`'s current
  thread (`preemptCurrentOnCore`: save its register context + re-enqueue it at
  its effective priority), dequeue `tid` (dequeue-on-dispatch), restore `tid`'s
  context, set `tid` as core `c`'s current thread.  Fail-closed: a `tid` that
  does not resolve to a TCB → `.schedulerInvariantViolation`; a `tid` whose
  affinity binds it to a core other than `c` → `.threadOnDifferentCore` (the
  `affinityAdmitsCore` gate).  Single-state form (mirrors the SM5.A
  `chooseThreadOnCore` split).
- **SM5.B.2/.5/.6/.8** (staged `Scheduler/Operations/PerCoreSwitchToThread.lean`):
  `switchToThreadOnCoreLockSet` — the *complete* cross-domain footprint over
  SM5.A's `SchedLockId`: the object-store **write** lock (`schedObjStoreLockId`,
  guarding the `getTcb?` resolutions *and* the preempted thread's
  register-context save) + the per-core run-queue **write** lock, in plan §4.4
  ascending order (`switchToThreadOnCoreLockSet_object_before_runQueue`); the
  object-store *table* lock subsumes the dynamically-discovered preempted-thread
  TCB write that a static `LockId.tcb tid`-only set would under-lock (the same
  under-locking class SM5.A's audit closed).  Switch-semantics theorems
  `switchToThreadOnCore_sets_current` (Thm 3.2.1),
  `_preempts_previous` (Thm 3.2.2), `_rejects_remote` / `_ok_of_admits`
  (Thm 3.2.3), `_runQueueOnCore_excludes_current` (SM5.B.5),
  `_independent_of_other_core` (SM5.B.6, via the `preemptCurrentOnCore` frame
  lemmas `_currentOnCore` / `_runQueueOnCore_ne` / `_runQueueOnCore_self_active`);
  totality + the decidable `switchToThreadOnCoreSucceeds` /
  `switchToThreadOnCoreRejectsRemote` (SM5.B.8).
- **SM5.B.7** FFI seam: Rust `ffi_switch_to_thread(tid, core_id) -> u64` records
  the per-core current-thread id into a `PER_CPU_CURRENT_THREAD` slot (Relaxed;
  fail-closed out-of-range with the same u64-before-cast bound check as the
  per-core stats FFI); `ffi_per_core_current_thread(core_id) -> u64` reads it
  back.  Lean `@[extern]` bindings (`Platform.FFI.ffiSwitchToThread` /
  `ffiPerCoreCurrentThread`) + typed wrappers (`Concurrency.switchToThreadHw` /
  `perCoreCurrentThreadHw`) with pass-through marker theorems.  New `build.rs`
  scanner `scan_ffi_rs_exposes_switch_to_thread_exports` pins both exports.  No
  Lean caller at SM5.B; SM5.C's per-core dispatch loop is the consumer.
- **Tests (SM5.B.9)**: `tests/SmpSwitchToThreadSuite.lean`
  (`lake exe smp_switch_to_thread_suite`) — 35 surface anchors, 9
  elaboration-time theorem-application examples, 25 runtime assertions over the
  real `switchToThreadOnCore` computation (the eight switch scenarios +
  the SM5.B.2 lock-set witnesses + the SM5.B.4 affinity algebra); 6 new Rust
  `ffi::tests::sm5b7_*` (HAL 712 → 718).  `PerCoreSwitchToThread` registered in
  `Platform/Staged.lean` + `staged_module_allowlist.txt` (44 staged-only
  modules; production/staged partition gate green).  Tier-2 + Tier-3 wired.
...
**WS-SM SM5.E completion LANDED at v0.31.46** (production idle-aware dispatcher +
full preservation surface; closes every in-scope SM5.E gap flagged after the
v0.31.45 first cut and — per maintainer directive — pulls the SM5.I per-core
idle-aware dispatcher forward so idle threads are **live in the running kernel**,
demonstrated in the trace).

- **Refactor**: idle identities (`idleThreadIdBase` / `idleThreadId` + injectivity)
  moved `Platform.Boot` → `Scheduler/IdleThread.lean` so the in-`Core.lean`
  dispatcher can reference them (`createIdleThread` + installers stay in `Boot`).
- **Production dispatcher** (`Scheduler/Operations/Core.lean`, the SM5.I seed):
  `idleDispatchableOnCore` + `dispatchIdleOnCore` + `scheduleOrIdleOnCore` run core
  `c`'s idle thread when `scheduleEffectiveOnCore` finds nothing runnable, instead of
  `current = none` (conditional-fallback idle-if-installed-else-`none`, sound +
  backward-compatible; layered on `scheduleEffectiveOnCore` so the SM5.D proof base
  is untouched).
- **Staged `PerCoreDispatch.lean`**: the headline `scheduleOrIdleOnCore_runs_idle`
  + the `dispatchIdleOnCore` frame lemmas + the soundness surface (objects-invExt /
  currentThreadValid / queueCurrentConsistent / runQueueWellFormed).
- **Trace demo**: a `MainTraceHarness` `[IDLE-…]` scenario shows legacy `schedule`
  → `current = none` vs. idle-aware → `current = some 65536`; the fixture grows by 6
  additive lines (every prior line byte-identical).
- **`PerCoreIdle` completion**: the `enqueueIdleThreadOnCore` per-core
  invariant-preservation surface (incl. the soundness-critical conditional
  `queueCurrentConsistent` under the documented `current ≠ idle` precondition), the
  `enqueueIdleThreadOnCoreLockSet` cross-domain footprint (SM5.A–D parity),
  `idleThread_no_starvation`, the decidable `idleAvailableOnCoreB` companion, and the
  `idleThread_core_locality_forall` `∀c` aggregate.
- **Verification**: inventory 26 → 58 entries; `SmpIdleSuite` + tier-3 expanded;
  partition gate 51 staged-only modules; axiom-clean; Tier 0+1+2+3 green; trace
  additive-only.  Tracked debt (SM5.I / SM5.H / post-1.0): folding the idle dispatch
  into `scheduleEffectiveOnCore` in place; the unconditional invariant-backed
  `chooseThreadOnCore_always_succeeds`; per-(core,domain) idle; wiring
...
**WS-SM SM5.F LANDED at v0.31.50** (per-core priority inheritance protocol; plan
§3.6 / §5).  All 10 sub-tasks landed in one cut.  Under SMP a thread `t` on core
`c` blocked on a resource held by a thread `t'` on core `c'` boosts `t'` to `t`'s
priority; if `c' ≠ c`, the boost emits a `.reschedule` SGI to `c'`.
...
holder's bucket migrates on (its home core, not always `bootCoreId`) and (2)
whether a remote core must be poked with a `.reschedule` SGI.  Taking only a
per-core slice would *under-boost* and re-introduce priority inversion.

- **SM5.F.1** `computeMaxWaiterPriorityOnCore` (`Scheduler/PriorityInheritance/Compute.lean`,
  production) — the per-core analytic *slice* of the global max-waiter priority,
  plus `optPriorityVal`, `computeMaxWaiterPriorityOnCore_le_global` (the per-core ≤
  global decomposition), and `_frame`.
- **SM5.F.2** `updatePipBoostOnCore` + `pipBoostWithWake`
  (`Scheduler/PriorityInheritance/Propagate.lean`, production).  `updatePipBoostOnCore`
  is `updatePipBoost` with the bucket migration generalised from `bootCoreId` to an
  explicit home core `c`; recovered at `c = bootCoreId`
  (`updatePipBoost_eq_updatePipBoostOnCore_bootCore`, an `rfl`), preserving the
  single-core PIP proof base verbatim.  `pipBoostWithWake` boosts the holder on its
  home core and emits a `.reschedule` SGI iff remote + material; full frame /
  blocking-graph / `invExt` preservation.
- **SM5.F.3** `pipBoost_perCore_consistent` (Thm 3.6.1).
- **SM5.F.4** `propagatePipChainCrossCore` (`Propagate.lean`, production) — donation
  chain across cores (each holder boosted on its own home core, remote SGIs
  collected); `invExt` + blocking-acyclicity preservation.
- **SM5.F.5 / SM5.F.6** `restoreToReadyOnCore` / `restoreToReadyWithWake`
  (`Lifecycle/Suspend.lean`, production) — per-core resume re-ready (restore IPC
  clear → recompute `pipBoost` from the GLOBAL graph → per-core enqueue) +
  cross-core resume wake.
- **SM5.F.7** `blockingServerOnCore` + `blockingGraphOnCore_consistent`.
- **SM5.F.8** `blockingAcyclic_perCore` (the per-core slice is a sub-walk of the
  acyclic global chain).
- **SM5.F.9** `priorityInheritance_perCore_witness` (aggregate soundness).
- **SM5.F.10** `tests/SmpPipSuite.lean` (`smp_pip_suite`, 60+ anchors / 9 examples /
  24 runtime assertions on a concrete cross-core blocking fixture).
...
categories).  Two private `blockingServer_*` helpers in `Propagate.lean`
de-privatised for reuse.  **AK7 baseline re-anchored** (`raw_match_total` 122 →
126, `raw_lookup_tid` 810 → 814): the per-core PIP transitions necessarily mirror
the raw-based D4-era single-core PIP (the `rfl` bridges + decomposition require
identical raw object reads), so the raw floor legitimately rises by the structural
...
**WS-SM SM5.G LANDED at v0.31.51** (per-core domain scheduling; plan §3.7 / §5).
All 6 sub-tasks landed in one cut.  Each core independently rotates its **own**
domain schedule, so different cores can be in different scheduling domains
simultaneously — maximising parallelism while still bounding per-domain CPU share
(plan §4.2).  Built on the SM4.B per-core `SchedulerState` domain fields
(`activeDomainOnCore` / `domainScheduleIndexOnCore` / `domainTimeRemainingOnCore`)
and the SM5.A `chooseThreadOnCore` selector.

- **SM5.G.1** (the active-domain query): the per-core active-domain query is the
  existing `SchedulerState.activeDomainOnCore` accessor — the form plan §3.7's
  `s.scheduler.activeDomainOnCore c` literally uses.  No redundant `SystemState`-level
  wrapper is added (the no-inferior-layer discipline); the accessor is recapped via
  the rotation-query lemmas `advanceDomainOnCore_rotates` / `_activeDomainOnCore_ne`.
- **SM5.G.2** (`advanceDomainOnCore` + cyclic theorem): `advanceDomainOnCore` is the
  *pure* per-core domain rotation, advancing **only** core `c`'s domain triple
  (`activeDomain` / `domainTimeRemaining` / `domainScheduleIndex`), never the run
  queue, current thread, or object store.  The §3.7 pseudocode's unconditional
  modular-arithmetic / `.getD ⟨0⟩` form is realised as a fail-closed single
  `Option`-lookup match (`domainSchedule[(idx+1) % length]?`): on an empty schedule
  `(idx+1) % 0 = idx+1` is out of bounds of the empty list, so the lookup is `none`
  and the rotation no-ops — subsuming the empty-schedule case without a separate
  `| [] =>` arm and never fabricating a phantom domain (mirroring
  `switchDomainOnCore`'s single-domain no-op).  Plus the full frame surface, the
  single-step index/domain/time formulas, the boundedness witness
  `advanceDomainOnCore_index_lt`, the `advanceDomainOnCoreN` `k`-fold iteration (the
  core-only toolchain has no `Function.iterate`), and the **cyclic theorem**
  `advanceDomainOnCore_cyclic` (iterating `domainSchedule.length` times returns the
  schedule index to its start — the defining round-robin property; requires
  `idx < length`, which the scheduler invariant maintains).  The bridge
  ...
  abstract rotation into the live domain-switch path (the two transitions differ only
  in their run-queue / current / register-context effects, never in the domain
  triple).
- **SM5.G.3** (`activeDomainOnCore_isInDomainSchedule`):
  `advanceDomainOnCore_establishes_activeDomainOnCore_isInDomainSchedule`
  *unconditionally* establishes the SM4.C predicate on the rotated core (the rotation
  always lands on a domain genuinely in the schedule); `_preserves_…_ne` /
  `_preserves_isInDomainSchedule_smp` frame the untouched cores and lift to the
  system-wide form; the plan §3.7 **Theorem 3.7.1** literal membership form
  `activeDomainOnCore_isInDomainSchedule_mem` (+ `_mem_of_smp` discharged from
  `schedulerInvariant_smp_crossSubsystem`).  The §3.7 statement is necessarily under a
  `domainSchedule ≠ []` hypothesis — under an empty schedule the SM4.C predicate's
  single-domain disjunct holds, which the membership-in-`map` form cannot express.
- **SM5.G.4** (`chooseThreadOnCore_respects_activeDomain`): a thread selected by
  ...
  domain barrier (temporal isolation between domains) is honoured by selection, never
  dispatching a thread of a different domain.  Proven via the new fold-eligibility
  lemmas `chooseBestRunnableBy_result_eligible` / `chooseBestInBucket_result_eligible`
  (mirroring SM5.A's `_result_mem` induction); plus the budget-aware companion
  `chooseThreadEffectiveOnCore_respects_activeDomain` (the effective selector's domain
  fact was already inside `chooseThreadEffectiveOnCore_selected_has_budget`, surfaced
  here under the SM5.G plan-name for symmetry).
- **SM5.G.5** (cross-core domain independence): `advanceDomainOnCore st c` frames
  ...
  `c' ≠ c` (`advanceDomainOnCore_independent_of_other_core` / `_perCore_independence`,
  via the SM5.A `chooseThreadOnCore_frame`); the `advanceDomainOnCoreLockSet`
  footprint (the single core-`c` run-queue WRITE lock over SM5.A's `SchedLockId` — the
  rotation reads no object store, so no object lock) structurally pins the rotation to
  core `c`, with the cross-core disjointness witness
  `advanceDomainOnCoreLockSet_disjoint_of_ne` (the lock-discipline counterpart of the
  semantic independence frame).

Modules: staged `SeLe4n/Kernel/Scheduler/Operations/PerCoreDomain.lean` (38 theorems
+ 3 defs, ~608 LoC) + `PerCoreDomainInventory.lean` (39-entry typed inventory in 6
categories — rotation / cyclic / bridge / domainSchedule / respects / independence —
with the `pcdt!` compile-time identifier-validation macro + per-category counts +
partition-sum + kernel-sound `Nodup` witnesses).  Both staged via `Platform.Staged`
(partition gate now 55 staged-only modules, was 53 at SM5.F); SM5.I's per-core run
loop is the first runtime exerciser.  Tests: new `tests/SmpDomainSuite.lean`
(`lake exe smp_domain_suite`) — 43 surface anchors, 7 elaboration-time
theorem-application examples, 27 runtime assertions across the SM5.G.6 scenarios
(single-domain no-op, multi-domain rotation, cyclic return-to-start, post-rotation
in-domain membership, selection respecting the active-domain barrier, cross-core
independence) + the SM5.G.5 lock-set witnesses + the inventory partition counts.
Tier-2 (negative) + Tier-3 (invariant surface) wired.  All new theorems axiom-clean
(only the foundational `propext` / `Quot.sound` / `Classical.choice`, verified via
`#print axioms`); default build green (324 jobs); trace fixture byte-identical (SM5.G
is purely additive — the new module is staged; production is untouched).  Items
deferred past v1.0.0 with correctness impact: NONE.  Follow-on: SM5.H (per-core CBS),
...
**WS-SM SM5.G deep-audit pass LANDED at v0.31.53** (per-core domain scheduling; the
live-transition invariant-preservation closure).  A code-first audit (reading the
implementation, not its docstrings — checking soundness, security, and mathematical
correctness) confirmed the v0.31.52 cut sound, secure, and proved with no dependency
beyond the foundational `propext` / `Quot.sound` / `Classical.choice`, with no shortcut
that made the codebase less secure.  It closed one substantive completeness asymmetry +
one stale def docstring.  All additions carry the same foundational-only dependency
(verified via `#print axioms`); default build green (324 jobs); trace byte-identical
(purely additive / staged); the AK7 cascade floor is unchanged (`RAW_LOOKUP_TID` 814).
The staged `PerCoreDomain.lean` theorem inventory grows 67 → 75 entries (the
`livePreservation` category 7 → 15).

- **The asymmetry (the finding).**  The v0.31.52 cut introduced the two SM5.G per-core
  domain invariants `domainScheduleIndexInBoundsOnCore` (the schedule index stays
  `< domainSchedule.length`) and `domainConsistentOnCore` (the active domain equals the
  schedule entry at the current index) and proved that `scheduleDomainOnCore`
  *preserves* the SM4.C *membership* predicate — but it did **not** prove the live
  `scheduleDomainOnCore` / `switchDomainOnCore` transitions maintain these two new
  invariants.  An invariant established by the abstract rotation yet not shown
  maintained by the real transitions is half-delivered: SM5.I could not assume it
...
- **§11 — 8 new live-transition preservation theorems.**
  `domainScheduleIndexInBoundsOnCore_frame` / `domainConsistentOnCore_frame` (the
  per-other-core frames — a write to core `c'`'s domain triple leaves core `c`'s
  invariant untouched); `idleFallbackOnCore_domainScheduleIndexOnCore` /
  `scheduleEffectiveOnCore_domainScheduleIndexOnCore` /
  `decrementDomainTimeOnCore_domainScheduleIndexOnCore` (the schedule-index frames the
  re-dispatch path needs — none of the idle fallback, the effective dispatch, or the
  pure domain-time decrement touches the schedule index);
  ...
  the active domain equal to the entry at the current index — `switchDomainOnCore`'s
  domain effect is exactly the rotation, so consistency is *established*
  unconditionally; `scheduleDomainOnCore` inherits it through the context save/restore
  frames).
...
anchors and 2 elaboration examples; the inventory partition-count guards updated to the
...
theorem re-verified to depend only on the three foundational Lean axioms; the staged
build (208 jobs) + production/staged partition gate (55 staged-only modules) green; no
compiler / linter warnings on a forced clean rebuild; the domain barrier
(`chooseThreadOnCore_respects_activeDomain` + the budget variant) re-traced as
genuinely enforced (the selection's fold-eligibility induction never admits an
out-of-domain thread, and `scheduleEffectiveOnCore` independently re-checks
...
- 32 in-source deferrals across **19** production files stated **in their own
...
| Three `@[export]` runtime-seam modules are staged-only, outside the production import closure, so a linked image would not carry their symbols | `scripts/staged_module_allowlist.txt`, `SeLe4n.lean` | RR5.15 |
| SM4.G residue: `bootFromPlatformWithIdleThreads` is proven correct and has no production caller, so `idleThreadEnqueuedOnCore` is never established at boot and `schedulerNoStall_smp`'s `hIdle` is discharged by hypothesis only | `SeLe4n/Platform/Boot.lean` | RR5.10–RR5.14 |
| `suspend_thread_inner` commits kernel state outside the kernel-entry lock | `SeLe4n/Platform/FFI.lean` | RR5.17 |
| Two `debug_assert!` lock/vector tripwires vanish from the release image | `rust/sele4n-hal/src/` | RR5.18 |
...
and its `sendFaultIPC` rights gate, the fail-closed dispositions, cross-core
...
| RR4 | Fault handling: full fault IPC with reply-based restart — **LANDED v0.34.44** (RR4.1–RR4.27: `Fault` + wire format, resolution + rights, fail-closed dispositions, delivery through the live `.call` chain, resume/restart, preservation + RR4.19 progress + RR4.20's production flow gate and non-interference, the `ExceptionModel`/`trap.rs` wiring, suite + fixture; **review round, PR #887**: `.tcbSetFaultHandler` as `TCB.faultHandler`'s only writer, kernel-origin exceptions halt instead of being delivered, the woken handler's frame staged, `.tcbResume` retires a pending fault, unknown syscall numbers delivered as faults; **review round 2**: the classifier upcall gated on readiness with the gated-seam set *derived* in `build.rs` and the ungated seams pinned as a reasoned allowlist, the MCS fault-tag layout cited and pinned by `faultLabel_ne_timeout`; **review round 3**: capability faults produced at the SVC seam (`syscallCapFaultOf`, on every syscall the refusal ledger does not record, the phase flag per seL4's blocking rule, the `SVC` as the restart PC), the SVC number read at full width, the readiness scanner dominance-aware and the routing check on the assignment relation, the not-ready abort fallback halting, the rights predicate defined from its clause inventory, the fault-handler lock set covering the validated endpoint with the CSpace walk's interior registered as an uncovered domain; **review round 4**: the build-time scanners moved to a statement-level view, refusing nested and inverted shapes; **review round 5**: a delivered syscall fault is outcome tag 2 (`.faulted`) on which the trap layer halts, never the blocked-resume sentinel; **review rounds 6 and 7**: the readiness guard is bound to the executing PE, the routing and decode scanners locate their statements and arms, an aliased upcall fails closed, exemptions reconcile by occurrence) | 27 |
...
## WS-DT — IPC `ipcInvariantFull` de-threading (**CLOSED v0.34.43**, opened v0.31.157, registered v0.34.26)

**Status**: CLOSED — WS-RR RR3.1–RR3.26 (v0.34.43) closed D1, D6 and **both**
halves of D8: the de-threading (RR3.1–RR3.14) and the dispatch payoff
(RR3.15–RR3.26).
**Closed by**: **WS-RR phase RR3** (`SMP_RELEASE_READINESS_PLAN.md` §RR3,
26 sub-tasks), which absorbed D1, D6 and D8 and retired the plan at RR3.26.
**Plan** (retired):
[`docs/dev_history/planning/IPC_INVARIANT_DETHREADING_PLAN.md`](dev_history/planning/IPC_INVARIANT_DETHREADING_PLAN.md).
...
**The payoff, landed (RR3.15–RR3.26, v0.34.43).**  Three theorems now carry
the bundle across dispatch, layered the way the dispatcher is.
`dispatchCapabilityOnly_preserves_ipcInvariantFull` (`SeLe4n/Kernel/API.lean`,
**production**) covers every capability-gated arm — cspace, retype, VSpace
with the shootdown and icache-broadcast variants, service, sched-context,
TCB fields, suspend/resume — under the pre-state pack
...
(`SeLe4n/Kernel/IPC/Invariant/DispatchPayoff.lean`).  Every hypothesis in both
packs is a **pre-state** fact, so the RR3.1 gate's zero-binding rule holds
over the payoff tier too.  The per-arm layer the payoffs compose is
production: `SeLe4n/Kernel/IPC/Invariant/DispatchArmPreservation.lean`,
imported by `API.lean`, holds per-operation bundles for the capability,
retype (behind the eighteen-field `retypeTargetDetached`
revoke-and-suspend-before-retype discipline), VSpace, service, sched-context,
...
`Platform.Staged` while a linked kernel image does not.  Relocating the
payoffs to production rides with that surface's own promotion — a debt row
below.  The confinements are stated, not silent: `.notificationSignal` is
covered on the unbound-delivery path only (SM6.D's registered bound-delivery
debt); the `.replyRecv` composite excludes a live donation edge naming the
woken caller (the AUD-3 window between a bare reply and its donation return);
retype and suspend demand their quiescence packs, and suspension of a victim
still parked in an IPC queue needs the cancellation composite registered
below.  `docs/planning/ipc_dethreading_pending.txt` now carries **zero**
registrations, and the gate holds that register in both directions, so a new
residual cannot be parked there silently.  The standing constraint in
`CLAUDE.md` / `AGENTS.md` (added RR0.3, rewritten at RR3.14 and again at
RR3.26) states the payoff's location and confinements; RR8.3 retires it when
the staged tier relocates.
...
| The per-arm dispatch bundles the payoff needs (capability, lifecycle, VSpace, service, sched-context, TCB) | WS-RR | **CLOSED** RR3.15–RR3.23 (v0.34.43) |
| D8 — the two dispatch payoff theorems and their `CLAIM_EVIDENCE_INDEX.md` citation | WS-RR | **CLOSED** RR3.24, RR3.25 (v0.34.43) |
| The `consumeCallerReply` documented exception | WS-RR | **CLOSED** RR3.10 (v0.34.43) |
| Plan retirement to `docs/dev_history/planning/` | WS-RR | **CLOSED** RR3.26 (v0.34.43) |
| `schedContextBind` operation hardening — the bind arm's bundle takes the pre-state fact that the thread being bound is not a recorded donation owner (`capabilityDispatchQuiescence.boundThreadNotDonationOwner`); hardening the operation to refuse such a bind makes that field dischargeable from operation success alone | SchedContext subsystem (WS-SM) | SM10 |
...
| Trace composition — the payoffs conclude `ipcInvariantFull st'` while their packs require `ipcReachable st`, and `ipcReachable` has an inhabitation witness but no per-syscall preservation theorem, so the payoff applies once per pack discharge rather than chaining down a trace (PR #886 review); closing it means concluding the pack's state-shaped components (`objects.invExt`, `allTimeoutBudgetsNone`, `pendingMessageCapBadgesWellFormed`, `notificationWaiterConsistent`) alongside the bundle | IPC subsystem (WS-SM) | SM10 |
| Rendezvous-reachable witness interiors — the per-arm witness family (`DispatchPayoff` §7b, PR #886 review) exercises every indexed pack field to the retype/binding levers' boundary, and two interiors sit beyond it: `replyStage`'s `caller = some _` premise (a fresh reply's caller is pinned `none` by `retypeReplacementFresh`; only the call rendezvous creates a caller-carrying reply) and `replyRecvStage`'s resolver premise (a CSpace-resolved reply capability, which the same rendezvous supplies); closing them means a rendezvous-built witness state with the reachability components carried across the call composite | IPC subsystem (WS-SM) | SM10 |
| Elaborator-backed gate census — the RR3.1 gate re-implements a fragment of Lean's surface grammar in text, and the PR #886 review showed the spelling class does not converge under per-spelling fixes (eight consecutive rounds each taught the grammars one more valid command); the shared `_COMMANDS`/`_MODIFIERS` source and the `grammar_coverage` tripwire (same review, the churn-diagnosis cut) turn any unknown spelling into the gate's own loud failure. **CLOSED** v0.34.43 (same review, round 25): eight further findings in two rounds were each a fragment of the *elaborator's* semantics — telescopes, grouped binders, named-argument order, notation expansion, structure entailment, `∨`-entailment — so the registered structural closure was pulled forward: `SeLe4n/Testing/IpcDethreadingEnvironmentCensus.lean` walks the elaborated environment (a `run_cmd` over every family-marked proposition, entailment descending `∧`, structure fields and definitional unfoldings, with self-enforced threaded/clean witnesses), agrees with the text census at 146 statements / 0 threaded, and is built by `test_tier1_build.sh` as its own root; the text gate remains the fast pre-commit approximation, and its documented residuals (equality transport aside, positions the elaborator resolves) are the census's to hold | Tier-0 tooling | **CLOSED** v0.34.43 |
...
- **R2.B — `syscallDispatchFromAbi` and substantive `@[export]`
  ...
  at `rust/sele4n-hal/src/svc_dispatch.rs` and
  `rust/sele4n-hal/src/ffi.rs` with the actual
...
(`Scheduler/Operations/Core.lean`) silently returned
...
`Scheduler/Invariant.lean` requires that a bound thread's
  ...
  - **AN10 post-delivery AN10-residual-1 closure**: a fourth
    ...
    commits**:
      ...
      2. H7 wrapper (`b4fd653`): `removeRunnableValid` typed
         entry-point in `IPC/Operations/Endpoint.lean`.
    ...
    **Source-read correction**: `endpointQueueRemoveDual`
    (`IPC/DualQueue/Transport.lean`) confirmed 3-arm
...
- **AN8** (Rust HAL hardening, v0.30.9, **released**): 6 sub-tasks
  ...
  - **AN8 third-pass audit**: caught four more tightening items.
    (A) Stale `mmu.rs` "AG6 replaces it" comment updated with
...
- **AN1** (Critical-path blockers, v0.30.6, in progress): closes the
  ...
  - **AN1-C** (H-24 / RUST-M06): the 6 primary `WS-V/AG10` TODOs
    called out in the audit (`trap.rs` SVC FFI; `lib.rs` HAL
    ...
    `conformance.rs` V1 test block header;
    `Assumptions.lean` AG10-C section headers) are purely
...
- **AN3** (IPC subsystem, v0.30.6, complete): closes H-01 (Donation
  ...
  - **AN3-B (IPC-M01 / Theme 4.2)**: the 16-conjunct
    ...
    in `IPC/Invariant/Defs.lean`, `contextSwitchState_preserves_proofLayerInvariantBundle`
    in `Architecture/Invariant.lean`).  3 new runtime tests
...
- **AN5** (Scheduler / SchedContext + `eventuallyExits` closure, v0.30.6,
  ...
  - **AN5-B (SCH-M02..M05 MEDIUM batch)**:
    - **SCH-M01** — TCB case-dispatch factoring scope documented
      in-place at `Scheduler/Operations/Preservation.lean` with
...
- **W3 (Dead Code Elimination) COMPLETE** (v0.22.14): 8 sub-tasks (W3-A through W3-H).
  ...
  **W3-D**: Removed entire fast-projection cluster (~200 lines) from Projection.lean dead enforcement boundary items from Wrappers.lean orphaned HashSet bridge
...
L3-E: already resolved (pre-existing in `CallReplyRecv.lean`).
...
|-----------|---------|-------|-------------|
| **WS-RA** | v0.33.37–v0.33.38 | Syscall return ABI — the kernel returns seL4's exact ARM64 frame (`x0` result, `x1` MessageInfo with the kernel status in the top of its label range — ABI v3 at v0.34.44, v2's offset label retired — `x2`–`x5` message registers); `SYSCALL_ABI_VERSION = 3` pinned in Lean, `sele4n-types` and the HAL; blocked-waiter staging + per-arm return-shape family. **COMPLETE** (frame delivery at context restore owed to SM10.1) | RA.A–RA.B |
