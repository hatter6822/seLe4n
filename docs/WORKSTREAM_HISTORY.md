...
- **Phase AD4 COMPLETE** (v0.25.14): Cross-Subsystem Composition Proofs (F-08) — 4 sub-tasks (AD4-A through AD4-D). F-08: Added 13 cross-subsystem bridge lemmas to `CrossSubsystem.lean` (~343 lines): 1 core generic bridge (`crossSubsystemInvariant_objects_change_bridge`) that frame-preserves `registryDependencyConsistent` and `serviceGraphInvariant` while accepting caller-provided post-state proofs for 6 objects-reading predicates; 7 IPC operation bridges (`ipcSend`, `ipcReceive`, `ipcReply`, `ipcCall`, `ipcReplyRecv`, `notificationSignal`, `notificationWait`); 5 Scheduler/Lifecycle operation bridges (`schedule`, `handleYield`, `timerTick`, `suspendThread`, `resumeThread`). Coverage matrix documented in CrossSubsystem.lean. Full build (236 jobs), `test_full.sh` (Tier 0–3), and sorry/axiom scan all pass. Zero sorry/axiom.
