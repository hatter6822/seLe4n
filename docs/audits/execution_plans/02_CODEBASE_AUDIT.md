...
**Remaining `sorry` in `Invariant.lean` (line 531):** `bfs_complete_for_nontrivialPath` — deferred BFS completeness bridge. This is the sole proof debt in the file, annotated TPI-D07-BRIDGE, and operationally validated by cycle detection tests and the depth-5 dependency chain smoke test. **Closure path:** A detailed completeness proof roadmap is documented in [M2 §6](./milestones/M2_BFS_SOUNDNESS.md#6-completeness-proof-roadmap--the-harder-path), including ~18 prerequisite lemmas in 4 dependency tiers, a lexicographic induction measure `(countUnvisitedRegistered, frontier.length)` that avoids the fuel-recycling complication, and two fuel adequacy options (`serviceIndex` field vs. preconditioned hypothesis).
