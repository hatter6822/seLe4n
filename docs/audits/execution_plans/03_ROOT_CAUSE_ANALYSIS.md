...
> - **Gap 2 (BFS bridge):** Partially resolved — `bfs_complete_for_nontrivialPath` exists with focused `sorry` (TPI-D07-BRIDGE, line 531). Full BFS soundness suite deferred. **Completeness closure roadmap** now documented in [M2 §5-§7](./milestones/M2_BFS_SOUNDNESS.md) and sub-documents [M2A](./milestones/M2A_EQUATIONAL_THEORY.md)–[M2D](./milestones/M2D_COMPLETENESS_PROOF.md): 13 prerequisite lemmas, `bfsClosed` invariant, `serviceCountBounded` fuel adequacy, `(fuel, frontier.length)` lexicographic induction, and 10-step implementation sequencing plan.
