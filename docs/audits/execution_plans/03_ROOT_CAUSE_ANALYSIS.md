...
> - **Gap 2 (BFS bridge):** Fully resolved — `bfs_complete_for_nontrivialPath` proved under `serviceCountBounded` precondition (TPI-D07-BRIDGE closed). Full B1-B7 BFS soundness suite implemented.
> - **Gap 3 (edge-insertion decomposition):** Resolved — `nontrivialPath_post_insert_cases` proved (line 541-572).
>
> The preservation theorem is sorry-free. All three gaps are fully closed. Line references below reflect the pre-implementation state.
