...
with **97 atomic sub-tasks**, explicit dependencies, gate conditions, and scope
estimates. The plan addresses all 14 HIGH findings, all 39 MEDIUM findings, and
selects 28 LOW findings for inclusion based on security relevance, proof chain
...
| U-H03 | A-02 | Lifecycle | `deleteObject` relies on caller having first called `revokeCap`. No compile-time enforcement or runtime guard — `revokeBeforeDelete` is a proof obligation, not a code check. |
...
| U-H10 | IF-03 | InfoFlow | `NonInterferenceStep` 34-constructor completeness is manually maintained. Adding a new kernel operation without a corresponding NI constructor silently weakens the guarantee. No automated enforcement. |
...
| U1-E | U-H02 | **(Requires U1-D)** Update `retypeFromUntyped` preservation theorems to thread through the new post-allocation alignment check. The invariant proof must handle the new error branch. | `SeLe4n/Kernel/Lifecycle/Invariant.lean` | Medium |
...
| U1-L | U-H03 | Add runtime guard in `deleteObject`: before deleting, check that no CDT children exist for any capability in the object. Return `.revocationRequired` if children found. This makes the `revokeBeforeDelete` proof obligation a runtime check. | `SeLe4n/Kernel/Lifecycle/Operations.lean` | Medium |
...
| U2-C | U-H06 | **(Requires U2-B)** Update VSpace invariant proofs to thread through the new VAddr bounds. Add `vaddr_bounded` conjunct to `vspaceInvariantBundle` if needed, or prove VAddr bounds are maintained by all VSpace operations. | `SeLe4n/Kernel/Architecture/VSpaceInvariant.lean` | Medium |
| U2-D | U-H07 | Thread `MachineConfig.physicalAddressWidth` through VSpace bounds checking. Replace hardcoded `physicalAddressBound = 2^52` with `2^config.physicalAddressWidth`. This makes the bound platform-specific (44-bit for BCM2712, 52-bit for generic ARM64). | `SeLe4n/Kernel/Architecture/VSpace.lean` | Medium |
...
| U2-F | U-H07 | **(Requires U2-D)** Update VSpace invariant proofs and `vspaceMapPageChecked` to use the parameterized PA bound. Verify RPi5 `rpi5MachineConfig` (44-bit) passes all VSpace well-formedness checks. | `SeLe4n/Kernel/Architecture/VSpaceInvariant.lean`, `SeLe4n/Platform/RPi5/Board.lean` | Medium |
...
| U2-K | U-L03 | Make `AccessRightSet` constructor enforce `isWord64` at creation. Add `AccessRightSet.mk_checked` that validates `bits < 2^5`. Make the raw `mk` private or add a `valid` precondition. Prefer `edgeWellFounded` over BFS-based acyclic for CDT security checks where applicable. | `SeLe4n/Model/Object/Types.lean` | Medium |
...
| U2-M | U-M20 | Add compile-time completeness check for `allTablesInvExt`. Use a structure with one field per RHTable in `SystemState`, so adding a new table without updating `allTablesInvExt` produces a type error. | `SeLe4n/Model/State.lean` | Medium |
...
| ID | Finding | Description | Files | Est. |
|----|---------|-------------|-------|------|
| U4-A | U-H09 | Discharge `hProjection` hypothesis for `endpointSendDual` NI proof. Prove that `endpointSendDual` preserves `projectState` for unobservable domains by showing the operation only modifies objects within the sender's and endpoint's security domains. | `SeLe4n/Kernel/InformationFlow/Invariant/Operations.lean` | Large |
| U4-B | U-H09 | **(Requires U4-A)** Discharge `hProjection` for `endpointReceiveDual`. Same pattern as U4-A but for the receive path — receiver's domain and endpoint's domain are the only modified domains. | `SeLe4n/Kernel/InformationFlow/Invariant/Operations.lean` | Large |
| U4-C | U-H09 | **(Requires U4-B)** Discharge `hProjection` for `endpointCall` and `endpointReplyRecv`. These are compositions of send+block and receive+reply respectively. The proofs should compose from U4-A and U4-B. | `SeLe4n/Kernel/InformationFlow/Invariant/Operations.lean` | Large |
...
| U4-E | U-H10 | Add compile-time or CI-level completeness check for `NonInterferenceStep`. Create a theorem or `#check` that the 34 constructors match the exhaustive list of kernel API entry points. If a new operation is added without a constructor, compilation fails. | `SeLe4n/Kernel/InformationFlow/Invariant/Composition.lean` | Medium |
...
| U4-H | U-M05 | **(Requires U4-G)** Update `crossSubsystemInvariant` preservation theorems to include `serviceGraphInvariant` preservation. Thread through existing proofs from `Acyclicity.lean`. | `SeLe4n/Kernel/CrossSubsystem.lean` | Medium |
...
| U4-K | U-M31 | Prove self-contained `ipcInvariantFull` preservation for Send, Receive, Call, and ReplyRecv by internalizing the `dualQueueSystemInvariant` hypothesis. Compose from existing individual proofs. | `SeLe4n/Kernel/IPC/Invariant/Structural.lean` | Large |
...
| U4-N | U-M19 | Prove `descendantsOf` transitive closure fuel sufficiency for deep CDT trees. Show that `descendantsOfBounded` with fuel = CDT node count returns all transitive descendants. | `SeLe4n/Model/Object/Structures.lean` | Large |
...
| ID | Finding | Description | Files | Est. |
|----|---------|-------------|-------|------|
| U5-A | U-M02 | Extract shared dispatch logic from `dispatchWithCap` and `dispatchWithCapChecked` into a common helper function. The helper takes the 7 identical match arms. Both dispatchers call the helper and override only the 7 policy-gated arms. | `SeLe4n/Kernel/API.lean` | Medium |
...
| U5-D | U-M02 | **(Requires U5-A)** Verify the refactored dispatch paths produce identical results by updating or extending the `checkedDispatch_subsumes_unchecked` theorem. Replace the trivial `True` placeholder (U-L20) with actual semantic equivalence under permissive policy. | `SeLe4n/Kernel/API.lean` | Medium |
...
| ID | Finding | Description | Files | Est. |
|----|---------|-------------|-------|------|
| U6-A | U-M08 | Add formal MMIO abstraction boundary. Declare device register reads as `opaque` (platform-dependent) to prevent proofs from assuming RAM-like behavior (idempotency, commutativity). Add `MmioRead` and `MmioWrite` structures that encode device-specific semantics. | `SeLe4n/Platform/RPi5/MmioAdapter.lean` | Medium |
| U6-B | U-M08 | **(Requires U6-A)** Update any proofs that depend on memory-read semantics for MMIO addresses. These proofs should now carry an explicit `MmioSafe` hypothesis or use the opaque declarations. | `SeLe4n/Kernel/Architecture/VSpace.lean` | Medium |
| U6-C | U-M09 | Strengthen `registerContextStable` in RPi5 runtime contract. Replace the permissive `∨ st'.scheduler.current ≠ st.scheduler.current` disjunct with a requirement that the new register state matches the scheduled thread's saved context. | `SeLe4n/Platform/RPi5/RuntimeContract.lean` | Medium |
| U6-D | U-M09 | **(Requires U6-C)** Update RPi5 proof hooks to use the strengthened contract. Verify that existing preservation proofs still hold under the tighter predicate. | `SeLe4n/Platform/RPi5/ProofHooks.lean` | Medium |
...
| U6-G | U-M15 | **(Requires U6-F)** Prove boot-to-runtime invariant bridge: single theorem from `bootFromPlatform` output through freeze to `proofLayerInvariantBundle`. Compose from existing boot validity (`bootFromPlatform_valid`) and freeze correctness (`apiInvariantBundle_frozenDirect`). | `SeLe4n/Platform/Boot.lean`, `SeLe4n/Model/FreezeProofs.lean` | Large |
...
| U7-B | U-L02 | Remove dead types and functions from `Architecture/Assumptions.lean` (`TransitionSurface`, `InvariantSurface`, `ContractRef`, `ExtendedBootBoundaryContract`), `RPi5/MmioAdapter.lean` (`MmioOp`, `MmioOpKind`, `MemoryBarrier`, `hasAppropriateBarriers`, `isMmioPeripheralAddress`, `isValid`), and `InformationFlow/Policy.lean` (`bibaPolicy`, `bibaSecurityFlowsTo`, `defaultGenericLabelingContext`, `threeDomainExample`). | Multiple files | Medium |
| U7-C | U-L02 | **(Requires U7-B)** Remove dead functions from `Prelude.lean` (`ObjId.valid`, `Deadline.none`, `Deadline.immediate`), `Machine.lean` (`alignedRead`, `alignedWrite`), `FrozenState.lean` (`FrozenMap.fold`, `FrozenSet.mem`, `minObjectSize`, `objectCapacity`), `Selection.lean` (`chooseThreadInDomain`), `RunQueue.lean` (`maxPriorityValue`, `filterToList`, `rotateHead`, `mem_rotateHead`), and `VSpaceBackend.lean` (`hashMapVSpaceBackend`). Verify no downstream references for each. | Multiple files | Medium |
...
| U7-E | U-L02 | Remove superseded invariant bundles: `schedulerInvariantBundle` (3-tuple), `kernelInvariant` (4-tuple), `capabilityInvariantBundleFull`, `WithMintCompleteness`, `WithCdtMaps` and their ~500 lines of preservation theorems. Only the canonical bundles (`schedulerInvariantBundleFull`, `capabilityInvariantBundle`) should remain. | `SeLe4n/Kernel/Scheduler/Invariant.lean`, `SeLe4n/Kernel/Capability/Invariant/Defs.lean` | Medium |
...
| U7-H | U-H12 | **(Requires U7-G)** Add `beq_symmetric` lemma for the updated `BEq RHTable` instance under `noDupKeys` + `invExt`. This provides formal assurance that the instance is well-behaved for kernel-state tables. | `SeLe4n/Kernel/RobinHood/Bridge.lean` | Medium |
...
| U7-J | U-M37 | Refactor high-heartbeat Robin Hood proofs. Target all proofs under `maxHeartbeats 800000` (4× default). Factor `get_after_insert_eq` and `get_after_erase_eq` into smaller helper lemmas that each complete within default heartbeats. | `SeLe4n/Kernel/RobinHood/Invariant/Lookup.lean` | Large |
| U7-K | U-L21 | Replace fragile `simp [Function.update]` chains in scheduler preservation proofs with named lemmas. Extract `function_update_same`, `function_update_ne`, and similar patterns into reusable `@[simp]` lemmas. | `SeLe4n/Kernel/Scheduler/Operations/Preservation.lean` | Medium |
| U7-L | U-L07 | Add formal commutativity proofs between builder and frozen operations: prove that `freeze ∘ builderOp = frozenOp ∘ freeze` for the core operations (store, lookup). These bridge the two phases and justify the separate-but-equivalent design. | `SeLe4n/Model/FreezeProofs.lean` | Large |
...
*Workstream plan created 2026-03-24. Baseline: seLe4n v0.20.7.
Four audit sources, 12 erroneous findings excluded, 81 confirmed findings
organized into 8 phases with 97 atomic sub-tasks.*
