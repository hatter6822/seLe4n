...
unconditionally from the bundle. §3.B entries are a mix of substantively
discharged (4) and closure-form-with-recipe (6); see the per-row notes.
§3.C entries are closure-form by design — `schedule` and `revokeService`
both call into kernel-internal sub-machines whose projection witnesses are
caller obligations at composition time.
...
|---|---------|-----------|-----------|---------------|--------------------|
| A.1 | `cspaceCopy_preserves_capabilityInvariantBundle` | `SeLe4n/Kernel/Capability/Invariant/Preservation/CopyMoveMutate.lean:34` | `hCdtPost` | `cspaceCopy_cdt_hypothesis_discharged_at` (`CrossSubsystem.lean:2892`) | `#check @SeLe4n.Kernel.cspaceCopy_cdt_hypothesis_discharged_at` |
| A.2 | `cspaceMove_preserves_capabilityInvariantBundle` | `SeLe4n/Kernel/Capability/Invariant/Preservation/CopyMoveMutate.lean:88` | `hCdtPost` | `cspaceMove_cdt_hypothesis_discharged_at` (`CrossSubsystem.lean:2900`) | `#check @SeLe4n.Kernel.cspaceMove_cdt_hypothesis_discharged_at` |
| A.3 | `cspaceMintWithCdt_preserves_capabilityInvariantBundle` | `SeLe4n/Kernel/Capability/Invariant/Preservation/CopyMoveMutate.lean:173` | `hCdtPost` | `cspaceMintWithCdt_cdt_hypothesis_discharged_at` (`CrossSubsystem.lean:2907`) | `#check @SeLe4n.Kernel.cspaceMintWithCdt_cdt_hypothesis_discharged_at` |
| A.4 | `cspaceMutate_preserves_capabilityInvariantBundle` | `SeLe4n/Kernel/Capability/Invariant/Preservation/CopyMoveMutate.lean:239` | (bundle transfer; no `hCdtPost`) | `cspaceMutate_cdt_hypothesis_discharged_at` (`CrossSubsystem.lean:2916`) — index-completeness | `#check @SeLe4n.Kernel.cspaceMutate_cdt_hypothesis_discharged_at` |
| A.5 | `cspaceDeleteSlot_preserves_capabilityInvariantBundle` | `SeLe4n/Kernel/Capability/Invariant/Preservation/Delete.lean:127` | (bundle transfer; no `hCdtPost`) | `cspaceDeleteSlot_cdt_hypothesis_discharged_at` (`CrossSubsystem.lean:2924`) — index-completeness | `#check @SeLe4n.Kernel.cspaceDeleteSlot_cdt_hypothesis_discharged_at` |
| A.6 | `cspaceRevoke_preserves_capabilityInvariantBundle` | `SeLe4n/Kernel/Capability/Invariant/Preservation/Delete.lean:191` | (bundle transfer; no `hCdtPost`) | `cspaceRevoke_cdt_hypothesis_discharged_at` (`CrossSubsystem.lean:2933`) — index-completeness | `#check @SeLe4n.Kernel.cspaceRevoke_cdt_hypothesis_discharged_at` |
...
The classification post-AN12 is:

- **5 fully substantive** (no closures): `cspaceDelete`, `serviceQuery`,
  `tcbSetIPCBuffer`, `vspaceMap`, `vspaceUnmap`.
- **3 hybrid** (substantive body + ONE legit external closure):
  `tcbSetPriority` and `tcbSetMCPriority` (take `hSchedProj` for the
  optional preemption-schedule call), `serviceRevoke` (takes
  `hServiceProjEq` for fold-induction at projection layer; see §3.C).
- **6 closure-form with discharge recipes**: `schedContextConfigure`,
  `schedContextBind`, `schedContextUnbind`, `lifecycleRetypeDirectWithCleanup`,
  `tcbSuspend`, `tcbResume` — listed below.

| # | Theorem | File:Line | Hypothesis | Discharge recipe (frame lemmas) | Reachability check |
|---|---------|-----------|-----------|---------------------------------|--------------------|
| B.1 | `schedContextConfigure_preserves_projection` | `SeLe4n/Kernel/InformationFlow/Invariant/Operations.lean:3626` | `hProjEq` | `objects_insert_preserves_projection_high` × 2 (SC + bound TCB) + `migrateRunQueueBucket_preserves_projection` + `schedContextBind_frame_runQueue_rebucket` | `#check @SeLe4n.Kernel.InformationFlow.schedContextConfigure_preserves_projection` |
| B.2 | `schedContextBind_preserves_projection` | `SeLe4n/Kernel/InformationFlow/Invariant/Operations.lean:3666` | `hProjEq` | `objects_insert_preserves_projection_high` × 2 + `schedContextBind_frame_runQueue_rebucket` + `projectState_scThreadIndex_eq` | `#check @SeLe4n.Kernel.InformationFlow.schedContextBind_preserves_projection` |
| B.3 | `schedContextUnbind_preserves_projection` | `SeLe4n/Kernel/InformationFlow/Invariant/Operations.lean:3692` | `hProjEq` | `projectState_scheduler_current_cleared_when_high` + `removeRunnable_preserves_projection` + `objects_insert_preserves_projection_high` × 2 + `projectState_replenishQueue_eq` + `projectState_scThreadIndex_eq` | `#check @SeLe4n.Kernel.InformationFlow.schedContextUnbind_preserves_projection` |
| B.4 | `lifecycleRetypeDirectWithCleanup_preserves_projection` | `SeLe4n/Kernel/InformationFlow/Invariant/Operations.lean:3722` | `hProjEq` | `lifecyclePreRetypeCleanup` field-frame lemmas + `scrubObjectMemory_preserves_projection` (memory-ownership-`none` arm) + `storeObject_preserves_projection` | `#check @SeLe4n.Kernel.InformationFlow.lifecycleRetypeDirectWithCleanup_preserves_projection` |
| B.5 | `suspendThread_preserves_projection` | `SeLe4n/Kernel/InformationFlow/Invariant/Operations.lean:3787` | `hProjEq` | 9-phase recipe in theorem docstring (G1 lookup → G9 schedule); composes `revertPriorityInheritance` chain + `removeRunnable_preserves_projection` + `cancelDonation_preserves_projection` (B.6 helper) + `storeObject_preserves_projection` × 2 | `#check @SeLe4n.Kernel.InformationFlow.suspendThread_preserves_projection` |
| B.6 | `cancelDonation_preserves_projection` | `SeLe4n/Kernel/InformationFlow/Invariant/Operations.lean:3750` | `hProjEq` | 3-arm split (`.unbound` `rfl` / `.bound` SC unbind / `.donated` returnDonation); donor-high derivable from `donationOwnerValid` | `#check @SeLe4n.Kernel.InformationFlow.cancelDonation_preserves_projection` |
| B.7 | `resumeThread_preserves_projection` | `SeLe4n/Kernel/InformationFlow/Invariant/Operations.lean:3845` | `hProjEq` | `resumeThread_frame_insert` (substantive, 3806) + `resumeThread_frame_ensureRunnable` (substantive) | `#check @SeLe4n.Kernel.InformationFlow.resumeThread_preserves_projection` |

**Notes for §3.B**

- All seven closure-form theorems were chosen for the closure pattern (over
  full substantive discharge) because Lean 4.28.0's `split` / `split_ifs`
  tactic interacts poorly with `Except.ok`-wrapped deeply-nested match
  conditions. The frame lemmas listed in the **Discharge recipe** column
  are themselves substantively proven at the same module so a future caller
  can compose them in ≈25–60 LOC per arm.
- B.5 is the longest recipe (≈100 LOC) because `suspendThread` is the most
...
| # | Theorem | File:Line | Hypothesis | Discharge site | Reachability check |
|---|---------|-----------|-----------|---------------|--------------------|
| C.1 | `setPriorityOp_preserves_projection` | `SeLe4n/Kernel/InformationFlow/Invariant/Operations.lean:3356` | `hSchedProj` | `schedule_preserves_projection` (`Operations.lean:678`) composed at API dispatch | `#check @SeLe4n.Kernel.InformationFlow.setPriorityOp_preserves_projection` |
| C.2 | `setMCPriorityOp_preserves_projection` | `SeLe4n/Kernel/InformationFlow/Invariant/Operations.lean:3434` | `hSchedProj` | `schedule_preserves_projection` (`Operations.lean:678`) composed at API dispatch | `#check @SeLe4n.Kernel.InformationFlow.setMCPriorityOp_preserves_projection` |
| C.3 | `schedContextBind_preserves_projection` (schedule arm) | `SeLe4n/Kernel/InformationFlow/Invariant/Operations.lean:3666` | inside `hProjEq` (B.2) | covered by §3.B B.2 recipe; the schedule arm is captured inside the closure body | `#check @SeLe4n.Kernel.InformationFlow.schedContextBind_preserves_projection` |
| C.4 | `schedContextConfigure_preserves_projection` (schedule arm) | `SeLe4n/Kernel/InformationFlow/Invariant/Operations.lean:3626` | inside `hProjEq` (B.1) | covered by §3.B B.1 recipe | `#check @SeLe4n.Kernel.InformationFlow.schedContextConfigure_preserves_projection` |
| C.5 | `revokeService_preserves_projection` | `SeLe4n/Kernel/InformationFlow/Invariant/Operations.lean:3572` | `hServiceProjEq` | RHTable fold induction at composition time; documented in the theorem's
  docstring "Design note on hypothesis `hServiceProjEq`" block | `#check @SeLe4n.Kernel.InformationFlow.revokeService_preserves_projection` |
...
- **§3.A — 6 of 6 rows DISCHARGED** with substantively proven companion
  bridges.
- **§3.B — 4 substantive + 7 closure-form** (the closure-form rows have
  named discharge recipes via pre-proven frame lemmas).
- **§3.C — 5 closure-form** (all with named composition recipes at the
