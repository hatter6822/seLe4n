...
#### C-02 — Three scripts referenced by the v0.29.0 plan did not exist *(RESOLVED in this same commit)*
**Original file**: `docs/audits/AUDIT_v0.29.0_WORKSTREAM_PLAN.md` (multiple sites); `scripts/` tree
**Severity at finding**: CRITICAL — at the time this finding was surfaced the
plan was the *current* active-audit plan cited by `CLAUDE.md`, and it directed
auditors / CI to invoke scripts that were not in the repo.
**Evidence (historical)**: `Grep` for
`find_large_lean_files|sync_documentation_metrics|sync_readme_from_codebase_map`
returned only `docs/audits/AUDIT_v0.29.0_WORKSTREAM_PLAN.md` and a single
historical file under `docs/dev_history/`. `ls scripts/` confirmed none of the
three files existed.

**Resolution** (landed in the same PR as this audit document):
1. Implemented the three scripts with proper project conventions
   (shebang, GPL-3.0+ header, `set -euo pipefail`, shellcheck-clean, `--check`
   modes):
   - `scripts/find_large_lean_files.sh` — ranks Lean modules + docs by LoC;
     emits CLAUDE.md-style bullets; `--check` diffs against CLAUDE.md's
     "Known large files" list.
   - `scripts/sync_readme_from_codebase_map.sh` — pushes `readme_sync`
     values from `docs/codebase_map.json` into `README.md` and
     `docs/spec/SELE4N_SPEC.md`; idempotent; `--check` reports drift.
   - `scripts/sync_documentation_metrics.sh` — orchestrator that
     regenerates the codebase map, runs the README/spec sync, advisory-
     checks the CLAUDE.md large-files list, and finally runs
     `test_docs_sync.sh`.
2. Archived the now-closed plan file:
   `docs/audits/AUDIT_v0.29.0_WORKSTREAM_PLAN.md` →
   `docs/dev_history/audits/AUDIT_v0.29.0_WORKSTREAM_PLAN.md`, matching the
   WS-AM precedent for AUDIT_v0.28.0_WORKSTREAM_PLAN.md.
3. Updated all ~30 in-tree references (CLAUDE.md, CHANGELOG.md,
   SELE4N_SPEC.md, WORKSTREAM_HISTORY.md, ERRATA/DEFERRED links,
   `tests/Ak9PlatformSuite.lean`, `rust/sele4n-hal/src/ffi.rs`).

All three scripts pass shellcheck under the existing Tier 0 hygiene
pattern. Verified that `check_version_sync.sh`,
`check_website_links.sh`, and the Tier 0 hygiene gate still PASS after
the move.
...
- **DOC-M04** CLAUDE.md large-file list refreshed AK10-E claims
  `CHANGELOG.md (~9 453 lines)`, matched in spot check. After the C-02
  resolution the plan file lives at
  `docs/dev_history/audits/AUDIT_v0.29.0_WORKSTREAM_PLAN.md (~4 721 lines)`
  and the CLAUDE.md entry has been updated to reflect the new path.
  Running `scripts/find_large_lean_files.sh --check` surfaces any future
  drift against this table automatically.
