...
| **SM2.C-T** — `RwLockExecution` carries no timestamps, so `writerWaitDepth` and every bound built on it (`lockContention_delay_bounded`, the CC-5 alphabet, the trace capacity) is denominated in *lock operations*, not ticks.  A holder may occupy its critical section for an arbitrarily long real interval with no operation recorded, so CC-5 reads as a **timing** channel only under an explicit per-step cost ceiling, which `FineLockFlow.lean` §SM8.D.3 takes as the hypothesis `tCs` of `elapsedBetween` rather than leaving in prose.  The stronger form puts timestamps on the execution datatype itself and denominates `maxDelay` in ticks | Honest, not unsound: the weaker unit is *stated* at every site that uses it and the bridge theorem carries its ceiling as a named hypothesis, so nothing claims a wall-clock bound it has not got.  Closing it changes the core execution datatype **every SM2.C liveness theorem quantifies over** — a foundation move for that surface, not a strengthening that can ride along with a lock-implementation cut | **WS-LC** phase LC5, target v0.34.54 — [`SMP_LOCK_DATATYPE_COMPLETION_PLAN.md`](planning/SMP_LOCK_DATATYPE_COMPLETION_PLAN.md) §7; LC5.10 retires this row.  **RR6 deliberately did not absorb it** (RR6.26); raised by SM8.D's review rounds against a phase already CLOSED, and registered here so it no longer depends on `SMP_INFORMATION_FLOW_PLAN.md` §SM8.D prose for its existence.  Site: `SeLe4n/Kernel/InformationFlow/FineLockFlow.lean` §3 |
| **SM2.C-C** — a refused 2PL growing phase cannot withdraw a request it has already queued.  **Partially closed at v0.34.50–52**: `RwLockOp.cancel` exists, preserves all five INV-R conjuncts (`rwLock_cancel_preserves_wf`), carries its own payoff theorems, and is related by **both** refinement bridges — the ticket-FIFO one since v0.34.51, with a tombstoned ledger, skip-aware promotion and a position-based FIFO capstone.  The **deployed** lock carries it since v0.34.52: `QueuedRwLock::cancel` publishes a per-core withdrawal, arbitrates against the previous holder's skip loop with a compare-exchange, and reaches Lean through six new FFI symbols; it is covered by four loom models (whose decisiveness is proved by relation-breaking mutation), miri under strict provenance, and a fifth Tier-5 alphabet letter driving both oracles.  **Both consumers emit one at v0.34.53**: `withLockSet`'s shrinking phase and the revalidated entry's refusal path are one definition, `unwindAll`, which withdraws before it releases — the order is what makes `unwindAll_leaves_no_queued_request` hold with no distinctness or resolvability condition on the footprint.  Nothing of this row is open; **LC5.10 retires it**.  `RevalidatedEntryOutcome.refused` carries the **released** state and `releaseAll` is genuinely applied, but `releaseRead` / `releaseWrite` are the identity for a non-holder (`rwLock_release_by_nonholder_preserves_waiters`), so where the growing phase found a member *contended* the core is queued rather than holding, and the unwind releases what was granted while leaving what was merely requested — which can still be promoted later and strand the lock | Bounded and fail-closed rather than unsound: a stranded promotion grants a lock to a core that will not use it, which the FIFO discipline then hands on at the next release; it cannot grant to the wrong core, break exclusion, or lose a waiter.  The docstring states it exactly, so a reader cannot take "released" to mean "fully unwound".  Closing it adds an `RwLockOp` constructor, which changes `applyOp`, **all five INV-R invariants** and every `cases op` in the SM2.C liveness surface | **WS-LC** — LC2 (the ticket-FIFO refinement, v0.34.51), LC3 (the deployed lock, v0.34.52) and LC4 (the consumers, v0.34.53) have all landed; the row is retired by LC5.10 — [`SMP_LOCK_DATATYPE_COMPLETION_PLAN.md`](planning/SMP_LOCK_DATATYPE_COMPLETION_PLAN.md) §§4–6; LC5.10 retires this row.  **RR6 deliberately did not absorb it** (RR6.26) for the same datatype-ripple reason as SM2.C-T.  Sites: `SeLe4n/Kernel/InformationFlow/FineLockFlow.lean` (`RevalidatedEntryOutcome`), `SeLe4n/Kernel/Concurrency/Locks/WithLockSet.lean` |
