...
| CDT edges are node-stable and slot-observed CDT is projection of node graph through slot mapping; `cspaceMove` performs pointer/backpointer updates instead of whole-edge rewrites. | `SeLe4n/Model/Object.lean`, `SeLe4n/Model/State.lean`, `SeLe4n/Kernel/Capability/Operations.lean`, `SeLe4n/Kernel/Capability/Invariant.lean` | `source /root/.elan/env && lake build`, `./scripts/test_smoke.sh`, `./scripts/test_full.sh` | Build + smoke/full tiers validate node-based CDT operations, preservation proofs, and trace/negative-state coverage. |
