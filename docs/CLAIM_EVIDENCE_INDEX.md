...
| The labeling a hardware boot installs is bound in the platform contract, admitted and `LabelingContextValid` by construction, and its declared separation witnesses are installed threads of the boot state | `SELE4N_SPEC.md` §6.7, `CLAUDE.md` | `lake exe syscall_dispatch_suite` (SD-046, SD-055); Tier 3 | `PlatformBinding.deploymentLabeling : DeploymentLabeling`, `PlatformBinding.labeling_admitted`, `PlatformBinding.labeling_valid`, `rpi5_deploymentLabeling` (`rfl`), `bootAndInitialisePlatform_eq_checked_boot`, `declaredWitnessesInstalled`, `uninstalledSeparationWitnessBootError` |
...
| Every core the binding declares boots with its idle thread enqueued on its own run queue, and the RPi5 binding declares every model core | `SELE4N_SPEC.md` §6.5.5, `CLAUDE.md` | `lake exe smp_idle_suite`, `lake exe syscall_dispatch_suite` (SD-045, SD-055) | `bootFromPlatformCheckedWithIdleThreads_idle_available` (production), `…_idleThreadEnqueuedOnCore` (staged, discharges `schedulerNoStall_smp`'s `hIdle`); `PlatformBinding.declaredCores`, `bootFromPlatformCheckedWithIdleThreadsFor_allCores`, `rpi5_cores_eq_allCores`, `bootAndInitialisePlatform_rpi5_all_cores` |
...
| The production boot state is `threadStateConsistent` — a boot-state theorem; the inactive-flag relation the live decisions read is `threadInactiveFlagConsistent`, and its preservation is registered debt — and the checked boot reserves the idle slots and every reference to them, with its own diagnostic | `SELE4N_SPEC.md` §6.5.5, `CLAUDE.md`, `docs/REGISTERED_DEBT.md` | `lake exe smp_idle_suite`; Tier 3 | `queuedIdleThread`, `bootFromPlatformCheckedWithIdleThreads_threadStateConsistent`, `…_threadInactiveFlagConsistent`, `idleSlotsReserved`, `bootObjectReferencesReservedIdleSlot`, `bootFromPlatformChecked_ok_idleSlotsFreshAt` |
...
| Each release-surviving tripwire halts on its declared failure predicate — the whole condition, not a token of it — in every profile | `CLAUDE.md`, `docs/REGISTERED_DEBT.md` | `cargo build -p sele4n-hal` (build.rs self-check with polarity mutations) | `release_surviving_tripwire_status`, `RELEASE_SURVIVING_TRIPWIRES`, `verify_release_surviving_tripwire_polarity` |
