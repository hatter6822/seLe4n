...
| That any lock-delay bound is denominated in time | `RwLockExecution` carries no per-step cost, so every bound built on `writerWaitDepth` counts *lock operations*; reading one as wall-clock needs the explicit ceiling `FineLockFlow.lean` §3 takes as a hypothesis | WS-LC LC5 |
