...
| A release by a non-holder and a re-acquisition by a holder are the **deployed** lock's no-ops, decided by its per-core held word; the refinement represents the holders (`queuedHeldSim`) and derives the no-op blocks rather than assuming them, and the undeployed CAS-retry bridge claims no no-op at all | `SELE4N_SPEC.md`, `CLAUDE.md`, GitBook 12 | `lake build SeLe4n.Kernel.Concurrency.Locks.QueuedRwLockRefinement`; `cargo test -p sele4n-hal --features host_tools --bin rw_lock_oracle`; `./scripts/test_loom_queued_rw_lock.sh` (`unwind_by_a_non_holder_never_touches_the_holder`, `every_pair_of_units_is_safe`); `./scripts/test_tier3_invariant_surface.sh` | `QueuedRwLock::held`, `queuedHeldSim`, `queuedBlock.releaseRead_noop`, `check_holders`, `scan_queued_rw_lock_protocol_intact` |
