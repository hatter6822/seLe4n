...
| Information-flow completeness: extended projection covers all security-relevant state. | `SeLe4n/Kernel/InformationFlow/Projection.lean`, `SeLe4n/Kernel/InformationFlow/Invariant.lean`, `tests/InformationFlowSuite.lean` | `./scripts/test_full.sh` | WS-F3: `ObservableState` extended with 3 fields (activeDomain, irqHandlers, objectIndex); CNode slot filtering via `projectKernelObject`; 7+ NI preservation theorems; `projectKernelObject_idempotent`/`_objectType` safety proofs; enforcement-NI bridge for `serviceRestartChecked`; WS-F3 test suite with IRQ/objectIndex/CNode/7-field low-equivalence checks. |
