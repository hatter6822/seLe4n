...
| The labeling a hardware boot installs is bound in the platform contract, admitted by construction | `SELE4N_SPEC.md` §6.7, `CLAUDE.md` | `lake exe syscall_dispatch_suite` (SD-046); Tier 3 | `PlatformBinding.deploymentLabeling` + `deploymentLabelingAdmitted`, `rpi5_deploymentLabeling` (`rfl`), `bootAndInitialisePlatform_eq_checked_boot` |
