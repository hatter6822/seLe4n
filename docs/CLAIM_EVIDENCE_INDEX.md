...
| **Non-compositional preservation** | Proves preservation by re-proving invariant components from scratch on post-state, discarding pre-state evidence. Structurally valid but masks lack of engagement with the operation's state transformation. (Identified by v0.11.6 audit H-01; **resolved by WS-E2** — all capability preservation proofs now compositionally derive post-state invariant components from pre-state components.) | Medium (weaker than compositional proofs) — **no instances remain in capability proofs** |
| **Tautological / Meta-property** | Proves a property that holds for *all* states by construction (e.g., `cspaceSlotUnique_holds` exploiting pure-function determinism). (Identified by v0.11.6 audit C-01; **resolved by WS-E2** — reclassified as meta-properties, genuine state-dependent `cspaceCNodeSlotsNoDup` added to `capabilityInvariantBundle`.) | Low (retained for documentation, no longer counted as high-assurance evidence) |
