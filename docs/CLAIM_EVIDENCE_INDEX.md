...
| That a refused two-phase-locking growing phase fully unwinds | The unwind releases what was granted and leaves what was merely requested, because neither `RevalidatedEntryOutcome.refused` nor `withLockSet`'s shrinking phase emits a withdrawal yet — the *operation* they would emit exists at every level (spec, both refinements, the deployed lock), so this is a wiring gap, not a missing primitive | WS-LC LC4 |
