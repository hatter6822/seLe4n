...
| Every kernel entry the HAL links against is defined — by the built archive, the HAL's assembly, or a reconciled expected-unresolved entry | `SELE4N_SPEC.md`, `CLAUDE.md` | `./scripts/check_kernel_entry_exports.py` (Tier 1, after `lake build SeLe4n:static`) | `nm --defined-only` over `.lake/build/lib/libseLe4n_SeLe4n.a`; every HAL `extern "C"` declaration is required (`classify_link_requirements`), so a rename on either side fails |
...
| The production boot state is `threadStateConsistent`, and the checked boot reserves the idle slots | `SELE4N_SPEC.md` §6.5.5, `CLAUDE.md` | `lake exe smp_idle_suite`; Tier 3 | `queuedIdleThread`, `bootFromPlatformCheckedWithIdleThreads_threadStateConsistent`, `idleSlotsReserved`, `bootFromPlatformChecked_ok_idleSlotsFreshAt` |
