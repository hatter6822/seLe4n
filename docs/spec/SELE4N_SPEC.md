...
- WS-E4 dual-queue endpoint operations (`endpointSendDual`/`endpointReceiveDual`) use intrusive-list queue boundaries (`Endpoint.sendQ`/`Endpoint.receiveQ`) with per-thread links (`TCB.queuePPrev`/`TCB.queueNext`, with compatibility mirror `queuePrev`); executable invariant checks enforce `intrusiveQueueWellFormed` over queue boundaries and links (head/tail coherence, cycle-free reachability, and pprev/next link consistency), while `negative_state_suite` covers send/receive block-rendezvous FIFO/drain paths, duplicate-wait rejection via `alreadyWaiting`, explicit queue-link runtime assertions, and O(1) arbitrary unlink semantics via stored `pprev`, including malformed-link rejection (`illegalState`) when queue metadata is inconsistent.
