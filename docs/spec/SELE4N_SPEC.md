...
2. machine-checked invariant preservation (1,224 theorem/lemma declarations),
...
| Attribute | Value |
|-----------|-------|
| **Package version** | `0.16.16` (`lakefile.toml`) |
...
| **Next workstreams** | **WS-M** Capability subsystem audit & remediation — Phase 1 (WS-M1) **COMPLETED** (v0.16.14), Phase 2 (WS-M2) **COMPLETED** (v0.16.15), Phase 3 (WS-M3) **PLANNED** (v0.16.16): IPC capability transfer — 20 subtasks covering `CapTransferResult`/`CapTransferSummary` types, `ipcTransferSingleCap`/`ipcUnwrapCaps` operations, IPC wrapper integration, API wiring, 4 test scenarios. Resolves L-T03 (last WS-L deferred item). **WS-L** IPC subsystem — **PORTFOLIO COMPLETE** (v0.16.9–v0.16.13). WS-K **PORTFOLIO COMPLETE** (v0.16.0–v0.16.8). WS-J1 **PORTFOLIO COMPLETE** (v0.15.4–v0.15.10). **Next: WS-M3 implementation → M4–M5, then Raspberry Pi 5 hardware binding** |
