...
2. machine-checked invariant preservation (1,304 theorem/lemma declarations),
...
| Attribute | Value |
|-----------|-------|
| **Package version** | `0.17.3` (`lakefile.toml`) |
| **Lean toolchain** | `v4.28.0` (`lean-toolchain`) |
| **Production LoC** | 41,068 across 72 Lean files |
...
| **Next workstreams** | **WS-N** Robin Hood HashMap — **N1 COMPLETED** (v0.17.1), **N2 COMPLETED** (v0.17.2: `resolveCapAddress` leaf-level occupancy fix + strengthened theorems), **N3 COMPLETED** (v0.17.3: full `Std.HashMap`/`Std.HashSet` → `KernelHashMap`/`KernelHashSet` migration across 20 source + 6 test files, bridge lemma redirection, fold argument order fix). Foundation with machine-checked bridge lemmas (14 HashMap + 6 HashSet). N4–N5 pending. **WS-M** Capability subsystem audit & remediation — **PORTFOLIO COMPLETE** (v0.16.14–v0.17.0). **WS-L** IPC subsystem — **PORTFOLIO COMPLETE** (v0.16.9–v0.16.13). WS-K **PORTFOLIO COMPLETE** (v0.16.0–v0.16.8). **Next: WS-N4–N5, then Raspberry Pi 5 hardware binding** |
