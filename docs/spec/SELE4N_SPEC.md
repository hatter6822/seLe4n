...
| Attribute | Value |
|-----------|-------|
| **Package version** | `0.33.3` (`lakefile.toml`) |
| **Lean toolchain** | `v4.28.0` (`lean-toolchain`) |
| **Production LoC** | 240,683 across 267 Lean files |
| **Test LoC** | 49,844 across 68 Lean test suites |
| **Proved declarations** | 7,918 theorem/lemma declarations (zero sorry/axiom) |
...
| **Active workstream** | **WS-SM (SMP multi-core completion) IN FLIGHT** — closes at v1.0.0 with a bootable verified SMP microkernel on Raspberry Pi 5. Current sub-phase: **SM7.F.3 round-generation-tagged descriptors LANDED (v0.32.105) — SM7.F CLOSED**: a shootdown round's deferred catch-up now drains only the rounds its *own* commit opened, so a concurrently-committed round's freshly-posted descriptors survive for its own catch-up instead of being swallowed (the SM7.B v0.32.79 model-fidelity debt).  Descriptors carry `generation : Nat`, `TlbShootdownState` a monotone `roundGeneration`, and the live seam recovers its window from the same `(pre, post)` diff it already uses; every landed SM7.A/B round theorem carries over through the exactness bridges, since under round serialisation the window drain **is** the whole-queue drain.  The Rust mirror closed a **High**-severity (once bootable) hazard: with a Boolean acknowledgment vector, a `.tlbShootdownReq` SGI left pending by an earlier round could be delivered inside a later round's `reset → publish` window, retire the *previous* round's operands and satisfy the new round's wait with that target's TLB still stale — an under-invalidation.  Acknowledgments now carry the generation they discharged (`acked_gen`, `fetch_max`), the handler latches it before any TLB work, and the reset is gone entirely along with the window it lived in.  **Audit cut (v0.32.110)**: the 12th `proofLayerInvariantBundle` conjunct (`pendingBounded`) is now carried across the live catch-up — SM7.B proved it for the single-view handler, but v0.32.81 swapped the live fold to the per-core handler and v0.32.105 restricted it to the round window, leaving the transition `completeShootdownRounds` runs without a bound proof; the capacity-bound justification ("at most one round's descriptors in flight per target") was false and corrected — the constant is sound, since the bound is maintained by fail-closed construction rather than by that counting argument.  Prior: **SM7.E tests + fixtures LANDED (v0.32.103)** — the SM7 closure phase: the four-core concurrent-unmap storm (with the per-core handler commutativity theorem `handleTlbShootdownReqOnCorePerCore_comm` it surfaced — SM7.B had proven the claim only for the single-view handler, while the live catch-up fold runs the per-core one), the cross-cluster mock on both the TLB and instruction-cache sides, the `smp_tlb_shootdown` golden trace fixture, the surface anchors, and the Tier-4 concurrent-unmap stress exerciser.  Prior: **SM7.D cache maintenance broadcast CLOSED — LANDED (v0.32.94), closure cut (v0.32.95), residual closure (v0.32.96), SECURITY fixes (v0.32.97, v0.32.100)** — v0.32.100 closes a **High**-severity (once bootable) omission on the re-type path: the scrub's zeroing stores were never cleaned to the Point of Unification, and the `IC IALLUIS` that followed guaranteed the next instruction fetch would re-read the previous owner's code from the stale PoU copy through any later executable mapping of the frame; the new `cleanRangeIallu` operand cleans exactly the scrubbed extent before the invalidate (`retypeIcacheOp_cleans_scrub_extent`), and v0.32.97 closes a **High**-severity confused deputy found via a PR #845 review and confirmed pre-existing: the syscall gate verified only that the caller held *a* capability with the required right, never that its target matched the operand, so `.vspaceMap` / `.vspaceUnmap` / `.vspaceUnifyInstruction` acted on a caller-supplied ASID and a thread holding a writable capability to its own TCB could unmap pages in any address space; `vspaceCapAuthorizesAsid` now requires the capability to name the root `resolveAsidRoot` yields for that ASID, fail-closed when unbound — the residual closure adds the `.vspaceUnifyInstruction` code-publication syscall (`SyscallId` 29, count 30), seLe4n's equivalent of seL4's `seL4_ARM_Page_Unify_Instruction`: a **pure cache** transition (`vspaceUnifyInstructionPage`, provably no page-table/TLB/object-store change) that lets a subject which *writes* instructions publish them — the dual of the destroy-side maintenance, and previously unfulfillable by user code — fail-closed on both authority legs (`.asidNotBound` / `.translationFault`), requiring the `.write` right, emitting the dedicated `.unifyPage` operand (distinct from `.ivauPage` because the clean-to-PoU must precede the invalidate) realised in the HAL as the canonical `DC CVAU` → `DSB` → `IC IVAU` → `DSB` → `ISB` page sequence, and corrects the v0.32.95 emission ledger from a single-operand join to a **coverage preorder over a list** (adding `.unifyPage` showed `iallu` is not a lattice top — `IC IALLUIS` issues no `DC CVAU`, so collapsing into it would have dropped a required clean-to-PoU; the ledger now appends and reduces only where one entry provably covers another, with `covers` grounded in the model's own effect); the closure cut fixes the `IC IVAU` page-vs-line granularity (one page operand expands to `icacheLinesPerPage` = 64 instructions, seL4's `invalidateCacheRange_I` shape) and makes the runtime emit the model's **exact** operand via the emission ledger `SystemState.pendingIcacheMaintenance` (read and cleared in the same atomic step that commits the transition), so an executable unmap emits a targeted page loop, a data-page unmap emits nothing at all, and a retype emits `IC IALLUIS` whether or not it posted a shootdown round; the data-side clean-to-Point-of-Unification dual is registered as a checked obligation (`kernelCodeWriteSites_owe_pou_clean` + tripwire) with emission scoped to SM9.E — the cache-side companion of SM7.C, closing the *instruction*-cache half of SMP-C4.  New production module `Architecture/PerCoreCacheModel.lean` mounts `SystemState.perCoreICache : Vector ICacheState numCores` and defines the per-core instruction-cache model over it: `icFetchOnCore` (the hardware fetch), `icInvalidateOnCore` (`IC IALLU` — reaches only the executing PE, and `…_icacheOnCore_ne` **states the SMP hazard**), and `icInvalidateBroadcast` (`IC IALLUIS` / `IC IVAU` — the inner-shareable broadcast), with the headline `icInvalidateBroadcast_reaches_all_cores` as the instruction-side analogue of Theorem 3.3.1.  `icacheCoherent_perCore` — on every core, every cached line still has a live *executable* mapping — is the **14th `proofLayerInvariantBundle` conjunct**, and it needs no pending-allowance disjunct because instruction-cache maintenance is a synchronous broadcast instruction rather than a queued acknowledge round.  On the data side, `dcMaintenanceAllCores` takes **no target set at all** (`dcMaintenanceByVA_reaches_all_cores`) — the absence of a reach parameter is the formal content of "`DC` by VA at the Point of Coherency is already system-wide" — and the DMA scope boundary is a machine-checked tripwire (`modeledCoherentAgents_no_dma_master`) rather than prose.  Live behind `.vspaceUnmap` (targeted `IC IVAU` for executable mappings, provably inert otherwise) and `.lifecycleRetype` (unconditional `IC IALLUIS`, since a retype re-purposes scrubbed memory whose physical lines the abstract state cannot enumerate).  Rust HAL 789 tests.  Prior sub-phase: **SM7.C per-core TLB model LANDED (v0.32.80); operative cut (v0.32.81)** — the new production module `Architecture/PerCoreTlbModel.lean` mounts `SystemState.perCoreTlb : Vector TlbState numCores` (the SMP generalisation of the scalar boot-core `tlb`, added **alongside** it — the scalar stays the pre-SMP single-core layer, `perCoreTlb` is the SMP model the SM7.B protocol drives; both cohere empty at boot) and defines the per-core model over it under the SM4.B path-a discipline (`tlbOnCore`/`setTlbOnCore` accessors + store/load algebra + frame lemmas): `tlbInsertOnCore` (the hardware page-table walker filling one core; leaves others empty — the SMP asymmetry), `tlbInvalidateOnCore` (a local invalidation reaching exactly this core; leaves others stale — the precise SMP hazard), and `tlbInvalidateOnAllCores` (runs the SM7.B `tlbShootdownBroadcast` — posting to `tlbShootdown` — **and** evolves every core's view via the protocol's `shootdownRoundViews`, so the mounted field is a genuine consumer of the shootdown state machine, not a parallel structure).  `tlbInvalidationConsistent_perCore` (∀ core, its view matches the page tables) is the **13th `proofLayerInvariantBundle` conjunct**, generalising the 9th (`tlbConsistent st st.tlb`) and threaded exactly like SM7.B's 12th (`pendingBounded`): boot witness, definitional transport through the three adapter preservation proofs (which touch only machine, and — for the context switch — scheduler.current, none of which the conjunct reads), the Boot general bridge (`bootFromPlatform_perCoreTlb_eq`), freeze wholesale.  `tlbShootdown_invalidates_perCore` mounts **Theorem 3.3.1** (`tlbShootdownBroadcast_invalidatesAllCores`) on the field — after a covering `tlbInvalidateOnAllCores` no core retains any covered entry (the SMP-C4 use-after-unmap closure).  `tlbConsistency_cross_subsystem` is the memory-subsystem capstone (protocol × TLB-model × page-tables): a covering invalidation both removes every stale entry on every core **and** preserves per-core consistency.  Carried through freeze (`FrozenSystemState.perCoreTlb` + `freeze_preserves_perCoreTlb := rfl`) / congruence (`OffSchedulerAgrees.perCoreTlb`) / boot; `perCoreTlb` kept **out** of the IF projection (a TLB view is a covert timing channel, like `machine.timer`).  Zero sorry/axiom; golden trace byte-identical; `tests/SmpTlbShootdownSuite.lean` §1 (60 `#check` anchors — the v0.32.80 landing + operative-cut/completeness/NI + PR #844 initiator-drain/capstone symbols) + §5.1–§5.2 (15 runtime assertions) + §5.3 (16, incl. the PR #844 live-catch-up initiator drain).  Round-generation-tagged descriptors (the SM7.B v0.32.79 model-fidelity debt) remains a separately-scoped follow-on (a `TlbShootdownState` descriptor-type change orthogonal to the per-core view model; no hardware hazard).  **v0.32.81 completion cut — the model made operative on the live path**: v0.32.80 landed `perCoreTlb` as a parallel spec model (the view function `shootdownRoundViews` took its arguments free-standing); the field now evolves by the round's **real per-descriptor drain**.  The new operational handler `handleTlbShootdownReqOnCorePerCore` runs the SM7.B single-view handler (`handleTlbShootdownReqOnCore`) **and** retires the *same* drained operands on core `c`'s own view, and the live `SyscallDispatchEntry.completeShootdownRounds` catch-up fold is swapped to it — **trace byte-identical**, since the per-core handler's `tlb`/`tlbShootdown` effects are *definitionally* the single-view's (`foldl_handleTlbShootdownReqOnCorePerCore_agrees`) and only the projection-invisible `perCoreTlb` additionally evolves.  Theorem 3.3.1 is now operative on the mounted field via the round the live seam runs (`shootdownRoundPerCore` + `shootdownRoundPerCore_perCoreTlb` — the real-drain view **equals** `shootdownRoundViews` — + `shootdownRoundPerCore_invalidates_perCore`), with the every-round two-model bridge `shootdownRoundPerCore_tlb_eq` (the per-core round's `tlb` = the SM7.B single-view `shootdownRound`'s always, not just at boot; the scalar `tlb` stays the all-cores-conflated model, `perCoreTlb` its per-core refinement — deliberately not pointwise-equal).  Completeness: the insert-side invariant preservation (`tlbInsertOnCore_preserves_tlbInvalidationConsistent_perCore`), the overflow-safe `tlbInvalidateOnAllCoresCoalescing`, and computable `tlbConsistentCheck`/`tlbInvalidationConsistentCheck_perCore` decision procedures.  Hardening: `FrozenSystemState.perCoreTlb` is now **required** (a silent freeze-drop is a compile error), the explicit NI witness `perCoreTlb_write_preserves_projection`, and suite §5.3 (operational round + bridge + coalescing + decidable checker).  Prior: **SM7.B shootdown protocol LANDED (v0.32.76) + completion cut (v0.32.77) + debt-closure cut (v0.32.78) — the plan-§3.2 protocol complete and LIVE, every landing deferral closed and every tracked-debt item closed or narrowed**: `Architecture/TlbShootdownProtocol.lean` (production) lands the invalidation-effect semantics on FFI encodings (`tlbEntryMatches` — the hardware's TLBI operand comparison, so the caller-side encoders `encodePageInvalidation`/`encodeAsidInvalidation` cover their entries unconditionally and collisions only over-invalidate), `tlbShootdownLocal` (B.1), `tlbShootdownBroadcast` (B.2: masked round open + posting fold + the exact `.tlbShootdownReq` SGI list, fail-closed, with posting-exactness/ack-shape/frame/capacity theorems) plus the total coalescing form behind the live wrappers (overflow collapses to a covered full flush — no request is ever lost), and the `.tlbShootdownReq` handler transitions (B.3: the TLB effect lands at the *acknowledgment*, so a set ack flag constructively means "my view is clean"; the composed handler projects onto the SM7.A `completeShootdownOnCore` and is duplicate-SGI idempotent).  **Theorem 3.3.1** (B.8, `tlbShootdownBroadcast_invalidatesAllCores`): after a covered round no core's TLB view retains any covered entry — proven over the per-core view vector `shootdownRoundViews` (closed form via idempotence) with the non-vacuity bridge to the real transitions (`handleTlbShootdownReqOnCore_applies_posted_op` + `tlbShootdownBroadcast_posts_singleton`), the unmap instantiation, and the real-pipeline corollaries (`shootdownRound_tlb_no_matching_entry` / `_quiescent` / `_allAcked`).  `TlbShootdownWait.lean`: `shootdownAck_release_acquire` (B.4 — the target's TLBI retirement happens-before the initiator's post-observation access, against the SM2.A memory model, with a concrete decide-checked witness), the constructive `shootdown_wait_loop_terminates` (B.5), and the verdict-exact `shootdown_timeout_handling` (B.6 — the runtime's fail-closed timeout panic fires only on a genuinely hung round; the 540 000-tick budget is pinned to the HAL constant on both sides).  `TlbShootdownLockSet.lean` (B.7): the cross-domain `TlbShootdownLockId` order (object < round < queue; the SM7.A audit's round-serialisation contract as theorems `object_lt_round`/`round_lt_queue`), `lockSet_tlbShootdown_correct` (strictly ascending — the SM3 lock-ladder shape), duplicate-freedom, and footprint honesty against the live commit's diff-recovered write set (`lockSet_tlbShootdown_covers_commit`) — the registered SM7.B.7 obligation discharged.  **Wired callers** (B.9–B.11): the live `.vspaceUnmap`/`.vspaceMap` arms route through `vspaceUnmapPageWithShootdown`/`vspaceMapPageCheckedWithShootdownFromState` (caller's core via `determineExecutingCore`; the WS-K-D delegation theorems and the AC4-D enforcement-boundary registry updated in lockstep); `tlbFlushBy{ASID,Page}WithShootdown` cover the targeted-flush ops; `asidAllocateWithShootdown` is the previously-missing kernel-level consumer of `AsidPool.allocate.requiresFlush` (B.10); `lifecycleRetypeDirectWithCleanupShootdown` (B.11, live behind `.lifecycleRetype`) closes a genuine pre-existing gap — retyping a live VSpaceRoot destroyed an entire address space with **no TLB maintenance at all**; the dead ASID is now flushed locally and `.aside1`-broadcast (non-VSpaceRoot retypes provably unchanged).  B.12: `tlbShootdown_outer_correct` — the round is state-identical under `.outer` sharing (only the emitted instruction variant changes; the live entry's domain is `rfl`-pinned to the RPi5 binding).  **Live seam**: `SyscallDispatchEntry.completeShootdownRounds` — after each syscall commit the diff-recovered round (`shootdownChangedTargets`/`shootdownPostedOps`) runs under THE global round lock (realised as the CAS try-lock `SHOOTDOWN_ROUND_LOCK`, acquired cooperatively — a lock-waiter services its own pending shootdown obligation between retries, since a blind spin with IRQs masked could never satisfy the in-flight round waiting on it): Rust masked flag reset, `.tlbShootdownReq` SGIs at **online** targets only (the SM7.A P1 obligation), the initiator's `tlbiForSharing` broadcast TLBIs, the bounded `allAcked` wait (timeout ⇒ fail-closed panic), and the handler catch-up commit restoring quiescence; `TlbiForSharing` is promoted to production (staged partition 57 → 56).  **Rust HAL 755 → 769** (clippy-clean): the round lock, the deadline-exact `wait_all_acked_bounded`, the boot-registered `.tlbShootdownReq` handler (local `tlbi vmalle1` → release `ack_set` → `sev`; fail-closed no-ack on a bad core id), `online_mask`, and the `gic::dispatch_irq_with_iar` trap refactor — closing the SM1.F deferred-SGI-dispatch note with genuine `source_cpu` and fixing a pre-existing GICv2 defect (SGI `GICC_EOIR` writes must echo the IAR's source-CPU field per GIC-400 TRM §4.4.5; the masked EOI stranded per-source SGI instances active on any multi-core build).  Golden trace byte-identical; zero sorry/axiom; `tests/SmpTlbShootdownSuite.lean` 81 → 150 assertions / 20 groups (Theorem 3.3.1 computed over per-core views; the live map → unmap → shootdown pipeline on a built VSpace state).  **v0.32.77 completion cut — every landing deferral closed**: `pendingBounded st.tlbShootdown` joins `proofLayerInvariantBundle` as the **12th conjunct** (boot witness + the three adapter preservation proofs + the Boot general bridge via `bootFromPlatform_tlbShootdown_eq`; carriage proven through every live shootdown-aware transition on a new `…_tlbShootdown_eq` frame family); handler commutativity (`handleTlbShootdownReqOnCore_comm` — catch-up order is a convention) + the coalescing-round capstones (`coalescingRound_restores_quiescent`/`_allAcked`) + the positive diff characterization (`shootdownChangedTargets_coalescing_of_quiescent`) + Theorem 3.3.1's total-posting remote case; remap-only map rounds with the ok-implies-fresh model fact (`vspaceMapPageCheckedWithFlushFromState_ok_fresh` — a successful map is always fresh, so the map path never posts today and the round rides the unmap of unmap-then-map); the least-index wait (`waitAllAckedBounded_least`) + the round-lock CAS model + the cross-round publication chain (`shootdownRoundLock_release_acquire`, with the 4-core multi-pair B.4 witness); entry hardening (named `shootdownRoundLockAcquireFuel`, one `CORE_READY` snapshot per round, the vmalle1-dominance `collapseShootdownOps`, `shootdownSharingDomain` derived from `PlatformBinding.sharingDomain`, the local self-service `tlbiLocalFullFlush`); the CSpaceAddr retype sibling `lifecycleRetypeWithCleanupShootdown` (the storeObject-sweep closure); Rust `_in` handler/lock forms with genuine false→true ack tests + an 8-thread CAS mutex stress (HAL 769 → 772); suite 22 groups / 160 runtime assertions incl. the live `.vspaceUnmap` `dispatchSyscall` scenario; `scripts/test_qemu_smp_shootdown.sh` seeded (Tier-4-registered; SKIPs until the SM9.E image).  **v0.32.78 debt-closure cut — every SM7.B tracked-debt item closed or narrowed**: the `.tlbShootdownReq` handler retires the round's EXACT operands per-descriptor (`tlb::tlbi_local`) instead of a blanket `vmalle1`, matching the Lean `handleTlbShootdownReqOnCore` — the initiator publishes the collapsed operands into a seqlock-guarded `ShootdownOpMailbox` under the round lock before the SGIs, and the handler retires a stable snapshot per-descriptor with a fail-safe local `vmalle1` fallback on any torn read / overflow / undecodable operand (HAL 772 → 780, trace byte-identical); the formal refinement narrows to operand-for-operand (residual: the SM9.E linked-runtime proof); B.10 is a confirmed no-safety-gap completeness deferral (no runtime ASID-reuse path exists) with closure target SM8; step-4d direct-ack is closed by design (the spin wait + masked SVC path make it informationless); the `withLockSet` shootdown slice is closed (`withLockSet_preserves_pendingBounded`); and the host-test starvation flake is closed (host-test yields already present via `cpu::wfe()`; the authoritative `test_rust.sh` builds before testing; the mutex-stress test caps contenders at `available_parallelism()`).  **v0.32.79 PR #839 review-P1 cut**: the shootdown target mask now reads the IRQ-serviceable `smp::CORE_IRQ_READY` flag (published by the secondary after `enable_irq`) for both the round reset and the SGI targets, rather than the primary's `CORE_READY` release handshake — so a released-but-not-IRQ-ready or timer-dead secondary can no longer hang a round into the SM7.B.6 fail-closed panic (HAL 780 → 782; Lean FFI-backed, docstring-only); the concurrent posting/catch-up round-lock finding is recorded as model-fidelity tracked debt (no hardware hazard — each round's TLB maintenance rides its own `(pre,post)` diff + blocking ack wait), closure target the SM7.C round-generation model change.  Prior: **SM7.A shootdown descriptor + state LANDED (v0.32.72); completion cut (v0.32.73)**: the SM7 state layer (plan §5, all six sub-tasks).  **v0.32.73 completion cut**: the state is **mounted in the kernel state** — `SystemState.tlbShootdown : TlbShootdownState := .initial` (the plan §4.1 "ConcurrencyState" placement, the SM3.A.10 `objStoreLock` precedent; `default_tlbShootdown_{initial,quiescent,pendingBounded}`), with the pure `TlbInvalidation` operand module extracted from the staged `TlbiForSharing` so `Model/State` stays FFI-free, and `Architecture.TlbShootdown` promoted to production (partition 58 → 57); the plan §4.1 capacity argument is formal (`beginRound_foldlM_enqueueShootdown_isSome` + the round capstone `shootdownRound_restores_quiescent` — a completed round restores quiescence, closing the cross-round induction); a total overflow escape hatch `enqueueShootdownOrCoalesce` collapses a full queue to one full-flush descriptor (never loses a request, unconditionally bounded); the per-core `ShootdownQueueLockId` (decidable total order, ascending-core acquisition) is the ready SM7.B.7 lock-set seam; and the ack-flag FFI seam is live (`ffi_shootdown_*` + typed `CoreId` wrappers + `shootdownAck_ffi_core_in_range`; HAL 743 → 750 tests).  Suite 51 → 73 assertions / 11 groups.  **v0.32.74 audit cut**: three-lane adversarial audit — the round-serialisation contract is corrected (the ack vector carries no round identity ⇒ rounds must be serialised system-wide; the §3.2 VSpaceRoot-lock precondition is insufficient across distinct VSpaces; the new provably-unique `ShootdownRoundLockId` is the registered SM7.B.7 serialiser, all claim sites restated) and the coalescing coverage is completed (`enqueueShootdownOrCoalesce_pending_covered`); suite 73 → 75 assertions; everything else verified sound.  **v0.32.75 (review P1 — offline-core masking)**: an offline core's never-set ack flag can no longer hang a round — the Rust `reset_for_round` masks by `CORE_READY` (offline cores born-acknowledged; safe because every secondary executes `tlbi vmalle1` in `init_mmu_secondary` before enabling its MMU, so a core that comes online mid-round holds no stale translation the round could have missed), the Lean model mirrors it with `beginShootdownRoundFor` (target-masked round: `beginShootdownRoundFor_ackOnCore_iff`, the hypothesis-free masked capstone `shootdownRoundFor_restores_quiescent`, and `beginShootdownRoundFor_allCores_eq` collapsing the all-targets case to the unmasked round), and the SM7.B obligations extend to online-only target sets (no rounds concurrent with core bring-up); suite 75 → 81 assertions / 12 groups, HAL 743 → 755.  `SeLe4n/Kernel/Architecture/TlbShootdown.lean`: `TlbShootdownDescriptor` (the typed SM1.E.4 `TlbInvalidation` operand — one descriptor type covers the SM7.B.9 page-unmap, SM7.B.10 ASID-retire, and SM7.B.11 full-flush callers — plus the initiating `CoreId`); `TlbShootdownState` with `pendingShootdowns : Vector (List TlbShootdownDescriptor) numCores` + `shootdownAck : Vector Bool numCores` under the SM4.B path-a accessor discipline (`@[simp]` store/load algebra, `ext_perCore`, quiescent boot state); `enqueueShootdown` (FIFO tail-append, **fail-closed `none` at the `maxPendingPerCore = 16` bound** — a silently dropped descriptor is the SMP-C4 stale-TLB hazard; success ⟺ below-capacity); `drainShootdowns` (the `.tlbShootdownReq` handler's whole-queue FIFO drain — the completeness half of Theorem 3.3.1's remote case; exhaustive; deliberately ack-free so the runtime retires TLBIs before acknowledging); `acknowledgeShootdown` (monotone) + `beginShootdownRound` (initiator born-acknowledged, targets down — `beginShootdownRound_ackOnCore_iff`) + the decidable `allAcked` / `shootdownQuiescent` / `pendingBounded` predicates, `…_preserves_pendingBounded` across every operation, and the SM7.B.5 wait-loop-termination anchor `allCores_foldl_acknowledgeShootdown_allAcked`.  Rust realisation `rust/sele4n-hal/src/shootdown.rs` (HAL 724 → 743 tests, clippy-clean): `SHOOTDOWN_ACK` — one cache-line-aligned `AtomicBool` per core, booting quiescent all-`true` — with `ack_set` (Release, the SM7.B.4 release edge) / `ack_is_set` + `all_acked` (Acquire) / `reset_for_round` (Relaxed; publication rides SM1.F.8's dsb-before-SGIR), fail-closed bounds panics, `_in_slice` testable forms.  Zero sorry/axiom.  Tests: `tests/SmpTlbShootdownSuite.lean` (`smp_tlb_shootdown_suite`, the SM7.E.1 seed — 51 assertions / 7 groups incl. the 16-fill / 17th-reject / drain-restores capacity walk and a full 4-core state-level shootdown round trip ending quiescent), Tier-2 + Tier-3 wired.  Plan: [`docs/planning/SMP_TLB_SHOOTDOWN_PLAN.md`](../planning/SMP_TLB_SHOOTDOWN_PLAN.md) §5.  Prior: **SM6.F tests + fixtures LANDED (v0.32.67); depth cut (v0.32.68) — SM6 (A–F) complete** (depth cut v0.32.68: the aggregates add SchedContext donation round trips, capability transfer, info-flow-checked dispatch, the live `dispatchSyscall` `.call` path, cancellation×IPC composition, scheduler contention, a three-waiter drain, and the review-#3 bound-signal badge-leak gate — IPC suite 80→130 assertions/14 groups, notification 58→76/10): the SM6 closure phase (plan §SM6.F, all six sub-tasks), closing the two remaining substantive §8 acceptance-gate items on the live operations.  `tests/SmpIpcSuite.lean` (`smp_ipc_suite`, 130 assertions / 14 scenario groups + the golden-trace check): end-to-end pipelines composing the SM6.A/SM6.C transitions with the SM5 per-core scheduler (`handleRescheduleSgiOnCore` dispatch on the SGI's target core) — the **2-thread cross-core call/reply round trip** and the **4-thread SMP rendezvous** (two interleaved client/server pairs across all four cores, cross-pair framing + payload isolation), plus the cross-core send/receive rendezvous, client-first ordering, the server `endpointReplyRecvOnCore` steady-state loop, fail-closed error paths, state-resolved 2PL footprints, and live-dispatch coherence.  `tests/SmpNotificationSuite.lean` (`smp_notification_suite`, 76 assertions / 10 groups): the wait → cross-core signal → SGI → handler-dispatch round trip, multi-waiter head-first drain (per-waiter home cores + badge isolation), `Badge.bor` accumulation + the non-blocking consume, the remote bound-TCB delivery round trip, the bind/unbind lifecycle, error paths, and independence framing.  `tests/fixtures/smp_ipc_4core.expected` (+ `.sha256`): the deterministic 16-line `[smp-ipc-4core]` golden trace, byte-for-byte verified in-suite and auto-gated by the Tier-2 companion walk.  `scripts/test_qemu_smp_ipc.sh`: the Tier-4 QEMU `-smp 4` handshake exerciser (SKIPs until the SM9.E bootable image, the SM5-sibling discipline).  Surface anchors: in-suite `#check` blocks + Tier-3 grep anchors.  The plan §8 acceptance gate is fully checked.  Prior: **SM6.E cancellation across cores LANDED (v0.32.60; completed v0.32.61; PR-review cuts v0.32.62–65; audit closure v0.32.66)** — the suspend pipeline's cancellation sub-operations lifted to SMP in the production `SeLe4n/Kernel/IPC/CrossCore/Cancellation.lean`: `descheduleThread` (the SM5.C `wakeThread` dual — home-core deschedule via `removeRunnableOnCore`, `.reschedule` SGI iff the victim was actively current on a remote home core, ghost-guard mirrored), the cross-core composite `cancelIpcBlockingOnCore` (the suspend pipeline's G2+G4+G7 slice across cores: single-core object teardown + home-core deschedule + remote poke), and the per-core donation cancellation (`cancelBoundDonationOnCore` purges the SchedContext's replenishments from the **victim's home core's** queue per SM5.H affinity-consistency, fixing the single-core arm's `bootCoreId` hardcode; `cancelBoundDonationOnCore_bootCoreId` is the `rfl` bridge) — under the plan §3.1 lock-set footprints `lockSet_cancelIpcBlocking` / `lockSet_cancelDonation` (state-resolved forms, `permittedKinds .tcbSuspend` consistency, member-by-member coverage by the reply-extended `lockSet_tcbSuspend` — closing the SM6.D reply-fold footprint gap: `permittedKinds .tcbSuspend` += `.reply`, `lockSet_tcbSuspend` += optional `consumedReplyId`, 5-extension footprint of size 8 = `maxLockSetSize`).  2PL atomicity: `cancelIpcBlocking_atomic_under_lockSet` / `cancelDonation_atomic_under_lockSet` (exact plan names) + the OnCore companions.  Flagship `cancellation_cross_core_correct`: remote poke ∧ full home-core deschedule ∧ per-core locality ∧ object-level fidelity.  `objects.invExt` preservation down the whole cancellation surface (incl. the store-sweep helpers and the de-privatised `returnDonatedSchedContext_preserves_objects_invExt`).  Axiom-clean; `tests/SmpCancellationSuite.lean` (107 runtime assertions, 17 scenario groups, Tier-2 + Tier-3 wired).  **v0.32.61 completion cut** (full record: plan §SM6.E completion note): the live `.tcbSuspend` cross-core dispatch is CLOSED — `suspendThreadOnCore` (the complete per-core suspend, `resumeThreadOnCore`'s mirror) behind the `.tcbSuspend` arm, the diff seam's new descheduled-current SGI rule (`crossCoreSgiBody_remote_deschedule`, single-core inert), and the AN9-D bracket flipped to the commit-then-fire FFI seam `suspend_thread_cross_core` (trace byte-identical, Rust HAL 724 green); cancellation NI LANDED as the staged `CancellationNI` module (every SM6.E-new effect substantive; the blocked-arm composites consume exactly the AK6-F.18 single-core teardown projection obligation); the `ipcInvariant` conjunct of the whole bundle is CLOSED across the entire cancellation surface (fold keystone `fold_preserves_of_lookup`, the state-correcting `notificationQueueWellFormed_filter_correct`, seventeen `_preserves_ipcInvariant` lemmas) with the remaining `ipcInvariantFull` conjuncts tracked; the pre/post-resolution frames are discharged closed-form (`cancelIpcBlockingOnCore_eq_descheduleThread_closed`, `invExt`-only).  Plus observational atomicity (the guarded observer layer + the victim-`ipcState` capstone `cancelIpcBlockingOnCore_observer_atomic`), the donated-arm replenishment migration to the owner's home core, the SM5.H purge/placement corollaries, and fixes for two pre-existing single-core defects (the notification sweep's sole-waiter `.waiting`-with-`[]` invariant break; `suspendThread`'s dead post-G4 G7 reschedule guard).  **v0.32.62 PR-review cut (PR #831 P2 + root causes)**: a third pre-existing single-core defect fixed — the suspend pipeline's PIP revert ran at the *victim before* the teardown cleared its `ipcState` (a fixed-point no-op: a server retained the suspended victim's donated priority indefinitely); both suspend forms now use `timeoutThread`'s D4-N capture → clear → revert-from-server order, the per-core suspend reverts via the SM5.F.4 cross-core walk (`propagatePipChainCrossCore`, per-home-core bucket migration), the `suspend_thread_cross_core` FFI entry fires the diff-recovered SGIs (`computeCrossCoreSgis` — remote re-bucketing pokes ride the diff), and the chain-walk obligation is declared (`pipChainStart_tcbSuspend`; SM3.B inventory 99, chainStart 4).  **v0.32.63 (review 2) — disinheritance scheduling points**: a suspend deboosting a *still-current* server now runs the gated local preemption point when the executing core's own current thread lost effective priority (`currentEffectivePrio?`/`currentDeboostedFrom`; the G7 dispatch factored into `suspendRescheduleOnCore` with arm-level SGI lemmas), and `crossCoreSgiBody` gains the deboosted-current rule (`crossCoreSgiBody_remote_deboost_current`) poking a remote core that keeps running a deboosted server; raises fire nothing, single-core inert.  **v0.32.64 (review 3) — scheduler-lock footprint closure**: `suspendThreadOnCoreSchedLockSet` gains the executing core's run-queue write lock (shared `sortedSchedCorePair` segment shape), the SM3.C.11 chain-walk contract requires each step to hold the member's TCB + home-core run-queue write locks (state-discovered re-bucketing, outside any static footprint), and the preemption gate's TCB-field comparison is documented sound under `boundThreadPriorityConsistent`.  **v0.32.65 (review 4) — running-core suspend + write-set honesty**: an unbound victim still current on a secondary core is now descheduled and poked on THAT core (`runningCoreOf?`; the diff seam's current rules re-keyed to the pre-state running core), and the cancellation sweeps insert only changed objects with the splice's neighbour-TCB writes declared footprint members (`cancelSpliceNeighbors?`).  **v0.32.66 (audit closure)**: the running-core lock in the suspend scheduler footprint, the EDF deadline dimension in the diff rules, the `currentThreadUniqueAcrossCores` invariant slice, the AK6-F.18 projection-sketch correction, and the donation-side observer capstone; the three-auditor audit found no CVE-class issues (authority chain sound, all error arms fail-closed).  Prior sub-phase: **SM6.D IPC across-core invariant bundle LANDED (v0.32.58; completed v0.32.59)** — the full IPC invariant bundle restricted to per-core views and proven preserved by every IPC operation, and (v0.32.59) by every **cross-core** transition and the capability-carrying trio: the production transfer layer `SeLe4n/Kernel/IPC/Invariant/LookupCongruence.lean` (per-conjunct pointwise-lookup congruences + the `OffSchedulerAgrees` relation and step congruences) feeds per-operation agreement dichotomies (`…_post_agrees`), giving unconditional `…_preserves_ipcInvariantFull{,_perCore}` for `notificationSignalOnCore`/`notificationWaitOnCore`/`endpointReplyOnCore`/`endpointReceiveDualOnCore` and the compositionally-closed `endpointReplyRecvOnCore` (disjunctive reply-id premise covering seL4-MCS one-object reuse), plus `endpointSendDualWithCaps`/`endpointReceiveDualWithCaps`/`endpointCallWithCaps_preserves_ipcInvariantFull_perCore` behind the live `.send` dispatch; remaining tracked debt (bound-delivery whole-bundle; `withLockSet` bundle carriage) is recorded in the plan's SM6.D scope note.  `ipcInvariantFull_perCore` (`SeLe4n/Kernel/IPC/Invariant/PerCoreBundle.lean`, production) mirrors the twenty-conjunct `ipcInvariantFull` one named field per conjunct: thread-subject conjuncts restrict to the threads homed on core `c` (`threadHomeCore` = `cpuAffinity` defaulting to `bootCoreId`, provably the operational wake target — `determineTargetCore_eq_threadHomeCore`); shared-object clauses (notification well-formedness, badges, per-endpoint dual-queue well-formedness, head-disjointness, the backward reply-linkage clause) are carried whole in every core's view; the scheduler-reading `passiveServerIdle` uses the SM4.D per-core form.  The decomposition is exact (`ipcInvariantFull_smp_iff_full_and_passive_smp`), the four plan-named conjuncts (SM6.D.3–D.6) ship per-conjunct exactness iffs, and the boot state satisfies every core's view (`default_ipcInvariantFull_smp`).  Preservation (SM6.D.2, `PerCoreBundlePreservation.lean`, production): send / receive / call / reply (+ replyRecv) / signal / wait each preserve every core's bundle view, plus the cross-core call flagship `endpointCallOnCore_preserves_ipcInvariantFull_perCore` (staged with SM6.A) — nineteen conjuncts ride the single-core whole-bundle theorems through the exact-decomposition bridges, and the per-core `passiveServerIdle` slice rides a new core-parameterised pullback-frame family (`passiveServerIdleFrameOnCore`) built on the SM4.B per-core independence algebra with **no idle-core assumption**.  All axiom-clean; tests `tests/SmpCrossCoreCallSuite.lean` §SM6.D + Tier-3 anchors.  Prior sub-phases: SM6.C reply path across cores (v0.31.77, live `.reply`/`.replyRecv` dispatch), the SM6.D reply-object hardening series (v0.31.115→v0.32.57: per-object reply locks, `replyCallerLinkage`, server-first receive stash, capability-authorized `.reply`, the atomic reply fold), SM6.B notifications across cores (v0.31.68→76, live bound-notification delivery), SM6.A endpoint call across cores (v0.31.65→67, live cross-core `.call` + SGI seam). Plan: [`docs/planning/SMP_CROSS_CORE_IPC_PLAN.md`](../planning/SMP_CROSS_CORE_IPC_PLAN.md) §5. |
...
#### 11.2.4 Per-core observers under SMP (WS-SM SM8.A)
...
Not yet covered at SM8.A: per-core *preservation* of non-interference
across transitions (`crossCoreNonInterference`), the lock-contention
timing channel, and per-core declassification audit. These are WS-SM
SM8.B–SM8.D; see
[`docs/planning/SMP_INFORMATION_FLOW_PLAN.md`](../planning/SMP_INFORMATION_FLOW_PLAN.md).
