...
| Attribute | Value |
|-----------|-------|
| **Package version** | `0.33.1` (`lakefile.toml`) |
| **Lean toolchain** | `v4.28.0` (`lean-toolchain`) |
| **Production LoC** | 240,363 across 267 Lean files |
| **Test LoC** | 49,395 across 68 Lean test suites |
| **Proved declarations** | 7,897 theorem/lemma declarations (zero sorry/axiom) |
...
1. **The observable state's components partition into shared and
   ...
   restricted to the observer's core. The partition is total by
   construction: `ObservableState.ext_fragments` rebuilds an observable
   state from its two fragments, so a component added to `ObservableState`
   and registered in neither fragment fails to compile.
...
3. **Visibility is monotone in clearance.** `onCore_label_monotone`
   proves that raising the observer's clearance can only widen what it
   sees, over the `ObservableState.visibilityLe` preorder. This is a
   visibility order rather than component equality, because a wider
   clearance may reveal more of an object it can already see: CNode slot
   redaction is the only observer-dependent part of object projection
...
components remain unconditionally visible to every observer under
scheduling transparency (`onCore_schedulingTransparency`), and under SMP
...
decides a strictly weaker *slice* relation, and the strictness is proved
rather than asserted (`perCoreSlice_erases_register_content`,
`perCoreSlice_erases_shared_content`), so a decided slice equality can
never be read as observable equality.
