...
| Attribute | Value |
|-----------|-------|
| **Package version** | `0.33.37` (`lakefile.toml`) |
| **Lean toolchain** | `v4.28.0` (`lean-toolchain`) |
| **Production LoC** | 266,029 across 279 Lean files |
| **Test LoC** | 56,226 across 69 Lean test suites |
| **Proved declarations** | 8,910 theorem/lemma declarations (zero sorry/axiom) |
...
- Policy-checked wrappers (`endpointSendDualChecked`, `cspaceMintChecked`, `registerServiceChecked`) exercised by default in trace and probe harnesses. `enforcementBoundary` classifies 40 operations (12 policy-gated, 24 capability-only, 4 read-only; pinned by `enforcementBoundaryExtended_count`). Includes SchedContext ops (WS-Z8), thread lifecycle (D1), priority management (D2), IPC buffer (D3), VSpace/service ops (AC4-D), the live declassification entry point (WS-SM SM8.C) and the two-phase-locking bracket (WS-SM SM8.E.3). (WS-Q1: `serviceRestartChecked` removed, `registerServiceChecked` added — service lifecycle simplified to registry-only model.)
