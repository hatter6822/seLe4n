...
| **Production LoC** | 302,288 across 296 Lean files |
| **Test LoC** | 65,174 across 69 Lean test suites |
| **Proved declarations** | 10,042 theorem/lemma declarations (zero sorry/axiom) |
...
**De-threading status** (WS-RR RR3.1–RR3.14, v0.34.43).  Every
...
bound on a post-state across all fifty-nine such statements, with the conjunct
set, the bundle family and each bundle's pre-state all derived from the sources
rather than listed.  The pre-state conditions that replaced them are collected
and, where possible, *derived* in
`SeLe4n/Kernel/IPC/Invariant/Reachability.lean` (`ipcReachable`, proved
inhabited by the boot state).  What does **not** yet exist is a top-level
dispatch payoff — `dispatchWithCap_preserves_ipcInvariantFull` and
`dispatchSyscall_preserves_ipcInvariantFull` — so `ipcInvariantFull` is not yet
claimed as an end-to-end machine-checked property of a syscall; both are
registered with closure targets in
`docs/planning/ipc_dethreading_pending.txt`, which the gate checks in both
directions.
