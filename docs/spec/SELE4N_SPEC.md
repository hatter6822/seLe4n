...
| Attribute | Value |
|-----------|-------|
| **Package version** | `0.30.4` (`lakefile.toml`) |
...
| **Active workstream** | **WS-AK Phase AK9 COMPLETE** (v0.30.4) — Platform / Boot / DTB / MMIO. 8 sub-tasks (AK9-A..AK9-H): **AK9-A** `mmioRead32` / `mmioRead64` with 4/8-byte alignment + region-local range checks (P-H01); **AK9-B** `BootBoundaryContract.objectStoreNonEmpty` renamed to `objectStoreEmptyAtBoot` matching its true semantic (P-H02); **AK9-C** `irqHandlersReferenceNotifications` validates each IRQ handler ObjId refers to a notification (P-M01); **AK9-D** `isDeviceRangeWithinRegion` tightens multi-byte writes to stay inside a single `.device` region (P-M02); **AK9-E** `budgetSufficientCheck` fails closed on missing / wrong-variant SchedContext bindings (P-M03); **AK9-F** DTB + MachineConfig validation — `classifyMemoryRegionChecked`, `applyMachineConfigChecked` (rejects zero/>52 PA width), `findMemoryRegPropertyChecked` with Except fuel propagation (P-M04/M05/M07); **AK9-G** `bootEnableInterruptsOp` + `bootFromPlatformWithInterrupts` mirror the HAL Phase-3 IRQ re-enable (P-M06); **AK9-H** LOW-tier batch documentation for 13 accepted gaps / resolutions (P-L1..P-L13). 21 regression tests in `tests/Ak9PlatformSuite.lean` covering every sub-task. Plan: [`AUDIT_v0.29.0_WORKSTREAM_PLAN.md`](../audits/AUDIT_v0.29.0_WORKSTREAM_PLAN.md) §12. Portfolio AK1..AK10 targeted at v1.0.0 release (AK1–AK9 complete; AK10 in progress). Prior: WS-AM (v0.30.0), WS-AJ (v0.28.1–v0.29.0), WS-AI (v0.27.7–v0.28.0), WS-AH (v0.27.2–v0.27.6), WS-AG–WS-B. **Next: WS-AK Phase AK10 (Testing, Documentation & Closure).** |
