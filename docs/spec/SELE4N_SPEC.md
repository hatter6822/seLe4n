...
seLe4n is built in incremental slices, each landing its transitions together
with the proofs that cover them: a live kernel transition never lands ahead of
its own invariant surface.  Every merged PR ships its own patch version and its
own [`CHANGELOG.md`](../../CHANGELOG.md) entry, so the documented state and the
tree never diverge by more than one commit.

The per-slice record is [`docs/REGISTERED_DEBT.md`](../REGISTERED_DEBT.md);
the working rules are [`docs/DEVELOPMENT.md`](../DEVELOPMENT.md).
...
| **IPC queuing** | Intrusive linked list | Dual-queue model (`sendQ`/`receiveQ`) with O(1) arbitrary removal; `blockedOnCall` state for call/reply semantics; reply-target scoping for confused-deputy prevention; formal `dualQueueSystemInvariant` with doubly-linked integrity (WS-H5) and back-pointer agreement — `queuePPrev` against `queuePrev`, which is what makes the O(1) removal's own precondition discharged rather than assumed (`dualQueueRemovalGuardHolds`, WS-RR RR8.3).  That pairing is *pointwise*, so it cannot reach a queue's **head**, whose `queuePrev = none` is legitimate; the head's back-pointer is therefore a clause of the **queue's** own boundary (`intrusiveQueueWellFormed`'s P2, `v0.35.106`), preserved through a fifth bundle conjunct that a thread heads at most one endpoint queue (`endpointQueueHeadDisjoint`) — itself a consequence of `queueHeadBlockedConsistent` rather than a new assumption (`queueHeadExclusive`), so a removal's `queuePPrev` hypothesis is now discharged for **any** member of a well-formed queue, head included (`dualQueueRemovalGuardHolds_of_member`).  Both the pairing and the head-disjointness conjunct are **checked at runtime** as well as proved (`queuePPrevAgreesWithPrevChecks`, `endpointQueueHeadDisjointChecks`); the second arrived a cut later than its conjunct, and until `v0.35.108` no check in `stateInvariantChecksFor` was cross-endpoint at all, so a state the bundle refuses — two endpoints sharing one singleton head — passed the whole harness surface; all three removals write one definition of what unlinking does — `queueUnlinkPredecessor` / `queueUnlinkSuccessor` to the neighbours (WS-OD OD3.9) and `queueRemoveBoundary` to the queue's own head and tail (WS-RR RR8.4), the latter reading `q.tail = some tid` rather than inferring tail-ness from the removed thread's `queueNext`, with the two readings' agreement checked by the removal's guard.  The removal's whole store-free precondition is one **model**-level predicate over an `IntrusiveQueue` and a `TCB` (`dualQueueRemovalEnabled`, `Model/Object/Types.lean`), so the frozen execution mirror — which imports no kernel module — refuses exactly what the live removal refuses rather than a subset of it (`v0.35.59`); its *discharges* stay in the kernel layer, where the invariants that entail them are |
...
A specification says what the kernel is, not how it got there.  The record of
completed workstreams — scope, versions and evidence — is
[`docs/REGISTERED_DEBT.md`](../REGISTERED_DEBT.md); what each version
changed is [`CHANGELOG.md`](../../CHANGELOG.md); what is in flight and what
blocks it is that file's *Current status*.
...
### 6.2.1 DTB peripheral-discovery depth
...
- **The arena is a link-time constant.**  `link.ld` places a `NOLOAD` section
  ...
  kernel's reserved extent (§6.2.8).  No firmware value or device-tree field
  sizes it.  It lies in the RAM the constant boot map covers (§6.2.7), so translation is never enabled over tables that leave it unmapped.
...
- **The map.**  `boot_mapping_for(addr, layout)` is a function of the address
  ...
  in **every** configuration's Lean map, and the Normal window to equal the
  kernel's extent the table declares.  All other RAM, the rest of the first
  gigabyte included, is mapped after the verified Lean parse has chosen the
  board (BP4.6, §6.2.12): before that it is unmapped, a lost resource rather
  than a false claim.
- **W^X at EL1.**  Inside the kernel's extent:
...
- **The section boundaries are the linker's.**  They are assigned inside the
  ...
  mapped executable.  Its RAM region ends at `KERNEL_RESERVED_END`, which an
  `ASSERT` holds since BP7.10.
...
- **Where it runs.**  `lean_entry::enter_lean_kernel` calls
  ...
  identity map; `link.ld` places the image in the kernel's reserved extent,
...
The boot map (§6.2.7) is built from constants before anything is parsed, so it
covers only the kernel's reserved extent `[0, 256 MiB)` (since BP7.10; the first
gigabyte before it).  All other RAM a board has is mapped once the verified
Lean parser has read the device tree and the binding has chosen the
configuration.

- **The extent is derived.**  `bootRamExtensionsOf map` is every RAM region of
  `map` reaching past `rpi5KernelReservedEnd`, clipped to it and non-empty
  (`bootRamExtensionOf?`; clipped at `rpi5GuaranteedRamTop` until BP7.10).
  `mem_bootRamExtensionsOf` (every extension is RAM of the map, outside the
  extent, non-empty) and `bootRamExtensionsOf_covers` (every RAM address of the
  map outside the extent is in one) state that it is exactly that RAM.  The
  ...
  `rpi5BootRamExtensions_values` evaluates the five uncut variants —
  `[256 MiB, 1 GiB)` on every board, and on every board above 1 GiB a second
  region `[1 GiB, ramSize)`, the BCM2712's DRAM being contiguous from 0
  (§6.2.14).  `rpi5BootRamExtensions_admissible` proves, over every admissible
  configuration, that each is one the HAL accepts: non-empty, 2 MiB aligned, past
  the extent, inside the tables' 512 GiB reach, and either inside the first
  gigabyte or whole gigabytes.
...
- **What the HAL writes.**  `mmu::extend_boot_tables` decides every refusal
  ...
  level-1 block per whole gigabyte and a 2 MiB block per block in a gigabyte
  with a level-2 table (`level2_table`: the first gigabyte since BP7.10, and
  the device window's) — each Normal, writable and never executable, and only
  into entries the tables leave invalid.  A partial gigabyte elsewhere is
  refused, and so is a range starting inside the kernel's extent
  (`InsideKernelReserved`).  Because no valid
  ...
  `is_boot_cacheable_range` is the union of the kernel's extent and the record
...
- **Driven, not mirrored.**  `tests/fixtures/boot_map.expected` carries each
  configuration's `extend` lines — the five variants and three firmware-cut
  configurations.  The HAL's `the_boot_map_agrees_with_the_lean_map` applies
  them and requires the extended Normal window to be exactly that
  configuration's RAM on every configuration, both by walking the tables and
  through `ram_range_covered`.
- **Handed to the root task** (WS-BP BP4.7).  The deployment's objects are a
  ...
  `initialObjectsFor`).  `rpi5RootTaskUntypeds v` is one normal-memory untyped
  per extension, at ids `6, 7, …` and root-CNode slots `5, 6, …` (since BP7.10,
  which retired the two fixed first-gigabyte untypeds), and
  `rpi5RootTaskUntypeds_regions` states its regions **are**
  `rpi5BootRamExtensions v`.  Every gate holds over every admissible
  configuration (§6.2.17), and
  ...
  `rpi5DeploymentBootStateAt_untypedInstalled` says the boot installs each.
...
- **DRAM is contiguous from 0** on every variant (`axi` `ranges` map
  ...
  `[0, ramSize)` (cut in the first gigabyte to the firmware's account since
  BP7.10, §6.2.17) and the boot maps at most one extension above the first
  gigabyte.
...
### 6.2.17 The firmware's memory account (WS-BP BP7.10, v0.36.3)

A Raspberry Pi 5's firmware writes `/memory@0` as three ranges — `[0, 0x80000)`,
`[0x80000, 0x3FC00000)` and `[0x40000000, top)` on a Pi 5 8 GiB Rev 1.1
(`[0x80000, 0x3FB00000)` on a CM5 4 GiB Rev 1.0; accounts read 2026-09-25) —
withholding the top of the first gigabyte for itself, by a board- and
firmware-dependent amount.  Every variant declared `[0, ramSize)` whole and
`machineConfigCovers` requires every declared RAM byte to be in the account, so
no variant was covered and the bridge refused every real board; and the boot
map described the withheld memory as the kernel's own writable RAM, while the
root task's `[256 MiB, 1 GiB)` untypeds would have let it retype VideoCore
memory had the board been accepted.
...
- **A configuration carries its first-gigabyte top.**  `BCM2712Config.lowRamTop`
  ...
  reaching the floor always covers the top it binds.
- **Decided on the real accounts.**  `rpi5VariantFor_rpi5_firmware_account`
  binds `{8 GiB, 0x3FC00000}`, `rpi5VariantFor_cm5_firmware_account`
  `{4 GiB, 0x3FA00000}` (rounded down off the granule), and
  `rpi5VariantFor_kernel_extent_not_ram` refuses an account that does not
  report the kernel's extent as RAM.
- **Admissibility replaces membership.**  `BCM2712Config.Admissible` (the uncut
  form a member, the top admissible) is what every deployment theorem is stated
  over, and `rpi5VariantFor_admissible` replaces `rpi5VariantFor_mem`.  The
  gates are re-proved over every admissible configuration: the extent-free ones
  by `decide` through the extent-erasure projection
  `PlatformConfig.withoutExtents` (`wellFormed_of_withoutExtents`), the untyped
  placement symbolically, the machine configuration by
  `MachineConfig.wellFormed_of_within` (shrinking regions preserves
  well-formedness).
...
### 6.5 Hardware Binding Architecture
...
│  - Transitions: SeLe4n/Kernel/API.lean (38 syscalls)     │
...
back into the faulting thread's registers (`error_frame_regs(VM_FAULT)`,
`error_frame_regs(USER_EXCEPTION)`), and `dispatchSynchronousException`'s
non-SVC arms returned `.error .vmFault` / `.error .userException` — an EL0
thread that took a page fault was resumed at the faulting instruction with a
number in `x1` and no way to make progress, which is a livelock, not a fault
handler.  RR4 replaces that path with seL4's fault IPC:

- **The fault value** (`SeLe4n/Model/Fault.lean`, production) is seL4's
  `seL4_Fault_t` — `vmFault` / `capFault` / `unknownSyscall` /
  `userException` — carried on the TCB as `pendingFault : Option ThreadFault`
  (seL4's `tcbFault`), together with the `FaultContext` snapshot (`faultIP`,
  `sp`, `lr`, `spsr`, an 8-register GPR window) the handler needs to diagnose
  and restart.  It lives in `Model/` rather than `Architecture/` because the
  TCB carries it and `Model/Object/Types.lean` cannot import an architecture
  module — the same split seL4 makes between `shared_types.bf` and the arch
  fault encoders.
- **The wire format** (`Architecture/Fault.lean`, production) is seL4's
  `seL4_Fault_tag` numbering (`nullFault` 0, `capFault` 1, `unknownSyscall`
  2, `userException` 3, `vmFault` 6) with seL4's message lengths (4 / 4 / 13
  / 5).  `encodeFault` / `decodeFault` round-trip
  (`decodeFault_encodeFault`), and the label travels in the new
  `IpcMessage.label` field, which **only kernel-originated messages set** —
  a thread holding a send capability to a fault endpoint cannot mint a
  `seL4_Fault_tag`, and restoring seL4's sender-side label pass-through is
  registered debt rather than a silent omission.
- **Delivery** (`IPC/CrossCore/Fault.lean`, production) resolves the
  faulting thread's `faultHandler` CPtr through its own CSpace, requires the
  handler capability to carry **both** `.write` and `.grant` (seL4's send +
  grant/grantReply gate), and then performs a real `.call` through
  `endpointCallCrossCoreDispatch` — the live cross-core call chain — so a
  *passive* fault handler receives the faulting thread's SchedContext
  donation and can actually run.  A thread whose handler cannot be resolved,
  or whose call fails, is **suspended with its fault recorded**
  (`faultSuspend`), which is seL4's `handleDoubleFault` outcome: never
  resumed at the faulting instruction.
- **The reply that reaches it is the ordinary one.**  A fault handler holds
  nothing but the reply capability the fault Call gave it, so it answers with
  `seL4_Reply`; the live `.reply` dispatch arm is therefore seL4's
  `doReplyTransfer`, branching on the answered thread's `pendingFault`
  (`replyTransferOnCore`, `IPC/CrossCore/Fault.lean`, production).  On an
...
- **Restart** decodes the handler's reply arm-for-arm with seL4's
  `handleFaultReply` (`decodeFaultReply`), yielding either
  `.restart frame` — a `FaultRestartFrame` staged through the *same*
  `RegisterFile.stageReturnFrame` the WS-RA syscall return uses — or
  `.abandon`, which suspends the thread and clears the fault.  `SPSR_EL1` is
  carried outbound but **never written back**, the fail-closed counterpart of
  seL4's `sanitiseRegister`: a fault handler cannot promote its client to
  EL1 or unmask its interrupts.
- **The policy** gates the delivery: the live entry calls
  `faultDeliverOnCoreChecked` (`IPC/CrossCore/Fault.lean`, production), which
  applies the same `endpointFlowGate` the `.call` syscall arm applies — the
  ...
  policy could refuse.  A denied flow takes the **fail-closed suspend, not an
  error**: returning an error would leave the faulting thread runnable at the
  faulting instruction, which is the livelock reintroduced through the policy
  layer.  `faultEntry` reads the deployment context through
  `Platform.FFI.getKernelLabelingContext`, exactly as
  `syscallDispatchCrossCoreEntry` does.
- **The handler capability** is gated by seL4's `sendFaultIPC` predicate —
  send, and grant **or** grant-reply (`faultHandlerCapAuthorized`,
  `resolveFaultHandler_authorized`).  The reply link is structural in this
  model, so the disjunct is a policy gate rather than a mechanism
  prerequisite; the audit round replaced a send-and-grant reading that would
...
- **The register window** the message reports and a resume reinstalls is the
  ...
  executing core's successor runs through `scheduleLocalSuccessorLive`.
- **The return ABI is version 3**: a delivered fault message's label is its
  ...
  and a handler's decoder reads a successful receive carrying the tag.
- **What reaches hardware registers** is `MR0`-`MR3`: no receive path writes
  `MR4` onward into the receiver's IPC buffer yet (the WS-RA four-register
  residual, with its first consumer here), so an `unknownSyscall` or
  `userException` handler on hardware sees its first four words until that
  write lands — registered debt, closure target RR7.
- **Progress** is the payoff: `faultDeliverOnCore_not_dispatchable`
  (`IPC/Invariant/FaultProgress.lean`, production) proves that after fault
  delivery the faulting thread is neither on any core's run queue nor any
  core's current thread — on *both* dispositions, delivered and suspended —
  so the fault cannot re-fire in a loop.  The theorem needed a lemma the
  tree did not have: the cross-core priority-inheritance chain walk
  (`propagatePipChainCrossCore`) never adds a run-queue member, proven by
  induction on its fuel.  `faultDeliverOnCoreChecked_not_dispatchable` carries
  it through the policy gate, so a refusal cannot reintroduce the livelock.

The hardware seam is `@[export lean_handle_fault]`
(`SeLe4n/Kernel/FaultEntry.lean`, production), invoked by `trap.rs`'s
`deliver_fault` under the per-core `lean_ready` gate and inside the global
kernel-entry lock.  Classification is exported too — `@[export
...
A fault raised at the SVC seam is a distinct outcome (`SyscallOutcome.faulted`,
...
per-core Lean runtime initialization — the thing that would flip
`lean_ready` — is SM10.1's obligation.  `deliver_fault` therefore prints the
delivery and takes `cpu::fatal_halt()`.  This path is unreachable at
v0.34.44 (no core sets `lean_ready` anywhere in the tree, so the trap
...
- `TCB.faultHandler` had no writer outside the test fixtures, so on a live
  ...
  **target's** CSpace and refuses at set time anything
  ...
  `.cspaceRevoke`, 37 since WS-BP BP7.1 added `.untypedRetype`, and 38 since
  it added `.untypedReset`).
...
**AI1-C (v0.27.7), per-core since SM5, commit-coupled since the v0.34.1
review closure**: The timer tick accounting path has been unified —
single-path, one owner, one site. The IRQ path
(`trap.rs::handle_irq_per_core` → `timer::per_core_timer_tick_isr`) re-arms
the hardware timer via `reprogram_timer()`, records the per-core diagnostic
tick counter, and drives the verified Lean per-core scheduler tick under the
kernel-entry lock, gated on the per-core Lean-runtime readiness mask
(`lean_ready`); it never touches the global `TICK_COUNT`. That counter
shadows the Lean model clock, and its sole incrementer is
`ffi_timer_advance_tick_count`, called by the Lean entry
(`perCoreTimerTickEntry`) **iff the atomically-committed run-loop step
advanced `machine.timer`** (the flag is definitionally the committed
state's clock delta — `perCoreTimerTickStepWithClockAdvance_flag_def`), so
the shadow equals the model clock on every arm: pre-readiness ticks,
non-boot cores and fail-closed entries advance neither.
`ffi_timer_reprogram()` is re-arm-only, so a second incrementer cannot
reappear — preserving the M-26 fix that eliminated the dual-path bug where
both the IRQ handler and FFI bridge incremented the tick count, causing
double-counting on hardware; a `build.rs` scanner additionally fails the
build if the ISR body regrows an incrementer.
...
- `@[export lean_syscall_dispatch_cross_core]`
  ...
  restore, SM10.1).  The former `syscall_dispatch_inner` export and
...
- Cache coherency model proofs: the single-view `CacheModel.lean` predicates
  are trivially satisfied under simulation, but the SMP layer
  (`PerCoreCacheModel.lean`, WS-SM SM7.D) is **not** — its `icacheCoherent_perCore`
  (the 14th `proofLayerInvariantBundle` conjunct) distinguishes a stale cached
  line from a maintained one, and its broadcast-reach theorems separate the
  PE-local `IC IALLU` from the domain-wide `IC IALLUIS` / `IC IVAU`
...
### 6.7 Insecure Default Labeling Context Guard
...
### 8.6 Memory Scrubbing on Delete
...
> **Severity: High once bootable (SM10.1); not exploitable today.** This is a
...
> the exact leak `scrubObjectMemory` exists to prevent. Closure target: SM10.1,
...
### 8.7 TLB Flush Requirements for Production Paths
...
### 8.8 Frozen IPC Queue Semantics
...
### 8.9 Object Well-Formedness Validation
...
### 8.10 Checked Dispatch and MMIO Adapter
...
- **No other source of memory authority**: an in-place retype refuses a
  ...
  (`bootSafeObject`), and the pre-retype cleanup refuses to destroy one in
...
### 8.10.6 IPC Capability-Transfer NI Symmetry
...
### 8.10.8 Scheduler / Lifecycle Behaviour Symmetry
...
[`docs/REGISTERED_DEBT.md`](../REGISTERED_DEBT.md) `WS-RC R5`
...
### 8.11 buildChecked Runtime Invariant Validation
...
#### 8.12.4 Capability-Controlled Thread Binding
...
#### 8.12.6 Invariant Composition & Cross-Subsystem
...
#### 8.12.7 SchedContext Donation
SchedContext donation enables **passive servers** — threads that consume zero
CPU when idle by borrowing the client's SchedContext during IPC Call/Reply, **at
every call depth** since WS-OD OD4 (v0.35.2; see *The push writes it* below).

**Protocol**: (1) Client calls server via `endpointCall`. If server is passive
(`.unbound`), the client's *effective* SchedContext — the one it holds, bound or
itself donated — is donated via `donateSchedContext`, which also pushes a frame
onto that context's reply stack. (2) Server executes on the client's budget. (3)
Server replies via `endpointReply` — `returnDonatedSchedContext` pops the head
frame and returns the SC to the thread the stack names: the original owner at the
bottom, the outer caller one level up. (4) Server becomes passive (`.unbound`,
removed from RunQueue).
...
- `passiveServerIdle`: unbound non-runnable threads are
  ...
  **And the first DONATING arm declares one** (WS-RR RR8.12 Cut 8a-ii,
  ...
  nothing about it (SM8.D's CC-5).

  **And on a plain `Send` rendezvous it is `[]` too** (PR #897 Codex review,
  `v0.35.112`).  That last sentence was written about the block path and not applied
  ...
  **`Call`**, so every ordinary `seL4_Send` rendezvous declared two replenish-queue
  write locks for a migration that provably does not happen.  It keys on
  `receiveRendezvousCallSender?` now, narrowed by `rendezvousSenderIsCall` — the
  **pre**-state sibling of `rendezvousDequeuedCall`, which must be spelled separately
  because a dequeued `Call` sender is `.blockedOnCall` before the leg runs and
  `.blockedOnReply` after it; asking the post-state question at the pre-state would
  answer `false` for exactly the sender that *will* donate, and a footprint that
  omits a written lock is false where one wider than its operation is merely
  expensive.  The licence is
  `endpointReceiveDualWithCapsOnCore_not_dequeuedCall_of_blockedOnSend` (the leg
...
**The MCS reply stack** (WS-OD OD2, v0.34.125).  This is the structure that
makes donation *transitive*; it landed with **no behavioural change**, before the
push that writes it (WS-OD OD4, below).  At OD2 the invariant was vacuously true
of every state the tree reached: the one transition that wrote the fields below
was the donation pop, whose writing arm needs a context that already heads a
stack, and nothing constructed one until OD4's push.  That is the point: the
invariant and its frames exist *before* the transitions that must preserve them,
so no live transition is ever ahead of its own proofs.
...
- **The stack is doubly linked** (`v0.35.4`).  `Reply.prev` names the frame
  below, and `Reply.next : ReplyStackLink` names the frame **above** — or, on the
  head frame alone, the scheduling context whose stack this is (`.head sc`).  A
  sum type rather than two fields, so "heads a context", "has a frame above" and
  "off every stack" are three states of one value and no object can claim two at
  once.  The context is therefore recorded at the head only, which is what makes
  taking a frame out of the **middle** of a stack an `O(1)` repair of its two
  neighbours (seL4's `reply_remove_tcb`) rather than a walk clearing a per-frame
  context field below the cut.  A single-linked encoding cannot do that, and a
  frame it could not remove stayed on its stack with its caller gone: the Reply
  object and the scheduling context could then never be retyped, and the Reply
  could never be linked to a new caller.
- **`Reply.wellFormed` is no longer `True`** — seL4's `reply_unlink` invariant, a
  stack link in **either** direction only on a Reply whose caller is still
  blocked on it (`caller = none → prev = none ∧ next = none`).  `Reply.isFree` is
  its decidable form and the one spelling of "this Reply may be linked to a new
  caller or retyped", shared by the link guard, the stash admission, the retype
  guard and the boot check.
- **`donationChainWellFormed`** (`SeLe4n/Kernel/IPC/Invariant/Defs.lean`) — five
  clauses: every stored Reply is locally well formed; a context's head resolves
  to a Reply heading that context and conversely; every `prev` link is answered
  by the frame it names (`prevLinkReciprocal`); and each context's stack, walked
  from its head, **terminates** (`donationChainFrom`, a fuel-bounded walk).  Each
  link is validated by the target's own **upward** link and never by its
  `caller`: Reply objects are re-linked to new callers, so a stale link over a
  reused Reply would otherwise let a donation return read the *new* caller and
  hand the original thread's scheduling context to an unrelated thread, in
...
- **A conjunct of `ipcReachable`, not of `ipcInvariantFull`**, which keeps its
  ...
  kinds; `donationChainWellFormed_of_frame` is the payoff.  Its third clause
  (`callerKept`, `v0.35.4`) is what keeps the frame honest about the one step
  that is not one: a transition may not consume a linked frame's caller while
  framing its links, because `Reply.wellFormed` reads both.
...
pop itself (`storeDonationHeadPop`: the consumed head's links cleared, then the
frame below **re-headed** to this context, which is what keeps the new head's
upward link reciprocal), the target's `donationReturnBinding scId newOwner?`, and
the server's `.unbound`.
...
**The pop preserves it** (WS-OD OD3.8, v0.34.132).  Almost every transition in
the tree discharges `donationChainWellFormed` through `donationChainFrame` — it
writes no `Reply.prev`, no `Reply.next` and no `SchedContext.scReply`, so the two
projections the walk reads are fixed.  The pop is the exception the frame family
was designed against, and
...
- **The argument is acyclicity, and acyclicity is derived rather than assumed.**
  ...
  `not_mem_donationChainFrom_of_unlinked` (a Reply carrying no upward link is on
  no chain at all).  The first is
...
**The push writes it, and the chain becomes transitive** (WS-OD OD4, v0.35.2).
This is the row the workstream exists for, and the first one that **changes
behaviour**.  Two edits make donation transitive, and they land in the same cut
...
- **`donateSchedContext` pushes a reply-stack frame.**  Five object writes since
  ...
  Reply's `prev := sc.scReply` (the old head, read off the same object the first
  store rewrites) and `next := some (.head clientScId)`, the **old head's**
  `next := some (.frame pushRid)` — the store that makes the new frame reachable
  from below, and without which a middle frame could never be removed in `O(1)` —
  the donor's `.unbound` and the server's `.donated clientScId clientTid`.  A
  frame cannot be pushed onto itself, which is refused rather than assumed.  The
  frame is the donor's **own**
  `replyObject` — the Reply the `Call` rendezvous linked to it — so the push
  declares nothing the `.call` footprint had not
  (`lockSet_endpointCallOnCore_covers_donationPush`), and `maxLockSetSize` does
  not move.
- **The push is fail-closed on its frame** (`donationPushFrame?`).  A donor with
  no reply object, one whose reply id resolves to nothing, and one whose reply
  already donates are three distinguishable refusals.  A donation with no stack
  frame is the defect the whole chain design exists to refuse: the next pop would
  then clear an *outer* caller's frame and settle a scheduling context on the
  wrong thread, across a domain boundary.
- **The push preserves the chain** (`donateSchedContext_preserves_donationChainWellFormed`,
  WS-OD OD4.5).  The dual of OD3.8, and with both the predicate is preserved by
  every transition rather than by every transition but one.  Freshness is what
  makes the new frame safe and is *checked* rather than assumed: the refusal
  above puts the pushed Reply on **no** context's chain
  (`not_mem_donationChainFrom_of_unlinked`), so prepending it cannot revisit
  a member and the old walk transports verbatim.
- **The migration's source is the intermediate donor's home**
  (WS-OD OD4.6).  `replenishQueueAffinityConsistentOnCore` puts a context's CBS
  replenishments on its **bound thread's** home core; at depth `n` that thread is
  the intermediate server making this call, which is what the dispatch already
  passes.  Proved rather than inherited from the depth-1 reading: a successful
  depth-≥ 2 donation exhibits the pre-state context with
  `boundThread = some caller`.
- **Every pop resolves its new owner** (WS-OD OD4.4).  All six call sites now run
  `returnDonatedSchedContextResolved`, which reads `replyStackOuterCaller?` on its
  own pre-state and hands the answer to the pop; at depth 1 the answer is `none`
  and the site is the pre-OD4 behaviour
  (`returnDonatedSchedContextResolved_eq_legacy_of_no_stack`).  The obligation the
  resolution carries is `replyStackOuterCallerValid`, with two call-shaped
  siblings (`cleanupDonationStackValid` for the pre-receive cleanup,
  `cancelDonationStackValid` for the cancellation reclaim), and it is vacuous on
  every store where no context heads a stack.

**The teardown paths are chain-aware** (WS-OD OD5, v0.35.2).

- **A Reply that still carries a stack link cannot be freshened** (OD5.1, plan
  §3.4's confused deputy).  `linkReply` refuses anything but a **free** Reply
  (`Reply.isFree`: no caller, no link in either direction) — a linked Reply is a
  live frame of some context's stack, and re-linking it to a new caller would make
  the pop that later reaches it hand the original thread's context to an unrelated
  thread, driven by object reuse.  **Refused rather than cleared**: clearing would
  take the frame off a stack the context still heads, so the walk would stop
  mid-chain and the store would satisfy no chain at all.  A head's links are
  cleared by the pop that follows the reply leg in the same transition, which is
  why `Reply.consumed` leaves them in place on a head and clears them everywhere
  else.
- **A cancelled *middle* caller is spliced out of the stack** (OD5.2, re-decided
  by WS-HP HP6.8 at `v0.35.45`).  All three candidate answers are named
  (`CancelledMiddleCallerPolicy`), one is chosen
  (`cancelledMiddleCallerPolicy = .spliceOutTheCut`) and the choice is proved:
  `cancelledMiddleCaller_splices_at_cut` binds the innermost live caller
  `.donated scId outer` — still owed outward — and leaves **every other thread's
  TCB unchanged**, which is exactly what `reclaimToCancelledThread` would not do.
  `spliceThreadReplyFrameOut` takes the cancelled frame off the middle of the stack
  in `O(1)` before its caller link is consumed, so no frame is left on a stack with
  its caller gone, and since HP6.3 it *repairs* the pair either side of the cut
  rather than breaking it: the frame above takes the cut frame's own downward link,
  the frame below links back up at the frame above, and the cut frame's own `prev`
  is cleared (seL4's `reply_unlink` downward half).  **Up to `v0.35.44` it severed**
  — `.severAtCut`, which is upstream's own non-head branch (§8.12.8's correction at
  `v0.35.40`) — and the cost was stated rather than hidden: the original owner's
  reservation ended up with the innermost live caller and no later pop carried it
  below the cut.  The splice is therefore an **improvement on** seL4-MCS rather than
  parity with it; the `severAtCut` constructor is kept because it names the
  behaviour upstream still has.  From the cancellation end the same policy is two
  theorems — the reclaim fires for the holder of the context the victim's own frame
  heads, and declines below the cut.
- **The suspend pipeline pops twice at depth ≥ 2** (OD5.3).  The G2 teardown's
  reply arm can rebind the victim `.donated scId outer`, and the arm selector
  below it re-reads the binding from the **post-teardown** TCB — so the `.donated`
...
- **Retype refuses both halves of a live stack** (OD5.4).  A Reply that is not
  ...
  this context".  Both are `.revocationRequired`, `O(1)`, and fail-closed.
...
so `resolveEffectivePrioDeadline` reads a real reservation rather than falling
back to the legacy TCB priority, the second so the CBS engine charges it.  The
`onCore` instance adds the replenishment migration, so the reservation's refills
land on the core that now runs the server.

**The stack is doubly linked, and no frame is left on it with its caller gone**
(`v0.35.4`).  WS-OD's closure audit found that `severAtCut` was implemented by
*leaving a frame behind*: a cancelled middle caller's frame stayed on its stack
with its `caller` consumed, the later pop read it as the bottom of the stack and
bound the context outright, and the frame then **headed** that context forever.
Nothing could clear it — the Reply object and the SchedContext could never be
retyped, and the Reply could never be linked to a new caller — so an ordinary
`.tcbSuspend` on a client at call depth ≥ 3 pinned two kernel objects
permanently.  Reachable with no more authority than a TCB write right over a
...
- **A validated frame below the head whose caller was consumed is now an
  ...
  not name.
- **`maxLockSetSize` moved to 16**, because the pop now *writes* the frame below
  the head (it re-heads it) and the suspend pipeline's second pop reads and
  writes one frame further down.  The per-lock cost and envelope that implies are
...
7. **The removal did not preserve the donation accounting, and that cost was
   the `severAtCut` policy's — closed at depth ≥ 3 by WS-HP HP6 (`v0.35.45`).**
   Taking a caller out of the *middle* of a chain was destructive to which thread
   ends up owning the scheduling context: the removal moved no context, and the
   later pop donated to whatever the remaining stack said was outermost.  On
   `owner → middle → server`, a delegate answering `owner` out of order left
   `owner` `.unbound` permanently and the server's in-order reply then settled the
   context `.bound` on `middle` — where the in-order unwind would have left it
   `.donated … owner`, still owed outward.  A callee that delegates its caller's
   reply capability to a confederate could therefore capture that caller's
   reservation; the authority required is already the authority to unblock the
   victim.

   **The splice closes it, and the payoff is stated.**  Since HP6.8 the removal
   writes the cut frame's own downward link into the frame above and links the frame
   below back up at it (`removeCallerReplyFrame_splices_reciprocally`), so the
   ...
   **It was the policy's, not the chain's, and depth two cannot show that.**  A
   ...
   `spliceOutTheCut` write the same value into the frame above and the two are
   indistinguishable there.  `tests/SmpIpcSuite.lean` §3.22 is the depth-three
   witness where they differ; up to `v0.35.44` it measured the sever's cost and
   since `v0.35.45` it measures the splice's payoff, with two negatives spelling the
   retired values so the assertions are known to discriminate.  §3.20 pins the
   depth-two halves, and that they pass **byte-identically** across the policy flip
   is the measurement that the change is confined to depth ≥ 3.
...
6. **Below the cut the reclaim declines on the stack.**
   ...
   construction.  That is the removal policy seen from the cancellation end, and the
   fact HP6.8 consumed to make the splice live.
...
#### 8.12.11 The removal splices rather than severs — WS-HP HP6 (`v0.35.45`)
...
**The removal is three stores.**  `spliceReplyFrameStores` writes
`above.prev := some below`, `below.next := some (.frame above)`, and
`rid.prev := none` — the last being seL4's `reply_unlink` downward half.  The third
is not optional: without it the cut frame keeps a `prev` that nothing below names
back, which falsifies `donationChainWellFormed.prevLinkReciprocal` at the cut
frame, so the bare splice would owe a relaxed predicate and
`SeLe4n/Testing/ReplyStackWriteCensus.lean` would have had to accept a half-step
where the sever stated its result outright.  With it the invariant is preserved
**outright** across the removal
...
`donationAccountingPreserved_atCallDepthThree`, which is the workstream's payoff:
at reply-stack depth ≥ 3 a middle removal leaves the reservation **owed outward**
and the pop that answers the bottom frame delivers it home.  Both are stated with
no key-distinctness hypothesis, derived instead from the store's contents, because
`ReplyId.toObjId` and `ThreadId.toObjId` are two wrappers over one `ObjId` and a
numerical collision is representable.  The first is stated at the *removal* rather
than at the splice, because the consume that follows clears the cut frame's
remaining link and a claim about the splice alone would say nothing about whether
that clear disturbs the pair it just built.

**The ordering that was enforced.**  The splice re-heads a frame whose recorded
reply server is by then gone and `.unbound`, so under a binding-driven pop trigger
answering it would run no pop and leave a consumed frame heading a context — the
object pinning `v0.35.4` closed.  The trigger therefore moved first, for the reply
...
master, 13.0.0, 12.1.0, 12.0.0 and 11.0.0 — so seL4-MCS strands the reservation at
depth ≥ 3 too.  The `CancelledMiddleCallerPolicy.severAtCut` constructor is kept
for that reason: it names the behaviour this kernel diverged from and that upstream
still has.

**What is not closed, and the measurement that says so.**  At depth **two** the
frame below the cut is the stack's bottom, so both policies write `none` into the
...
band only through priority inheritance.  That is seL4-MCS's split, where both
parameters are TCB fields (`tcb->tcbPriority`, `tcb->tcbDomain`) and the
scheduling context carries budget and period.

The chain walk (`propagatePipChainCrossCore`, completed on the `.receive` arm at
`v0.34.141`) is therefore the only priority route between a client and the
server it calls.  Before that cut the donated reservation supplied both
thread-owned parameters for the donee, which crossed authority in both
directions: `.tcbSetPriority` on the donee wrote the **client's** reservation,
and `schedContextConfigure` on the client's reservation rewrote the **server's**
own priority and domain — the latter migrating a thread between partitions on a
capability that says nothing about it.  Every reader and writer of a thread-owned
parameter now classifies through `ownScId?`, and each reader is pinned to
`SystemState.threadBasePriority` by theorem.

**Bounded inversion**: Priority inversion is bounded by
`objectIndex.length × WCRT(effectiveSchedParams)` ticks (parametric in WCRT).

### 8.14 Bounded Latency Theorem
...
### 8.14.1.1 RPi5 Canonical Deployment Specialisation
...
### 8.15 Hardware Integration Roadmap
...
#### 8.15.1 Architecture Module Integration
...
1. `ExceptionModel.lean` — ARM64 exception classification and dispatch.
   **Partially integrated at WS-RR RR4 (v0.34.44)**: the syndrome layer this
   module owned — `SynchronousExceptionClass`, `ExceptionContext`,
   `extractExceptionClass`, `extractInstructionSyndrome`,
   `classifySynchronousException` — moved *down* into the production
   `Architecture/Fault.lean`, which does not import `Kernel/API.lean` and so
   is not in the cycle.  The live hardware path is now
   `@[export lean_classify_synchronous_exception]` +
   `@[export lean_handle_fault]` (`Kernel/FaultEntry.lean`, production).
   The `dispatchException` / `dispatchSynchronousException` *wrappers* remain
   staged, because they still import `Kernel/API.lean` for the SVC arm's
   `syscallEntry`
...
2. Wire `ExceptionModel.dispatchException` into the syscall entry path
   ...
   **Non-SVC half done** at WS-RR RR4 (v0.34.44): the cycle was resolved for
   the classification and fault-delivery half by extraction rather than
   abstraction — `Architecture/Fault.lean` and `IPC/CrossCore/Fault.lean` are
   below `API.lean` in the import order, so `Kernel/FaultEntry.lean` reaches
   the hardware without the wrapper.  What remains is the SVC arm, which is
   already live through `lean_syscall_dispatch_cross_core` (§6.5.5) and would
   only gain uniformity, not reachability, from routing through
   `dispatchException`.
...
6. ~~Import `CacheModel` into Architecture invariant bundle.~~ **DONE** at
   ...
   contributes `icacheCoherent_perCore` as the 14th
   `proofLayerInvariantBundle` conjunct, and `CacheModel` is production.
...
API.lean is the gating prerequisite before integration of the *wrappers* is
possible.  It is no longer a prerequisite for reaching hardware: WS-RR RR4
demonstrated that the load-bearing content (classification, fault
construction, fault delivery) can be — and now is — production code below
`API.lean`, leaving only the dispatch wrappers above it.

#### 8.15.2 Budget-Aware Scheduler Activation
...
### 11.2 Information Flow and Non-Interference Boundary
...
#### 11.2.1 Service-presence covert channels
...
#### 11.2.2 Architecture assumption consumer index
...
#### 11.2.3 Single-core kernel model witness
...
#### 11.2.4 Per-core observers under SMP
The NI boundary above is stated for a single observer at a security
clearance. Under SMP an observer is a **pair** `(c, L)` of a core and a
clearance (WS-SM SM8.A, `PerCoreObserver` in
`Kernel/InformationFlow/ObservableStatePerCore.lean`): an attacker thread
is bound to a core by `cpuAffinity`, and every cross-core leakage path the
SMP kernel opens — scheduling decisions, cross-core IPC, lock contention —
is a per-core operation. The state such an observer sees is
`ObservableState.onCore ctx c L s`, which at `bootCoreId` is
*definitionally* the single-core `projectState`, so every guarantee above
is the boot-core member of the per-core family.

Three properties bound what the per-core observer can learn.

1. **The observable state's components partition into shared and
   per-core.** Seven components (`objects`, `services`, `irqHandlers`,
   `objectIndex`, `domainSchedule`, `memory`, `serviceRegistry`) are
   filtered by the observer's *label* only and are identical from every
   core; six (`runnable`, `current`, `activeDomain`,
   `domainTimeRemaining`, `domainScheduleIndex`, `machineRegs`) are
   restricted to the observer's core. The partition is a **bijection**:
   `ObservableState.ofFragments` reassembles a state from the pair and
   `ofFragments_eta` proves the round trip, so a component added to
   `ObservableState` and registered in neither fragment leaves
   `ofFragments` unable to supply it — the "fails to compile" property is
   a checked fact, not a review convention. `onCore_isProjection_of_globalProjection`
   states the consequence as an exact `iff`: the observer learns the
   global projection's shared fragment paired with its own core's
   per-core fragment, all of it and nothing beyond.

2. **The read set is bounded, and excludes every other core.**
   `onCore_perCore_independence` proves the per-core observable state is a
   function of the six shared state components plus the observer core's
   five scheduler slots and its register bank — nothing else. A write to
   another core's scheduler slot or register bank is therefore invisible,
   as are components outside the read set on *any* core: the CBS
   replenishment queue, the diagnostic timeout log, the SchedContext
   thread index, the machine timer (the deliberate `ObservableState`
   exclusion of §11.2, now restated per core), and the SM7.C per-core TLB
   view.

3. **Visibility is monotone in clearance.** `onCore_label_monotone`
   (and its ∀-core aggregate `onCore_label_monotone_smp`) proves that
   raising the observer's clearance can only widen what it sees, over the
   `ObservableState.visibilityLe` preorder. The two list components are
   compared by `List.Sublist` rather than membership — both filter the
   *same* underlying list, so order is preserved, and a run queue's order
   is its dispatch order. Only `objects` is compared by visibility, because
   a wider clearance may reveal more of an object it can already see: CNode
   slot redaction is the only observer-dependent part of object projection
   (`projectKernelObject_observer_independent_off_cnode`), it is monotone
   (`projectCNode_lookup_monotone`, lifted to the observable-state layer by
   `onCore_objects_cnode_slot_monotone`), and elsewhere the projected value
   is identical (`onCore_objects_label_invariant_off_cnode`).

**Accepted covert channel CC-1 is per core.** The four scheduling
components pass through unfiltered — `onCore_schedulingTransparency`
states them against core `c`'s *raw* scheduler state, so it says what the
observer gets rather than merely that two clearances agree. Under SMP each
core carries its own `activeDomain` / `domainTimeRemaining` /
`domainScheduleIndex`, so the channel documented in §11.2 exists once per
core rather than once per system. The system-wide `domainSchedule` is
shared.

**Two channels registered at SM8.A: CC-6 and CC-7.** The SM7.C per-core
TLB view (`SystemState.perCoreTlb`) and the SM7.D per-core
instruction-cache view (`SystemState.perCoreICache`) are proven outside
the per-core observable read set (`onCore_perCoreTlb`,
`onCore_perCoreICache`; also `onCore_tlbShootdown`,
`onCore_pendingIcacheMaintenance`, `onCore_tlb`), so no model-level flow
exists through them. That is exactly the CC-2 machine-timer situation, and
it gets the same treatment: the exclusion is a statement about the
*model*, while a real observer measures TLB and instruction-cache
...
> **v0.33.5 review round 4.**  Three live arms were still unaudited — the
> bound-delivery `.signal`, `.receive` rendezvousing with a blocked sender, and
...
> have taken it to thirty-one transitions and twenty-three live arms, per the current
...
> with the statement of what is not; and the per-core enforcement boundary now
> audits the cross-core wrappers the live dispatch reaches rather than the
> single-core name table.
...
per-core writes stay on core `c'` is invisible to an observer on core
`c ≠ c'` unless it moves the shared, label-filtered half of the view.  The
proof is the SM8.A partition applied twice: the per-core fragment is frozen
by the frame premise `observableSlotsConfinedToCore st st' c'` (every
per-core slot outside core `c'` unchanged, register banks included — under
SM5.I a real obligation, not a structural fact), and the shared fragment by
`sharedViewUnchanged`, which is the plan's "does not mutate any object with
`labelOf o ⊑ L`" together with its "does not signal an observable
notification" (signalling writes the notification's object).

The plan discharges Theorem 3.3.1 from serializability.  That argument is
**not available on the live path**: SM3.C.9 still defers wrapping the
`@[export]` bodies in `withLockSet`, and v0.32.142 serialises kernel entry
with one global ticket lock rather than the per-object fine locks.  The
theorem is therefore proven from the frame premises directly — strictly
weaker assumption, strictly stronger result — and
`crossCoreNonInterference_of_disjoint_lockSet` supplies the plan's argument
as a bridge, so once SM3.C.9 lands it becomes a corollary.

**`nonInterference_perCore`** follows: at `bootCoreId` the per-core view *is*
`projectState`, so the existing single-core `step_preserves_projection`
applies verbatim; elsewhere it is Theorem 3.3.1 at `c' = bootCoreId`.  All
thirty-five `KernelOperation` variants get a lift, and **thirty-one derive
the confinement premise from the operation's own semantics** rather than
assuming it — which discharges, for those operations, the obligation the
SM4.C / SM4.D per-core preservation theorems carry as an `hOtherIdle`
hypothesis.  The four catch-all constructors (`syscallDispatchHigh`,
`endpointCallWithDonationHigh`, `endpointReplyWithReversionHigh`,
`handleInterrupt`) carry no operational hypothesis and genuinely range over
remote-core writes, so they take the premise explicitly;
`perCoreConfinementDerived_count` pins the 31/4 split.

**The per-object `lock` is no longer projected.**  `RwLockState` carries
`writerHeld : Option CoreId`, `readers : List CoreId` and
`waiters : List (CoreId × AccessMode)` — every field a core identity — and
`projectKernelObject` previously passed it through.  An observer able to see
an object would therefore read off which cores were operating on it: the
per-thread *placement* channel WS-SM SM5.B closed by stripping
`TCB.cpuAffinity`, re-opened through a different field and on every object
kind.  The field is now erased structurally on every projected arm, per the
same discipline SM5.B states — not justified by "no live operation sets it
yet".  With the erasure, the SM3 two-phase-locking bracket is
non-interference transparent **unconditionally**
(`withLockSet_preserves_projection`: no hypothesis on the lock set, none on
contention), which is what leaves the lock-contention channel CC-5 a
hardware *timing* channel and nothing more.

**The accepted covert channels are data.**  `acceptedCovertChannelsPerCore`
lists CC-1 … CC-7 as `CovertChannel` records carrying the plan's channel
number, the WS-W mitigation note, the severity, whether the channel is
...
view moves *if and only if* the shared fragment moves — six of the thirteen
components carry no cross-core flow at all.

**Per-core declassification audit (WS-SM SM8.C, v0.33.7).**
Declassification is the one path allowed to move information *down* the
lattice, and until this cut the tree gated it without recording it: nothing
constructed a `DeclassificationEvent`, so the audit trail was a type with no
writer.  `declassifyStoreOnCore` is the producer — the same gate, threading an
append-only log, appending exactly one event per authorized downgrade, with the
state effect provably identical to the unaudited gate, so auditing adds a record
and not a transition.  `DeclassificationEvent.originatingCore` is undefaulted
(a default would attribute every event to the boot core) and
`authorizationBasis` is typed, so the kernel can check its own records while an
external consumer reads the same rendered strings as before.
`declassifyStoreFromCore` reads the source domain off the subject the executing
core is running, so an event's subject is a fact about the state rather than a
caller's claim.  The per-core audit views partition the log exactly
(`declassificationAuditLog_partitions_by_core`), and the theorem that decides the
design is `crossCoreChain_not_within_one_view`: a declassification chain that
crosses cores is contained in **no** single core's view, so one log per core —
the natural SMP implementation — would lose the composed downgrade.  Eight
cross-core declassification rules ship as data, each supplying a proof of its own
claim; the substantive ones are laundering (per-hop authorization does not
compose, detectable only from the trail) and that a per-endpoint policy override
can never authorize a downgrade.

The same cut wires the per-endpoint flow policy WS-E5/H-04 specified and nothing
ever read: `LabelingContext.endpointPolicy` is now consulted by the four
endpoint-keyed gates through `endpointFlowGate`, which **conjoins** the global
...
acquire and every release, so the question the phase answers is what an observer
— and what an integrity policy — can learn from those writes.  The answer to the
first is **nothing**, and it is proven in the form that settles the field rather
than one operation: the observer's view **factors through** lock erasure
(`projectKernelObject_setLock` — overwriting an object's lock with an *arbitrary*
`RwLockState` leaves the projected object literally identical), and
`lockWritesOnly` lifts that to states, with instances for the acquire, the
release, both 2PL folds and the bracket.  It is deliberately not the claim that
the state is unchanged, which is false under fine locks
(`KernelObject.updateLock_not_identity`).  Reader multiplicity is therefore
unobservable (`readerMultiplicity_not_observable`, instantiated at the
*reachable* two-reader state the RwLock spec constructs), and so is writer
exclusion — including to the blocked acquirer itself
(`blockedAcquirer_observes_nothing`), whose observer is the very core sitting in
the wait queue.

What is left of the lock-contention channel is timing, and SM8.D **bounds** it
rather than only registering it: `lockContention_delay_bounded` composes the
RwLock spec's tight wait-depth cap with its admission-step bound to give a
...
and a run capacity — the same construction the scheduling channel CC-1 is costed
by, so the two accepted timing channels are comparable.  The ceiling is
denominated in **lock operations**, not seconds; reading it as elapsed time
needs a per-critical-section bound, which `lockContention_wallClock_bounded`
carries as an explicit hypothesis.  Bounded is not closed:
`lockContentionChannel_two_codes_reachable` exhibits two fair, in-premise
executions in which the *same* contending core reads different codes.  (The
weaker `lockContentionAlphabet_at_least_two` counts the *allocated* alphabet and
is deliberately not the witness — the two codes it counts are precisely the two
an accepted acquisition cannot produce.)

Integrity is proven in **both** directions.  seLe4n's `integrityFlowsTo`
deliberately reverses standard BIBA (the dimension tracks authority delegation,
not data purity), so a result about one order would say nothing about a
deployment configured with the other; SM8.D states the property over an
arbitrary write rule and instantiates it at both, with `writeRules_differ` the
witness that those are two claims.  `lockWrite_carries_no_subject_data` is why
erasing the lock word is an abstraction and not a way of defining the write away:
two objects with wildly different content but the same lock word get the same
lock word out, so nothing the writing subject holds reaches the field.

Finally, the witness for the live path.  `syscallEntryUnderLockSet` is the shape
the deferred `withLockSet` migration installs at the `@[export]` bodies, and its
...
and `lockContention_unbounded_without_fairness` exhibits the execution in which
a queued core is never admitted at all.  Its arithmetic likewise separates what
is grounded from what is not — `numCores - 1` is the shipped hardware's real
factor, while the release budget is an explicit placeholder pending tuning, so
the concrete alphabet figure is a property of that symbol rather than of the
kernel.  And the observation the bound is about is keyed to the acquisition that
made it: a core meeting the same lock twice has its second wait reported rather
than swallowed, which is what distinguishes the enqueue-relative admission step
from the execution's first one.
