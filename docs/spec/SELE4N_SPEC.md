...
| **Production LoC** | 287,653 across 287 Lean files |
| **Test LoC** | 64,210 across 69 Lean test suites |
...
been unified — single-path, one owner, one site. The IRQ path
...
tick counter, and — on the **boot core only** — advances the global
`TICK_COUNT` (the single-authority clock; the Lean side mirrors it exactly:
only the boot core's committed run-loop step advances `machine.timer`, via
`tickClockedState`). It also drives the verified Lean per-core scheduler tick
under the kernel-entry lock, gated on the per-core Lean-runtime readiness
mask (`lean_ready`). `ffi_timer_reprogram()` is re-arm-only, so a second
incrementer cannot reappear — preserving the M-26 fix that eliminated the
dual-path bug where both the IRQ handler and FFI bridge incremented the tick
count, causing double-counting on hardware.
