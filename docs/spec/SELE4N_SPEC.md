...
- WS-E4 CDT abstraction now separates stable derivation-node identity from CSpace slot identity: derivation edges are node-level, slots map to nodes (`slotNodes`) with node backpointers (`nodeSlots`), and `cspaceMove` performs slot-to-node relocation (`moveSlotNode`) rather than whole-tree edge rewriting. The abstraction theorem `mem_projectedEdges_iff` states that observable CSpace CDT edges are exactly the projection of node edges through slot mapping.
