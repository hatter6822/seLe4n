...
- Capability derivation modeling uses stable CDT node identities: derivation edges are node→node, CSpace slots project to nodes through explicit `slotToNode`/`nodeToSlot` bindings, and `cspaceMove` is implemented as slot-node rebinding with backpointer fixup (`rebindSlotNode`). The abstraction theorem `projectedEdges_is_slot_projection` states that the CSpace-observed CDT is exactly the slot projection of the stable node graph.
