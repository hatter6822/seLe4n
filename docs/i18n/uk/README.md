...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-0.36.2-blue" alt="Версія" />
...
- **Протокол успадкування пріоритетів** — транзитивне поширення пріоритету з машинно перевіреною відсутністю взаємних блокувань (`blockingAcyclic`) та обмеженою глибиною ланцюга. Запобігає необмеженій інверсії пріоритетів
- **Теорема обмеженої затримки** — машинно перевірена межа WCRT: `WCRT = D × L_max + N × (B + P)`, доведена у 8 модулях liveness, що охоплюють монотонність бюджету, тайминг поповнення, семантику yield, вичерпання смуги та ротацію доменів
...
- **N-доменний інформаційний потік** — параметризовані політики потоків, що узагальнюють бінарний поділ seL4. Межа примусового застосування (enforcement) з 43 точками входу та доведеннями неінтерференції (non-interference) для кожної операції (індуктивний тип `NonInterferenceStep` з 35 конструкторами), а також обмежений, закритий при відмові (fail-closed) журнал аудиту декласифікації з мандатно-контрольованим читачем
- **Складений шар доведень** — `proofLayerInvariantBundle` об'єднує 16 пакетів інваріантів підсистем (ядро планувальника + розширення CBS, мандати, IPC + зв'язка IPC–планувальник, життєвий цикл, сервіси, VSpace, міжпідсистемні, узгодженість TLB, узгодженість очікувачів сповіщень, межі pending/ack для TLB shootdown, поядерна інвалідація TLB та когерентність I-кешу, а також межа журналу аудиту декласифікації) в єдине зобов'язання верхнього рівня, що перевіряється від завантаження до всіх операцій
- **Трифазна архітектура стану** — фаза побудови зі свідками інваріантів переходить у заморожене незмінне представлення з доведеною еквівалентністю пошуку. 24 заморожені операції дзеркалять активний API
...
| **Продуктовий код (Lean LoC)** | 420 740 рядків у 343 файлах |
| **Тестовий код (Lean LoC)** | 85 588 рядків у 71 тест-сьюті |
| **Доведені декларації** | 13 928 декларацій theorem/lemma (нуль sorry/axiom) |
...
tests/                           Виконувані тест-сьюти + фікстури
```
...
| **Інформаційний потік** | Бінарний поділ high/low | N-доменна конфігурована політика з межею enforcement із 43 точками входу (кількість зафіксовано `enforcementBoundaryExtended_count`), доведеннями неінтерференції по операціях та мандатно-контрольованим журналом аудиту для кожної авторизованої декласифікації |
...
| **Обмежена затримка** | Немає формальної межі WCRT | `WCRT = D × L_max + N × (B + P)`, доведено у 8 модулях liveness |
...
| **Доведення** | Isabelle/HOL, post-hoc | Type-checker Lean 4, поєднані з переходами — нуль sorry/axiom (кількість доведених декларацій — у таблиці [Поточний стан](#поточний-стан)) |
| **Платформа** | HAL рівня C | Typeclass `PlatformBinding` з типізованими контрактами меж |

## Що далі

Активний робочий потік — **WS-SM** (завершення багатоядерного SMP),
який об'єднав решту фаз усунення зауважень WS-RC зі SMP-специфічним
планом фаз SM0–SM10 і закривається на **v1.0.0** завантажуваним
верифікованим SMP-мікроядром на Raspberry Pi 5. Фази SM0–SM9
завершено — базові типи SMP та ієрархія блокувань, SMP-ініціалізація
Rust HAL, верифіковані примітиви блокувань, пооб'єктні блокування,
поядерний стан планувальника та планування, міжядерний IPC, TLB
shootdown та обслуговування кешів, інформаційний потік SMP, а також
