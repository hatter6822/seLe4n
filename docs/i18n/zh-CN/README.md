...
| **版本** | `0.19.6` |
| **Lean 工具链** | `v4.28.0` |
| **生产代码行数** | 57,506 行，分布于 100 个文件 |
| **测试代码行数** | 7,559 行，分布于 10 个测试套件 |
| **已证明的声明** | 1,756 个定理（theorem）/引理（lemma）声明（零 sorry/axiom） |
| **目标硬件** | Raspberry Pi 5 (BCM2712 / ARM Cortex-A76 / ARMv8-A) |
| **最新审计** | [`AUDIT_COMPREHENSIVE_v0.18.7_PRE_BENCHMARK.md`](../../audits/AUDIT_COMPREHENSIVE_v0.18.7_PRE_BENCHMARK.md) 和 [`AUDIT_COMPREHENSIVE_v0.18.7_KERNEL_RUST.md`](../../audits/AUDIT_COMPREHENSIVE_v0.18.7_KERNEL_RUST.md) —— 双重综合审计（115+ 项发现，0 项严重） |
...
- **WS-S** —— 预基准强化（7 个阶段，S1–S7，83 个子任务）**已完成**（v0.19.0–v0.19.6）。针对 v0.18.7 双重综合审计（115+ 项发现）的全部内容。计划：[`AUDIT_v0.18.7_WORKSTREAM_PLAN.md`](../../audits/AUDIT_v0.18.7_WORKSTREAM_PLAN.md)。结项报告：[`WS_S_CLOSURE_REPORT.md`](../../audits/WS_S_CLOSURE_REPORT.md)。
- **WS-R** —— 综合审计修复（8 个阶段，R1–R8，111 个子任务）**已完成**（v0.18.0–v0.18.7）。计划：[`AUDIT_v0.17.14_WORKSTREAM_PLAN.md`](../../dev_history/audits/AUDIT_v0.17.14_WORKSTREAM_PLAN.md)。
- **Raspberry Pi 5 硬件绑定** —— ARMv8 页表遍历、GIC-400 中断路由、引导序列（下一工作流）

此前的工作流组合（WS-B 至 WS-R）已全部完成。此前的审计（v0.8.0–v0.9.32）、里程碑结项报告和历史 GitBook 章节已归档至 [`docs/dev_history/`](../../dev_history/README.md)。
