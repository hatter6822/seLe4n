...
- **840 theorem/lemma declarations** across 7 kernel subsystems.
...
- **Phase 1–3 correctness fixes** (WS-H1..H9): IPC call-path semantic bug fixed, lifecycle safety guards, build/CI infrastructure hardened, capability invariant redesign, IPC dual-queue structural invariant, scheduler proof completion, enforcement-NI bridge with 4 new enforcement wrappers and projection hardening, non-interference coverage extended to >80% of kernel operations with 31 NonInterferenceStep constructors.
...
|---------|-----------|--------|
| v0.12.15 | WS-H | Phase 1–3 **Completed** (WS-H1..H9: IPC fix, lifecycle guards, CI hardening, capability invariant redesign, dual-queue structural invariant, scheduler proof completion, enforcement-NI bridge & missing wrappers, NI coverage extension to >80%, WS-H7/H8/H9 gap closure v0.13.5) |
