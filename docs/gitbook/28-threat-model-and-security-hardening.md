...
Out of scope for the current phase: hardware runtime exploit mitigation and cryptographic attestation of generated machine code. Non-interference coverage is >80% of kernel operations with 35 `NonInterferenceStep` constructors and a 42-entry enforcement boundary (count pinned by `enforcementBoundaryExtended_count`).
