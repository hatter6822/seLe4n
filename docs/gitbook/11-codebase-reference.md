...
Machine-readable mirror: `docs/codebase_map.json` (generated by `./scripts/generate_codebase_map.py --pretty`, validated with `--check`, and auto-refreshed post-merge by `.github/workflows/codebase_map_sync.yml`) exports module + declaration inventory with commit-derived cache metadata for website cache invalidation.
