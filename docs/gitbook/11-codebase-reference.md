...
Machine-readable mirror: `docs/codebase_map.json` (generated by `./scripts/generate_codebase_map.py --pretty`, validated with `--check`, and auto-refreshed post-merge by `.github/workflows/codebase_map_sync.yml`) exports module + declaration inventory with stable source-digest cache metadata (`source_sync.source_digest`) plus `repository.head` git metadata. Declaration entries include additive `called` arrays for internal declaration invocations (empty when none are detected); docs-sync checks compare the stable subset so branch/merge-only churn does not fail CI.
...
- `SeLe4n/Kernel/Capability/Operations.lean`
  - `resolveCapAddress` multi-level CSpace resolution with guard/radix bit extraction (WS-H13).
- `SeLe4n/Kernel/Capability/Invariant.lean`
  - `cspaceEnrichmentInvariantBundle` (`cspaceDepthConsistency` ∧ `cspaceWellFormedGuards`) with
    preservation across insert/delete/mint/copy/move (WS-H13).
...
- `SeLe4n/Kernel/Service/Operations.lean`
  - backing-object verification in `serviceStart`/`serviceStop` (WS-H13/A-29);
    restart rollback semantics in `serviceRestart` (WS-H13/A-30).
- `SeLe4n/Kernel/Service/Invariant.lean`
  - `serviceCountBounded` preservation for start/stop/restart (WS-H13/A-31);
    `serviceCountBounded_transfer` generic transfer lemma.
