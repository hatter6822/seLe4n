...
| A fault is delivered, never returned: no execution path resumes a thread at the instruction that faulted without a handler decision (`faultDeliverOnCore_not_dispatchable`, both dispositions) | `lake exe fault_handling_suite`; `./scripts/test_full.sh` (Tier 3 anchors); `sha256sum -c tests/fixtures/fault_handling_4core.expected.sha256` |
