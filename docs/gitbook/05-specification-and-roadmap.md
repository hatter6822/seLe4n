...
| Active workstream | **WS-AL in progress** (branch `claude/review-ak7-workstream-QAUBL`) — AK7 cascade-closure workstream resolving the three deferred AK7 items (AK7-E / AK7-F / AK7-I) before v1.0.0. **Phase AL0 complete**: 17-metric baseline + monotonicity CI guard. **Phase AL1 (AK7-I.cascade) RESOLVED**: `Capability.requireNotNull` guard wired at `cspaceMint` / `cspaceCopy` / `cspaceMove`; bridge lemma `requireNotNull_some_eq`; seven preservation proofs patched; three end-to-end regression tests; `docs/audits/AUDIT_v0.29.0_DEFERRED.md` row marked RESOLVED. **Phase AL2 (AK7-F foundational layer) complete**: five per-variant `getX?` helpers (`getTcb?`, `getSchedContext?`, `getEndpoint?`, `getNotification?`, `getUntyped?`) + exhaustive discrimination lemmas (10 rejection for `getTcb?` + 4 mirrors) + 5 `_eq_some_iff` unfolding lemmas + `getTcb?_eq_none_iff` complement + 8 runtime tests. Gate: `lake build` (260 jobs, 0 warnings) + `test_smoke.sh` + `test_full.sh` + `ak7_regression_suite` (**55 checks**, up from 38 at v0.29.13) + `cargo test --workspace` (415 tests) + zero sorry/axiom. Phases AL3–AL11 remain (304 consumer call-site migrations, `storeObjectChecked` kind-guard, AK7-E dispatch sentinel guards, v1.0.0 closure). Prior: **WS-AK Phase AK7 COMPLETE** (v0.29.13) — Foundational Model baseline with 11 sub-tasks AK7-A..AK7-K; AK6 (v0.29.9) + AK6-F audit remediation (v0.29.12), AK5 (v0.29.5–v0.29.6), AK4 (v0.29.4–v0.29.8), AK3 (v0.29.3), AK2 (v0.29.2), AK1 (v0.29.1), **WS-AJ PORTFOLIO COMPLETE** (v0.28.1–v0.29.0), WS-AI (v0.27.7–v0.28.0), WS-AH (v0.27.2–v0.27.6), WS-AG (v0.26.0–v0.27.1), WS-AF–WS-B. **Next (after WS-AL completes):** AK8 (Capability/Lifecycle/Data Structures), AK9 (Platform), AK10 (Closure); then WS-V (AG10: Hardware Integration). |
