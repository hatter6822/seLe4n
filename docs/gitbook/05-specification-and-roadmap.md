...
| Latest audit | [`AUDIT_v0.29.0_COMPREHENSIVE`](../audits/AUDIT_v0.29.0_COMPREHENSIVE.md) — pre-1.0 full-kernel audit (2 CRITICAL, 23 HIGH, ~76 MED, ~101 LOW). WS-AK Phases AK1–AK6 complete (v0.29.1–v0.29.12); remaining phases AK7–AK10 target v1.0.0. |
| Active workstream | **WS-AK Phase AK6 COMPLETE** with audit remediation (v0.29.12). Pre-1.0 Release Hardening (v0.29.0 audit) — Phase AK6: Information Flow + SchedContext Correctness. Delivered in v0.29.9 as 10 sub-tasks (AK6-A..AK6-J: 2 HIGH, 6 MEDIUM, 7 LOW findings). AK6-F per-arm per-op theorem coverage extended across v0.29.10–v0.29.11 (14/14 arms named). v0.29.12 audit remediation: substantiveness tiering corrected (5 truly substantive / 3 hybrid with legitimate external closure / 6 closure-form with documented discharge recipes via pre-proven frame lemmas), 3 new frame lemmas (`resumeThread_frame_insert`, `resumeThread_frame_ensureRunnable`, `schedContextBind_frame_runQueue_rebucket`), and runtime test coverage for AK6-G (pendingMessage/timedOut stripping), AK6-H (`isInsecureDefaultContext`), AK6-I (`cbs_bandwidth_bounded_tight` arithmetic) added to `tests/InformationFlowSuite.lean`. Prior: AK5 (v0.29.5–v0.29.6), AK4 (v0.29.4–v0.29.8), AK3 (v0.29.3), AK2 (v0.29.2), AK1 (v0.29.1), **WS-AJ PORTFOLIO COMPLETE** (v0.28.1–v0.29.0), WS-AI (v0.27.7–v0.28.0), WS-AH (v0.27.2–v0.27.6), WS-AG (v0.26.0–v0.27.1, 10 phases, 67 sub-tasks), WS-AF–WS-B. **Next:** AK7–AK10 (continued pre-1.0 hardening); then WS-V (AG10: Hardware Integration). |
