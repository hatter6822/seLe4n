...
| Active workstream | **WS-AL COMPLETE (v0.29.14)** — AK7 cascade closure with COMPILE-TIME TYPE ENFORCEMENT. All three AK7 deferred cascades (AK7-E / AK7-F / AK7-I) structurally RESOLVED with the Lean type system (not just runtime checks) enforcing the discipline. **AK7-I.cascade RESOLVED via AL1b** (supersedes reverted AL1 runtime-guard approach): new `NonNullCap := { cap : Capability // cap.isNull = false }` subtype; `mintDerivedCap`'s signature tightened from `Capability → …` to `NonNullCap → …` so the Lean compiler rejects any caller feeding a null cap; new distinct `.nullCapability` error code (discriminant 50, synced to Rust ABI) replaces the prior overloaded `.invalidCapability`; `cspaceMint` / `Copy` / `Move` promote via `Capability.toNonNull?`; 14 preservation theorems patched; 7 runtime tests (`al1b_01..07`) including `al1b_07_nullCapability_distinct_from_invalidCapability` as a direct regression test against reintroducing the overloading. **AK7-F.cascade RESOLVED via AL6**: `storeObjectKindChecked` wrapper rejects cross-variant object-store writes with `.error .invalidObjectType`; 3 substantive correctness theorems + 5 runtime tests. Plus AL2 foundational layer: 5 per-variant `getX?` helpers + 23 discrimination lemmas + 8 runtime tests. **AK7-E.cascade RESOLVED via AL7 + AL8** (AL8 supersedes AL7's dispatch-only design): all 8 capability-only handler signatures (`suspendThread`, `resumeThread`, `setIPCBufferOp`, `setPriorityOp`, `setMCPriorityOp`, `schedContextConfigure`, `schedContextBind`, `schedContextUnbind`) now accept `ValidThreadId` / `ValidObjId` directly — removing the sentinel-check produces an immediate compile error. New `validateObjIdArg` dispatch helper; 8 NI-preservation + 2 authority-bounded + 9 IPC-buffer frame theorems signature-updated; test suites promoted via `⟨tid, by decide⟩`. **AL0 monotonicity CI guard** captures 17 metrics. **AL10 integration gate**: version 0.29.14 (patch); 5 cross-cutting runtime tests. **AL11 closure**: DEFERRED doc rewritten with all three cascades marked RESOLVED. Gate: `lake build` (260 jobs, 0 warnings) + `test_smoke.sh` + `test_full.sh` + `test_tier2_negative.sh` (302 checks) + `information_flow_suite` (143 checks) + `ak7_regression_suite` (**73 checks**, up from 38 at v0.29.13) + `suspend_resume_suite` (21) + `priority_management_suite` (24) + `ipc_buffer_suite` (17) + `cargo test --workspace` (415 tests) + `cargo clippy --workspace -- -D warnings` (0 warnings) + `ak7_cascade_check_monotonic.sh` PASS + `check_version_sync.sh` PASS + zero sorry/axiom. Residual hygiene (non-gating, **3 items down from 4** — `AK7-E.hygiene` CLOSED by AL8): **AK7-F.reader.hygiene** (304 consumer call-site migration), **AK7-F.writer.hygiene** (~50 `storeObject` → `storeObjectKindChecked`), **AL6-C.hygiene** (preservation proof for `lifecycleObjectTypeLockstep`). All improve readability / defense-in-depth / proof tightness without affecting correctness; AL1b/AL6/AL8 close the attack surfaces independently. Prior: **WS-AK Phase AK7 COMPLETE** (v0.29.13) — Foundational Model baseline with 11 sub-tasks AK7-A..AK7-K; AK6 (v0.29.9) + AK6-F audit remediation (v0.29.12), AK5 (v0.29.5–v0.29.6), AK4 (v0.29.4–v0.29.8), AK3 (v0.29.3), AK2 (v0.29.2), AK1 (v0.29.1), **WS-AJ PORTFOLIO COMPLETE** (v0.28.1–v0.29.0), WS-AI (v0.27.7–v0.28.0), WS-AH (v0.27.2–v0.27.6), WS-AG (v0.26.0–v0.27.1), WS-AF–WS-B. **Next:** AK7-F reader/writer hygiene (incremental), AK8 (Capability/Lifecycle/Data Structures), AK9 (Platform), AK10 (Closure); then WS-V (AG10: Hardware Integration). |
