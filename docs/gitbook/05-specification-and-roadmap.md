...
| Latest audit | [`AUDIT_v0.29.0_COMPREHENSIVE`](../audits/AUDIT_v0.29.0_COMPREHENSIVE.md) — pre-1.0 full-kernel audit (2 CRITICAL, 23 HIGH, ~76 MED, ~101 LOW). WS-AK Phases AK1–AK4 complete (v0.29.1–v0.29.8); remaining phases AK5–AK10 target v1.0.0. |
| Active workstream | **WS-AK Phase AK4 COMPLETE** (v0.29.8). Pre-1.0 Release Hardening (v0.29.0 audit) — Phase AK4: ABI Bridge — Decode, Types, Validation (8 sub-tasks AK4-A through AK4-H; 1 CRITICAL, 2 HIGH, 4 MEDIUM, 8 LOW). Key fixes: AK4-A (R-ABI-C01 CRITICAL) IPC-buffer merge via `decodeSyscallArgsFromState` + `ipcBufferReadMr` — unblocks `serviceRegister`/`schedContextConfigure`; AK4-B (R-ABI-H02 HIGH) `decodeServiceRegisterArgs` validation tightened to match Rust constants (`methodCount ≤ 1024`, `max*Size ≤ 960`, strict `requiresGrant`); AK4-C (R-ABI-H01 HIGH) `SchedContextId` newtype in Rust with sentinel guard; AK4-E (R-ABI-M02) `decodeCSpaceMintArgs` rejects `rights > 0x1F`; AK4-F (R-ABI-M04) `IpcBuffer::set_mr` symmetry; AK4-G end-to-end `tests/AbiRoundtripSuite.lean` closes audit §9.6 testing gap. Prior: AK3 (v0.29.3), AK2 (v0.29.2), AK1 (v0.29.1), **WS-AJ PORTFOLIO COMPLETE** (v0.28.1–v0.29.0), WS-AI (v0.27.7–v0.28.0), WS-AH (v0.27.2–v0.27.6), WS-AG (v0.26.0–v0.27.1, 10 phases, 67 sub-tasks), WS-AF–WS-B. **Next:** AK5 (Rust HAL Boot Hardening) through AK10 (Testing, Documentation & Closure); then WS-V (AG10: Hardware Integration). |
