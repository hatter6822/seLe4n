...
6. WS-E4 dual endpoint queues remain intrusive-list backed (`Endpoint.sendQ`/`receiveQ` + `TCB.queuePrev`/`queueNext`) with duplicate-wait rejection (`alreadyWaiting`) and executable intrusive-link integrity checks in `negative_state_suite`.
