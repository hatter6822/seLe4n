...
| Active workstream | **WS-AK Phase AK10 COMPLETE — PORTFOLIO CLOSED** (v0.30.6). Phase AK10 (Testing, Documentation & Closure) closes the 10-phase WS-AK remediation portfolio. AK10-A fixture verification (227-line byte-identical match); AK10-B doc sync across README + 10 i18n + SELE4N_SPEC + WORKSTREAM_HISTORY + GitBook + CHANGELOG; AK10-C **patch-only** version bump v0.30.5 → v0.30.6 per maintainer direction (v1.0.0 release tag deferred to manual maintainer action); AK10-D `AUDIT_v0.29.0_ERRATA.md` with six clarifications (E-1 S-H03 verification, E-2 R-HAL-M12 scope, E-3 A-H01 three-layer extent, E-4 R-HAL-H02 partial DSB/ISB, E-5 NI-H02 composition scope, E-6 finding-count arithmetic 202 not 201); AK10-E CLAUDE.md large-file list refresh; AK10-F residual LOW verification (zero `sorry`/`axiom`/`native_decide`); AK10-G website manifest PASS; AK10-H WORKSTREAM_HISTORY WS-AK portfolio summary; AK10-J `AUDIT_v0.29.0_DEFERRED.md` with 11 items (7 → WS-V hardware-binding: TLB+cache composition, tlbBarrierComplete, MMU/Device BarrierKind, suspendThread atomicity, VSpaceRoot boot exclusion, SVC FFI; 4 → post-1.0 hygiene: 17-deep tuple refactor, eventuallyExits, ValidObjId cascade, ObjKind cascade). Prior: AK9 (v0.30.4–v0.30.5), AK8 (v0.30.1–v0.30.3), WS-AM (v0.30.0), WS-AL (v0.29.14), AK7 (v0.29.13), AK6 (v0.29.9 + v0.29.12), AK5 (v0.29.5–v0.29.6), AK4 (v0.29.4–v0.29.8), AK3 (v0.29.3), AK2 (v0.29.2), AK1 (v0.29.1), **WS-AJ PORTFOLIO COMPLETE** (v0.28.1–v0.29.0), WS-AI (v0.27.7–v0.28.0), WS-AH (v0.27.2–v0.27.6), WS-AG (v0.26.0–v0.27.1), WS-AF–WS-B. **Next:** WS-V / AG10 hardware-binding integration (RPi5 first-silicon bring-up; consumes AK3/AK5 outputs). |
