...
| Test LoC | 14,890 across 24 suites |
...
| Active workstream | **WS-AN Phase AN0 COMPLETE — Pre-flight** (v0.30.6). Opens WS-AN, the pre-1.0 remediation workstream for the v0.30.6 comprehensive audit ([`AUDIT_v0.30.6_COMPREHENSIVE.md`](../audits/AUDIT_v0.30.6_COMPREHENSIVE.md), 196 findings: 2 actionable CRITICAL + 23 actionable HIGH + 71 MEDIUM + 59 LOW + 40 INFO, after C-02 already-resolved and H-22 downgraded). 11 phases (AN0..AN10), 79 sub-tasks, ~144 sub-sub-task commits per [`AUDIT_v0.30.6_WORKSTREAM_PLAN.md`](../audits/AUDIT_v0.30.6_WORKSTREAM_PLAN.md). **AN0-A** baseline snapshot — [`AUDIT_v0.30.6_WS_AN_BASELINE.txt`](../audits/AUDIT_v0.30.6_WS_AN_BASELINE.txt) captures the complete WS-AN start state: `lake build` 260 jobs / 0 warnings; `cargo test --workspace` 414 passing / 0 failed; `cargo clippy --workspace -- -D warnings` 0 warnings; Theme 4.7 monolithic file LOCs (IPC/Invariant/Structural.lean 7626 / InformationFlow/Invariant/Operations.lean 3768 / Scheduler/Operations/Preservation.lean 3633 / Capability/Invariant/Preservation.lean 2461 / Lifecycle/Operations.lean 1473); fixture byte-identical to `tests/fixtures/main_trace_smoke.expected` (227 lines); zero `sorry`/`axiom`/`native_decide` in production; 24 Lean test suites / ~1614 assertions. The baseline also records the prior retirement of `scripts/ak7_cascade_baseline.sh` + `scripts/ak7_cascade_check_monotonic.sh` + `docs/audits/AL0_baseline.txt` in commit `8d9e61f` — the three attack surfaces they tracked are now closed structurally at the type system (`NonNullCap` subtype for null-cap, `storeObjectKindChecked` wrapper for kind-swap, `Valid{Thread,Obj,SchedContext,CPtr}Id` subtypes for sentinel dispatch). **AN0-B** — plan itself committed to `claude/audit-workstream-planning-AUBX4` (b2ee03c + 7ac49e7 + cad00cb, merged via PR #736). **AN0-C** — branch-policy reminder. **Next:** AN1 critical-path blockers (C-01 README "Latest audit" pointer, C-03 pre-commit hook auto-installer, H-24 stale Rust HAL TODOs). Prior: **WS-AK Phases AK1–AK10 COMPLETE — PORTFOLIO CLOSED** (v0.29.1–v0.30.6); AK10-A fixture verification; AK10-B doc sync; AK10-C v0.30.5→v0.30.6 patch bump; AK10-D `AUDIT_v0.29.0_ERRATA.md` with six clarifications; AK10-J `AUDIT_v0.29.0_DEFERRED.md` with 11 post-1.0 hardening items (7 hardware-binding, 4 proof-hygiene). Earlier: WS-AM (v0.30.0), WS-AL (v0.29.14), AK7 (v0.29.13), AK6 (v0.29.9+v0.29.12), AK5 (v0.29.5–v0.29.6), AK4 (v0.29.4–v0.29.8), AK3 (v0.29.3), AK2 (v0.29.2), AK1 (v0.29.1), **WS-AJ PORTFOLIO COMPLETE** (v0.28.1–v0.29.0), WS-AI (v0.27.7–v0.28.0), WS-AH (v0.27.2–v0.27.6), WS-AG (v0.26.0–v0.27.1), WS-AF–WS-B. |
