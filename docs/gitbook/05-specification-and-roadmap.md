...
6. WS-E4 dual endpoint queues remain intrusive-list backed (`Endpoint.sendQ`/`receiveQ` + `TCB.queuePPrev`/`queueNext`, mirrored `queuePrev`) with runtime intrusive-queue invariant checks (head/tail coherence, cycle-free traversal, pprev/next linkage) and duplicate-wait rejection (`alreadyWaiting`).
