...
| Active workstream | **WS-RC IN FLIGHT** (v0.30.11 → v0.31.0 → v1.0.0, branch `claude/audit-workstream-planning-XsmKS`). Closes the v0.30.11 audit cycle via 15 phases (R0..R14). Pre-v1.0 must-fix: R0 (audit baseline), R1 (one-line IPC NI symmetry), R2 (FFI dispatch wiring), R3 (boot VSpace threading), R4 (structural-invariant promotions including R4.D witness theorem for DEEP-CAP-02 false positive), R5 (scheduler/lifecycle symmetry), R6 (architecture/info-flow completeness). Post-FFI hygiene: R7..R12 (capability deferred items, test renames, pre-commit hardening, inline polish, documentation accuracy, CI hygiene + R12.B/C/D structural enforcement gates that close DEEP-ARCH-01, DEEP-RUST-01/02, DEEP-ARCH-02 false positives at the CI level). R13 reserved for emergent items; R14 the post-1.0 maintainability backlog. WS-RC R0 prep landed: SPDX header on `SeLe4n.lean` (R10.1), audit errata file (R0.2), staged-module allowlist + partition gate (R12.B.1/2 — caught 2 stale "STATUS: staged" markers on production-wired contract files), orphan-fields gate (R12.D.1), Tier-0 wiring of both gates. Release path: `v0.31.0` "verified specification release" once R0+R1+R8..R12 land; `v1.0.0` "bootable verified microkernel" once R2..R6 additionally land. |
