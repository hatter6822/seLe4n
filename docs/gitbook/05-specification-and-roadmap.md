...
| Active workstream | **WS-AK Phase AK7 COMPLETE** (v0.29.13). Pre-1.0 Release Hardening (v0.29.0 audit) — Phase AK7: Foundational Model (Prelude / Machine / Model). 11 sub-tasks (AK7-A..AK7-K) addressing 2 HIGH, 11 MEDIUM, 15 LOW findings. Highlights: AK7-A `freezeMap_indexMap_invExtK` witness closes F-H01 auto-grow concern; AK7-B `apiInvariantBundle_frozenDirectFull` adds 12 field-agreement conjuncts for machine/TLB/CDT/scheduler closing F-H02 vacuous-preservation; AK7-C `readMemChecked`/`writeMemChecked` bounds-checked memory; AK7-D `MessageInfo.mkChecked` validating constructor; AK7-E `ValidObjId`/`ValidThreadId`/`ValidSchedContextId`/`ValidCPtr` subtype baseline; AK7-F `ObjectKind`+`KindedObjId` discriminator baseline with structural disjointness; AK7-G `TCB.ext` sanctioned extensionality; AK7-H `FrozenMap.wellFormed`; AK7-I `Capability.isNull` seL4_CapNull predicate; AK7-J `cdtNextNodeBounded`+`noPhysicalFrameCollision`+`TlbEntry.asidGeneration`; AK7-K F-L5 reverse permission round-trip + F-L15 CdtNodeId sentinel convention. Prior: AK6 (v0.29.9) + AK6-F audit remediation (v0.29.12), AK5 (v0.29.5–v0.29.6), AK4 (v0.29.4–v0.29.8), AK3 (v0.29.3), AK2 (v0.29.2), AK1 (v0.29.1), **WS-AJ PORTFOLIO COMPLETE** (v0.28.1–v0.29.0), WS-AI (v0.27.7–v0.28.0), WS-AH (v0.27.2–v0.27.6), WS-AG (v0.26.0–v0.27.1, 10 phases, 67 sub-tasks), WS-AF–WS-B. **Next:** AK8 (Capability/Lifecycle/Data Structures), AK9 (Platform), AK10 (Closure); then WS-V (AG10: Hardware Integration). |
