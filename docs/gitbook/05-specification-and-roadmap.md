...
| Active workstream | **WS-SM IN FLIGHT** (v0.31.2 → v0.31.3 → v0.32.x → v0.33.x → v0.34.x → v1.0.0). **Current: SM0–SM9 landed (SM9 declassification completion CLOSED at v0.33.100); WS-RA complete; **WS-RR (SMP release readiness) IN FLIGHT** — the pre-SM10 remediation SM10 is blocked on: RR0 (v0.34.26), RR1 the aarch64 cross gate + TLBI broadcast discipline (v0.34.41), RR2 the live-path invariant gaps (v0.34.42), RR3 `ipcInvariantFull` de-threading + dispatch payoff (v0.34.43), RR4 fault handling with reply-based restart (v0.34.44); RR5–RR8 remain, then SM10 (release closure) — see CLAUDE.md's phase table, [`SMP_RELEASE_READINESS_PLAN.md`](../planning/SMP_RELEASE_READINESS_PLAN.md) and `docs/WORKSTREAM_HISTORY.md` for the per-phase record; the SM0 closeout below is the phase this row was written at.** Unified workstream merging WS-RC's remaining R6..R14 phases with the SMP-specific SM-phases (SM0..SM10). Closes at v1.0.0 with a bootable verified SMP microkernel on Raspberry Pi 5. **WS-SM Phase SM0 (Foundations & honesty patches) COMPLETE** at v0.31.3 (branch `claude/complete-sm0-foundations-gldW8`): 21 sub-tasks across 6 categories. Foundational types (`Concurrency.Types`/`Locks/Kind`/`Locks`/`Sgi` with `numCores`/`CoreId`/`SharingDomain`/`LockKind`/`LockId`/`SgiKind`/`BklState`); AN12-B inventory hardening (NoDup witnesses + build-time `@`-anchors closing SMP-H3, 6-way `archAssumptionConsumer_distinct_6` closing SMP-H2); Rust HAL scaffolding hardening (`.smp_stacks` zeroed at boot closing SMP-M3, TPIDR_EL1 set on `secondary_entry` closing SMP-M4, `MAX_SECONDARY_CORES + 1 == 4` compile-time pin closing SMP-L2); documentation honesty patches (every `dev_history/` cross-reference removed from production sources, every "deferred to WS-V" SMP claim repointed to the WS-SM phase plans, DEF-R-HAL-L20 disposition rewritten to "PARTIALLY RESOLVED — scaffolding only; full activation in WS-SM"); testing infrastructure (`tests/SmpFoundationsSuite.lean` with 41 runtime assertions; tier-4 SMP boot-check stub); WS-RC merge (R0..R5 archived; R6..R14 absorbed into SM0..SM9 per the SM0.Q.1 absorption mapping; sub-portfolio plans archived to `docs/dev_history/audits/`). Plan: [`SMP_MULTICORE_COMPLETION_PLAN.md`](../planning/SMP_MULTICORE_COMPLETION_PLAN.md). SM0 sub-plan: [`SMP_FOUNDATIONS_PLAN.md`](../planning/SMP_FOUNDATIONS_PLAN.md). Items deferred past v1.0.0 with correctness impact: NONE. |
| Predecessor workstream | **WS-RC R0..R5 LANDED** (v0.31.2). The remediation workstream that closed the v0.30.11 audit cycle landed phases R0..R5 (audit baseline, IPC NI symmetry, FFI dispatch wiring, boot VSpace threading, structural-invariant promotions, scheduler/lifecycle symmetry); R6..R14 absorbed into WS-SM per the SM0.Q.1 absorption mapping (see [`AUDIT_v0.30.11_WORKSTREAM_PLAN.md §15`](../audits/AUDIT_v0.30.11_WORKSTREAM_PLAN.md)). WS-RC sub-portfolio plans (R4 close-out, R5 deferred-work substantive completion) archived to [`docs/dev_history/audits/`](../dev_history/audits/) per WS-SM SM0.Q.2. |
| Predecessor workstream (WS-AN) | **WS-AN Phase AN12 COMPLETE — PORTFOLIO CLOSED** (v0.30.11). AN12 lands the cross-cutting Theme 4.1 closure-form discharge index (`docs/dev_history/audits/AUDIT_v0.30.6_DISCHARGE_INDEX.md`, marker theorem `closureForm_discharge_index_documented` in `SeLe4n/Kernel/CrossSubsystem.lean`), Theme 4.4 SMP-latent assumption inventory (`SeLe4n/Kernel/Concurrency/Assumptions.lean` with 8-entry `smpLatentInventory`, mirrored at SPEC §6.8), DOC-M01..M08 batch (SPDX headers added to all 247 `.lean`/`.rs` files; `CONTRIBUTING.md` rewritten), DOC LOW batch (`docs/audits/README.md`, `docs/CLAUDE_HISTORY.md`), in-place RESOLVED annotations on 14 of 15 absorbed `AUDIT_v0.29.0_DEFERRED.md` rows (DEF-F-L9 retained as a post-1.0 cosmetic refactor with no correctness impact), version bump 0.30.10 → 0.30.11. **Items deferred past v1.0.0 with correctness impact: NONE.** Prior phases AN0..AN11 detail in [`docs/WORKSTREAM_HISTORY.md`](../WORKSTREAM_HISTORY.md). |
| Active workstream (legacy) | **WS-AK Phase AK10 COMPLETE — PORTFOLIO CLOSED** (v0.30.6). Phase AK10 (Testing, Documentation & Closure) closes the 10-phase WS-AK remediation portfolio. AK10-A fixture verification (227-line byte-identical match); AK10-B doc sync across README + 10 i18n + SELE4N_SPEC + WORKSTREAM_HISTORY + GitBook + CHANGELOG; AK10-C **patch-only** version bump v0.30.5 → v0.30.6 per maintainer direction (v1.0.0 release tag deferred to manual maintainer action); AK10-D `AUDIT_v0.29.0_ERRATA.md` with six clarifications (E-1 S-H03 verification, E-2 R-HAL-M12 dead-code removal — `b .` replaces unreachable `restore_context` in both SError entries per the audit's original remediation, E-3 A-H01 three-layer extent, E-4 R-HAL-H02 partial DSB/ISB, E-5 NI-H02 composition scope, E-6 finding-count arithmetic 202 not 201); AK10-E CLAUDE.md large-file list refresh; AK10-F residual LOW verification (zero `sorry`/`axiom`/`native_decide`); AK10-G website manifest PASS; AK10-H WORKSTREAM_HISTORY WS-AK portfolio summary; AK10-J `AUDIT_v0.29.0_DEFERRED.md` with 11 items (7 hardware-binding: TLB+cache composition, tlbBarrierComplete, MMU/Device BarrierKind, suspendThread atomicity, VSpaceRoot boot exclusion, SVC FFI; 4 proof-hygiene: 17-deep tuple refactor, eventuallyExits, ValidObjId cascade, ObjKind cascade) — all recorded as **post-1.0 hardening candidates; no currently-active plan file tracks them** (WS-V and AG10 are both closed workstreams per WORKSTREAM_HISTORY.md and cannot be reused as deferral buckets). Prior: AK9 (v0.30.4–v0.30.5), AK8 (v0.30.1–v0.30.3), WS-AM (v0.30.0), WS-AL (v0.29.14), AK7 (v0.29.13), AK6 (v0.29.9 + v0.29.12), AK5 (v0.29.5–v0.29.6), AK4 (v0.29.4–v0.29.8), AK3 (v0.29.3), AK2 (v0.29.2), AK1 (v0.29.1), **WS-AJ PORTFOLIO COMPLETE** (v0.28.1–v0.29.0), WS-AI (v0.27.7–v0.28.0), WS-AH (v0.27.2–v0.27.6), WS-AG (v0.26.0–v0.27.1), WS-AF–WS-B. **Next:** hardware-binding / proof-hygiene items tracked per-ID in `AUDIT_v0.29.0_DEFERRED.md`; a future workstream picking any up should reference the file and update its row. |
...
## Completed: Raspberry Pi 5 Hardware Binding (WS-AG, v0.26.0–v0.27.1)

This section originally listed the hardware binding as the next milestone;
all five items shipped with WS-AG (see chapter 10 for the full record):

1. ~~Populate RPi5 runtime contract with hardware-validated predicates.~~ DONE.
2. ~~Implement ARMv8 multi-level page table walk as a `VSpaceBackend` instance.~~ DONE (AG6).
3. ~~Implement GIC-400 interrupt routing with IRQ acknowledgment.~~ DONE.
4. ~~Bind timer adapter to ARM Generic Timer (CNTPCT_EL0).~~ DONE.
5. ~~Define boot sequence as a verified initial state construction.~~ DONE (WS-RC R3 wired the boot VSpace into production).
...
integrated into `ipcInvariantFull` bundle (9-conjunct as of V3-G6/J/K; 20 conjuncts today). `checkBounds_iff_bounded`
...
## Completed: WS-J1 (with Raspberry Pi 5 hardware binding then queued next)
