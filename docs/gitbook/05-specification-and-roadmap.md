...
| Active workstream | **WS-AL COMPLETE (v0.29.14)** — AK7 cascade closure. All three AK7 deferred cascades (AK7-E / AK7-F / AK7-I) structurally RESOLVED at their primary attack surfaces. **AK7-I.cascade RESOLVED via AL1**: `Capability.requireNotNull` guard wired at `cspaceMint` / `cspaceCopy` / `cspaceMove`; bridge lemma + 7 patched preservation proofs + 3 end-to-end regression tests. **AK7-F.cascade RESOLVED via AL6**: new `storeObjectKindChecked` wrapper in `SeLe4n/Model/State.lean` rejects cross-variant object-store writes with `.error .invalidObjectType` (discriminant 49, synced to Rust ABI); 3 substantive correctness theorems + 5 runtime tests. Plus AL2 foundational layer: 5 per-variant `getX?` helpers + 23 discrimination lemmas (10 rejection + 4 mirror + 5 iff + 1 complement) + 8 runtime tests. **AK7-E.cascade RESOLVED via AL7**: two private helpers `validateThreadIdArg` / `validateSchedContextIdArg` wired at all 8 capability-only dispatch arms, rejecting sentinel IDs with `.error .invalidArgument` before handler entry. **AL0 monotonicity CI guard** captures 17 metrics and enforces per-metric direction on every commit. **AL10 integration gate**: version bumped 0.29.13 → 0.29.14 (patch release, 14 version-bearing files synced); 5 cross-cutting runtime tests (`al10_01..05`) including a defense-in-depth test that exercises all three cascades in one scenario. **AL11 closure**: `docs/audits/AUDIT_v0.29.0_DEFERRED.md` rewritten — all three `AK7-*.cascade` rows marked **RESOLVED** with commit SHAs. Gate at v0.29.14: `lake build` (260 jobs, 0 warnings) + `test_smoke.sh` + `test_full.sh` + `test_tier2_negative.sh` (302 checks) + `information_flow_suite` (143 checks) + `ak7_regression_suite` (**69 checks**, up from 38 at v0.29.13) + `cargo test --workspace` (415 tests) + `cargo clippy --workspace -- -D warnings` (0 warnings) + `ak7_cascade_check_monotonic.sh` PASS + `check_version_sync.sh` PASS + zero sorry/axiom. Residual hygiene tracked (non-gating): **AK7-E.hygiene** (handler-signature tightening) + **AK7-F.hygiene** (304 consumer call-site migration to typed helpers). Both improve readability without affecting correctness; structural guards (AL6, AL7) close the attack surfaces independently. Prior: **WS-AK Phase AK7 COMPLETE** (v0.29.13) — Foundational Model baseline with 11 sub-tasks AK7-A..AK7-K; AK6 (v0.29.9) + AK6-F audit remediation (v0.29.12), AK5 (v0.29.5–v0.29.6), AK4 (v0.29.4–v0.29.8), AK3 (v0.29.3), AK2 (v0.29.2), AK1 (v0.29.1), **WS-AJ PORTFOLIO COMPLETE** (v0.28.1–v0.29.0), WS-AI (v0.27.7–v0.28.0), WS-AH (v0.27.2–v0.27.6), WS-AG (v0.26.0–v0.27.1), WS-AF–WS-B. **Next:** AK7-E/F hygiene (incremental), AK8 (Capability/Lifecycle/Data Structures), AK9 (Platform), AK10 (Closure); then WS-V (AG10: Hardware Integration). |
