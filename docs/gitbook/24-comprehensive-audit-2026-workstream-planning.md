...
- **M-03:** Fixed-priority + EDF tie-breaking — `chooseBestRunnable` upgraded to three-level deterministic selection (priority > deadline > FIFO). `Deadline` typed identifier; `TCB.deadline` field modeling seL4 MCS sporadic-server semantics. EDF replacement predicate with `isBetterCandidate_irrefl` (FIFO stability) and `isBetterCandidate_asymm` (strict ordering) proofs. `edfCurrentHasEarliestDeadline` invariant predicate. Determinism theorems: `chooseBestRunnable_deterministic`, `chooseThread_deterministic`, `chooseThreadInDomain_deterministic`. 3 fixture-backed EDF test scenarios (EDF-01/02/03).
