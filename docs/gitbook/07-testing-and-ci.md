...
- **WS-W Phase W5 (Test Infrastructure & Coverage, v0.22.16):** Consolidated 3 duplicate `expectError` implementations into shared `Helpers.lean` with `msgPrefix` parameter (W5-A). Documented 11-suite OID range table (W5-B). Added chain33 service lifecycle tests — 17 assertions covering `registerInterface`, `registerService` (5 error paths), `serviceRegisterDependency`, `revokeService` (W5-C). Improved `buildChecked` error reporting with check numbers (W5-D). Added mutation testing assertions to chain1/chain5 (W5-E). Removed unused `_hCap` parameter (W5-F). Documented `resolveExtraCaps` silent-drop behavior (W5-G).
