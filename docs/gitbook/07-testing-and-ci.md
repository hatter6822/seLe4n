...
- **WS-I3 (coverage expansion — operations, v0.15.2):** R-06 — new `tests/OperationChainSuite.lean` with 6 compositional operation chains; R-07 — scheduler stress coverage (16-thread repeated scheduling, same-priority deterministic selection, and multi-domain scheduling constraints); R-08 — declassification runtime checks added to `tests/InformationFlowSuite.lean`. Tier 2 now executes OperationChainSuite via `scripts/test_tier2_negative.sh`. Closes R-06, R-07, R-08.
