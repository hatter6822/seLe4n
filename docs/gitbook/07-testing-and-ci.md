...
Current stage context: **All prior workstreams completed through WS-L (v0.16.13). WS-M Phase 1 complete (v0.16.14). WS-M Phase 2 complete (v0.16.15). WS-M Phase 3 plan refined (v0.16.16). Testing tiers enforce regression protection and evidence continuity across 38,868 production LoC, 4,115 test LoC, and 1,240 proved declarations.**
...
- **WS-M (Capability subsystem audit, v0.16.13– — in progress):** End-to-end audit of 5 Capability files (3,520 LoC). 14 findings: M-P01–P05 (performance), M-G01–G04 (proof gaps), M-T01–T03 (test coverage), M-D01–D02 (documentation). Phase M1 (v0.16.14): proof strengthening — 7 new proved declarations. Phase M2 (v0.16.15): performance optimization — M2-A fused revoke (`revokeAndClearRefsState` single-pass), M2-B CDT `parentMap` index for O(1) `parentOf` lookup, M2-C reply lemma extraction and new field preservation lemmas for NI proofs; `parentMapConsistent` runtime check; M-P01/P02/P03/P05 resolved. Phase M3 (v0.16.16): IPC capability transfer plan refined — 20 subtasks modeling seL4 receive-side cap unwrapping with Grant-right gate; 4 test scenarios planned (SCN-IPC-CAP-TRANSFER-*). Phase M4 adds multi-level resolution edge cases, strict revocation stress tests. See [WS-M workstream plan](../audits/AUDIT_v0.16.13_CAPABILITY_SUBSYSTEM_WORKSTREAM_PLAN.md).
