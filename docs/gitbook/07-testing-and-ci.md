...
- **WS-H12 (v0.13.8):** Scheduler & IPC semantic alignment — dequeue-on-dispatch (`scheduleDequeue`, `handleYieldDequeue`, `timerTickDequeue`), per-TCB register context (`registerContext`, `scheduleWithContext`), IPC message bounds (120 regs, 3 caps with `wellFormed`/`isWellFormed`), legacy IPC deprecation. 13 new trace scenarios (H12-01a..H12-03d). `ipc_wf_elim` proof helper for bounds-check elimination. 898 theorem/lemma declarations (zero sorry/axiom).
