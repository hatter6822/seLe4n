...
- `RunQueue` (`Scheduler/RunQueue.lean`, WS-G4/WS-H6) — priority-bucketed run queue with bidirectional structural invariants: `flat_wf` (flat→membership) and `flat_wf_rev` (membership→flat). The theorem `membership_implies_flat` closes the reverse bridge needed by scheduler proof work. `chooseBestInBucket` bucket-first scheduling: O(k) max-priority bucket scan with full-list fallback. `remove` computes filtered bucket once for both `byPriority` and `maxPriority` (v0.12.15 refinement). Implicit `membership` ↔ `threadPriority` consistency maintained by insert/remove API (runtime-verified by `runQueueThreadPriorityConsistentB`).
