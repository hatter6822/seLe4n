...
- **WS-RC R6 (DEEP-ARCH-03, DEEP-IF-01/02, DEEP-IPC-04) (v0.31.3)** — Architecture / InformationFlow completeness closes the four spec-completeness findings.
  - **R6.A (DEEP-ARCH-03)** — Lean-level GIC dispatch bridge from `ExceptionModel` to `InterruptDispatch`.  New `InterruptOp` algebra (`.ack | .eoi | .handle`) + `interruptDispatchSchedule : InterruptId → List InterruptOp` encoding the AN8-C canonical `[.ack, .eoi, .handle]` ordering symbolically.  New `classifyIrqInterruptId` with the consistency theorem `classifyIrqInterruptId_iff_acknowledgeInterrupt_ok`.  Bridge theorem `exception_irq_dispatches_via_interrupt_dispatch` proves three properties simultaneously for a valid IRQ INTID (schedule canonical, dispatch equals `interruptDispatchSequence`, always succeeds).  Bundle composition: `GicDispatchBridgeBundle` / `architectureGicDispatchBridgeBundle` co-located with the bridge in `ExceptionModel.lean` to avoid the `SeLe4n.Kernel.API → Architecture/Invariant → ExceptionModel → API` import cycle; architecture-invariant-family anchor `r6a_gicDispatchBridge_in_architecture_invariant_family : True := trivial` in `Architecture/Invariant.lean`.
  - **R6.B (DEEP-IF-01)** — Verification-only closure: `DeclassificationPolicy` pre-existed at `Policy.lean:879` and is consumed via the import chain `InformationFlow.Enforcement.Soundness.lean → InformationFlow.Enforcement.Wrappers.lean → InformationFlow.Policy.lean`.  Witness theorems anchor the closure: `r6b_declassificationPolicy_defined` (existence) + `r6b_declassificationPolicy_none_denies_all` (semantic non-triviality).
  - **R6.C (DEEP-IF-02)** — SecurityDomain lattice completion (the section flagged truncated in the predecessor deep audit is now structurally complete).  Order primitives `SecurityDomain.le`/`.lt`/`.sup`/`.inf` via `Nat.max`/`Nat.min` on the underlying identifier; Lean-core typeclass instances `LE`/`LT`/`Max`/`Min` (the `⊔`/`⊓` notation is a Mathlib extension this project does not depend on, so `max`/`min` is used throughout).  Eight per-pair lattice laws (six semilattice — `sup_idem`/`sup_comm`/`sup_assoc` + `inf_idem`/`inf_comm`/`inf_assoc` — plus the two absorption laws `absorb_sup_inf`/`absorb_inf_sup`).  Order characterisation theorems `le_sup_left`/`le_sup_right`/`sup_le`/`inf_le_left`/`inf_le_right`/`le_inf`/`le_iff_sup_eq`/`le_iff_inf_eq`.  Prop bundles `SecurityDomainIsSemilatticeSup`/`SecurityDomainIsSemilatticeInf`/`SecurityDomainIsLattice` with witness theorems `securityDomain_isSemilatticeSup`/`securityDomain_isSemilatticeInf`/**`securityDomain_isLattice`** (the lattice witness theorem closing the R6.C.2 obligation).  Bridge theorems `DomainFlowPolicy.linearOrder_canFlow_iff_le` and `domainFlowsTo_linearOrder_iff_le` connect the `flowsTo` relation under `linearOrder` to the lattice's `≤`.
  - **R6.D (DEEP-IPC-04)** — Verification-only closure: the theorem `cleanupPreReceiveDonationChecked_never_errors_under_ipcInvariantFull` pre-existed at `IPC/Invariant/Defs.lean:2630`, was sorry-free, and discharged via the named projection `hInv.donationOwnerValid` (AN3-B.2 named-projection migration).  Plan-compliant alias `cleanupPreReceiveDonation_never_errors_under_ipcInvariantFull` (line 2669).
  - Tests: 10 new R6.A tests in `tests/InterruptDispatchSuite.lean` (t14..t23); 19 new R6.B/R6.C tests in `tests/InformationFlowSuite.lean::runR6InformationFlowCompletenessChecks` (r6b_01..04 + r6c_01..15); 46 new R6 surface anchors in `tests/LivenessSuite.lean`.
  - Discharge index: [`AUDIT_v0.30.11_DISCHARGE_INDEX.md`](../audits/AUDIT_v0.30.11_DISCHARGE_INDEX.md) §3.I (12 rows I.1–I.12).
