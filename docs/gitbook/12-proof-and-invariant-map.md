...
- **Cross-subsystem invariant bundle** (`CrossSubsystem.lean`):
  ...
  - **WS-RC R3 (DEEP-BOOT-01, v0.30.11)**: closes the boot-VSpace threading gap by admitting boot-safe VSpaceRoots into `bootSafeObject` / `bootSafeObjectCheck` and threading the canonical RPi5 boot VSpace through `bootFromPlatformChecked` via the new `installBootVSpaceRoot` builder operation. New companion theorems: `bootSafeVSpaceRootCheck_iff` (Bool↔Prop equivalence), `rpi5BootVSpaceRoot_bootSafeCheck` (the canonical root passes the runtime gate), `rpi5BootVSpaceRoot_mappings_invExt` (`RHTable.invExt` discharge for the 6-mapping table), `bootSafeObjectCheck_admits_rpi5BootVSpaceRoot` and `bootSafeObject_admits_rpi5BootVSpaceRoot` (admission witnesses at the boot-pipeline layer), `installBootVSpaceRoot_objects_lookup` and `installBootVSpaceRoot_asidTable_lookup` (post-state lookup witnesses), `bootFromPlatformChecked_admits_bootVSpace` (sibling of the equality theorem for the with-boot-VSpace case). New runtime gates `bootVSpaceRootObjIdDistinct` and `bootVSpaceRootSafe` enforce ObjId distinctness and boot-safety at the gated boot path. The `installBootVSpaceRoot` builder composes `Builder.createObject` with an `asidTable` insertion so the boot VSpace is resolvable by ASID after boot. `PlatformConfig` gains `bootVSpaceRoot : Option BootVSpaceRootEntry := none`; the `PlatformBinding` typeclass (lifted to `Platform/Contract.lean` for the entry structure) gains the same field. RPi5 binding sets it to `some rpi5BootVSpaceRootEntry`; sim binding sets it to `some simBootVSpaceRootEntry` (a minimal single-mapping root for parity). The pre-R3 theorem `bootFromPlatform_proofLayerInvariantBundle_general` gains a precondition `hNoVSpaceInInitial` (no VSpaceRoots in `initialObjects`) — boot VSpaceRoots are now exclusively introduced via the gated `bootFromPlatformChecked` path. Regression coverage: 8 TPH-015 tests in `tests/TwoPhaseArchSuite.lean` + 5 new assertions in `tests/An9HardwareBindingSuite.lean`. Removes `Platform.RPi5.VSpaceBoot` and `Architecture.AsidManager` from `scripts/staged_module_allowlist.txt` — they enter production via Boot.lean's import.
