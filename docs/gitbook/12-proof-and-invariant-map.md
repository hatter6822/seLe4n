...
- **WS-RC R5 (DEEP-SUSP-01/02, DEEP-SCH-02..06) (v0.31.0)** — scheduler / lifecycle behaviour symmetry closes seven audit findings whose remediation is a behavioural symmetry or function-split.
  ...
  - **R5 deferred-work SUBSTANTIVE COMPLETION (v0.31.2)** — the deferred-completion plan ([`WS_RC_R5_DEFERRED_COMPLETION_PLAN.md`](../audits/WS_RC_R5_DEFERRED_COMPLETION_PLAN.md)) closes the four "AVOIDED" / "UNDER-DELIVERED" items.  **Phase P** (foundational): `blockingAcyclic_of_subgraph`, `blockingChain_subgraph_prefix`, `computeMaxWaiterPriority_frame`, `waitersOf_frame`, `effectiveSchedParams_frame`, `effectiveSchedParams_frame_per_field`, `getSchedContext?_frame`, `objects_insert_non_tcb_non_sc_preserves_boundThreadDomainConsistent`, `objects_update_sync_domain_preserves_boundThreadDomainConsistent` (with `schedContextBindingConsistent` strengthening per ERRATA-R5-2).  **Phase Q (R5.B.2)** substantive: `restoreToReady_preserves_blockingAcyclic`, `ensureRunnable_preserves_computeMaxWaiterPriority`, `resumeThread_postState_shape`, `resumeThread_preserves_blockingAcyclic` (no `hProp` closure), `resumeThread_pipBoost_consistent_with_blocking_graph` (no `hProp` closure).  **Phase R (R5.G.3)** substantive: `schedContextConfigure_preserves_boundThreadDomainConsistent_caseC`, `schedContextConfigure_preserves_boundThreadDomainConsistent_scOnly`, `schedContextConfigure_preserves_boundThreadDomainConsistent` (no `hProp`).  **Phase S (R5.C.1)** full deprecation: `effectivePriority` def + 3 helper theorems + bridge theorem all DELETED; remaining callers migrated to `effectiveSchedParams`.  Discharge index §3.H rows H.19–H.25 added; H.16 marked SUBSTANTIVE; H.9 marked RETIRED.
  - Plan: [`docs/audits/AUDIT_v0.30.11_WORKSTREAM_PLAN.md §9`](../audits/AUDIT_v0.30.11_WORKSTREAM_PLAN.md#9-phase-r5--scheduler-lifecycle-behaviour-symmetry-deep-susp-0102-deep-sch-0206); deferred-completion: [`WS_RC_R5_DEFERRED_COMPLETION_PLAN.md`](../audits/WS_RC_R5_DEFERRED_COMPLETION_PLAN.md).
- **WS-RC R6 (DEEP-ARCH-03, DEEP-IF-01, DEEP-IF-02, DEEP-IPC-04) (v0.31.2)** — architecture / information-flow completeness closes the four spec-completeness DEEP findings of v0.30.11.
  - **R6.A (DEEP-ARCH-03)** — formal GIC dispatch bridge from `ExceptionModel` to `InterruptDispatch`.  New `InterruptOp` algebra (`.ack id`, `.eoi id`, `.handle id`), symbolic plan `interruptDispatchPlan : InterruptId → List InterruptOp` returning the AN8-C-ordered list `[.ack id, .eoi id, .handle id]`, five plan-ordering witnesses (`interruptDispatchPlan_length`, `_ack_head`, `_eoi_second`, `_handle_third`, `_decomposes`), and the bridge theorem `exception_irq_dispatches_via_interrupt_dispatch` proving both the plan ordering (`rfl`) and the runtime delegation `dispatchException .irq ≡ interruptDispatchSequence`.  Convenience form `exception_irq_dispatches_when_handled` (raw-INTID side).  `GICDispatchBridge` Prop bundle + `gicDispatchBridge_holds` witness package the bridge for downstream consumers.  Source: `SeLe4n/Kernel/Architecture/ExceptionModel.lean`.
  - **R6.A.3 (DEEP-ARCH-03)** — `ArchitectureInvariantBundle (st : SystemState) : Prop` composite joining `proofLayerInvariantBundle st` (from `Architecture/Invariant.lean`) with the static `gicDispatchPlanInvariant : Prop := ∀ id, GICDispatchBridge id` (discharged by `gicDispatchPlanInvariant_holds`).  Constructor `ArchitectureInvariantBundle.of_proofLayer`, default-state witness `default_system_state_architectureInvariantBundle`, three adapter-primitive preservation theorems (`advanceTimerState_preserves_architectureInvariantBundle`, `writeRegisterState_preserves_architectureInvariantBundle`, `contextSwitchState_preserves_architectureInvariantBundle`), and two projections (`toProofLayer`, `toGicDispatchPlan`).  The composite lives in `Architecture/ExceptionModel.lean` (not `Architecture/Invariant.lean`) to avoid the import cycle `Kernel.API → Architecture.Invariant` / `Architecture.ExceptionModel → Kernel.API`; a "WS-RC R6.A.3" cross-reference section in `Architecture/Invariant.lean` points readers at the composite's actual location.
  - **R6.B (DEEP-IF-01) — discharge-only** — `DeclassificationPolicy` already defined at `Kernel/InformationFlow/Policy.lean:879` with `canDeclassify`, `isDeclassificationAuthorized`, and the `none` (strictest) constructor.  `Enforcement/Soundness.lean` consumes it via `Enforcement/Wrappers.lean → InformationFlow/Policy.lean`.  R6.B records the structure in the discharge index (§3.I row I.11).
  - **R6.C (DEEP-IF-02)** — `SecurityDomain` lattice completion under the canonical `linearOrder` policy.  Pre-R6.C the H-04 section at `Kernel/InformationFlow/Policy.lean:484` promised "reflexivity, transitivity, antisymmetry ... proved generically under policy constraints" but only delivered a pre-order.  R6.C adds `SecurityDomain.sup` / `_inf` (Nat-indexed sup/inf) + `Max` and `Min` instances; six lattice laws (`sup_assoc`, `sup_comm`, `sup_self` idempotency, `inf_assoc`, `inf_comm`, `inf_self`); two absorption laws (`absorb_sup_inf`, `absorb_inf_sup`); the two order-characterising bridges `linearOrder_canFlow_iff_sup_eq` and `_inf_eq`; antisymmetry (`linearOrder_canFlow_antisymm` — the third pre-order law promised by the spec header) + the `antisymmetric` predicate + `DomainFlowPolicy.linearOrder_antisymm`; and the `SecurityDomainLattice` Prop bundle + `securityDomain_complete_lattice` witness.
  - **R6.D (DEEP-IPC-04) — discharge-only** — `cleanupPreReceiveDonationChecked_never_errors_under_ipcInvariantFull` at `Kernel/IPC/Invariant/Defs.lean:2630` proves the cleanup-error branch unreachable under `ipcInvariantFull` + participant non-reservation, sorry-free.  Plan-named alias `cleanupPreReceiveDonation_never_errors_under_ipcInvariantFull` retained.  R6.D records both names in the discharge index (§3.I row I.20).
  - Tests: `runR6CSecurityDomainLatticeChecks` (17 lattice runtime assertions in `InformationFlowSuite`), `test_t14..t18` (5 GIC dispatch tests in `InterruptDispatchSuite` including bundle elaboration), `runR6PhaseChecks` (6 negative guards in `NegativeStateSuite`: plan head is `.ack`, plan index 1 is `.eoi`, misorder distinguishable from canonical plan, `linearOrder` asymmetric on distinct domains, `sup`/`inf` respect ordering — guarding against `Nat.max`/`Nat.min` swap), and 32 new `#check` surface anchors in `LivenessSuite` covering every R6 theorem, structure, and bridge.  Discharge index [`AUDIT_v0.30.11_DISCHARGE_INDEX.md`](../audits/AUDIT_v0.30.11_DISCHARGE_INDEX.md) §3.I records 20 rows (I.1..I.20).
  - Plan: [`docs/audits/AUDIT_v0.30.11_WORKSTREAM_PLAN.md §10`](../audits/AUDIT_v0.30.11_WORKSTREAM_PLAN.md).
  - **R6 deferred-completion (v0.31.2)** — four items identified by the post-R6-landing audit as "not literally satisfied" are now closed:
    - **Marker theorem** `closureForm_ws_rc_extensions_documented : True := trivial` added to `Kernel/CrossSubsystem.lean` per the discharge-index update protocol §6 (triggered by §3.D/§3.E/§3.F/§3.H/§3.I becoming substantive).
    - **Plan-named aliases** `flowsTo_iff_sup_eq` and `flowsTo_iff_inf_eq` (as `abbrev` aliases of `SecurityDomain.linearOrder_canFlow_iff_*_eq`) in `Kernel/InformationFlow/Policy.lean`, plus two renamed legacy-label bridges (`securityFlowsTo_to_linearOrder_canFlow`, `securityFlowsTo_implies_embedded_sup_eq` — the earlier names `integrityFlowsTo_to_linearOrder_canFlow` and `_iff_embedded_sup_eq` were misleading: the hypothesis is `securityFlowsTo` not `integrityFlowsTo`, and the embedding is one-directional not iff) and a substantive integrity bridge: `integrityToNat` encoding, `integrityFlowsTo_iff_integrityToNat_le` (BIBA-inverted equivalence on the encoded integrity sub-lattice), and `integrityFlowsTo_iff_canFlow_via_integrityToNat` (connecting `integrityFlowsTo` to the SecurityDomain lattice via the encoded integrity component).
    - **In-house Mathlib-compatible typeclass hierarchy** in `Kernel/InformationFlow/Policy.lean`: `Preorder`, `PartialOrder`, `Sup`, `Inf`, `SemilatticeSup`, `SemilatticeInf`, `Lattice` with the same class names and field names as Mathlib so a future Mathlib import is a drop-in replacement.  Four generic lattice-law theorems (`SemilatticeSup.sup_assoc'`, `_sup_comm'`, `Lattice.absorb_sup_inf'`, `_absorb_inf_sup'`) proven from the typeclass axioms (demonstrating mathematical soundness — the laws are theorems, not extra axioms).  `SecurityDomain` instances for every class in the hierarchy, discharged by `Nat.le_max_*`, `Nat.min_le_*`, `Nat.le_antisymm` (all Lean core, no Mathlib).  Plus `LE`/`LT`/`Decidable LE/LT` instances for `SecurityDomain` with `_iff_id_*` simp lemmas.
    - **GIC plan invariant added as 12th conjunct of `proofLayerInvariantBundle`** in `Kernel/Architecture/Invariant.lean` (closing the audit plan's literal §10.6 R6.A.3 requirement).  New upstream module `Kernel/Architecture/GicDispatchPlanCore.lean` (imports only `SeLe4n.Prelude`) hosts `InterruptId`, `InterruptOp`, `interruptDispatchPlan`, the five plan-ordering witnesses, and `gicDispatchPlanStaticInvariant : Prop` + `_holds`.  `Architecture/InterruptDispatch.lean` now imports this upstream module (replacing its local `InterruptId` abbrev).  `Architecture/ExceptionModel.lean` keeps the runtime-delegation half of the bridge.  `proofLayerInvariantBundle` now has 12 conjuncts (was 11); the constructor, all three preservation theorems, and `Platform/Boot.lean`'s `bootFromPlatform_proofLayerInvariantBundle_general` updated to discharge the 12th component via `gicDispatchPlanStaticInvariant_holds`.
    - Tests: 5 new IF runtime typeclass-dispatch checks, 2 new NegativeStateSuite typeclass-dispatch guards + marker witness check, 22 new LivenessSuite surface anchors covering the in-house typeclass hierarchy, instances, plan-named aliases, integrity bridge, upstream static invariant, and the marker theorem.  Discharge index §3.I extended with 6 new rows (I.21–I.26).
    - R6 is now in **full literal plan compliance** with §10.6.  Items deferred past v1.0.0 with correctness impact: **NONE**.
