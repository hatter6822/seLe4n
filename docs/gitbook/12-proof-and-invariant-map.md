...
- **Cross-subsystem invariant bundle** (`CrossSubsystem.lean`):
  ...
  - **AN7-D.2 (PLT-M02/PLT-M03, v0.30.8)**: new `SeLe4n/Platform/RPi5/VSpaceBoot.lean` module establishes the canonical **Raspberry Pi 5 boot VSpaceRoot** (`rpi5BootVSpaceRoot`). Ships `VSpaceRootWxCompliant`, `VSpaceRootWellFormed`, and `bootSafeVSpaceRoot` predicates on a per-root basis, plus four substantively-proven theorems (`_asid`, `_wxCompliant`, `_wellFormed`, `_bootSafe`). Kernel image layout mirrors `rust/sele4n-hal/src/mmu.rs::BOOT_L1_TABLE` — kernel text RX, kernel data RW, stack RW, UART0/GIC distributor/GIC CPU-interface RW non-executable. **Closes DEF-P-L9** (VSpaceRoot boot exclusion deferred item). The full cascade rewrite of `bootSafeObject` to admit well-formed VSpaceRoots in the production boot sweep is tracked for AN9 hardware-binding closure (AN9-E cross-reference).
