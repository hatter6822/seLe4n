...
- **WS-RC R6 (DEEP-ARCH-03, DEEP-IF-01, DEEP-IF-02, DEEP-IPC-04) (v0.31.2)** — architecture / information-flow completeness closes the four spec-completeness DEEP findings of v0.30.11.
  ...
  - **R6 deferred-completion (v0.31.2)** — four items identified by the post-R6-landing audit as "not literally satisfied" are now closed:
    ...
    - **Plan-named aliases** `flowsTo_iff_sup_eq` and `flowsTo_iff_inf_eq` (as `abbrev` aliases of `SecurityDomain.linearOrder_canFlow_iff_*_eq`) in `Kernel/InformationFlow/Policy.lean`, plus substantive integrity bridge `integrityFlowsTo_to_linearOrder_canFlow` and combined witness `securityFlowsTo_iff_embedded_sup_eq` connecting the legacy BIBA-inverted integrity comparison to the `SecurityDomain` lattice via `embedLegacyLabel`.
