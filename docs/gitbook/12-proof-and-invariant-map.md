...
- **WS-RC R4 close-out (v0.31.0)** — completes the structural-invariant retirement that the v0.30.11 R4 partial landing left open.  **The state-level `cspaceSlotUnique` and `uniqueWaiters` predicates have been deleted entirely**; the substantive content is carried structurally by `UniqueSlotMap.hWF` (on `CNode.slots`) and `NoDupList.hNodup` (on `Notification.waitingThreads`).  No backwards-compatibility shims remain.
  ...
  - Plan: [`docs/dev_history/audits/WS_RC_R4_CLOSEOUT_PLAN.md`](../dev_history/audits/WS_RC_R4_CLOSEOUT_PLAN.md) (archived per WS-SM SM0.Q.2). Version bumped 0.30.11 → 0.31.0.
- **WS-RC R5 (DEEP-SUSP-01/02, DEEP-SCH-02..06) (v0.31.0)** — scheduler / lifecycle behaviour symmetry closes seven audit findings whose remediation is a behavioural symmetry or function-split.
  - **R5.A (DEEP-SUSP-02)** — `cancelDonation` split into `cancelBoundDonation` (in-place SchedContext unbind) and `cancelDonatedDonation` (return-to-original-owner via `cleanupDonatedSchedContext`); `cancelDonation` retained as a thin dispatcher.  Six new preservation theorems lift the existing `scheduler.runQueue/current` and `serviceRegistry` invariants through the split.  Source: `SeLe4n/Kernel/Lifecycle/Suspend.lean` + `SeLe4n/Kernel/Lifecycle/Invariant/SuspendPreservation.lean`.
  - **R5.B (DEEP-SUSP-01)** — `resumeThread` re-derives `pipBoost` from the post-suspend blocking graph via `PriorityInheritance.computeMaxWaiterPriority`.  Structural witnesses: `restoreToReady_objectIndex_eq`, `restoreToReady_objects_eq_at_tid`, `resumeThread_pipBoost_consistent_post_restore`.
  - **R5.C (DEEP-SCH-02)** — total `effectiveSchedParams : SystemState → TCB → Priority × Deadline × DomainId` unifies the divergent `effectivePriority` (`Option`) and `resolveEffectivePrioDeadline` (total) APIs.  Bridge witnesses: `effectiveSchedParams_priority_deadline_eq_resolve`, `effectivePriority_some_eq_effectiveSchedParams`.
  - **R5.D (DEEP-SCH-03)** — IPC-state-clearing transition shared between `cancelIpcBlocking` (suspend G2) and `resumeThread` (H3) extracted as `restoreToReady`.  The private `clearTcbIpcFields` retained as a `@[inline]` back-compat shim; `clearTcbIpcFields_eq_restoreToReady` bridges the two names.
  - **R5.E (DEEP-SCH-04)** — new `KernelError.missingSchedContext` (discriminant 52) replaces the silent `(state, false)` no-preempt fallback in `timerTickBudget`'s bound-budget branch.  Rust `KernelError` enum mirror grows in lock-step (range 0..=52, sentinel range 53..=254); `FrozenOps.frozenTimerTickBudget` mirrored.
  - **R5.F (DEEP-SCH-05)** — formal precondition assertion theorems for `RunQueue.rotateToBack`: `rotateToBack_requires_membership` (from `wellFormed` + membership, derives `∃ p, threadPriority[tid]? = some p`) and `rotateToBack_priority_eq_threadPriority` (corollary).
  - **R5.G (DEEP-SCH-06)** — domain-propagation block in `schedContextConfigure` mirroring the existing priority-propagation block.  Witness theorems: `schedContextConfigure_bound_tcb_domain_eq`, `schedContextConfigure_domain_noop_when_eq`.
  - Tests: 13 new Lean regression tests (4 R5.A + 3 R5.B + 2 R5.D + 1 R5.E + 3 R5.G; R5.B's `sr027b_resumeRecomputesPipBoostWithWaiters` is the substantive with-waiter PIP-recomputation test added at the audit pass) plus 1 R5.E discriminant pin in `SyscallDispatchSuite`, 24 R5 surface anchors in `LivenessSuite`, 2 new Rust tests for the new discriminant.  AK7 cascade monotonicity preserved at the v0.30.11 floor.
  - **R5 audit pass additions (non-behavioural)**: `cancelBoundDonation_preserves_projection` / `cancelDonatedDonation_preserves_projection` closure-form IF helpers in `Kernel/InformationFlow/Invariant/Operations.lean` (per-arm symmetry with AK6-F.17); `schedContextConfigure_preserves_boundThreadDomainConsistent` closure-form preservation in `Kernel/SchedContext/Invariant/Preservation.lean`; R5.E comment correction (the `.error` short-circuit does NOT advance the timer; pre-audit comment misclaimed otherwise); substantive R5.B test for the with-waiter recomputation path.
  - **R5 deferred-work SUBSTANTIVE COMPLETION (v0.31.2)** — the deferred-completion plan ([`WS_RC_R5_DEFERRED_COMPLETION_PLAN.md`](../dev_history/audits/WS_RC_R5_DEFERRED_COMPLETION_PLAN.md)) closes the four "AVOIDED" / "UNDER-DELIVERED" items.  **Phase P** (foundational): `blockingAcyclic_of_subgraph`, `blockingChain_subgraph_prefix`, `computeMaxWaiterPriority_frame`, `waitersOf_frame`, `effectiveSchedParams_frame`, `effectiveSchedParams_frame_per_field`, `getSchedContext?_frame`, `objects_insert_non_tcb_non_sc_preserves_boundThreadDomainConsistent`, `objects_update_sync_domain_preserves_boundThreadDomainConsistent` (with `schedContextBindingConsistent` strengthening per ERRATA-R5-2).  **Phase Q (R5.B.2)** substantive: `restoreToReady_preserves_blockingAcyclic`, `ensureRunnable_preserves_computeMaxWaiterPriority`, `resumeThread_postState_shape`, `resumeThread_preserves_blockingAcyclic` (no `hProp` closure), `resumeThread_pipBoost_consistent_with_blocking_graph` (no `hProp` closure).  **Phase R (R5.G.3)** substantive: `schedContextConfigure_preserves_boundThreadDomainConsistent_caseC`, `schedContextConfigure_preserves_boundThreadDomainConsistent_scOnly`, `schedContextConfigure_preserves_boundThreadDomainConsistent` (no `hProp`).  **Phase S (R5.C.1)** full deprecation: `effectivePriority` def + 3 helper theorems + bridge theorem all DELETED; remaining callers migrated to `effectiveSchedParams`.  Discharge index §3.H rows H.19–H.25 added; H.16 marked SUBSTANTIVE; H.9 marked RETIRED.
  - Plan: [`docs/audits/AUDIT_v0.30.11_WORKSTREAM_PLAN.md §9`](../audits/AUDIT_v0.30.11_WORKSTREAM_PLAN.md#9-phase-r5--scheduler-lifecycle-behaviour-symmetry-deep-susp-0102-deep-sch-0206); deferred-completion: [`WS_RC_R5_DEFERRED_COMPLETION_PLAN.md`](../dev_history/audits/WS_RC_R5_DEFERRED_COMPLETION_PLAN.md).
...
- **Cross-subsystem invariant bundle** (`CrossSubsystem.lean`):
  ...
  - **AN7-D.2 (PLT-M02/PLT-M03, v0.30.8)**: new `SeLe4n/Platform/RPi5/VSpaceBoot.lean` module establishes the canonical **Raspberry Pi 5 boot VSpaceRoot** (`rpi5BootVSpaceRoot`). Ships `VSpaceRootWxCompliant`, `VSpaceRootPaddrBounded`, `VSpaceRootWellFormed` (4-conjunct), and `bootSafeVSpaceRoot` predicates on a per-root basis, plus five substantively-proven theorems (`_asid`, `_wxCompliant`, `_paddrBounded`, `_wellFormed`, `_bootSafe`). The 4-conjunct well-formedness ensures: (1) `asid.val ≤ maxAsidValue`, (2) every mapping is W^X compliant, (3) at least one mapping present, (4) every mapping's `paddr.toNat < 2^44` (BCM2712 PA bound). Kernel image layout mirrors `rust/sele4n-hal/src/mmu.rs::BOOT_L1_TABLE` — kernel text RX, kernel data RW, stack RW, UART0/GIC distributor/GIC CPU-interface RW non-executable. **Closes DEF-P-L9** (VSpaceRoot boot exclusion deferred item). The full cascade rewrite of `bootSafeObject` to admit well-formed VSpaceRoots in the production boot sweep landed in **WS-RC R3 (DEEP-BOOT-01, v0.30.11)**, see below.
  - **WS-RC R3 (DEEP-BOOT-01, v0.30.11)**: closes the boot-VSpace threading gap by admitting boot-safe VSpaceRoots into `bootSafeObject` / `bootSafeObjectCheck` and threading the canonical RPi5 boot VSpace through `bootFromPlatformChecked` via the new `installBootVSpaceRoot` builder operation. New companion theorems: `bootSafeVSpaceRootCheck_iff` (Bool↔Prop equivalence), `rpi5BootVSpaceRoot_bootSafeCheck` (the canonical root passes the runtime gate), `rpi5BootVSpaceRoot_mappings_invExt` (`RHTable.invExt` discharge for the 6-mapping table), `bootSafeObjectCheck_admits_rpi5BootVSpaceRoot` and `bootSafeObject_admits_rpi5BootVSpaceRoot` (admission witnesses at the boot-pipeline layer), `installBootVSpaceRoot_objects_lookup` and `installBootVSpaceRoot_asidTable_lookup` (post-state lookup witnesses), `bootFromPlatformChecked_admits_bootVSpace` (sibling of the equality theorem for the with-boot-VSpace case). Four runtime gates enforce config-level boot safety: `noVSpaceRootsInInitialObjects` (forbids VSpaceRoots in `initialObjects` because `Builder.createObject` does NOT update `asidTable`), `bootVSpaceRootObjIdDistinct` (no ObjId collision), `bootVSpaceRootObjIdNonSentinel` (no `ObjId.sentinel` per H-06/WS-E3), and `bootVSpaceRootSafe` (passes `bootSafeVSpaceRootCheck`). The `installBootVSpaceRoot` builder composes `Builder.createObject` with an `asidTable` insertion so the boot VSpace is resolvable by ASID after boot. `PlatformConfig` gains `bootVSpaceRoot : Option BootVSpaceRootEntry := none`; the `PlatformBinding` typeclass (with the entry structure lifted to `Platform/Contract.lean`) gains the same field. RPi5 binding sets it to `some rpi5BootVSpaceRootEntry` at non-sentinel `ObjId.ofNat 1`; sim binding sets it to `some simBootVSpaceRootEntry` (a minimal single-mapping root for parity). The pre-R3 theorem `bootFromPlatform_proofLayerInvariantBundle_general` gains a precondition `hNoVSpaceInInitial` (no VSpaceRoots in `initialObjects`) — boot VSpaceRoots are now exclusively introduced via the gated `bootFromPlatformChecked` path. Regression coverage: 11 TPH-015 tests in `tests/TwoPhaseArchSuite.lean` (3 added in audit pass: TPH-015i VSpaceRoot-in-initialObjects rejection, TPH-015j sentinel-ObjId rejection, TPH-015k unsafe-VSpaceRoot rejection) + 5 new assertions in `tests/An9HardwareBindingSuite.lean`. Removes `Platform.RPi5.VSpaceBoot` and `Architecture.AsidManager` from `scripts/staged_module_allowlist.txt` — they enter production via Boot.lean's import.
  ...
  - **AH5-C (M-08)**: Composition coverage assessment documented — 10 predicates yield 45 pairwise analyses; remaining gap scenarios (IPC queue ↔ service registry, capability revocation ↔ service lifecycle); full formal composition proof deferred to post-1.0 hardening (separate workstream; not in WS-SM SMP scope)
...
> 54 once round 37 re-routed `.tcbSetAffinity` — `enforcementBoundaryPerCore_count`
> is the machine-checked figure).
...
* **The 2PL bracket promoted** (SM8.E.3): `enforcementBoundary` 39 → 40
  (12 policy-gated, 26 capability-only, 4 read-only), appended last so the
...
to declassify at all until reboot.  The capacity refusal is correct — a
downgrade the kernel authorized and did not record is the failure SM8.C exists
to exclude — so the fix is a reader and a drain, not a softer bound.

* **The clearance-filtered, re-indexed view** (SM9.A.1).  `auditLogVisibleTo` is
  a genuine `Sublist` and — the no-gap-leak property —
...
* **The persistent timestamp epoch** (SM9.A.1a), sequenced **before** drain
  ...
  new entry collides with the surviving `2` — which falsifies
  `declassificationAuditLog_timestamp_identifies_event`.
  `SystemState.declassificationAuditEpoch` makes the stamp `epoch + log.length`;
  the counterexample is kept as `preEpochTimestamp_reused_after_drain` so a
  regression fails to build, and the timestamp layer moved down to
  `AuditRecord.lean` (SM7.A's `TlbInvalidation` extraction) so the production
  drain can state its preservation.
* **The chunk protocol** (SM9.A.2).  Every exported field is an unbounded `Nat`
  — `SecurityDomain.id`, `ObjId.val`, the timestamp — and `x0` carries 63 usable
  bits, so a fixed low/high pair would only move the truncation point to `2^64`.
  Values ship through 32-bit chunks with exact reconstruction
  (`auditReadField_reconstructs`), failing closed above `maxAuditFieldChunks`
  with `KernelError.auditFieldTooLarge` rather than truncating; the *chunk
  coordinates* are themselves single words, so "total for any `Nat`" was never
  available.  The basis **designation** ships the same way, since exporting the
...
* **The reader-visibility discipline** (SM9.A.4a).  Adding a reader changes what
  ...
  **false** (`lowEquivalent_does_not_determine_visible_view`) and cannot be the
  lemma.  `auditObservationalEquivalence`'s clause set is a **total function** on
  `ReadableStructure`, not a list: `readableStructure_list_gate_insufficient`
  refutes the list design, and `AuditReadOp` is *fused* with `ReadableStructure`
  so a read operation cannot exist without naming what it reads.
* **Authority is a `CapTarget`, not a right** (SM9.A.9).  `syscallLookupCap`
  never constrains `cap.target`, so a rights-only gate would repeat the
  v0.32.97 confused-deputy class exactly — a thread holding a writable
  capability to its own TCB would drain the trail.  `CapTarget.auditTrail` is
  fieldless (the trail is a singleton `SystemState` component) and
  `extractAuditAuthority` is the gate.
* **The live arms** (SM9.A.10).  `SyscallId.auditRead = 31` / `.auditDrain = 32`
  (count 33), both `.word`-shaped so WS-RA's `writeReturnFrameToTcb` carries the
...
### Layer 3 under SMP — the data-carrying declassification (WS-SM SM9.C)

SM8.C's `.declassify` authorizes a downgrade and records it while moving no user
data: the store it performs is the model's *simulation* of a transfer, and
performing one from userspace would let a caller install a chosen `KernelObject`
at a chosen id.  SM9.C is the syscall that moves the data.

`notificationSignalDeclassifiedOnCore` (`InformationFlow/DeclassifiedSignal.lean`,
**production** — the live arm calls it) is a real SM6.B bound signal: the badge
lands where the ordinary signal would put it, a waiter or the notification's
bound TCB is woken on its **own** home core, and the committed post-state is
literally SM6.B's with `declassificationAuditLog` replaced
(`notificationSignalDeclassifiedOnCore_frame`).  That frame is what lets the
whole IPC invariant surface transfer rather than be re-proven.

* **Two hops, not one** (SM9.C.1).  The live checked `.notificationSignal`
  already gates `notification → receiver` as well as `signaler → notification`
  — added at v0.31.73 to stop a badge leak into a low bound TCB — so a
  declassifying variant gated only on the notification would have been strictly
  *weaker* than the syscall it wraps while carrying stronger authority.
  `declassifiedSignalHopAuthorization` runs per hop against the base lattice
  first and the configured declassification policy second, and each hop carries
  its **own** refusal discriminant (`DeclassifiedSignalHop.refusal_injective`).
  The second is the new `KernelError.declassificationDeniedAtReceiver` (56): a
  monitor reading a bare "denied" cannot tell an unauthorized caller from an
  authorized caller aimed at an unauthorized sink, and those call for opposite
  responses.

* **One event per authorized downgrade, and no invented edge.**
  `declassifiedSignal_audits_each_hop` records the two hops in order, the first
  naming the notification and the second the receiver's own TCB;
  `declassifiedSignal_no_invented_edge` is the property a *single* record could
  not have — collapsing `high → mid` and `mid → low` into one entry would put a
  direct `high → low` edge in the trail that no run of
  `declassificationDecision` ever returned `.ok` for.  And
...
* **The actor field.**  A second-hop event's `srcDomain` is the *notification's*
  domain, which is nobody's subject domain — so SM8.C's attributability rule,
  which read `srcDomain`, could not be stated of it.  `DeclassificationEvent`
  gains a **required** `actor : DeclassificationActor` (a default would
  attribute every event to whatever it names while compiling everywhere), and
  `attributionFromRunningSubject_over_actor` is SM8.C's rule restated over it.
  `secondHop_actor_differs_from_flowSource` is what makes the field necessary
  rather than redundant.  The visibility filter reads it
  (`auditLogVisibleTo_cleared_actor`), so a third exported domain is disclosed
  only to a reader that dominates it.

* **A retired trail invariant.**  `auditTrailDestinationsAreTargetDomains`
  (SM9.A round 5) is **false** of a second-hop event: its destination is a
  *thread* domain while its target is that thread's TCB.  Retired, with the
  object-identity discipline moved into `auditEntryVisibleTo` itself — strictly
  stronger, since `auditVisibleEntry_target_domain_flows` becomes unconditional.

* **A footprint is not an authorization** (SM9.C.5).
  `declassifiedSignalEffectFootprint` is defined **once** (notification ⊕
  resolved receiver ⊕ SM6.B's own `notificationSignalBoundWriteSet`) and read by
  both the confinement proof and the non-interference theorem, so the two cannot
  name different cores.  It takes **no policy**, which is what makes
...
* **Declassification-relative non-interference** (SM9.C.6).  This is the tree's
  ...
  domain pair `declassificationDecision` authorized, and the object effect
  bounded to the ordinary signal's.  The sharpest negative is that this
  transition is **not** plain non-interference at all — the footprint's core
  really changes — which is exactly why the statement is relative to a
  footprint.

* **The NI taxonomy that deliberately did not grow** (SM9.C.7).  Every
  `NonInterferenceStep` constructor concludes the projection is *unchanged*
  ...
  reported as coverage of the whole.  SM8.C set the precedent —
  `declassifyObjectFromCore` is likewise absent from `KernelOperation` — so the
  inventory that grew is `CrossCoreTransition` (28 → 29, live arms 21 → 22,
  remote writers 22 → 23, delegation-backed 13 → 14), and `KernelOperation`'s
  docstring records the exclusion with a Tier-3 negative pinning it.

Registries: `SyscallId.declassifySignal = 33`, count 33 → **34**; the
enforcement boundary 42 → **43** canonical / 57 → **58** per-core,
**policy-gated** and naming the live arm; the lock-set inventory 107 → **109**,
with `lockSet_declassifySignal` *composed* from `lockSet_notificationSignal`
plus the state-level write its trail append needs
(`…_extends_notificationSignal` is the tie, so the notification half cannot
drift from the syscall it wraps) and `auditState_footprints_share_serialization`
gaining a fourth conjunct — this is the one trail writer whose footprint is
*dominated* by object-level members and would otherwise look like an ordinary
IPC syscall.  Three total-match tables each forced a recorded decision as part
of adding the syscall: `refusalSeamClass` → `.records` (so its refusals reach
the SM9.B ledger rather than bypassing it, exactly as that phase's docstring
predicted), `syscallReturnShape` → `.unit`, `syscallChecksTargetFirst` →
`false`.  The per-core routing gate passes with **zero** allowlisted exceptions.
...
their `none` defaults, so the one footprint shape that can reach five members
had no size bound at all and the WCRT reasoning built on `maxLockSetSize` did not
cover the path SM6.B added (now general — `3 + 3 = 6 ≤ 8`, constant untouched);
and `endpointQueueRemoveDual_machine_eq`'s ~120-line duplicated branch walk is
re-derived from a new generic `endpointQueueRemoveDual_frame`.

Runtime coverage: §11.1–§11.6 of `tests/SmpInformationFlowSuite.lean` (679 →
...
2. **Donation chain acyclicity**: `donationChainAcyclic` — no circular SchedContext
   ...
   sub-relation is deferred to post-1.0 hardening (separate workstream;
   not in WS-SM SMP scope)
