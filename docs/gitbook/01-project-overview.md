...
- **WS-F** (v0.12.2 audit remediation): closing proof gaps identified by two independent audits.
  ...
  - **WS-F3** (Information-flow completeness): **COMPLETED** — `ObservableState` extended with 3 fields (activeDomain, irqHandlers, objectIndex); CNode slot filtering; 7+ NI theorems; WS-F3 test suite.
  - WS-F4..F8: planning.
