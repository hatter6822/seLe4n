...
- **WS-D4:** Kernel design hardening (medium; **completed** — F-07, F-11, F-12)
  - Service dependency cycle detection with BFS reachability (F-07, TPI-D07 partially closed). **Done.**
    - **Risk 0 resolved:** Vacuous invariant corrected via Strategy B (declarative acyclicity).
      Layer 0 definitions (`serviceEdge`, `serviceReachable`, `serviceHasNontrivialPath`) committed.
      TPI-D07 proof infrastructure plan added. M0 baseline lock in progress.
