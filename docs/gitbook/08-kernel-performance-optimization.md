...
## 4b. WS-M capability performance optimizations (v0.16.14–v0.16.19)
...
| **M-P04** | `descendantsOf` materializes full O(N+E) set upfront | `streamingRevokeBFS` level-by-level BFS traversal; `cspaceRevokeCdtStreaming` avoids full materialization | v0.16.19 |
| **M-P05** | `endpointReply_preserves_capabilityInvariantBundle` duplication | Unified via extracted lemmas from M-P03 | v0.16.15 |

### M-P04: Streaming BFS revocation (v0.16.19)

`cspaceRevokeCdtStreaming` performs level-by-level BFS revocation via
`streamingRevokeBFS`, processing one CDT level at a time without materializing
the full `descendantsOf` set. Each BFS step discovers only the immediate children
of the current frontier, avoiding O(N+E) upfront computation for deep derivation
trees.

Preservation is proved in two layers:
- `streamingRevokeBFS_step_preserves` — single-level invariant maintenance
