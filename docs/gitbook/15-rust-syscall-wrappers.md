...
| `ticket_lock` | Verified FIFO spinlock | **WS-SM SM2.B v0.31.9 — NEW MODULE**.  `TicketLock` `#[repr(C, align(64))]` cache-line aligned struct with two `AtomicU64` fields (`next_ticket`, `serving`); `acquire(&self) -> u64` (atomic `fetch_add(1, Acquire)` capture + spin-loop on `serving.load(Acquire)` with `wfe_bounded` parking); `release(&self)` (atomic `fetch_add(1, Release)` advance + `sev` broadcast + debug-assert misuse detection); `with_lock<F, R>(&self, f: F) -> R` RAII combinator + `TicketLockGuard<'a>` panic-safe guard.  21 unit tests including panic-safety, cross-thread mutex/FIFO stress, debug-assert misuse, cache-line alignment, const-fn pinning, signature pinning, 100-cycle monotone counter.  Refines the Lean spec at `SeLe4n/Kernel/Concurrency/Locks/TicketLock.lean` (all 6 substantive theorems machine-checked) |
| `rw_lock` | Verified reader-writer lock | **WS-SM SM2.C — NEW MODULE**.  `RwLock` `#[repr(C, align(64))]` cache-line aligned struct with one bit-packed `AtomicU64` `state` field (bit 63 = writer-held, bits 0..62 = reader count); `acquire_read` / `release_read` / `acquire_write` / `release_write` (audit-pass release-build hardening: `release_read` uses `fetch_sub(1, Release)` defending against state corruption on misuse; `release_write` uses `fetch_and(READER_MASK, Release)` preserving reader bits on misuse); `with_read<F, R>` / `with_write<F, R>` RAII combinators + `RwLockReadGuard<'a>` / `RwLockWriteGuard<'a>` panic-safe guards; `snapshot() -> (bool, u64)` introspection.  27 unit tests including 3 cross-thread stress tests (4-reader stress, mixed reader/writer, deterministic reader-multiplicity via two-phase `std::sync::Barrier`).  Refines the Lean spec at `SeLe4n/Kernel/Concurrency/Locks/RwLock.lean` (all 10 substantive theorems machine-checked) via the `RwLockRefinement.lean` bridge (the bridge documents the FIFO divergence: the Rust CAS-retry impl satisfies mutex + exclusion but not the spec's strict FIFO admission, since the Rust state has no explicit waiters queue) |
