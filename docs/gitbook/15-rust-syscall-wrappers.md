...
| `ffi` | Lean FFI bridge | 25 `#[no_mangle] extern "C"` exports for timer, GIC, TLB, MMIO, UART, interrupts, suspendThread bracket, cache+TLB composition, per-CPU core id (`ffi_current_core_id` at SM1.B.5), **`ffi_tlbi_for_sharing` / `ffi_send_sgi*` (WS-SM SM1.E.4 + SM1.F.6 v0.31.7)** (AG7 + AN9-A/D + WS-SM SM1.B/E/F) |
