...
| `smp` | Secondary-core bring-up | `SMP_ENABLED: AtomicBool` (default `false`), `CORE_READY: [AtomicBool; 4]`, `bring_up_secondaries`, `rust_secondary_main` (SM1.C full per-core init pipeline: MMU → VBAR → GIC → timer → IRQ → Lean kernel via `lean_secondary_kernel_main`); SM1.B back-compat re-exports of `PerCpuData`, `PER_CPU_DATA`, `PER_CPU_DATA_SLOT_SIZE*`, `per_cpu_slot_addr` (AN9-J v0.30.10 — DEF-R-HAL-L20; SM1.C v0.31.5 closes SMP-C2; v1.0.0 ships SMP merged but disabled) |
...
| `boot` | Boot sequence | `_start` → BSS zero → stack → UART → MMU → VBAR → GIC → timer → TPIDR_EL1 → IRQ → kernel; `install_exception_vectors()` (WS-SM SM1.C.2 v0.31.5 — extracted from formerly-private `set_vbar`, shared with `smp::rust_secondary_main`) |
| `gic` | GIC-400 driver | Distributor + CPU interface init, acknowledge/dispatch/EOI, `dispatch_irq<F>()` (AG5), **EOI-before-handler ordering** (AN8-C v0.30.9 — H-19, audit Option b), boot-time `self_check_distributor` (AN8-D RUST-M05); `init_cpu_interface_secondary(core_id)` (WS-SM SM1.C.3 v0.31.5 — per-core CPU interface init for secondaries) |
