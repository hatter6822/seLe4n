...
/-- §3.2: SM5.J.2 — the RPi5 §3.9 bound `≤ maxLockSetSize · 3 · tCs = 13·180 = 2340`,
...
11 and **WS-OD OD3.7** from 11 to 13, so the uniform envelope moved
1440 → 1620 → 1980 → 2340 µs.  What fits the 1 ms budget is the *typical*
...
private def runRpi5BoundChecks : IO Unit := do
  ...
  -- 9 → 11 at WS-OD OD3.5, 11 → 13 at WS-OD OD3.7).  Pinning only the product is
  ...
  assertBool "the per-lock, per-core cost 3 · tCs = 180 µs (the board's half)"
    (decide (3 * tCs60 = 180))
  assertBool "the RPi5 uniform envelope maxLockSetSize·3·60 = 2340 (moves with the ceiling)"
    (decide (maxLockSetSize * (3 * tCs60) = 2340))
  ...
  assertBool "chooseThreadOnCore lock-WCRT (360) ≤ RPi5 bound (2340)"
    (decide (WCRT_lockSet (chooseThreadOnCoreLockSet bootCoreId) tCs60 ≤ maxLockSetSize * (3 * tCs60)))
  assertBool "timerTickOnCore lock-WCRT (540) ≤ RPi5 bound (2340)"
  ...
  -- `1000 ÷ 39 = 25 µs` at WS-OD OD3.7's ceiling of thirteen (it was
  -- `1000 ÷ 33 = 30 µs` at OD3.5's eleven and `1000 ÷ 27 = 37 µs` at RR7.11's
  -- nine), and every declared footprint then fits by
  ...
  assertBool "the admissible-cost divisor is maxLockSetSize · (numCores − 1) = 39"
    (decide (maxLockSetSize * (numCores - 1) = 39))
  assertBool "…so the 1 ms tick admits a 25 µs per-lock critical section"
    (decide (admissibleCriticalSection rpi5TickBudgetMicros = 25))
