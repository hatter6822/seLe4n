...
/-- §3.2: SM5.J.2 — the RPi5 §3.9 bound `≤ maxLockSetSize · 3 · tCs = 11·180 = 1980`,
and the typical `|lockSet| ≤ 4` syscall fits the 1 ms (1000 µs) timer-tick budget
(`4 · 3 · 60 = 720 < 1000`).

**WS-RR RR7.11** moved `maxLockSetSize` from 8 to 9 and **WS-OD OD3.5** from
9 to 11, so the uniform envelope moved 1440 → 1620 → 1980 µs.  What fits the 1 ms budget is the *typical*
...
private def runRpi5BoundChecks : IO Unit := do
  ...
  -- 9 → 11 at WS-OD OD3.5).  Pinning only the product is what made this witness
  -- go stale twice: it reads as a claim about the board when it is a claim
  -- about the ceiling.
  assertBool "the per-lock, per-core cost 3 · tCs = 180 µs (the board's half)"
    (decide (3 * tCs60 = 180))
  assertBool "the RPi5 uniform envelope maxLockSetSize·3·60 = 1980 (moves with the ceiling)"
    (decide (maxLockSetSize * (3 * tCs60) = 1980))
  ...
  assertBool "chooseThreadOnCore lock-WCRT (360) ≤ RPi5 bound (1980)"
    (decide (WCRT_lockSet (chooseThreadOnCoreLockSet bootCoreId) tCs60 ≤ maxLockSetSize * (3 * tCs60)))
  assertBool "timerTickOnCore lock-WCRT (540) ≤ RPi5 bound (1980)"
  ...
  -- `1000 ÷ 33 = 30 µs` at WS-OD OD3.5's ceiling of eleven (it was `1000 ÷ 27 =
  -- 37 µs` at RR7.11's nine), and every declared footprint then fits by
  ...
  assertBool "the admissible-cost divisor is maxLockSetSize · (numCores − 1) = 33"
    (decide (maxLockSetSize * (numCores - 1) = 33))
  assertBool "…so the 1 ms tick admits a 30 µs per-lock critical section"
    (decide (admissibleCriticalSection rpi5TickBudgetMicros = 30))
