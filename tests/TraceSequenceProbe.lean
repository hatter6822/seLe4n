...
def probeBaseState (threadCount : Nat) : SystemState :=
  ...
  { (default : SystemState) with
    ...
    scheduler := (default : SchedulerState).setRunQueueOnCore SeLe4n.Kernel.Concurrency.bootCoreId (SeLe4n.Kernel.RunQueue.ofList (runnableThreads.map (fun tid => (tid, ⟨0⟩))))
  }
...
def stepOp (op : ProbeOp) (tid : SeLe4n.ThreadId) (st : SystemState) : StepOutcome :=
  ...
  | .send =>
      ...
      else match SeLe4n.Kernel.endpointSendDualChecked SeLe4n.Kernel.harnessLabelingContext probeEndpointId tid .empty default default default st with
      | .ok (_, st') => .mutated st'
