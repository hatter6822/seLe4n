...
private def fp001_singleLookup : IO Unit := do
  ...
  let fm := freezeMap rt
  expect "frozen get? matches insert" (fm.get? ⟨5⟩ == some 100)
  expect "frozen get? missing key" (fm.get? ⟨99⟩ == none)
  expect "source get? matches" (rt.get? ⟨5⟩ == fm.get? ⟨5⟩)
...
private def fp002_multiLookup : IO Unit := do
  let rt := ((RHTable.empty 16 : RHTable ObjId Nat).insert ⟨1⟩ 10)
    ...
    |>.insert ⟨3⟩ 30
  let fm := freezeMap rt
  expect "key 1" (rt.get? ⟨1⟩ == fm.get? ⟨1⟩)
  expect "key 2" (rt.get? ⟨2⟩ == fm.get? ⟨2⟩)
  expect "key 3" (rt.get? ⟨3⟩ == fm.get? ⟨3⟩)
  expect "missing" (rt.get? ⟨99⟩ == fm.get? ⟨99⟩)
...
private def fp003_emptyLookup : IO Unit := do
  ...
  let fm := freezeMap rt
  expect "empty get? key 0" (fm.get? ⟨0⟩ == none)
  expect "empty get? key 42" (fm.get? ⟨42⟩ == none)
  expect "source agrees" (rt.get? ⟨0⟩ == fm.get? ⟨0⟩)
...
private def fp004_overwrite : IO Unit := do
  let rt := ((RHTable.empty 16 : RHTable ObjId Nat).insert ⟨1⟩ 10)
    |>.insert ⟨1⟩ 99
  let fm := freezeMap rt
  expect "overwritten key" (fm.get? ⟨1⟩ == some 99)
  expect "source agrees" (rt.get? ⟨1⟩ == fm.get? ⟨1⟩)
...
private def fp005_largeLookup : IO Unit := do
  ...
  expect "all 10 entries match" allMatch
  expect "missing key 99" (fm.get? ⟨99⟩ == none)
...
private def fp006_irqHandlers : IO Unit := do
  ...
  let fss := freeze ist
  expect "empty irq lookup" (fss.irqHandlers.get? ⟨0⟩ == none)
  expect "empty irq lookup 5" (fss.irqHandlers.get? ⟨5⟩ == none)

/-- FP-007: ASID table field — freeze preserves lookup -/
private def fp007_asidTable : IO Unit := do
  let ist := mkEmptyIntermediateState
  let fss := freeze ist
  expect "empty asid lookup" (fss.asidTable.get? ⟨0⟩ == none)

/-- FP-008: Object types field — freeze preserves lookup -/
private def fp008_objectTypes : IO Unit := do
  let ist := mkEmptyIntermediateState
  let fss := freeze ist
  expect "empty objectTypes lookup" (fss.objectTypes.get? ⟨0⟩ == none)
...
private def fp009_cdtMaps : IO Unit := do
  ...
  expect "cdtChildMap empty" (fss.cdtChildMap.get? nodeId == none)
  expect "cdtParentMap empty" (fss.cdtParentMap.get? nodeId == none)
...
private def fp009c_objectsField : IO Unit := do
  ...
  expect "objects lookup none" ((fss.objects.get? ⟨0⟩).isNone)
...
private def fp013_cnodeRadixStructure : IO Unit := do
  ...
  match frozen with
  | .cnode fc =>
    expect "guardWidth preserved" (fc.guardWidth == 2)
    expect "guardValue preserved" (fc.guardValue == 1)
    expect "radixWidth preserved" (fc.radixWidth == 3)
  | _ => throw <| IO.userError "expected FrozenCNode"
...
private def fp014_preservesSize : IO Unit := do
  let rt := ((RHTable.empty 16 : RHTable ObjId Nat).insert ⟨1⟩ 10)
    ...
    |>.insert ⟨3⟩ 30
  let fm := freezeMap rt
  expect "data size equals entry count" (fm.data.size == 3)
...
private def fp015_preservesMembership : IO Unit := do
  let rt := ((RHTable.empty 16 : RHTable ObjId Nat).insert ⟨1⟩ 10)
    |>.insert ⟨2⟩ 20
  let fm := freezeMap rt
  expect "present key isSome" ((fm.get? ⟨1⟩).isSome)
  expect "present key isSome" ((fm.get? ⟨2⟩).isSome)
  expect "absent key isNone" ((fm.get? ⟨99⟩).isNone)
...
private def fp016_deterministic : IO Unit := do
  ...
  let fss1 := freeze ist
  let fss2 := freeze ist
  expect "same objects size" (fss1.objects.data.size == fss2.objects.data.size)
  expect "same irq size" (fss1.irqHandlers.data.size == fss2.irqHandlers.data.size)
    ...
    for j in List.range keys.size do
      if i != j then
        if fm.get? keys[i]! == fm.get? keys[j]! then
          allDistinct := false
  expect "all keys distinct values" allDistinct
...
private def fp018_totalCoverage : IO Unit := do
  let rt := ((RHTable.empty 16 : RHTable ObjId Nat).insert ⟨10⟩ 100)
    |>.insert ⟨20⟩ 200
  let fm := freezeMap rt
  expect "key 10 covered" ((fm.get? ⟨10⟩).isSome)
  expect "key 20 covered" ((fm.get? ⟨20⟩).isSome)
  expect "data size matches" (fm.data.size == 2)
...
private def fp020_fieldParity : IO Unit := do
  ...
  expect "objects accessible" (fss.objects.data.size == 0)
  expect "irqHandlers accessible" (fss.irqHandlers.data.size == 0)
...
private def fp021_lookupTransfer : IO Unit := do
  ...
  expect "transfer key 7" (rt.get? ⟨7⟩ == fm.get? ⟨7⟩)
  expect "transfer key 13" (rt.get? ⟨13⟩ == fm.get? ⟨13⟩)
  expect "transfer key 42" (rt.get? ⟨42⟩ == fm.get? ⟨42⟩)
  expect "transfer absent key" (rt.get? ⟨99⟩ == fm.get? ⟨99⟩)
