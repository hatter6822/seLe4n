...
def runInformationFlowChecks : IO Unit := do
  ...
  -- SM8.E.3 — the SM3 two-phase-locking bracket, capability-only, promoted here
  -- from the separate per-core list SM8.B introduced), declassifyObjectFromCore
  -- (WS-SM SM8.C — the live declassification entry point, policy-gated),
  ...
  expect "enforcement boundary: total 47 classified operations"
    (SeLe4n.Kernel.enforcementBoundary.length == 47)
  ...
  -- information-flow policy.  WS-SM SM9.A.11: 24 → 26 with the audit trail's
  ...
  expect "enforcement boundary has 30 capability-only"
    (coCount = 30)
  expect "enforcement boundary has 4 read-only"
    (roCount = 4)
  expect "enforcement boundary total is 47"
    (boundary.length = 47)
  ...
  expect "enforcementBoundaryExtended has 47 entries"
    (SeLe4n.Kernel.enforcementBoundaryExtended.length = 47)
