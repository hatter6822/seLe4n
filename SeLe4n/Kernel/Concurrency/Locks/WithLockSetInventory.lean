...
instance : Inhabited WithLockSetTheorem :=
  ⟨{ descriptionKey := 1, identifierKey := 1, _elabCheck := (), category := default }⟩

/-- The entry's description, unpacked from its key. -/
def WithLockSetTheorem.description (t : WithLockSetTheorem) : String := stringOfPacked t.descriptionKey

/-- The entry's fully-qualified name, unpacked from its key. -/
def WithLockSetTheorem.identifier (t : WithLockSetTheorem) : String := stringOfPacked t.identifierKey
...
theorem withLockSetTheorems_identifiers_nodup :
    (withLockSetTheorems.map (·.identifier)).Nodup :=
  nodup_map_stringOfPacked WithLockSetTheorem.identifierKey (fun t _ => t.identifierKey_wf)
    (by decide +kernel)

/-- WS-SM SM3.C: every inventory description is unique — the same key-level
argument as `withLockSetTheorems_identifiers_nodup`, over `descriptionKey`. -/
theorem withLockSetTheorems_descriptions_nodup :
    (withLockSetTheorems.map (·.description)).Nodup :=
  nodup_map_stringOfPacked WithLockSetTheorem.descriptionKey (fun t _ => t.descriptionKey_wf)
    (by decide +kernel)
