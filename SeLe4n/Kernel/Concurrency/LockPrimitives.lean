...
def lockPrimitives : List LockPrimitiveTheorem := [
  ...
  { description := "RwLock withdrawal safety: cancelling a queued request preserves all five INV-R conjuncts",
    ...
    category    := .rwLock },
  { description := "RwLock withdrawal exclusion: a cancel never touches writerHeld and admits only the reader run behind a withdrawn head",
    identifier  := `SeLe4n.Kernel.Concurrency.rwLock_cancel_admits_only_the_head_reader_run,
    category    := .rwLock },
