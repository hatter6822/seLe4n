...
structure PhaseTheoremEntry where
  ...
  /-- **Theorems** among those entries: the entries whose declaration type is a
      ...
      *not* `entryCount`: the inventories register a phase's whole surface, so
      217 of the 1138 entries are `def`s — lock-set footprints, per-core
...
/-- SM3's registered **entry** count is the sum of its five SM3.A–E
    inventories' lengths.  Of those 428 entries, 287 are propositions. -/
...
theorem smp_inventoried_theorem_count : smpInventoriedTheoremCount = 921 := by
  decide

/-- Entries in the same inventories: 1138, of which 217 are `def`s rather than
    proofs.  Kept beside the theorem count so the gap is a number a reader can
    see, not a caveat they have to be told. -/
theorem smp_inventoried_entry_count : smpInventoriedEntryCount = 1138 := by
  decide
...
theorem smp_inventoried_theorem_count_lt_entry_count :
    smpInventoriedTheoremCount + 217 = smpInventoriedEntryCount := by
  decide
