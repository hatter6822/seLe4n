...
/-- WS-SM SM5.H: a theorem entry in the SM5.H inventory.  Records a description and
...
instance : Inhabited PerCoreCbsTheorem :=
  ⟨{ descriptionKey := 1, identifierKey := 1, _elabCheck := (), category := default }⟩

/-- The entry's description, unpacked from its key. -/
def PerCoreCbsTheorem.description (t : PerCoreCbsTheorem) : String := stringOfPacked t.descriptionKey

/-- The entry's fully-qualified name, unpacked from its key. -/
def PerCoreCbsTheorem.identifier (t : PerCoreCbsTheorem) : String := stringOfPacked t.identifierKey
...
/-- WS-SM SM5.H: every inventory identifier is unique.  Kernel-checked, never
...
theorem perCoreCbsTheorems_identifiers_nodup :
    (perCoreCbsTheorems.map (·.identifier)).Nodup :=
  nodup_map_stringOfPacked PerCoreCbsTheorem.identifierKey (fun t _ => t.identifierKey_wf)
    (by decide +kernel)

/-- WS-SM SM5.H: every inventory description is unique — the same key-level
argument as `perCoreCbsTheorems_identifiers_nodup`, over `descriptionKey`. -/
theorem perCoreCbsTheorems_descriptions_nodup :
    (perCoreCbsTheorems.map (·.description)).Nodup :=
  nodup_map_stringOfPacked PerCoreCbsTheorem.descriptionKey (fun t _ => t.descriptionKey_wf)
    (by decide +kernel)
