...
other queue's tail). -/
theorem endpointQueueEnqueue_blockStore_establishes_endpointQueueTailBlockedConsistent
    ...
    (hFreshTid : ∀ (epId : SeLe4n.ObjId) (e : Endpoint),
      ...
      e.sendQ.tail ≠ some tid ∧ e.receiveQ.tail ≠ some tid)
    ...
    endpointQueueTailBlockedConsistent st2 := by
  have hObjInv1 := endpointQueueEnqueue_preserves_objects_invExt endpointId isReceiveQ tid st st1 hObjInv hEnq
  obtain ⟨epEnq, hEpEnq, hEnqTail, hOtherTail⟩ :=
    endpointQueueEnqueue_enqueued_is_tail endpointId isReceiveQ tid st st1 ep hObjInv hObj hEnq
  intro epId' ep' tl tcb hEp' hTcb
  have hEp'1 : st1.objects[epId']? = some (.endpoint ep') :=
    storeTcbIpcStateAndMessage_endpoint_backward st1 st2 tid blockState msg epId' ep' hObjInv1 hStore hEp'
  -- Resolve st1's endpoint at epId': the enqueue-updated endpoint (epId' = endpointId), or a
  -- pre-state endpoint (epId' ≠ endpointId).
  have hEpResolve : (epId' = endpointId ∧ ep' = epEnq) ∨
      (epId' ≠ endpointId ∧ st.objects[epId']? = some (.endpoint ep')) := by
    by_cases hEq : epId' = endpointId
    · left; refine ⟨hEq, ?_⟩
      have hEp'1' : st1.objects[endpointId]? = some (.endpoint ep') := hEq ▸ hEp'1
      simpa using (hEpEnq.symm.trans hEp'1').symm
    · right; exact ⟨hEq, endpointQueueEnqueue_endpoint_backward_ne endpointId isReceiveQ tid st st1
        epId' ep' hEq hObjInv hEnq hEp'1⟩
  refine ⟨fun hTl => ?_, fun hTl => ?_⟩
  · -- receiveQ.tail = some tl → tcb.ipcState = .blockedOnReceive epId'
    by_cases htid : tl = tid
    · subst tl
      have hState : tcb.ipcState = blockState :=
        storeTcbIpcStateAndMessage_ipcState_eq st1 st2 tid blockState msg hObjInv1 hStore tcb hTcb
      rcases hEpResolve with ⟨hEpIdEq, hEp'Eq⟩ | ⟨_, hPre⟩
      · subst epId'; subst ep'
        by_cases hR : isReceiveQ
        · rw [hState]; simpa [hR] using hBlock
        · have hOt : epEnq.receiveQ.tail = ep.receiveQ.tail := by simpa [hR] using hOtherTail
          rw [hOt] at hTl
          exact absurd hTl (hFreshTid endpointId ep hObj).2
      · exact absurd hTl (hFreshTid epId' ep' hPre).2
    · have hNeTl : tl.toObjId ≠ tid.toObjId := fun h => htid (ThreadId.toObjId_injective tl tid h)
      have hTcb1 : st1.objects[tl.toObjId]? = some (.tcb tcb) := by
        rw [← storeTcbIpcStateAndMessage_preserves_objects_ne st1 st2 tid blockState msg tl.toObjId hNeTl hObjInv1 hStore]
        exact hTcb
      obtain ⟨tcbPre, hTcbPre, hIpcEq⟩ :=
        endpointQueueEnqueue_tcb_ipcState_backward endpointId isReceiveQ tid st st1 tl tcb hObjInv hEnq hTcb1
      rw [← hIpcEq]
      rcases hEpResolve with ⟨hEpIdEq, hEp'Eq⟩ | ⟨_, hPre⟩
      · subst epId'; subst ep'
        by_cases hR : isReceiveQ
        · have hEt : epEnq.receiveQ.tail = some tid := by simpa [hR] using hEnqTail
          rw [hEt] at hTl
          exact absurd (Option.some.inj hTl).symm htid
        · have hOt : epEnq.receiveQ.tail = ep.receiveQ.tail := by simpa [hR] using hOtherTail
          rw [hOt] at hTl
          exact (hTail endpointId ep tl tcbPre hObj hTcbPre).1 hTl
      · exact (hTail epId' ep' tl tcbPre hPre hTcbPre).1 hTl
  · -- sendQ.tail = some tl → tcb.ipcState = .blockedOnSend epId' ∨ .blockedOnCall epId'
    by_cases htid : tl = tid
    · subst tl
      have hState : tcb.ipcState = blockState :=
        storeTcbIpcStateAndMessage_ipcState_eq st1 st2 tid blockState msg hObjInv1 hStore tcb hTcb
      ...
      · subst epId'; subst ep'
        by_cases hR : isReceiveQ
        · have hOt : epEnq.sendQ.tail = ep.sendQ.tail := by simpa [hR] using hOtherTail
          rw [hOt] at hTl
          exact absurd hTl (hFreshTid endpointId ep hObj).1
        · rw [hState]; simpa [hR] using hBlock
      · exact absurd hTl (hFreshTid epId' ep' hPre).1
    · have hNeTl : tl.toObjId ≠ tid.toObjId := fun h => htid (ThreadId.toObjId_injective tl tid h)
      have hTcb1 : st1.objects[tl.toObjId]? = some (.tcb tcb) := by
        rw [← storeTcbIpcStateAndMessage_preserves_objects_ne st1 st2 tid blockState msg tl.toObjId hNeTl hObjInv1 hStore]
        exact hTcb
      ...
      · subst epId'; subst ep'
        ...
        · have hOt : epEnq.sendQ.tail = ep.sendQ.tail := by simpa [hR] using hOtherTail
          rw [hOt] at hTl
          exact (hTail endpointId ep tl tcbPre hObj hTcbPre).2 hTl
        · have hEt : epEnq.sendQ.tail = some tid := by simpa [hR] using hEnqTail
          rw [hEt] at hTl
          exact absurd (Option.some.inj hTl).symm htid
      · exact (hTail epId' ep' tl tcbPre hPre hTcbPre).2 hTl

open SeLe4n.Model.SystemState in
/-- IPC de-threading D4 Slice 2b core (c) variant: same as
`endpointQueueEnqueue_blockStore_establishes_endpointQueueTailBlockedConsistent` but for the
`storeTcbIpcState` block-store (`endpointReceiveDual`'s block leg writes ipcState only, no
pendingMessage).  The proof is identical modulo the store lemma family. -/
theorem endpointQueueEnqueue_blockStoreIpc_establishes_endpointQueueTailBlockedConsistent
    (endpointId : SeLe4n.ObjId) (isReceiveQ : Bool) (tid : SeLe4n.ThreadId)
    (st st1 st2 : SystemState) (ep : Endpoint)
  ...
  intro epId' ep' tl tcb hEp' hTcb
  have hEp'1 : st1.objects[epId']? = some (.endpoint ep') :=
    storeTcbIpcState_endpoint_backward st1 st2 tid blockState epId' ep' hObjInv1 hStore hEp'
  -- Resolve st1's endpoint at epId': the enqueue-updated endpoint (epId' = endpointId), or a
  -- pre-state endpoint (epId' ≠ endpointId).
  have hEpResolve : (epId' = endpointId ∧ ep' = epEnq) ∨
      (epId' ≠ endpointId ∧ st.objects[epId']? = some (.endpoint ep')) := by
  ...
  · -- receiveQ.tail = some tl → tcb.ipcState = .blockedOnReceive epId'
    ...
    · subst tl
      have hState : tcb.ipcState = blockState :=
        storeTcbIpcState_ipcState_eq st1 st2 tid blockState hObjInv1 hStore tcb hTcb
      rcases hEpResolve with ⟨hEpIdEq, hEp'Eq⟩ | ⟨_, hPre⟩
      · subst epId'; subst ep'
        by_cases hR : isReceiveQ
        · rw [hState]; simpa [hR] using hBlock
    ...
    · have hNeTl : tl.toObjId ≠ tid.toObjId := fun h => htid (ThreadId.toObjId_injective tl tid h)
      have hTcb1 : st1.objects[tl.toObjId]? = some (.tcb tcb) := by
        rw [← storeTcbIpcState_preserves_objects_ne st1 st2 tid blockState tl.toObjId hNeTl hObjInv1 hStore]
        exact hTcb
      obtain ⟨tcbPre, hTcbPre, hIpcEq⟩ :=
        endpointQueueEnqueue_tcb_ipcState_backward endpointId isReceiveQ tid st st1 tl tcb hObjInv hEnq hTcb1
      rw [← hIpcEq]
...
open SeLe4n.Model.SystemState in
/-- `storeTcbQueueLinks` `queueNext`-backward: every post-state TCB's `queueNext` is either preserved
from the pre-state or equals the stored `next` (when the TCB is the store target). -/
theorem storeTcbQueueLinks_tcb_queueNext_backward
    ...
    ∃ tcb, st.objects[y.toObjId]? = some (.tcb tcb) ∧
      (tcb.queueNext = tcb'.queueNext ∨ tcb'.queueNext = next) := by
  ...
  · obtain ⟨origTcb, hOrig⟩ : ∃ t, st.objects[tid.toObjId]? = some (.tcb t) := by
    ...
    rw [hEq] at hTcb'
    rw [hTcbS] at hTcb'
    obtain rfl : tcbS = tcb' := by simpa using hTcb'
    exact ⟨origTcb, by rw [hEq]; exact hOrig, Or.inr hQNS⟩
  · rw [storeTcbQueueLinks_preserves_objects_ne st st' tid prev pprev next y.toObjId hEq hObjInv hStep] at hTcb'
    exact ⟨tcb', hTcb', Or.inl rfl⟩

open SeLe4n.Model.SystemState in
/-- `endpointQueueEnqueue` `queueNext`-backward (for threads other than the enqueued one): the
enqueue only ever sets `oldTail.queueNext := some enqueueTid` (and `enqueueTid.queueNext := none`), so
for any `anyTid ≠ enqueueTid`, its post-state `queueNext` is either preserved from the pre-state or
equals `some enqueueTid` (when `anyTid` is the old tail). -/
theorem endpointQueueEnqueue_tcb_queueNext_backward_ne
    (endpointId : SeLe4n.ObjId) (isReceiveQ : Bool)
          ...
          · revert hStep
            ...
            | none =>
              ...
              | ok pair =>
                ...
                rw [storeTcbQueueLinks_preserves_objects_ne pair.2 st' enqueueTid none
                  (some .endpointHead) none anyTid.toObjId hNe hInv1 hStep] at hTcb'
                by_cases hEqEp : anyTid.toObjId = endpointId
                · rw [hEqEp] at hTcb'
                  rw [storeObject_objects_eq' st endpointId _ pair hObjInv hStore] at hTcb'; cases hTcb'
                · rw [storeObject_objects_ne st pair.2 endpointId anyTid.toObjId _ hEqEp hObjInv hStore] at hTcb'
                  exact ⟨tcb', hTcb', Or.inl rfl⟩
            | some tailTid =>
              ...
              | some tailTcb =>
                ...
                | ok pair =>
                  ...
                  | ok st2 =>
                    ...
                    rw [storeTcbQueueLinks_preserves_objects_ne st2 st' enqueueTid (some tailTid)
                      (some (.tcbNext tailTid)) none anyTid.toObjId hNe hInv2 hStep] at hTcb'
                    obtain ⟨tcb2, hTcb2, hDisj⟩ := storeTcbQueueLinks_tcb_queueNext_backward pair.2 st2
                      tailTid tailTcb.queuePrev tailTcb.queuePPrev (some enqueueTid) hInv1 hLink1 anyTid tcb' hTcb'
                    by_cases hEqEp : anyTid.toObjId = endpointId
                    · rw [hEqEp] at hTcb2
                      rw [storeObject_objects_eq' st endpointId _ pair hObjInv hStore] at hTcb2; cases hTcb2
                    · rw [storeObject_objects_ne st pair.2 endpointId anyTid.toObjId _ hEqEp hObjInv hStore] at hTcb2
                      exact ⟨tcb2, hTcb2, hDisj⟩

/-- Slice-2c strict-match: a predecessor blocked on `endpointId` matching the enqueue direction (core
(b) form) and a target equal to the block-store state (matching the same direction, `hBlock` form)
strict-match per `queueNextTargetBlocked`. -/
theorem queueNextTargetBlocked_clause_of_predecessor_block
    (sA sB : ThreadIpcState) (endpointId : SeLe4n.ObjId) (isReceiveQ : Bool)
    (hA : (isReceiveQ = true → sA = .blockedOnReceive endpointId) ∧
          (isReceiveQ = false → sA = .blockedOnSend endpointId ∨ sA = .blockedOnCall endpointId))
    (hBlock : if isReceiveQ then sB = .blockedOnReceive endpointId
              else sB = .blockedOnSend endpointId ∨ sB = .blockedOnCall endpointId) :
    (∀ ep, sA = .blockedOnReceive ep → sB = .blockedOnReceive ep) ∧
    (∀ ep, (sA = .blockedOnSend ep ∨ sA = .blockedOnCall ep) →
      (sB = .blockedOnSend ep ∨ sB = .blockedOnCall ep)) := by
  by_cases hR : isReceiveQ
  · have hAR := hA.1 hR
    have hBR : sB = .blockedOnReceive endpointId := by simpa [hR] using hBlock
    refine ⟨fun ep h => ?_, fun ep h => ?_⟩
    · rw [hAR] at h; injection h with hEp; rw [hBR, hEp]
    · rw [hAR] at h; rcases h with h | h <;> simp at h
  · have hAS := hA.2 (by simpa using hR)
    have hBS : sB = .blockedOnSend endpointId ∨ sB = .blockedOnCall endpointId := by
      simpa [hR] using hBlock
    refine ⟨fun ep h => ?_, fun ep h => ?_⟩
    · rcases hAS with h' | h' <;> rw [h'] at h <;> simp at h
    · rcases hAS with h' | h' <;> rcases h with hh | hh <;> rw [h'] at hh <;>
        first
        | (injection hh with hEp; subst hEp; rcases hBS with hb | hb <;> simp [hb])
        | simp at hh
...
is preserved by the enqueue (`endpointQueueEnqueue_tcb_queueNext_backward_ne`) and both endpoints'
`ipcState`s are preserved through the enqueue + block-store, so it is framed from `hQNTB`. -/
theorem endpointQueueEnqueue_blockStore_establishes_queueNextTargetBlocked
    (endpointId : SeLe4n.ObjId) (isReceiveQ : Bool) (tid : SeLe4n.ThreadId)
    (st st1 st2 : SystemState) (ep : Endpoint)
    (blockState : ThreadIpcState) (msg : Option IpcMessage)
    (hObjInv : st.objects.invExt)
    ...
    (hTailFresh : ∀ (e : Endpoint) (tailTid : SeLe4n.ThreadId),
      st.objects[endpointId]? = some (.endpoint e) →
      (if isReceiveQ then e.receiveQ else e.sendQ).tail = some tailTid →
      ∀ (epId' : SeLe4n.ObjId) (ep' : Endpoint),
        st.objects[epId']? = some (.endpoint ep') →
        (epId' ≠ endpointId → ep'.sendQ.tail ≠ some tailTid ∧ ep'.receiveQ.tail ≠ some tailTid) ∧
        (epId' = endpointId → (if isReceiveQ then ep'.sendQ else ep'.receiveQ).tail ≠ some tailTid))
    ...
    (hEnq : endpointQueueEnqueue endpointId isReceiveQ tid st = .ok st1)
    (hStore : storeTcbIpcStateAndMessage st1 tid blockState msg = .ok st2) :
    queueNextTargetBlocked st2 := by
  have hObjInv1 := endpointQueueEnqueue_preserves_objects_invExt endpointId isReceiveQ tid st st1 hObjInv hEnq
  have hPred := endpointQueueEnqueue_predecessor_blocked endpointId isReceiveQ tid st st1 ep
    hObjInv hObj hDQ hTail hFreshTid hTailFresh hEnq
  obtain ⟨tcbTidEnq, hTidEnqObj, hTidEnqNext⟩ :=
    endpointQueueEnqueue_enqueued_queueNext_none endpointId isReceiveQ tid st st1 ep hObjInv hObj hEnq
  intro a b tcbA tcbB hA hB hN
  obtain ⟨tcbA1, hA1, hQNA1⟩ :=
    storeTcbIpcStateAndMessage_tcb_queueNext_backward st1 st2 tid blockState msg hObjInv1 hStore a tcbA hA
  have hN1 : tcbA1.queueNext = some b := by rw [hQNA1]; exact hN
  by_cases hbtid : b.toObjId = tid.toObjId
  · -- the new link `a → tid`
    by_cases hatid : a.toObjId = tid.toObjId
    · exfalso
      rw [hatid] at hA1; rw [hTidEnqObj] at hA1
      obtain rfl : tcbTidEnq = tcbA1 := by simpa using hA1
      rw [hTidEnqNext] at hN1; exact absurd hN1 (by simp)
    · have hA1' : st1.objects[a.toObjId]? = some (.tcb tcbA) := by
        rw [← storeTcbIpcStateAndMessage_preserves_objects_ne st1 st2 tid blockState msg a.toObjId
          hatid hObjInv1 hStore]
        exact hA
      have hBtidEq : b = tid := ThreadId.toObjId_injective b tid hbtid
      have hAtoTid : tcbA.queueNext = some tid := by rw [hN, hBtidEq]
      have hPredA := hPred a tcbA hA1' hAtoTid
      have hStateB : tcbB.ipcState = blockState :=
        storeTcbIpcStateAndMessage_ipcState_eq st1 st2 tid blockState msg hObjInv1 hStore tcbB (hbtid ▸ hB)
      rw [hStateB]
      exact queueNextTargetBlocked_clause_of_predecessor_block tcbA.ipcState blockState endpointId
        isReceiveQ hPredA hBlock
  · by_cases hatid : a.toObjId = tid.toObjId
    · exfalso
      rw [hatid] at hA1; rw [hTidEnqObj] at hA1
      obtain rfl : tcbTidEnq = tcbA1 := by simpa using hA1
      rw [hTidEnqNext] at hN1; exact absurd hN1 (by simp)
    · -- pre-existing link `a → b` (`a, b ≠ tid`)
      have hA1' : st1.objects[a.toObjId]? = some (.tcb tcbA) := by
        rw [← storeTcbIpcStateAndMessage_preserves_objects_ne st1 st2 tid blockState msg a.toObjId
          hatid hObjInv1 hStore]
        exact hA
      have hB1' : st1.objects[b.toObjId]? = some (.tcb tcbB) := by
        rw [← storeTcbIpcStateAndMessage_preserves_objects_ne st1 st2 tid blockState msg b.toObjId
          hbtid hObjInv1 hStore]
        exact hB
      obtain ⟨tcbA_st, hA_st, hDisjA⟩ := endpointQueueEnqueue_tcb_queueNext_backward_ne endpointId
        isReceiveQ tid st st1 a tcbA hObjInv hEnq hA1' hatid
      have hQNAst : tcbA_st.queueNext = some b := by
        rcases hDisjA with h | h
        · rw [h]; exact hN
        · rw [hN] at h; exact absurd (congrArg (·.toObjId) (Option.some.inj h)) hbtid
      obtain ⟨tcbA_st2, hA_st2, hIpcA⟩ := endpointQueueEnqueue_tcb_ipcState_backward endpointId
        isReceiveQ tid st st1 a tcbA hObjInv hEnq hA1'
      obtain ⟨tcbB_st, hB_st, hIpcB⟩ := endpointQueueEnqueue_tcb_ipcState_backward endpointId
        isReceiveQ tid st st1 b tcbB hObjInv hEnq hB1'
      have hAeq : tcbA_st = tcbA_st2 := by
        have := hA_st.symm.trans hA_st2; simpa using this
      have hPre := hQNTB a b tcbA_st tcbB_st hA_st hB_st hQNAst
      have hIpcAeq : tcbA_st.ipcState = tcbA.ipcState := by rw [hAeq]; exact hIpcA
      rw [← hIpcAeq, ← hIpcB]
      exact hPre

open SeLe4n.Model.SystemState in
/-- Slice-2c core (c) `storeTcbIpcState` variant: same as
`endpointQueueEnqueue_blockStore_establishes_queueNextTargetBlocked` but for the `storeTcbIpcState`
block-store (`endpointReceiveDual`'s block leg writes `ipcState` only, no `pendingMessage`). -/
theorem endpointQueueEnqueue_blockStoreIpc_establishes_queueNextTargetBlocked
    (endpointId : SeLe4n.ObjId) (isReceiveQ : Bool) (tid : SeLe4n.ThreadId)
    (st st1 st2 : SystemState) (ep : Endpoint)
    (blockState : ThreadIpcState)
    (hObjInv : st.objects.invExt)
  ...
  intro a b tcbA tcbB hA hB hN
  obtain ⟨tcbA1, hA1, hQNA1⟩ :=
    storeTcbIpcState_tcb_queueNext_backward st1 st2 tid blockState hObjInv1 hStore a tcbA hA
  have hN1 : tcbA1.queueNext = some b := by rw [hQNA1]; exact hN
  by_cases hbtid : b.toObjId = tid.toObjId
  · by_cases hatid : a.toObjId = tid.toObjId
    · exfalso
      ...
      rw [hTidEnqNext] at hN1; exact absurd hN1 (by simp)
    · have hA1' : st1.objects[a.toObjId]? = some (.tcb tcbA) := by
        rw [← storeTcbIpcState_preserves_objects_ne st1 st2 tid blockState a.toObjId hatid hObjInv1 hStore]
        exact hA
      have hBtidEq : b = tid := ThreadId.toObjId_injective b tid hbtid
      have hAtoTid : tcbA.queueNext = some tid := by rw [hN, hBtidEq]
      have hPredA := hPred a tcbA hA1' hAtoTid
      have hStateB : tcbB.ipcState = blockState :=
        storeTcbIpcState_ipcState_eq st1 st2 tid blockState hObjInv1 hStore tcbB (hbtid ▸ hB)
      rw [hStateB]
      exact queueNextTargetBlocked_clause_of_predecessor_block tcbA.ipcState blockState endpointId
        isReceiveQ hPredA hBlock
  · by_cases hatid : a.toObjId = tid.toObjId
    ...
    · have hA1' : st1.objects[a.toObjId]? = some (.tcb tcbA) := by
        ...
        exact hA
      have hB1' : st1.objects[b.toObjId]? = some (.tcb tcbB) := by
        rw [← storeTcbIpcState_preserves_objects_ne st1 st2 tid blockState b.toObjId hbtid hObjInv1 hStore]
        exact hB
      obtain ⟨tcbA_st, hA_st, hDisjA⟩ := endpointQueueEnqueue_tcb_queueNext_backward_ne endpointId
        isReceiveQ tid st st1 a tcbA hObjInv hEnq hA1' hatid
      have hQNAst : tcbA_st.queueNext = some b := by
...
(kind, endpoint). -/
theorem endpointQueuePopHead_popped_not_tail
    (endpointId : SeLe4n.ObjId) (isReceiveQ : Bool)
    ...
    ∀ (epId' : SeLe4n.ObjId) (ep' : Endpoint),
      st'.objects[epId']? = some (.endpoint ep') →
      ep'.receiveQ.tail ≠ some tid ∧ ep'.sendQ.tail ≠ some tid := by
  ...
  have hWF := hDQ.1 endpointId ep hObj
  simp only [dualQueueEndpointWellFormed, hObj] at hWF
  ...
  | true =>
    subst hRQ
    simp only [↓reduceIte] at hPoppedTail hOtherTail hRetHead
    have hHead : headTcb.ipcState = .blockedOnReceive endpointId :=
      (hQHBC endpointId ep tid headTcb hObj hPreTcb).1 hRetHead
    have hReNeTid : epP.receiveQ.tail ≠ some tid := by
      rw [hPoppedTail]
      cases hN : headTcb.queueNext with
      | none => simp
      | some n =>
        intro hpre
        obtain ⟨t, ht, htn⟩ := hWF.2.2.2 tid hpre
        rw [hPreTcb] at ht; rw [← (by simpa using ht : headTcb = t)] at htn
        rw [hN] at htn; exact absurd htn (by simp)
    intro epId' ep' hEp'
    refine ⟨fun hTl => ?_, fun hTl => ?_⟩
    · by_cases hEq : epId' = endpointId
      · have hep' : ep' = epP := by rw [hEq] at hEp'; simpa using (hEpP.symm.trans hEp').symm
        rw [hep'] at hTl; exact hReNeTid hTl
      · have hPre := endpointQueuePopHead_endpoint_backward_ne endpointId true st st' tid epId' ep' hEq hObjInv hStep hEp'
        have hb := (hTail epId' ep' tid headTcb hPre hPreTcb).1 hTl
        rw [hHead] at hb; exact hEq (Eq.symm (by simpa using hb))
    · by_cases hEq : epId' = endpointId
      · have hep' : ep' = epP := by rw [hEq] at hEp'; simpa using (hEpP.symm.trans hEp').symm
        rw [hep', hOtherTail] at hTl
        rcases (hTail endpointId ep tid headTcb hObj hPreTcb).2 hTl with h | h <;> rw [hHead] at h <;> simp at h
      · have hPre := endpointQueuePopHead_endpoint_backward_ne endpointId true st st' tid epId' ep' hEq hObjInv hStep hEp'
        rcases (hTail epId' ep' tid headTcb hPre hPreTcb).2 hTl with h | h <;> rw [hHead] at h <;> simp at h
  | false =>
    subst hRQ
    simp only [Bool.false_eq_true, ↓reduceIte] at hPoppedTail hOtherTail hRetHead
    have hHead := (hQHBC endpointId ep tid headTcb hObj hPreTcb).2 hRetHead
    have hSeNeTid : epP.sendQ.tail ≠ some tid := by
      rw [hPoppedTail]
      cases hN : headTcb.queueNext with
      | none => simp
      | some n =>
        intro hpre
        obtain ⟨t, ht, htn⟩ := hWF.1.2.2 tid hpre
        rw [hPreTcb] at ht; rw [← (by simpa using ht : headTcb = t)] at htn
        rw [hN] at htn; exact absurd htn (by simp)
    intro epId' ep' hEp'
    refine ⟨fun hTl => ?_, fun hTl => ?_⟩
    · by_cases hEq : epId' = endpointId
      · have hep' : ep' = epP := by rw [hEq] at hEp'; simpa using (hEpP.symm.trans hEp').symm
        rw [hep', hOtherTail] at hTl
        have hb := (hTail endpointId ep tid headTcb hObj hPreTcb).1 hTl
    ...
    · by_cases hEq : epId' = endpointId
      · have hep' : ep' = epP := by rw [hEq] at hEp'; simpa using (hEpP.symm.trans hEp').symm
        rw [hep'] at hTl; exact hSeNeTid hTl
      · have hPre := endpointQueuePopHead_endpoint_backward_ne endpointId false st st' tid epId' ep' hEq hObjInv hStep hEp'
        have hb := (hTail epId' ep' tid headTcb hPre hPreTcb).2 hTl
      ...
      | some _ =>
        ...
        | ok pair =>
          ...
          have hTail1 := endpointQueuePopHead_preserves_endpointQueueTailBlockedConsistent endpointId true st
            pair.2.2 pair.1 pair.2.1 hObjInv hInv hPop
          have hNotTail := endpointQueuePopHead_popped_not_tail endpointId true st pair.2.2 pair.1
          ...
          | ok st2 =>
            simp only [hMsg, Except.ok.injEq, Prod.mk.injEq] at hStep
            obtain ⟨_, hEq⟩ := hStep; subst hEq
            exact ensureRunnable_preserves_endpointQueueTailBlockedConsistent _ _ <|
              storeTcbReceiveComplete_preserves_endpointQueueTailBlockedConsistent pair.2.2 st2 pair.1
                (some msg) hTail1 hObjInv1 hMsg hNotTail
      | none =>
        cases hEnq : endpointQueueEnqueue endpointId false sender st with
        | error e => simp [hHead, hEnq] at hStep
        | ok st1 =>
...
open SeLe4n.Model.SystemState in
/-- IPC de-threading D4: `storeObject` of a Reply object preserves `endpointQueueTailBlockedConsistent`. -/
theorem storeObject_reply_preserves_endpointQueueTailBlockedConsistent
  ...
  storeObject_nonEndpointNonTcb_preserves_endpointQueueTailBlockedConsistent
    st st' rid.toObjId (.reply r) hObjInv (fun _ => by simp) (fun _ => by simp) hInv hStore

open SeLe4n.Model.SystemState in
/-- IPC de-threading D4: `consumeReply` preserves `endpointQueueTailBlockedConsistent`. -/
theorem consumeReply_preserves_endpointQueueTailBlockedConsistent
    (st st' : SystemState) (rid : SeLe4n.ReplyId)
    (hObjInv : st.objects.invExt) (hInv : endpointQueueTailBlockedConsistent st)
    (hStep : SystemState.consumeReply rid st = .ok ((), st')) :
    endpointQueueTailBlockedConsistent st' := by
...
open SeLe4n.Model.SystemState in
open SeLe4n.Model.SystemState in
/-- IPC de-threading D4 Slice 2b: `linkReply` preserves endpoints backward (the only write is the
Reply-object store at `rid.toObjId`, never an endpoint). -/
theorem linkReply_endpoint_backward
    (st st' : SystemState) (rid : SeLe4n.ReplyId) (caller : SeLe4n.ThreadId)
    (oid : SeLe4n.ObjId) (e : Endpoint)
    (hObjInv : st.objects.invExt)
    (hStep : SystemState.linkReply rid caller st = .ok ((), st'))
    (hEp : st'.objects[oid]? = some (.endpoint e)) :
    st.objects[oid]? = some (.endpoint e) := by
  unfold SystemState.linkReply at hStep
  cases hGet : st.getReply? rid with
  ...
  | some r =>
    ...
    · by_cases hEq : oid = rid.toObjId
      · subst hEq; rw [storeObject_objects_eq st st' rid.toObjId _ hObjInv hStep] at hEp; simp at hEp
      · rw [storeObject_objects_ne st st' rid.toObjId oid _ hEq hObjInv hStep] at hEp; exact hEp
    · simp at hStep
...
theorem linkCallerReply_endpoint_backward
  ...
  | ok p =>
    ...
    | some tcb =>
      simp only [hT] at hStep
      split at hStep
      · have hEp1 : st1.objects[oid]? = some (.endpoint e) := by
          by_cases hEq : oid = caller.toObjId
          · subst hEq; rw [storeObject_objects_eq st1 st' caller.toObjId _ hObjInv1 hStep] at hEp; simp at hEp
          · rw [storeObject_objects_ne st1 st' caller.toObjId oid _ hEq hObjInv1 hStep] at hEp; exact hEp
        exact linkReply_endpoint_backward st st1 rid caller oid e hObjInv hLink hEp1
      · simp at hStep

/-- IPC de-threading D4: `linkReply` preserves `endpointQueueTailBlockedConsistent`. -/
theorem linkReply_preserves_endpointQueueTailBlockedConsistent
    (st st' : SystemState) (rid : SeLe4n.ReplyId) (caller : SeLe4n.ThreadId)
    (hObjInv : st.objects.invExt) (hInv : endpointQueueTailBlockedConsistent st)
    (hStep : SystemState.linkReply rid caller st = .ok ((), st')) :
    endpointQueueTailBlockedConsistent st' := by
...
open SeLe4n.Model.SystemState in
/-- IPC de-threading D4: `linkCallerReply` preserves `endpointQueueTailBlockedConsistent`
(Reply store + a `replyObject` write — neither touches an endpoint queue or any `ipcState`). -/
theorem linkCallerReply_preserves_endpointQueueTailBlockedConsistent
    (st st' : SystemState) (caller : SeLe4n.ThreadId) (rid : SeLe4n.ReplyId)
    (hObjInv : st.objects.invExt) (hInv : endpointQueueTailBlockedConsistent st)
    (hStep : SystemState.linkCallerReply caller rid st = .ok ((), st')) :
    endpointQueueTailBlockedConsistent st' := by
  ...
  | ok p =>
    obtain ⟨⟨⟩, st1⟩ := p
    have hInv1 := linkReply_preserves_endpointQueueTailBlockedConsistent st st1 rid caller hObjInv hInv hLink
    have hObjInv1 := linkReply_preserves_objects_invExt st st1 rid caller hObjInv hLink
    simp only [hLink] at hStep
    cases hT : st1.getTcb? caller with
    | none => simp [hT] at hStep
    | some tcb =>
      ...
      · simp only [hRO, if_true] at hStep
        exact storeObject_tcb_preserveIpc_preserves_endpointQueueTailBlockedConsistent st1 st' caller tcb
...
theorem consumeCallerReply_preserves_endpointQueueTailBlockedConsistent
  ...
  | ok p =>
    obtain ⟨⟨⟩, st1⟩ := p
    have hInv1 := consumeReply_preserves_endpointQueueTailBlockedConsistent st st1 rid hObjInv hInv hConsume
    have hObjInv1 := consumeReply_preserves_objects_invExt st st1 rid hObjInv hConsume
    simp only [hConsume] at hStep
    cases hT : st1.getTcb? caller with
    | none =>
...
theorem consumeCallerReply_preserves_ipcInvariantFull
  ...
  refine ipcInvariantFull_of_core_replyCallerLinkage ?core hRCL'
    ...
    -- IPC de-threading D4 (Finding F-2): tail-blocked is now established from the pre-state
...
theorem endpointCall_preserves_queueNextBlockingConsistent
    (st st' : SystemState) (endpointId : SeLe4n.ObjId)
    (caller : SeLe4n.ThreadId) (msg : IpcMessage)
    (hInv : queueNextBlockingConsistent st)
    (hDQSI : dualQueueSystemInvariant st)
      ...
      | some _ =>
        ...
        | ok pair =>
          ...
          | ok st2 =>
            ...
            have hQNBC2 := storeTcbIpcStateAndMessage_ready_preserves_queueNextBlockingConsistent
              pair.2.2 st2 pair.1 (some msg) hQNBC1 hObjInv1 hMsg
            have hObjInvE : (ensureRunnable st2 pair.1).objects.invExt :=
              ensureRunnable_preserves_objects st2 pair.1 ▸ hObjInv2
            have hQNBCE := ensureRunnable_preserves_queueNextBlockingConsistent st2 pair.1 hQNBC2
            ...
            | ok st3 =>
              ...
              have hObjInv3 := storeTcbIpcStateAndMessage_preserves_objects_invExt _ st3 caller _ none
                hObjInvE hIpc
              have hQNBC3 := storeTcbIpcStateAndMessage_preserves_queueNextBlockingConsistent
                (ensureRunnable st2 pair.1) st3 caller (.blockedOnReply endpointId (some pair.1)) none
                hQNBCE hObjInvE hIpc (fun _ _ _ _ _ _ => trivial)
              ...
              | ok pL =>
                ...
                simp only [hLink, Except.ok.injEq, Prod.mk.injEq] at hStep
                obtain ⟨_, rfl⟩ := hStep
                exact removeRunnable_preserves_queueNextBlockingConsistent _ _ <|
                  linkServerStashedReply_preserves_queueNextBlockingConsistent st3 st5 caller pair.1
                    hObjInv3 hQNBC3 hLink
      | none =>
        cases hEnq : endpointQueueEnqueue endpointId false caller st with
        | error e => simp [hHead, hEnq] at hStep
        | ok st1 =>
          simp only [hHead, hEnq] at hStep
          have hObjInv1 := endpointQueueEnqueue_preserves_objects_invExt endpointId false caller st st1
            hObjInv hEnq
          have hQNBC1 := endpointQueueEnqueue_preserves_queueNextBlockingConsistent endpointId false
            caller st st1 hObjInv hInv hEnq
          cases hMsg : storeTcbIpcStateAndMessage st1 caller (.blockedOnCall endpointId) (some msg) with
          | error e => simp [hMsg] at hStep
          | ok st2 =>
            simp only [hMsg, Except.ok.injEq, Prod.mk.injEq] at hStep
            obtain ⟨_, rfl⟩ := hStep
            refine removeRunnable_preserves_queueNextBlockingConsistent _ _ <|
              storeTcbIpcStateAndMessage_preserves_queueNextBlockingConsistent st1 st2 caller
                (.blockedOnCall endpointId) (some msg) hQNBC1 hObjInv1 hMsg ?_ ?_
            · intro b tcbTid tcbB hSenderObj hSenderNext _
              obtain ⟨tcbS, hS, hSNext⟩ := endpointQueueEnqueue_enqueued_queueNext_none endpointId
                false caller st st1 ep hObjInv hObj hEnq
              rw [hSenderObj] at hS
              obtain rfl : tcbTid = tcbS := by simpa using hS
              rw [hSNext] at hSenderNext; exact absurd hSenderNext (by simp)
            · intro a tcbA _ hA hANext _
...
theorem cleanupPreReceiveDonation_preserves_endpointQueueTailBlockedConsistent
    (st : SystemState) (receiver : SeLe4n.ThreadId)
    (hObjInv : st.objects.invExt) (hInv : endpointQueueTailBlockedConsistent st) :
    endpointQueueTailBlockedConsistent (cleanupPreReceiveDonation st receiver) :=
  endpointQueueTailBlockedConsistent_of_endpoint_tcb_backward st (cleanupPreReceiveDonation st receiver)
    (fun eid ep hEp => cleanupPreReceiveDonation_endpoint_backward st receiver hObjInv eid ep hEp)
    (fun y tcb' hY => cleanupPreReceiveDonation_tcb_ipcState_backward st receiver hObjInv y tcb' hY)
    hInv
...
theorem endpointReceiveDual_preserves_queueNextBlockingConsistent
    (endpointId : SeLe4n.ObjId) (receiver : SeLe4n.ThreadId)
    (replyId : Option SeLe4n.ReplyId)
    (st st' : SystemState) (senderId : SeLe4n.ThreadId)
    (hInv : queueNextBlockingConsistent st)
  ...
  | some obj => cases obj with
    ...
    | endpoint ep =>
      ...
      | some _ =>
        ...
        | ok pair =>
          simp only [hHead, hPop] at hStep
          have hObjInv1 := endpointQueuePopHead_preserves_objects_invExt endpointId false st pair.2.2
            pair.1 pair.2.1 hObjInv hPop
          have hQNBC1 := endpointQueuePopHead_preserves_queueNextBlockingConsistent endpointId false st
            pair.2.2 pair.1 pair.2.1 hObjInv hInv hPop
          split at hStep
          · cases hMsg : storeTcbIpcStateAndMessage pair.2.2 pair.1
            ...
            | ok st2 =>
              ...
              have hObjInv2 := storeTcbIpcStateAndMessage_preserves_objects_invExt pair.2.2 st2 pair.1 _
                none hObjInv1 hMsg
              have hQNBC2 := storeTcbIpcStateAndMessage_preserves_queueNextBlockingConsistent pair.2.2
                st2 pair.1 (.blockedOnReply endpointId (some receiver)) none hQNBC1 hObjInv1 hMsg
                (fun _ _ _ _ _ _ => trivial) (fun _ tcbA _ _ _ _ => by cases tcbA.ipcState <;> trivial)
              cases hReplyId : replyId with
              | none => simp [hReplyId] at hStep
              | some rid =>
                simp only [hReplyId] at hStep
                ...
                | ok pLink =>
                  obtain ⟨_, stLinked⟩ := pLink
                  simp only [hLink] at hStep
                  have hObjInvLink := linkCallerReply_preserves_objects_invExt st2 stLinked pair.1 rid
                    hObjInv2 hLink
                  have hQNBCLink := linkCallerReply_preserves_queueNextBlockingConsistent st2 stLinked
                    pair.1 rid hObjInv2 hQNBC2 hLink
                  ...
                  | ok st3 =>
                    simp only [hPend] at hStep
                    obtain ⟨_, rfl⟩ := hStep
                    exact storeTcbIpcStateAndMessage_ready_preserves_queueNextBlockingConsistent stLinked
                      _ receiver pair.2.1.pendingMessage hQNBCLink hObjInvLink hPend
          · cases hMsg : storeTcbIpcStateAndMessage pair.2.2 pair.1 .ready none with
            | error e => simp [hMsg] at hStep
            | ok st2 =>
              simp only [hMsg] at hStep
              have hObjInv2 := storeTcbIpcStateAndMessage_preserves_objects_invExt pair.2.2 st2 pair.1 _
                none hObjInv1 hMsg
              have hQNBC2 := storeTcbIpcStateAndMessage_ready_preserves_queueNextBlockingConsistent
                pair.2.2 st2 pair.1 none hQNBC1 hObjInv1 hMsg
              have hObjInvR : (ensureRunnable st2 pair.1).objects.invExt :=
                ensureRunnable_preserves_objects st2 pair.1 ▸ hObjInv2
              have hQNBCR := ensureRunnable_preserves_queueNextBlockingConsistent st2 pair.1 hQNBC2
              cases hPend : storeTcbIpcStateAndMessage (ensureRunnable st2 pair.1) receiver .ready
                  pair.2.1.pendingMessage with
              | error e => simp [hPend] at hStep
              | ok st3 =>
                simp only [hPend] at hStep
                obtain ⟨_, rfl⟩ := hStep
                exact storeTcbIpcStateAndMessage_ready_preserves_queueNextBlockingConsistent _ _ receiver
                  pair.2.1.pendingMessage hQNBCR hObjInvR hPend
      | none =>
        cases hChecked : cleanupPreReceiveDonationChecked st receiver with
        | error _ => simp [hHead, hChecked] at hStep
        | ok stClean =>
          ...
          rw [hBridge] at hStep
          have hObjInvClean := cleanupPreReceiveDonation_preserves_objects_invExt st receiver hObjInv
          have hQNBCClean := cleanupPreReceiveDonation_preserves_queueNextBlockingConsistent st receiver
            hObjInv hInv
          have hTailClean := cleanupPreReceiveDonation_preserves_endpointQueueTailBlockedConsistent st
            receiver hObjInv hTail
          have hDQSIClean := cleanupPreReceiveDonation_preserves_dualQueueSystemInvariant st receiver
            hObjInv hDQSI
          have hFreshReceiverClean : ∀ (epId : SeLe4n.ObjId) (ep : Endpoint),
              (cleanupPreReceiveDonation st receiver).objects[epId]? = some (.endpoint ep) →
              ep.sendQ.head ≠ some receiver ∧ ep.sendQ.tail ≠ some receiver ∧
          ...
          have hRecvTailFreshClean : ∀ (ep : Endpoint) (tailTid : SeLe4n.ThreadId),
              ...
              ∀ (epId' : SeLe4n.ObjId) (ep' : Endpoint),
                ...
                (epId' = endpointId →
                  ep'.sendQ.tail ≠ some tailTid) :=
            fun ep tailTid hEp hTl epId' ep' hEp' =>
              hRecvTailFresh ep tailTid
                (cleanupPreReceiveDonation_endpoint_backward st receiver hObjInv endpointId ep hEp) hTl
                epId' ep'
                (cleanupPreReceiveDonation_endpoint_backward st receiver hObjInv epId' ep' hEp')
          have hObjClean := cleanupPreReceiveDonation_endpoint_forward st receiver hObjInv endpointId ep
            hObj
          ...
          | ok st1 =>
            ...
            have hObjInv1 := endpointQueueEnqueue_preserves_objects_invExt endpointId true receiver
              (cleanupPreReceiveDonation st receiver) st1 hObjInvClean hEnq
            have hQNBC1 := endpointQueueEnqueue_preserves_queueNextBlockingConsistent endpointId true
              receiver (cleanupPreReceiveDonation st receiver) st1 hObjInvClean hQNBCClean hEnq
            cases hIpc : storeTcbIpcState st1 receiver (.blockedOnReceive endpointId) with
            | error e => simp [hIpc] at hStep
            | ok st2 =>
              simp only [hIpc] at hStep
              have hObjInv2 := storeTcbIpcState_preserves_objects_invExt st1 st2 receiver _ hObjInv1 hIpc
              have hQNBC2 := storeTcbIpcState_preserves_queueNextBlockingConsistent st1 st2 receiver
                (.blockedOnReceive endpointId) hQNBC1 hObjInv1 hIpc
                (by
                  intro b tcbTid tcbB hRObj hRNext _
                  obtain ⟨tcbS, hS, hSNext⟩ := endpointQueueEnqueue_enqueued_queueNext_none endpointId
                    true receiver (cleanupPreReceiveDonation st receiver) st1 ep hObjInvClean hObjClean hEnq
                  rw [hRObj] at hS
                  obtain rfl : tcbTid = tcbS := by simpa using hS
                  rw [hSNext] at hRNext; exact absurd hRNext (by simp))
                (by
                  intro a tcbA _ hA hANext _
                  have h := (endpointQueueEnqueue_predecessor_blocked endpointId true receiver
                    (cleanupPreReceiveDonation st receiver) st1 ep hObjInvClean hObjClean hDQSIClean
                    hTailClean hFreshReceiverClean hRecvTailFreshClean hEnq a tcbA hA hANext).1 rfl
                  rw [h]; rfl)
              cases hGetR : st2.getTcb? receiver with
              | none =>
                simp only [hGetR, Except.ok.injEq, Prod.mk.injEq] at hStep
                obtain ⟨_, rfl⟩ := hStep
                exact removeRunnable_preserves_queueNextBlockingConsistent _ _ hQNBC2
              | some rTcb =>
                simp only [hGetR] at hStep
                cases hStash : storeObject receiver.toObjId
                    (.tcb { rTcb with pendingReceiveReply := replyId }) st2 with
                ...
                | ok pStash =>
                  ...
                  have hTcbPre : st2.objects[receiver.toObjId]? = some (.tcb rTcb) :=
                    (SystemState.getTcb?_eq_some_iff st2 receiver rTcb).mp hGetR
                  exact removeRunnable_preserves_queueNextBlockingConsistent _ _
                    (storeObject_tcb_preserveLinks_queueNextBlockingConsistent st2 stStashed receiver rTcb
                      { rTcb with pendingReceiveReply := replyId } hObjInv2 hTcbPre rfl rfl hQNBC2 hStash)

open SeLe4n.Model.SystemState in
/-- IPC de-threading D4 Slice 2b: `endpointReceiveDual` **establishes** `endpointQueueTailBlockedConsistent`.
    ...
    | endpoint ep =>
      ...
      | some _ =>
        ...
        | ok pair =>
          ...
          have hObjInv1 := endpointQueuePopHead_preserves_objects_invExt endpointId false st pair.2.2
            pair.1 pair.2.1 hObjInv hPop
          have hTail1 := endpointQueuePopHead_preserves_endpointQueueTailBlockedConsistent endpointId false st
            pair.2.2 pair.1 pair.2.1 hObjInv hInv hPop
          have hSenderNotTail := endpointQueuePopHead_popped_not_tail endpointId false st pair.2.2 pair.1
            pair.2.1 ep hObjInv hObj hDQSI hQHBC hInv hPop
          have hRecvNotTailPop := endpointQueuePopHead_fresh_not_tail endpointId false st pair.2.2 pair.1
            pair.2.1 ep receiver hObjInv hObj
            (fun epId e hE => ⟨(hFreshReceiver epId e hE).2.2.2, (hFreshReceiver epId e hE).2.1⟩) hPop
          split at hStep
          · cases hMsg : storeTcbIpcStateAndMessage pair.2.2 pair.1
                (.blockedOnReply endpointId (some receiver)) none with
            | error e => simp [hMsg] at hStep
            | ok st2 =>
              ...
              have hObjInv2 := storeTcbIpcStateAndMessage_preserves_objects_invExt pair.2.2 st2 pair.1 _
                none hObjInv1 hMsg
              have hTail2 := storeTcbIpcStateAndMessage_preserves_endpointQueueTailBlockedConsistent pair.2.2
                st2 pair.1 (.blockedOnReply endpointId (some receiver)) none hTail1 hObjInv1 hMsg hSenderNotTail
              cases hReplyId : replyId with
              | none => simp [hReplyId] at hStep
              | some rid =>
                simp only [hReplyId] at hStep
                ...
                | ok pLink =>
                  ...
                  have hObjInvLink := linkCallerReply_preserves_objects_invExt st2 stLinked pair.1 rid
                    hObjInv2 hLink
                  have hTailLinked := linkCallerReply_preserves_endpointQueueTailBlockedConsistent st2 stLinked
                    pair.1 rid hObjInv2 hTail2 hLink
                  have hRecvNotTailLinked : ∀ (epId : SeLe4n.ObjId) (e : Endpoint),
                      stLinked.objects[epId]? = some (.endpoint e) →
                      e.receiveQ.tail ≠ some receiver ∧ e.sendQ.tail ≠ some receiver := by
                    intro epId e hE
                    exact hRecvNotTailPop epId e
                      (storeTcbIpcStateAndMessage_endpoint_backward pair.2.2 st2 pair.1
                        (.blockedOnReply endpointId (some receiver)) none epId e hObjInv1 hMsg
                        (linkCallerReply_endpoint_backward st2 stLinked pair.1 rid epId e hObjInv2 hLink hE))
                  cases hPend : storeTcbIpcStateAndMessage stLinked receiver .ready
                      pair.2.1.pendingMessage with
                  | error e => simp [hPend] at hStep
                  | ok st3 =>
                    simp only [hPend] at hStep
                    obtain ⟨_, rfl⟩ := hStep
                    exact storeTcbIpcStateAndMessage_preserves_endpointQueueTailBlockedConsistent stLinked
                      _ receiver .ready pair.2.1.pendingMessage hTailLinked hObjInvLink hPend hRecvNotTailLinked
          · cases hMsg : storeTcbIpcStateAndMessage pair.2.2 pair.1 .ready none with
            | error e => simp [hMsg] at hStep
            | ok st2 =>
              simp only [hMsg] at hStep
              have hObjInv2 := storeTcbIpcStateAndMessage_preserves_objects_invExt pair.2.2 st2 pair.1 _
                none hObjInv1 hMsg
              have hTail2 := storeTcbIpcStateAndMessage_preserves_endpointQueueTailBlockedConsistent
                pair.2.2 st2 pair.1 .ready none hTail1 hObjInv1 hMsg hSenderNotTail
              have hObjInvR : (ensureRunnable st2 pair.1).objects.invExt :=
                ensureRunnable_preserves_objects st2 pair.1 ▸ hObjInv2
              have hTailR := ensureRunnable_preserves_endpointQueueTailBlockedConsistent st2 pair.1 hTail2
              have hRecvNotTailR : ∀ (epId : SeLe4n.ObjId) (e : Endpoint),
                  (ensureRunnable st2 pair.1).objects[epId]? = some (.endpoint e) →
                  e.receiveQ.tail ≠ some receiver ∧ e.sendQ.tail ≠ some receiver := by
                intro epId e hE
                rw [show (ensureRunnable st2 pair.1).objects = st2.objects from
                  ensureRunnable_preserves_objects st2 pair.1] at hE
                exact hRecvNotTailPop epId e
                  (storeTcbIpcStateAndMessage_endpoint_backward pair.2.2 st2 pair.1 .ready none epId e hObjInv1 hMsg hE)
              cases hPend : storeTcbIpcStateAndMessage (ensureRunnable st2 pair.1) receiver .ready
                  pair.2.1.pendingMessage with
              | error e => simp [hPend] at hStep
              | ok st3 =>
                simp only [hPend] at hStep
                obtain ⟨_, rfl⟩ := hStep
                exact storeTcbIpcStateAndMessage_preserves_endpointQueueTailBlockedConsistent
                  (ensureRunnable st2 pair.1) _ receiver .ready pair.2.1.pendingMessage hTailR hObjInvR hPend hRecvNotTailR
      | none =>
        cases hChecked : cleanupPreReceiveDonationChecked st receiver with
        | error _ => simp [hHead, hChecked] at hStep
        | ok stClean =>
          ...
          have hObjInvClean := cleanupPreReceiveDonation_preserves_objects_invExt st receiver hObjInv
          have hTailClean := cleanupPreReceiveDonation_preserves_endpointQueueTailBlockedConsistent st
            receiver hObjInv hInv
          have hFreshReceiverClean : ∀ (epId : SeLe4n.ObjId) (e : Endpoint),
              (cleanupPreReceiveDonation st receiver).objects[epId]? = some (.endpoint e) →
              e.sendQ.head ≠ some receiver ∧ e.sendQ.tail ≠ some receiver ∧
              e.receiveQ.head ≠ some receiver ∧ e.receiveQ.tail ≠ some receiver :=
            fun epId e hEp =>
              hFreshReceiver epId e (cleanupPreReceiveDonation_endpoint_backward st receiver hObjInv
                epId e hEp)
          have hObjClean := cleanupPreReceiveDonation_endpoint_forward st receiver hObjInv endpointId ep hObj
          cases hEnq : endpointQueueEnqueue endpointId true receiver
              (cleanupPreReceiveDonation st receiver) with
          ...
          | ok st1 =>
            ...
            | ok st2 =>
              simp only [hIpc] at hStep
              have hObjInv2 := storeTcbIpcState_preserves_objects_invExt st1 st2 receiver _ hObjInv1 hIpc
              have hTail2 := endpointQueueEnqueue_blockStoreIpc_establishes_endpointQueueTailBlockedConsistent
                endpointId true receiver (cleanupPreReceiveDonation st receiver) st1 st2 ep
                (.blockedOnReceive endpointId) hObjInvClean hObjClean hTailClean
                (fun epId e hE => ⟨(hFreshReceiverClean epId e hE).2.1, (hFreshReceiverClean epId e hE).2.2.2⟩)
                (by simp) hEnq hIpc
              cases hGetR : st2.getTcb? receiver with
              | none =>
                simp only [hGetR, Except.ok.injEq, Prod.mk.injEq] at hStep
                obtain ⟨_, rfl⟩ := hStep
                exact removeRunnable_preserves_endpointQueueTailBlockedConsistent _ _ hTail2
              | some rTcb =>
                simp only [hGetR] at hStep
                cases hStash : storeObject receiver.toObjId
                    (.tcb { rTcb with pendingReceiveReply := replyId }) st2 with
                ...
                | ok pStash =>
                  ...
                  have hTcbPre : st2.objects[receiver.toObjId]? = some (.tcb rTcb) :=
                    (SystemState.getTcb?_eq_some_iff st2 receiver rTcb).mp hGetR
                  exact removeRunnable_preserves_endpointQueueTailBlockedConsistent _ _
                    (storeObject_tcb_preserveIpc_preserves_endpointQueueTailBlockedConsistent st2 stStashed
...
open SeLe4n.Model.SystemState in
/-- IPC de-threading D4 Slice 2b: `ipcUnwrapCaps` frames `endpointQueueTailBlockedConsistent` (the
...
theorem endpointSendDualWithCaps_preserves_queueNextBlockingConsistent
  ...
  | ok pair =>
    rcases pair with ⟨_, stMid⟩
    have hQNBCMid := endpointSendDual_preserves_queueNextBlockingConsistent st stMid endpointId sender
      msg hInv hDQSI hTail hObjInv hFreshSender hSendTailFresh hSend
    have hObjInvMid := endpointSendDual_preserves_objects_invExt st stMid endpointId sender msg hObjInv hSend
    simp [hSend] at hStep
    cases hEp : st.getEndpoint? endpointId with
    | none => simp [hEp] at hStep; obtain ⟨_, rfl⟩ := hStep; exact hQNBCMid
    | some ep =>
      simp [hEp] at hStep
      cases hHead : ep.receiveQ.head with
      | none => simp [hHead] at hStep; obtain ⟨_, rfl⟩ := hStep; exact hQNBCMid
      | some receiverId =>
        simp [hHead] at hStep
        by_cases hEmpty : msg.caps = #[]
        ...
        · simp [hEmpty] at hStep
          ...
          | some recvRoot =>
            simp [hLookup] at hStep
            exact ipcUnwrapCaps_preserves_queueNextBlockingConsistent msg senderCspaceRoot recvRoot
              receiverSlotBase _ stMid st' summary hObjInvMid hQNBCMid hStep

open SeLe4n.Model.SystemState in
/-- IPC de-threading D4 Slice 2b: `endpointCallWithCaps` **establishes**
`queueNextBlockingConsistent` (`endpointCall` establish on `stMid` + optional `ipcUnwrapCaps`). -/
theorem endpointCallWithCaps_preserves_queueNextBlockingConsistent
  ...
  | ok pair =>
    rcases pair with ⟨_, stMid⟩
    have hQNBCMid := endpointCall_preserves_queueNextBlockingConsistent st stMid endpointId caller msg
      hInv hDQSI hTail hObjInv hFreshCaller hSendTailFresh hCall
    have hObjInvMid := endpointCall_preserves_objects_invExt st stMid endpointId caller msg hObjInv hCall
    simp [hCall] at hStep
    cases hEp : st.getEndpoint? endpointId with
    | none => simp [hEp] at hStep; obtain ⟨_, rfl⟩ := hStep; exact hQNBCMid
    | some ep =>
      ...
      | some receiverId =>
        ...
        · simp [hEmpty] at hStep
          ...
          | some recvRoot =>
            simp [hLookup] at hStep
            exact ipcUnwrapCaps_preserves_queueNextBlockingConsistent msg callerCspaceRoot recvRoot
              receiverSlotBase _ stMid st' summary hObjInvMid hQNBCMid hStep

open SeLe4n.Model.SystemState in
/-- IPC de-threading D4 Slice 2b: `endpointReceiveDualWithCaps` **establishes**
`queueNextBlockingConsistent` — the base `endpointReceiveDual` establish on `stMid`, then the
optional `ipcUnwrapCaps` (cap transfer writes only CNode caps, framing every `queueNext`/`ipcState`).
The cap-transfer leg is gated on the *receiver*'s delivered `pendingMessage` rather than the
...
theorem endpointReceiveDualWithCaps_preserves_queueNextBlockingConsistent
  ...
  | ok pair =>
    rcases pair with ⟨sid, stMid⟩
    have hQNBCMid := endpointReceiveDual_preserves_queueNextBlockingConsistent endpointId receiver
    ...
    | some receiverTcb =>
      simp [hTcb] at hStep
      cases hMsg : receiverTcb.pendingMessage with
      | none => simp [hMsg] at hStep; obtain ⟨⟨_, _⟩, rfl⟩ := hStep; exact hQNBCMid
      | some msg =>
        simp [hMsg] at hStep
        split at hStep
...
open SeLe4n.Model.SystemState in
/-- IPC de-threading D4 Slice 2b: `endpointSendDualWithCaps` **establishes**
`endpointQueueTailBlockedConsistent` (base `endpointSendDual` establish on `stMid` + optional
`ipcUnwrapCaps` frame). -/
theorem endpointSendDualWithCaps_preserves_endpointQueueTailBlockedConsistent
  ...
  | ok pair =>
    rcases pair with ⟨_, stMid⟩
    have hTailMid := endpointSendDual_preserves_endpointQueueTailBlockedConsistent st stMid endpointId
      sender msg hInv hDQSI hQHBC hObjInv hFreshSender hSend
    have hObjInvMid := endpointSendDual_preserves_objects_invExt st stMid endpointId sender msg hObjInv hSend
    simp [hSend] at hStep
    cases hEp : st.getEndpoint? endpointId with
    | none => simp [hEp] at hStep; obtain ⟨_, rfl⟩ := hStep; exact hTailMid
    | some ep =>
      simp [hEp] at hStep
      cases hHead : ep.receiveQ.head with
      | none => simp [hHead] at hStep; obtain ⟨_, rfl⟩ := hStep; exact hTailMid
      | some receiverId =>
        simp [hHead] at hStep
        by_cases hEmpty : msg.caps = #[]
        ...
        · simp [hEmpty] at hStep
          ...
          | some recvRoot =>
            simp [hLookup] at hStep
            exact ipcUnwrapCaps_preserves_endpointQueueTailBlockedConsistent msg senderCspaceRoot recvRoot
              receiverSlotBase _ stMid st' summary hObjInvMid hTailMid hStep

open SeLe4n.Model.SystemState in
/-- IPC de-threading D4 Slice 2b: `endpointCallWithCaps` **establishes**
`endpointQueueTailBlockedConsistent` (base `endpointCall` establish on `stMid` + optional `ipcUnwrapCaps`). -/
theorem endpointCallWithCaps_preserves_endpointQueueTailBlockedConsistent
  ...
  | ok pair =>
    rcases pair with ⟨_, stMid⟩
    have hTailMid := endpointCall_preserves_endpointQueueTailBlockedConsistent st stMid endpointId caller
      msg hInv hDQSI hQHBC hObjInv hFreshCaller hCall
    have hObjInvMid := endpointCall_preserves_objects_invExt st stMid endpointId caller msg hObjInv hCall
    simp [hCall] at hStep
    cases hEp : st.getEndpoint? endpointId with
    | none => simp [hEp] at hStep; obtain ⟨_, rfl⟩ := hStep; exact hTailMid
    | some ep =>
      ...
      | some receiverId =>
        ...
        · simp [hEmpty] at hStep
          ...
          | some recvRoot =>
            simp [hLookup] at hStep
            exact ipcUnwrapCaps_preserves_endpointQueueTailBlockedConsistent msg callerCspaceRoot recvRoot
              receiverSlotBase _ stMid st' summary hObjInvMid hTailMid hStep

open SeLe4n.Model.SystemState in
/-- IPC de-threading D4 Slice 2b: `endpointReceiveDualWithCaps` **establishes**
`endpointQueueTailBlockedConsistent` (base `endpointReceiveDual` establish on `stMid` + optional
...
theorem endpointReceiveDualWithCaps_preserves_endpointQueueTailBlockedConsistent
  ...
  | ok pair =>
    rcases pair with ⟨sid, stMid⟩
    have hTailMid := endpointReceiveDual_preserves_endpointQueueTailBlockedConsistent endpointId receiver
    ...
    | some receiverTcb =>
      simp [hTcb] at hStep
      cases hMsg : receiverTcb.pendingMessage with
      | none => simp [hMsg] at hStep; obtain ⟨⟨_, _⟩, rfl⟩ := hStep; exact hTailMid
      | some msg =>
        simp [hMsg] at hStep
        split at hStep
...
its enqueue-establish discharged by the receiver-freshness preconditions transported through the
reply phase). -/
theorem endpointReplyRecv_preserves_queueNextBlockingConsistent
    (endpointId : SeLe4n.ObjId) (receiver replyTarget : SeLe4n.ThreadId)
    (msg : IpcMessage) (replyId : Option SeLe4n.ReplyId) (st st' : SystemState)
    (hInvFull : ipcInvariantFull st)
    (hObjInv : st.objects.invExt)
    ...
    (hStep : endpointReplyRecv endpointId receiver replyTarget msg replyId st = .ok ((), st')) :
    queueNextBlockingConsistent st' := by
  have hQNBC := hInvFull.queueNextBlockingConsistent
  have hDQSI := hInvFull.2.1
  have hTail := hInvFull.endpointQueueTailBlockedConsistent
  unfold endpointReplyRecv at hStep
  simp only [show ¬(maxMessageRegisters < msg.registers.size) from by
    intro h; simp [h] at hStep, ↓reduceIte] at hStep
  simp only [show ¬(maxExtraCaps < msg.caps.size) from by
  ...
  | some replyTcb =>
    ...
    | blockedOnReply _ expectedReplier =>
      ...
      suffices ∀ st1, storeTcbIpcStateAndMessage st replyTarget .ready (some msg) = .ok st1 →
          (∀ stR, endpointReceiveDual endpointId receiver replyId (ensureRunnable st1 replyTarget) = .ok stR →
            queueNextBlockingConsistent stR.2) by
        cases expectedReplier with
        | none => simp at hStep
        | some expected =>
          simp only at hStep
      ...
      have hQNBC1 := storeTcbIpcStateAndMessage_ready_preserves_queueNextBlockingConsistent
        st st1 replyTarget (some msg) hQNBC hObjInv hMsg
      -- EQTB: `replyTarget` was `.blockedOnReply`, so by pre-state tail-blocked it is no endpoint tail.
      have hNotTail : ∀ (epId' : SeLe4n.ObjId) (ep' : Endpoint),
          st.objects[epId']? = some (.endpoint ep') →
          ep'.receiveQ.tail ≠ some replyTarget ∧ ep'.sendQ.tail ≠ some replyTarget := by
        intro epId' ep' hEp'
        refine ⟨fun hTl => ?_, fun hTl => ?_⟩
        · have := (hTail epId' ep' replyTarget replyTcb hEp'
            (lookupTcb_some_objects st replyTarget replyTcb hLookup)).1 hTl
          rw [hIpc] at this; cases this
        · rcases (hTail epId' ep' replyTarget replyTcb hEp'
            (lookupTcb_some_objects st replyTarget replyTcb hLookup)).2 hTl with h | h <;>
            rw [hIpc] at h <;> cases h
      have hEQTB1 := storeTcbIpcStateAndMessage_preserves_endpointQueueTailBlockedConsistent
        st st1 replyTarget .ready (some msg) hTail hObjInv hMsg hNotTail
      have hObjInvE : (ensureRunnable st1 replyTarget).objects.invExt :=
        ensureRunnable_preserves_objects st1 replyTarget ▸ hObjInv1
      have hDQSIE := ensureRunnable_preserves_dualQueueSystemInvariant st1 replyTarget hDQSI1
      have hQNBCE := ensureRunnable_preserves_queueNextBlockingConsistent st1 replyTarget hQNBC1
      have hEQTBE := ensureRunnable_preserves_endpointQueueTailBlockedConsistent st1 replyTarget hEQTB1
      -- Transport freshness conditions through the reply phase (endpoints unchanged by storeTcb + ensureRunnable).
      have hFreshReceiver' : ∀ (epId' : SeLe4n.ObjId) (ep' : Endpoint),
          (ensureRunnable st1 replyTarget).objects[epId']? = some (.endpoint ep') →
          ep'.sendQ.head ≠ some receiver ∧ ep'.sendQ.tail ≠ some receiver ∧
          ep'.receiveQ.head ≠ some receiver ∧ ep'.receiveQ.tail ≠ some receiver := by
      ...
      have hRecvTailFresh' : ∀ (ep' : Endpoint) (tailTid : SeLe4n.ThreadId),
        ...
        exact hRecvTailFresh ep' tailTid
          ...
          hTl epId' ep''
          (storeTcbIpcStateAndMessage_endpoint_backward st st1 replyTarget .ready (some msg) epId' ep'' hObjInv hMsg hEp'')
      -- Delegate to the receive-leg enqueue-establish.
      exact endpointReceiveDual_preserves_queueNextBlockingConsistent
        endpointId receiver replyId _ stR.2 stR.1
        hQNBCE hDQSIE hEQTBE hObjInvE hFreshReceiver' hRecvTailFresh'
        (by have : stR = (stR.1, stR.2) := Prod.ext rfl rfl; rw [this] at hRecv; exact hRecv)

open SeLe4n.Model.SystemState in
/-- IPC de-threading D4 Slice 2b: `endpointReplyRecv` **establishes** `endpointQueueTailBlockedConsistent`
from the pre-state.  Composes the reply phase (`storeTcbIpcStateAndMessage replyTarget .ready` +
`ensureRunnable`; the unblocked `.blockedOnReply replyTarget` is no endpoint tail, so the tail-blocked
+ head-blocked + dualQueue frames survive) with the `endpointReceiveDual` receive leg (tail-blocked
establisher), its preconditions (`endpointQueueTailBlockedConsistent`, `dualQueueSystemInvariant`,
`queueHeadBlockedConsistent`, `objects.invExt`, receiver-freshness) carried across the reply phase. -/
theorem endpointReplyRecv_preserves_endpointQueueTailBlockedConsistent
    (endpointId : SeLe4n.ObjId) (receiver replyTarget : SeLe4n.ThreadId)
    (msg : IpcMessage) (replyId : Option SeLe4n.ReplyId) (st st' : SystemState)
    (hInvFull : ipcInvariantFull st)
    (hObjInv : st.objects.invExt)
  ...
  have hDQSI := hInvFull.2.1
  have hTail := hInvFull.endpointQueueTailBlockedConsistent
  have hQHBC := hInvFull.queueHeadBlockedConsistent
  unfold endpointReplyRecv at hStep
  simp only [show ¬(maxMessageRegisters < msg.registers.size) from by
    intro h; simp [h] at hStep, ↓reduceIte] at hStep
  simp only [show ¬(maxExtraCaps < msg.caps.size) from by
  ...
  | some replyTcb =>
    ...
    | blockedOnReply _ expectedReplier =>
      ...
      suffices ∀ st1, storeTcbIpcStateAndMessage st replyTarget .ready (some msg) = .ok st1 →
          (∀ stR, endpointReceiveDual endpointId receiver replyId (ensureRunnable st1 replyTarget) = .ok stR →
            endpointQueueTailBlockedConsistent stR.2) by
        cases expectedReplier with
        | none => simp at hStep
        | some expected =>
          simp only at hStep
      ...
      have hEQTB1 := storeTcbIpcStateAndMessage_preserves_endpointQueueTailBlockedConsistent
        st st1 replyTarget .ready (some msg) hTail hObjInv hMsg hNotTail
      -- QHBC: `replyTarget` (`.blockedOnReply`) is no endpoint head either.
      have hNotHead : ∀ (epId' : SeLe4n.ObjId) (ep' : Endpoint),
          st.objects[epId']? = some (.endpoint ep') →
          ep'.receiveQ.head ≠ some replyTarget ∧ ep'.sendQ.head ≠ some replyTarget := by
        intro epId' ep' hEp'
        ...
        · have := (hQHBC epId' ep' replyTarget replyTcb hEp'
            (lookupTcb_some_objects st replyTarget replyTcb hLookup)).1 hH
          rw [hIpc] at this; cases this
        · rcases (hQHBC epId' ep' replyTarget replyTcb hEp'
            (lookupTcb_some_objects st replyTarget replyTcb hLookup)).2 hH with h | h <;>
            rw [hIpc] at h <;> cases h
      have hQHBC1 := storeTcbIpcStateAndMessage_preserves_queueHeadBlockedConsistent
        st st1 replyTarget .ready (some msg) hQHBC hObjInv hMsg hNotHead
      have hObjInvE : (ensureRunnable st1 replyTarget).objects.invExt :=
      ...
      have hFreshReceiver' : ∀ (epId' : SeLe4n.ObjId) (ep' : Endpoint),
        ...
        exact hFreshReceiver epId' ep'
          (storeTcbIpcStateAndMessage_endpoint_backward st st1 replyTarget .ready (some msg) epId' ep' hObjInv hMsg hEp')
      exact endpointReceiveDual_preserves_endpointQueueTailBlockedConsistent
        endpointId receiver replyId _ stR.2 stR.1
        hEQTBE hDQSIE hQHBCE hObjInvE hFreshReceiver'
        (by have : stR = (stR.1, stR.2) := Prod.ext rfl rfl; rw [this] at hRecv; exact hRecv)
...
theorem endpointQueuePopHead_preserves_queueHeadBlockedConsistent
  ...
  intro eid ep' hd tcbHd hEp' hTcb
  obtain ⟨tcbHdPre, hTcbHdPre, hIpcHd⟩ :=
    endpointQueuePopHead_tcb_ipcState_backward endpointId isReceiveQ st st' tid hd tcbHd hObjInv hStep hTcb
  by_cases hEid : eid = endpointId
  · subst eid
    rw [hEp'] at hEpPost
    obtain rfl : ep' = epPost := by simpa using hEpPost
    -- Pre-state head-blocked for the old popped head `tid`/`headTcb`.
    have hHeadBlocked := hQHBC endpointId ep tid headTcb hObj hHeadTcb
    cases hRQ : isReceiveQ with
    | true =>
      subst hRQ
      simp only [↓reduceIte] at hPoppedHead hOtherHead hHeadEq
      refine ⟨fun hHd => ?_, fun hHd => ?_⟩
      · -- receiveQ (popped): new head = headTcb.queueNext, blocked-receive via qNTB.
        rw [hPoppedHead] at hHd
        have hOldRecv : headTcb.ipcState = .blockedOnReceive endpointId := hHeadBlocked.1 hHeadEq
        have hLink := (hQNTB tid hd headTcb tcbHdPre hHeadTcb hTcbHdPre hHd).1 endpointId hOldRecv
        rw [← hIpcHd]; exact hLink
      · -- sendQ (other queue, unchanged): pre-state qHBC.
        rw [hOtherHead] at hHd
        have hPre := (hQHBC endpointId ep hd tcbHdPre hObj hTcbHdPre).2 hHd
        rw [← hIpcHd]; exact hPre
    | false =>
      subst hRQ
      simp only [Bool.false_eq_true, ↓reduceIte] at hPoppedHead hOtherHead hHeadEq
      refine ⟨fun hHd => ?_, fun hHd => ?_⟩
      · -- receiveQ (other queue, unchanged): pre-state qHBC.
        rw [hOtherHead] at hHd
        have hPre := (hQHBC endpointId ep hd tcbHdPre hObj hTcbHdPre).1 hHd
        rw [← hIpcHd]; exact hPre
      · -- sendQ (popped): new head = headTcb.queueNext, blocked-send/call via qNTB.
        rw [hPoppedHead] at hHd
        have hOldSend : headTcb.ipcState = .blockedOnSend endpointId ∨
            headTcb.ipcState = .blockedOnCall endpointId := hHeadBlocked.2 hHeadEq
        have hLink := (hQNTB tid hd headTcb tcbHdPre hHeadTcb hTcbHdPre hHd).2 endpointId hOldSend
        rw [← hIpcHd]; exact hLink
  · -- eid ≠ endpointId: endpoint + every head ipcState frames.
    have hEpPre : st.objects[eid]? = some (.endpoint ep') :=
      endpointQueuePopHead_endpoint_backward_ne endpointId isReceiveQ st st' tid eid ep' hEid hObjInv hStep hEp'
    have hPre := hQHBC eid ep' hd tcbHdPre hEpPre hTcbHdPre
    rw [← hIpcHd]
...
open SeLe4n.Model.SystemState in
/-- IPC de-threading D4: `endpointReply` preserves `endpointQueueTailBlockedConsistent` (it touches
no endpoint queue; the unblocked `target` was `.blockedOnReply`, so by pre-state tail-blocked it is
no endpoint tail). -/
theorem endpointReply_preserves_endpointQueueTailBlockedConsistent
    (st st' : SystemState) (replier target : SeLe4n.ThreadId) (msg : IpcMessage)
    (hObjInv : st.objects.invExt) (hInv : endpointQueueTailBlockedConsistent st)
    (hStep : endpointReply replier target msg st = .ok ((), st')) :
    endpointQueueTailBlockedConsistent st' := by
      ...
      | some expected =>
        ...
        · revert hStep
          ...
          | ok st'' =>
            ...
            simp only [Except.ok.injEq, Prod.mk.injEq] at hStep
            obtain ⟨_, rfl⟩ := hStep
            -- `target` is `.blockedOnReply`, so not an endpoint tail; discharge `hNotTail`.
            have hTargetObj : st.objects[target.toObjId]? = some (.tcb tcb) :=
              lookupTcb_some_objects st target tcb hLookup
            have hNotTail : ∀ (epId' : SeLe4n.ObjId) (ep' : Endpoint),
                st.objects[epId']? = some (.endpoint ep') →
                ep'.receiveQ.tail ≠ some target ∧ ep'.sendQ.tail ≠ some target := by
              intro epId' ep' hEp'
              refine ⟨fun hTl => ?_, fun hTl => ?_⟩
              · have := (hInv epId' ep' target tcb hEp' hTargetObj).1 hTl; rw [hIpc] at this; cases this
              · rcases (hInv epId' ep' target tcb hEp' hTargetObj).2 hTl with h | h <;> rw [hIpc] at h <;> cases h
            exact ensureRunnable_preserves_endpointQueueTailBlockedConsistent st'' target
              (storeTcbIpcStateAndMessage_preserves_endpointQueueTailBlockedConsistent
                st st'' target .ready (some msg) hInv hObjInv hMsg hNotTail)
        · simp at hStep
...
open SeLe4n.Model.SystemState in
/-- IPC de-threading D4: `notificationSignal` preserves `endpointQueueTailBlockedConsistent` (it
touches no endpoint queue; the woken head waiter is `.blockedOnNotification` (`hNWC`), so by
pre-state tail-blocked it is no endpoint tail). -/
theorem notificationSignal_preserves_endpointQueueTailBlockedConsistent
    (st st' : SystemState) (notificationId : SeLe4n.ObjId) (badge : SeLe4n.Badge)
    (hObjInv : st.objects.invExt) (hInv : endpointQueueTailBlockedConsistent st)
    (hNWC : notificationWaiterConsistent st)
    (hStep : notificationSignal notificationId badge st = .ok ((), st')) :
  ...
  · rename_i ntfn hObj
    ...
    | some headTail =>
      ...
      next => contradiction
      next st1 hSO =>
        have hInv1 := storeObject_nonEndpointNonTcb_preserves_endpointQueueTailBlockedConsistent
          st st1 notificationId (.notification _) hObjInv (fun _ => by simp) (fun _ => by simp) hInv hSO
        have hObjInv1 := storeObject_preserves_objects_invExt st st1 notificationId _ hObjInv hSO
        have hValEq : ntfn.waitingThreads.val = waiter :: rest.val :=
          (SeLe4n.NoDupList.tail?_eq_some_iff ntfn.waitingThreads waiter rest).mp hWaiters
        ...
        next st2 hSM =>
          ...
          have hNotTail : ∀ (epId' : SeLe4n.ObjId) (ep' : Endpoint),
              st1.objects[epId']? = some (.endpoint ep') →
              ep'.receiveQ.tail ≠ some waiter ∧ ep'.sendQ.tail ≠ some waiter := by
            intro epId' ep' hEp'
            refine ⟨fun hTl => ?_, fun hTl => ?_⟩
            · have := (hInv1 epId' ep' waiter wTcb hEp' hWTcb1).1 hTl; rw [hWIpc] at this; cases this
            · rcases (hInv1 epId' ep' waiter wTcb hEp' hWTcb1).2 hTl with h | h <;> rw [hWIpc] at h <;> cases h
          exact ensureRunnable_preserves_endpointQueueTailBlockedConsistent st2 waiter
            (storeTcbIpcStateAndMessage_preserves_endpointQueueTailBlockedConsistent
              st1 st2 waiter .ready _ hInv1 hObjInv1 hSM hNotTail)
    | none =>
      simp only [hWaiters] at hStep
      exact storeObject_nonEndpointNonTcb_preserves_endpointQueueTailBlockedConsistent
        st st' notificationId (.notification _) hObjInv (fun _ => by simp) (fun _ => by simp) hInv hStep
  · contradiction
  · contradiction

open SeLe4n.Model.SystemState in
/-- IPC de-threading D4 Slice 2b: `notificationWait` preserves `endpointQueueTailBlockedConsistent`.
It touches no endpoint queue; the only rewritten TCB is the running `waiter`, which is `.ready` in
the pre-state (`hWaiterReady`) — hence by pre-state tail-blocked it is no endpoint tail — and is
rewritten to `.ready` (badge-delivery branch) or `.blockedOnNotification` (block branch). -/
theorem notificationWait_preserves_endpointQueueTailBlockedConsistent
    (st st' : SystemState) (notificationId : SeLe4n.ObjId) (waiter : SeLe4n.ThreadId)
    (badge : Option SeLe4n.Badge)
    (hObjInv : st.objects.invExt) (hInv : endpointQueueTailBlockedConsistent st)
  ...
  · rename_i ntfn hObj
    split at hStep
    · -- badge-delivery branch: store notification idle then `storeTcbIpcState waiter .ready`
      split at hStep
      next => contradiction
      next st1 hSO =>
        have hInv1 := storeObject_nonEndpointNonTcb_preserves_endpointQueueTailBlockedConsistent
        ...
        next st2 hSI =>
          ...
          have hNe : waiter.toObjId ≠ notificationId := by
            intro hEq
            rw [hEq, storeObject_objects_eq st st1 notificationId _ hObjInv hSO] at hWTcb1
            simp at hWTcb1
          have hWTcbSt : st.objects[waiter.toObjId]? = some (.tcb wtcb1) := by
            rw [← storeObject_objects_ne st st1 notificationId waiter.toObjId _ hNe hObjInv hSO]
            exact hWTcb1
          have hReady1 : wtcb1.ipcState = .ready := hWaiterReady wtcb1 hWTcbSt
          have hNotTail : ∀ (epId : SeLe4n.ObjId) (ep : Endpoint),
              st1.objects[epId]? = some (.endpoint ep) →
              ep.receiveQ.tail ≠ some waiter ∧ ep.sendQ.tail ≠ some waiter := by
            intro epId ep hEp
            refine ⟨fun hTl => ?_, fun hTl => ?_⟩
            · have := (hInv1 epId ep waiter wtcb1 hEp hWTcb1).1 hTl; rw [hReady1] at this; cases this
            · rcases (hInv1 epId ep waiter wtcb1 hEp hWTcb1).2 hTl with h | h <;> rw [hReady1] at h <;> cases h
          exact storeTcbIpcState_preserves_endpointQueueTailBlockedConsistent
            st1 _ waiter .ready hInv1 hObjInv1 hSI hNotTail
    · -- block branch: store notification waiting then block the waiter `.blockedOnNotification`
      split at hStep
      · contradiction
      · rename_i waiterTcb hLookup
        split at hStep
      ...
      next st1 hSO =>
        ...
        next st2 hSI =>
          ...
          have hNotHead : ∀ (epId : SeLe4n.ObjId) (ep : Endpoint),
            ...
            intro epId ep hEp
            refine ⟨fun hHd => ?_, fun hHd => ?_⟩
            · have := (hInv1 epId ep waiter wtcb1 hEp hWTcb1).1 hHd; rw [hReady1] at this; cases this
            · rcases (hInv1 epId ep waiter wtcb1 hEp hWTcb1).2 hHd with h | h <;> rw [hReady1] at h <;> cases h
          exact storeTcbIpcState_preserves_queueHeadBlockedConsistent
            st1 _ waiter .ready hInv1 hObjInv1 hSI hNotHead
    · -- block branch: store notification waiting then block the waiter `.blockedOnNotification`
      split at hStep
      · contradiction
      · rename_i waiterTcb hLookup
...
theorem endpointReceiveDual_preserves_ipcInvariantFull
    ...
    -- threaded `hQNBC'`, now **established** via `endpointReceiveDual_preserves_queueNextBlockingConsistent`.
    (hFreshReceiver : ∀ (epId : SeLe4n.ObjId) (ep : Endpoint),
      st.objects[epId]? = some (.endpoint ep) →
      ep.sendQ.head ≠ some receiver ∧ ep.sendQ.tail ≠ some receiver ∧
      ep.receiveQ.head ≠ some receiver ∧ ep.receiveQ.tail ≠ some receiver)
...
theorem endpointCall_preserves_ipcInvariantFull
    ...
    -- threaded `hQNBC'`, now **established** via `endpointCall_preserves_queueNextBlockingConsistent`.
    (hFreshCaller : ∀ (epId : SeLe4n.ObjId) (ep : Endpoint),
      st.objects[epId]? = some (.endpoint ep) →
      ep.sendQ.head ≠ some caller ∧ ep.sendQ.tail ≠ some caller ∧
      ep.receiveQ.head ≠ some caller ∧ ep.receiveQ.tail ≠ some caller)
...
theorem notificationWait_preserves_ipcInvariantFull
    ...
    -- IPC de-threading D5: no thread carries a timeout budget (dischargeable global precondition).
    (hAllBudgetsNone : allTimeoutBudgetsNone st)
    -- IPC de-threading D4 Slice 2b: the running waiter is `.ready` (establishes
  ...
  exact ⟨notificationWait_preserves_ipcInvariant st st' notificationId waiter result hInv.1 hObjInv hStep,
   ...
   -- IPC de-threading D4 Slice 2c: queueHeadBlockedConsistent **established** from the pre-state — no
   -- endpoint queue is touched; the running waiter is `.ready`, hence not an endpoint head.
   notificationWait_preserves_queueHeadBlockedConsistent st st' notificationId waiter result
     hObjInv hInv.queueHeadBlockedConsistent hWaiterReady hStep,
...
theorem endpointReplyRecv_preserves_ipcInvariantFull
    ...
    -- `endpointReplyRecv_preserves_queueNextBlockingConsistent`.
    (hFreshReceiver : ∀ (epId : SeLe4n.ObjId) (ep : Endpoint),
      st.objects[epId]? = some (.endpoint ep) →
      ep.sendQ.head ≠ some receiver ∧ ep.sendQ.tail ≠ some receiver ∧
      ep.receiveQ.head ≠ some receiver ∧ ep.receiveQ.tail ≠ some receiver)
...
theorem endpointSendDualWithCaps_preserves_ipcInvariantFull
    ...
    -- IPC de-threading D4 Slice 2b: enqueue freshness (the running sender is `.ready`, hence not a
    -- queue member, and the old sendQ tail is not a cross-queue tail) — dischargeable, replacing the
    -- threaded `hQNBC'`, now **established** via
...
theorem endpointReceiveDualWithCaps_preserves_ipcInvariantFull
    ...
    -- queue member, and the old receiveQ tail is not a cross-queue tail) — dischargeable, replacing the
    -- threaded `hQNBC'`, now **established** via
    -- `endpointReceiveDualWithCaps_preserves_queueNextBlockingConsistent`.
    (hFreshReceiver : ∀ (epId : SeLe4n.ObjId) (ep : Endpoint),
      st.objects[epId]? = some (.endpoint ep) →
      ep.sendQ.head ≠ some receiver ∧ ep.sendQ.tail ≠ some receiver ∧
      ep.receiveQ.head ≠ some receiver ∧ ep.receiveQ.tail ≠ some receiver)
