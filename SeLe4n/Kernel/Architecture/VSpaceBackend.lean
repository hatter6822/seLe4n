...
/-- WS-G6/F-P05/WS-H11: The HashMap-backed `VSpaceRoot` satisfies the `VSpaceBackend` interface.
    ...
    `rootWF` is `root.mappings.invExt ∧ root.mappings.size < root.mappings.capacity`,
    which is the conjunction required by the RHTable erase/insert correctness lemmas. -/
instance hashMapVSpaceBackend : VSpaceBackend VSpaceRoot where
  ...
  rootWF root := root.mappings.invExt ∧ root.mappings.size < root.mappings.capacity
  ...
  lookup_after_map := fun root root' vaddr paddr perms hWF hMap =>
    VSpaceRoot.lookup_mapPage_eq root root' vaddr paddr perms hWF.1 hMap
  lookup_map_other := fun root root' vaddr vaddr' paddr perms hWF hNe hMap =>
    VSpaceRoot.lookup_mapPage_ne root root' vaddr vaddr' paddr perms hNe hWF.1 hMap
  lookup_after_unmap := fun root root' vaddr hWF hUnmap =>
    VSpaceRoot.lookup_unmapPage_eq_none root root' vaddr hWF.1 hUnmap
  lookup_unmap_other := fun root root' vaddr vaddr' hWF hNe hUnmap =>
    VSpaceRoot.lookup_unmapPage_ne root root' vaddr vaddr' hNe hWF.1 hWF.2 hUnmap
