...
-- ============================================================================
-- WS-SM SM6.D: MCS `Call` ABI (faithful reply-capability passing)
-- ============================================================================

/-- WS-SM SM6.D: argument structure for the MCS `Call` syscall.  Register
    mapping: `msgRegs[0]` = reply capability pointer — the slot in the caller's
    CSpace holding the single-use Reply cap the kernel links to the call.  Like
    `tcbBindNotification` (and unlike the legacy `replyRecv` raw thread-ID
    target), the reply authority is resolved through the caller's CSpace via
    `syscallLookupCap`, so a caller must *hold* a reply capability rather than
    name a raw `ReplyId`.  The invoked endpoint is the capability target; the
    message body comes from the standard message registers. -/
structure IpcCallArgs where
  replyCPtr : Nat
  deriving Repr, DecidableEq

/-- WS-SM SM6.D: decode MCS `Call` arguments (1 register: the reply capability
    pointer). -/
def decodeIpcCallArgs (decoded : SyscallDecodeResult)
    : Except KernelError IpcCallArgs := do
  let r0 ← requireMsgReg decoded.msgRegs 0
  pure { replyCPtr := r0.val }

/-- WS-SM SM6.D: encode MCS `Call` arguments into message registers.  Inverse of
    `decodeIpcCallArgs`. -/
@[inline] def encodeIpcCallArgs (args : IpcCallArgs) : Array RegValue :=
  #[⟨args.replyCPtr⟩]

/-- WS-SM SM6.D: `Call` decode fails iff fewer than 1 message register. -/
theorem decodeIpcCallArgs_error_iff (d : SyscallDecodeResult) :
    (∃ e, decodeIpcCallArgs d = .error e) ↔ d.msgRegs.size < 1 := by
  constructor
  · intro ⟨e, he⟩
    by_cases hlt : d.msgRegs.size < 1
    · exact hlt
    · exfalso
      simp only [decodeIpcCallArgs, bind, Except.bind,
        requireMsgReg, dif_pos (show 0 < d.msgRegs.size by omega),
        pure, Except.pure] at he
      nomatch he
  · intro h
...
/-- WS-SM SM6.D: round-trip — encoding then decoding `IpcCallArgs` recovers the
    original. -/
theorem decodeIpcCallArgs_roundtrip (args : IpcCallArgs) :
    decodeIpcCallArgs (stubDecoded (encodeIpcCallArgs args)) = .ok args := by
...
/-- WS-SM SM6.B: arguments for `tcbBindNotification` — `msgRegs[0]` is the
    **capability address** (CPtr) of the notification to bind to the
    capability-target TCB.  Resolved through the caller's CSpace (review #1):
    binding requires the caller to *hold* a notification capability, not merely
    name a raw `ObjId`. -/
structure TcbBindNotificationArgs where
  notificationCPtr : Nat
  deriving Repr, DecidableEq
