...
unchanged. Therefore CNode slot-key uniqueness transfers directly from the pre-state. -/
theorem storeServiceState_preserves_capabilityInvariantBundle
  ...
  have hObjEq : (storeServiceState sid { svc with status := newStatus } st).objects = st.objects := by
    simp [storeServiceState]
  have hUnique' : cspaceSlotUnique (storeServiceState sid { svc with status := newStatus } st) := by
    intro oid cn hObj
    rw [hObjEq] at hObj
    exact hUnique oid cn hObj
  have hSound' : cspaceLookupSound (storeServiceState sid { svc with status := newStatus } st) :=
    cspaceLookupSound_of_slotUnique _ hUnique'
  exact ⟨hUnique', hSound', hAttRule, lifecycleAuthorityMonotonicity_holds _⟩
