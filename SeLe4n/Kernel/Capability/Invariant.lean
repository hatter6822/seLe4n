...
- `cspaceMint_badge_notification_consistent`: Proves that badge values propagated
  through `cspaceMint` are faithfully delivered by notification signaling operations.
...
/-- WS-E2 / H-03: Badge preservation through capability minting.

When `cspaceMint` is called with an explicit badge override `some badge`,
the child capability stored in the destination slot carries exactly that badge.
This is the first link in the badge routing chain: the minted badge is faithfully
stored in the capability object. -/
...
/-- WS-E2 / H-03: Badge propagation through notification signaling (fresh notification).

When a notification has no waiters and no pending badge, signaling with badge `b`
stores exactly `b` as the pending badge. This is the second link in the badge routing
chain: the badge from the capability is faithfully transferred to the notification. -/
theorem notificationSignal_badge_stored_fresh
  ...
  -- After simplification, hSignal is about storeObject with pendingBadge := some badge
  exact ⟨{ state := .active, waitingThreads := [], pendingBadge := some badge },
    by rw [← storeObject_objects_eq st st' notifId _ hSignal], rfl⟩

/-- WS-E2 / H-03: Badge recovery through notification waiting.

When `notificationWait` succeeds and returns `some badge`, that badge was the
pending badge of the notification object in the pre-state. This is the third link:
the badge stored in the notification is faithfully delivered to the waiting thread. -/
theorem notificationWait_recovers_pending_badge
  ...
  | some obj =>
      ...
      | notification ntfn =>
          ...
          | none =>
              -- No pending badge: all paths return .error or .ok (none, ...)
          ...
          | some b =>
              -- Has pending badge: extract b = badge from the computation
...
/-- WS-E2 / H-03: End-to-end badge routing consistency.
...
sender identity encoded in badge values cannot be lost or substituted through the
IPC notification mechanism. -/
...
-- ============================================================================
-- WS-E2 / C-01: Compositional derivation infrastructure for non-trivial invariants
-- ============================================================================
...
/-- Transfer `cspaceSlotUnique` across an object-store update that preserves CNode structure.

WS-E2 / H-01 compositional pattern: when `storeObject` writes a non-CNode object (endpoint,
TCB, notification, VSpaceRoot), all CNode objects are unchanged and uniqueness transfers
...
theorem cspaceSlotUnique_of_storeObject_nonCNode
  ...
  · rw [hEq] at hObj
    ...
    rw [hEqObj] at hObj
    -- hObj : some obj = some (.cnode cn) — contradicts hNotCNode
...
/-- Transfer `cspaceSlotUnique` across a CNode-specific `storeObject`, given that the
new CNode satisfies `slotsUnique`. -/
...
/-- WS-E2 / H-01: Helper to transfer `cspaceSlotUnique` across endpoint-object stores.
Endpoint stores write a non-CNode object, so all CNode objects are unchanged. -/
...
/-- WS-E2 / H-01: Helper to transfer `cspaceSlotUnique` when objects are unchanged
(e.g., across `storeCapabilityRef` or `clearCapabilityRefs`). -/
...
theorem endpointSend_preserves_capabilityInvariantBundle
    ...
    capabilityInvariantBundle st' := by
  rcases hInv with ⟨hUnique, _hSound, hAttRule, _hLifecycle⟩
  -- Compositional: endpoint store preserves CNode objects (WS-E2 / H-01)
