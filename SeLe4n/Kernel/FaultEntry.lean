...
/-- WS-RR RR4.23: the verified step the fault entry commits — classify, build
the fault context from the faulting thread's saved registers, deliver.

Separated from the `BaseIO` entry so the whole decision is a pure function of
the pre-state and the syndrome, and so the tests exercise exactly what the
seam runs.  Returns the SGI list the entry fires after the commit, in the
shape `fireCrossCoreSgis` consumes.
...
def faultEntryStep (lctx : LabelingContext) (st : SystemState)
    (ectx : ExceptionContext) (coreId : UInt64) :
  ...
  if h : coreId.toNat < numCores then
    ...
    | some f =>
        ...
        | some tid =>
            ...
            let r := faultDeliverOnCoreChecked lctx st tid f fctx c
            (r.2.sgi.toList, r.1)
  else
...
theorem faultEntryStep_invalid_core (lctx : LabelingContext) (st : SystemState)
    (ectx : ExceptionContext)
    (coreId : UInt64) (h : ¬ coreId.toNat < numCores) :
    faultEntryStep lctx st ectx coreId = ([], st) := by
...
against the live kernel state, then fires the cross-core SGI the delivery
...
def faultEntry (coreId : UInt64) (esr elr spsr far : UInt64) : BaseIO Unit := do
  let lctx ← Platform.FFI.getKernelLabelingContext
  let sgis ← Platform.FFI.modifyGetKernelState (fun st =>
    faultEntryStep lctx st { esr := esr, elr := elr, spsr := spsr, far := far } coreId)
...
theorem faultEntry_def (coreId : UInt64) (esr elr spsr far : UInt64) :
    faultEntry coreId esr elr spsr far =
      (do
        let lctx ← Platform.FFI.getKernelLabelingContext
        let sgis ← Platform.FFI.modifyGetKernelState (fun st =>
          faultEntryStep lctx st { esr := esr, elr := elr, spsr := spsr, far := far } coreId)
...
thread they could leave runnable at a faulting instruction. -/
theorem faultEntryStep_not_dispatchable (lctx : LabelingContext) (st : SystemState)
    ...
    ¬ dispatchableOnCore (faultEntryStep lctx st ectx coreId).2 tid ⟨coreId.toNat, h⟩ := by
  ...
  | some f =>
      simp only [hCur]
      exact faultDeliverOnCoreChecked_not_dispatchable lctx st tid f _ _
