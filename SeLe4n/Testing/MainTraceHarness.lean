...
private def runLifecycleAndEndpointTrace (counter : IO.Ref Nat) (st1 : SystemState) : IO Unit := do
  ...
  match SeLe4n.Kernel.cspaceMintChecked SeLe4n.Kernel.harnessLabelingContext rootSlot mintedSlot (AccessRightSet.ofList [.read]) none st1 with
  | .error err => IO.println s!"[LEP-009] cspace mint error: {reprStr err}"
  | .ok (_, st2) =>
      match SeLe4n.Kernel.cspaceMintChecked SeLe4n.Kernel.harnessLabelingContext rootSlot siblingSlot (AccessRightSet.ofList [.read]) none st2 with
...
private def runCheckedPipelineTrace (counter : IO.Ref Nat) (_st1 : SystemState) : IO Unit := do
  ...
  -- WS-RR RR5.4: run the checked entry under the harness deployment labeling —
  -- a real two-domain context the fail-closed guard admits.  It used to be
  -- `testLabelingContext`, which passed only by evading the sentinel probe.
  let ctx := SeLe4n.Kernel.harnessLabelingContext
