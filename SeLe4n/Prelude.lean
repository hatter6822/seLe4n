...
/- W6-J (LOW-01): RHTable specification surface theorems.
   The following theorems (lines through `RHTable_fold_preserves`) characterize
   ...
   - `RHTable_insert_preserves_invExt` / `_invExtK`: invariant preservation
   - `RHTable_empty_invExt` / `_invExtK`: empty table invariants
