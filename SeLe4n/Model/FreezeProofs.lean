...
def apiInvariantBundle_frozenDirectFull (fst : FrozenSystemState) : Prop :=
  ∃ (sst : SystemState),
    ...
    -- WS-SM SM7.D.1: the per-core instruction-cache views are carried through
    -- freeze bitwise (the instruction-side twin of `perCoreTlb`).
    ...
    -- WS-SM SM9.D.3: the taint side table is carried bitwise, so the causal
    -- laundering analysis a frozen snapshot supports is the live one.
    sst.declassificationTaint = fst.declassificationTaint
...
theorem freeze_preserves_direct_invariants_full (ist : IntermediateState)
  ...
  -- WS-SM SM7.D.1: perCoreICache carried bitwise through freeze
  ...
  -- WS-SM SM9.D.3: the taint side table carried bitwise
  · exact (freeze_preserves_declassificationTaint ist).symm
